...
public class GraduateAuditServiceImpl extends BasicService implements GraduateAuditService {
  ...
  private Map<Course, List<CourseGrade>> populateCourseGradeMap(Long studentId, MajorType majorType,
      Boolean published, Map<CourseType, CourseType> replaceCourseTypes, AuditStandard auditStandard) {
    Map<Course, List<CourseGrade>> courseGradeMap = new HashMap<Course, List<CourseGrade>>();
    ...
    for (Iterator iter = scores.iterator(); iter.hasNext();) {
      ...
      if (replaceCourseTypes.containsKey(courseGrade.getCourseType())) {
        courseGrade.setCourseType(replaceCourseTypes.get(courseGrade.getCourseType()));
      }
    ...
    // 最好成绩放在最前
    for (Course course : courseGradeMap.keySet()) {
      List<CourseGrade> grades = courseGradeMap.get(course);
      Collections.sort(grades, new Comparator<CourseGrade>() {

        public int compare(CourseGrade g0, CourseGrade g1) {
          if (Boolean.TRUE.equals(g1.getIsPass())) {
            return 1;
          } else {
            return 0;
          }
      ...
      // 统计最优先的同名课程
      if (Boolean.TRUE.equals(auditStandard.getSameCourse())) {
        filterSameCourse_firstStep(priorityCourseMap, grades);
      }
    }

    // 按“已统计的”过滤出“成绩”
    if (MapUtils.isNotEmpty(priorityCourseMap)) {
      for (int i = 0; i < courseGradeMap.keySet().size();) {
        Course course = (Course) courseGradeMap.keySet().toArray()[i];
        List<CourseGrade> grades = courseGradeMap.get(course);
        filterSameCourse_secondStep(priorityCourseMap, grades);

        if (CollectionUtils.isEmpty(grades)) {
          courseGradeMap.remove(course);
        } else {
          i++;
        }
      }
  ...
  public void filterSameCourse(List<CourseGrade> grades) {
    filterSameCourse_secondStep(filterSameCourse_firstStep(null, grades), grades);
  }

  protected Map<String, CourseGrade> filterSameCourse_firstStep(Map<String, CourseGrade> priorityCourseMap,
      List<CourseGrade> grades) {
    Map<String, CourseGrade> resultMap = priorityCourseMap;
    if (null == resultMap) {
      resultMap = new HashMap<String, CourseGrade>();
    }
    for (CourseGrade courseGrade : grades) {
      String courseName = courseGrade.getCourse().getName();
      if (resultMap.containsKey(courseName)) {
        CourseType preCourseType = resultMap.get(courseName).getCourseType();
        // 是否可升为必修
        ...
        if (isOk1 || isOk2) {
          resultMap.put(courseName, courseGrade);
        }
      } else {
        resultMap.put(courseName, courseGrade);
      }
  ...
  protected void filterSameCourse_secondStep(Map<String, CourseGrade> priorityCourseMap,
      List<CourseGrade> grades) {
  ...
  private AuditStandard getAuditStandard(Long auditStandardId) {
    ...
    if (auditStandardId == null) {
      ...
      List rs1 = (List) utilService.load(CourseType.class, "isCompulsory", false);
      if (!rs1.isEmpty()) {
        Set set = new HashSet();
        for (Iterator iter = rs1.iterator(); iter.hasNext();) {
          set.add((CourseType) iter.next());
        }
        auditStandard.setConvertableCourseTypes(set);
      }
      auditStandard.setIsDualConvert(false);
  ...
  private TeachPlanAuditResult audit(Student student, MajorType majorType, Long auditStandardId,
    ...
    try {
      ...
      /**
       * 功能 '通识类'的父子类关系需要处理三个地方：培养计划，审核表转，课程成绩
       */
      Set courseGroups = teachPlan.getCourseGroups();// 计划里所有的课程组
      List replaceCourseTypes = new ArrayList();// 在计划里被替换的课程类型
      List<CourseType> courseTypes = utilDao.searchHQLQuery("from CourseType where name='通识类' and state=1");
      List<CourseType> subCourseTypes = utilDao.searchHQLQuery("from CourseType where name like '%通识类'");

      Map<CourseType, CourseType> courseTypeMaps = new HashMap<CourseType, CourseType>();
      CourseType courseType = null;
      boolean ifCon = false;
      if (!courseTypes.isEmpty()) {
        courseType = courseTypes.get(0);
        for (CourseType type : subCourseTypes) {
          courseTypeMaps.put(type, courseType);
        }

        for (Iterator iterator = courseGroups.iterator(); iterator.hasNext();) {
          CourseGroup courseGroup = (CourseGroup) iterator.next();
          if (courseGroup.getCourseType().getId().equals(courseType.getId())) {
            ifCon = true;
          }
        }
      }
      ...
      Map<Course, List<CourseGrade>> courseGradeMap = populateCourseGradeMap(student.getId(), majorType,
          Boolean.TRUE, ifCon ? courseTypeMaps : Collections.EMPTY_MAP, auditStandard);
      ...
      // 针对每一个组进行审核(包括任选课)
      Map<String, CourseType> coursefilterMap = populateCourseGroupAuditResult(courseGradeMap, student,
          teachPlan, result, auditStandard, auditTermList, auditedTypes);

      if (!courseGradeMap.isEmpty()) {
        // 其他没有具体课程要求的课程组(例如模块课)
        addCG2Group(courseGradeMap, teachPlan, auditStandard, result, auditedTypes, coursefilterMap);
      }
      // 任意选修课
      CourseGroupAuditResult publicGroupResult = populatePublicGroupResult(courseGradeMap, auditStandard,
          result, coursefilterMap);
      addConvertedCredit(result, publicGroupResult, auditStandard);

      boolean fullTerm = CollectionUtils.isEmpty(auditTermList) ? true : auditTermList.size() == teachPlan
          .getTermsCount().intValue();
      // 冲低学分
      checkOffsetCredit(result, fullTerm, student);

      result.check(fullTerm);
  ...
  private void checkOffsetCredit(TeachPlanAuditResult result, boolean fullTerm, Student std) {
    if (!fullTerm) { return; }
    ...
    Float offsetCredit = Float.valueOf(0);
    if (null != offsetCredits && offsetCredits.size() > 0) {
      offsetCredit = offsetCredits.get(0).getOffsetCredit();
    }
    for (Iterator iter = result.getCourseGroupAuditResults().iterator(); iter.hasNext();) {
      ...
      if (offsetCredit <= 0) break;
      if (rs.getCreditAuditInfo().getIsPass() || rs.getCourseType().getIsPractice()) continue;
      if (null == rs.getCreditAuditInfo().getCompleted()) rs.getCreditAuditInfo().addCompletedCredit(0f);

      // 还需要多少学分
      float needed = rs.getCreditAuditInfo().getRequired().floatValue();
      if (null != rs.getCreditAuditInfo().getCompleted()) needed -= rs.getCreditAuditInfo().getCompleted()
          .floatValue();
      if (null != rs.getCreditAuditInfo().getConverted()) needed -= rs.getCreditAuditInfo().getConverted()
          .floatValue();
      for (Object pcar : rs.getPlanCourseAuditResults()) {
        PlanCourseAuditResult courseAuditResult = (PlanCourseAuditResult) pcar;
        if (courseAuditResult.getCreditAuditInfo().getIsPass()) continue;
        if (courseAuditResult.getCourse().getName().contains("体育")) {
          needed -= courseAuditResult.getCourse().getCredits();
        }
      }
      if (needed <= 0) continue;

      if (offsetCredit.floatValue() > needed) {
        offsetCredit = Float.valueOf(offsetCredit.floatValue() - needed);
      } else {
        needed = offsetCredit;
        offsetCredit = 0f;
      }

      rs.getCreditAuditInfo().addCompletedCredit(needed);
      if (rs.getCreditAuditInfo().getCreditToComplete(false) <= 0
          && rs.getCreditAuditInfo().getRequiredNum() > 0) {
        rs.getCreditAuditInfo().setCompletedNum(rs.getCreditAuditInfo().getRequiredNum());
      }
      rs.setRemark("冲抵" + needed + "分");
    }
  }
   ...
   * @param auditStandard
   * @param result
   * @param coursefilterMap
   *          TODO
   * @return
   */
  private CourseGroupAuditResult populatePublicGroupResult(Map<Course, List<CourseGrade>> CGMap,
      AuditStandard auditStandard, TeachPlanAuditResult result, Map<String, CourseType> coursefilterMap) {
    ...
    if (!CGMap.isEmpty()
      ...
      for (Iterator iter = CGMap.keySet().iterator(); iter.hasNext();) {
        ...
        addPlanCourseAuditResult(publicGroupResult, gceList, coursefilterMap);
        iter.remove();
      }
    }
  ...
  private void addCG2Group(Map<Course, List<CourseGrade>> CGMap, TeachPlan teachPlan,
      ...
      Map<String, CourseType> coursefilterMap) {
    ...
    for (Iterator iter = CGMap.keySet().iterator(); iter.hasNext();) {
      ...
      if (!group.getPlanCourses().isEmpty() && Boolean.TRUE.equals(group.getCourseType().getIsCompulsory())) {
        continue;
      }
      // 查找group审核结果
      CourseGroupAuditResult groupResult = getCourseGroupResult(courseGroupAuditResultMap, result, firstCG);
      if (groupResult != null) {
        addPlanCourseAuditResult(groupResult, gceList, coursefilterMap);
        iter.remove();
      }
    }
  }
  ...
  private void addPlanCourseAuditResult(CourseGroupAuditResult groupResult, List gceList,
      Map<String, CourseType> coursefilterMap) {
    CourseGrade firstCG = (CourseGrade) gceList.get(0);
    if (coursefilterMap.containsKey(firstCG.getCourse().getName())
        && Boolean.FALSE.equals(firstCG.getCourseType().getIsCompulsory())) {
      return;
    } else {
      if (Boolean.TRUE.equals(firstCG.getCourseType().getIsCompulsory())
          && !coursefilterMap.containsKey(firstCG.getCourse().getName())) {
        // coursefilterMap.put(firstCG.getCourse().getName(), firstCG.getCourseType());
      }
    }
   ...
   * @return TODO
   */
  private Map<String, CourseType> populateCourseGroupAuditResult(Map<Course, List<CourseGrade>> CGMap,
      Student student, TeachPlan teachPlan, TeachPlanAuditResult result, AuditStandard auditStandard,
      List auditTermList, Set auditedTypes) {
    // 对审核的课程组进行排序,以防选修课程组中含有必修课组里面的课程。从而造成选修课学分满，而必修课未通过
    List courseGroups = new ArrayList(teachPlan.getCourseGroups());
    String cmpStr = "courseType.isCompulsory desc";
    PropertyComparator prtyCmt = new PropertyComparator(cmpStr);
    Collections.sort(courseGroups, prtyCmt);
    // 存储必修课的课程，以过滤与之同名的选修课程
    Map<String, CourseType> coursefilterMap = new HashMap<String, CourseType>();
    for (Iterator iterCourseGroup = courseGroups.iterator(); iterCourseGroup.hasNext();) {
      ...
      for (Iterator iterPlanCourse = courseGroup.getPlanCourses().iterator(); iterPlanCourse.hasNext();) {
        ...
        if (coursefilterMap.containsKey(planCourse.getCourse().getName())
            && Boolean.FALSE.equals(courseGroup.getCourseType().getIsCompulsory())) {
          continue;
        } else {
          if (Boolean.TRUE.equals(courseGroup.getCourseType().getIsCompulsory())
              && !coursefilterMap.containsKey(planCourse.getCourse().getName())) {
            // coursefilterMap.put(planCourse.getCourse().getName(), courseGroup
            // .getCourseType());
          }
        }
        // 审核某一门课程成绩
    ...
    for (Iterator iterator = substituteCourseList.iterator(); iterator.hasNext();) {
      ...
      if (courseMap.keySet().containsAll(sc.getOrigins()) && isSubstitutes(CGMap, sc)) {
        ...
        for (Iterator iterator2 = sc.getOrigins().iterator(); iterator2.hasNext();) {
          ...
          if (passed) {
            planCourseResult.getCreditAuditInfo().setCompletedNum(1);
  ...
  private void addPlanCourseAuditResult(CourseGroupAuditResult groupAuditResult,
    ...
    if (Boolean.TRUE.equals(planCourseResult.getCreditAuditInfo().getIsPass())) {
      ...
      groupAuditResult.getCreditAuditInfo().addCompletedNum();
    }
  }
  ...
  protected boolean isSubstitutes(Map<Course, List<CourseGrade>> gradeMap, SubstituteCourse substitute) {
    ...
    for (Iterator it1 = substitute.getOrigins().iterator(); it1.hasNext();) {
      ...
      if (null != grades && !grades.isEmpty()) {
        ...
        if (null != grade.getScore()) {
          ga1 += grade.getCredit().floatValue() * grade.getScore();
        }
    ...
    for (Iterator it1 = substitute.getSubstitutes().iterator(); it1.hasNext();) {
      ...
      if (null != grades && !grades.isEmpty()) {
        ...
        if (null != grade.getScore()) {
          ga2 += grade.getCredit().floatValue() * grade.getScore();
        }
    ...
    if (credit1 == 0 && fullGrade2) {
      ...
      if ((fullGrade1 && fullGrade2) && (credit1 > 0 && credit2 > 0)) {
        int rs = Float.compare(gpa1 / credit1, gpa2 / credit2);
        if (rs != 0) {
          return rs < 0;
        } else {
          return (ga1 / credit1) < (ga2 / credit2);
        }
      } else {
        return false;
      }
