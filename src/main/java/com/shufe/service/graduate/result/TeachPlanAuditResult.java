...
public class TeachPlanAuditResult {
  ...
  public TeachPlanAuditResult(Student student, TeachPlan plan, MajorType majorType) {
    this(plan);
    setStudent(student);
    setMajorType(majorType);
  }
  ...
  public CourseGroupAuditResult getCourseGroupAuditResult(CourseType type) {
    if (null == courseGroupAuditResults) return null;
    for (Iterator iter = courseGroupAuditResults.iterator(); iter.hasNext();) {
      CourseGroupAuditResult groupAuditResult = (CourseGroupAuditResult) iter.next();
      if (groupAuditResult.getCourseType().equals(type)) return groupAuditResult;
    }
    return null;
  }
  ...
  public void check(boolean fullTerm) {
    ...
    for (Iterator iter = getCourseGroupAuditResults().iterator(); iter.hasNext();) {
      CourseGroupAuditResult rs = (CourseGroupAuditResult) iter.next();
      if (Boolean.FALSE.equals(rs.getCreditAuditInfo().getIsPass())) {
        flag = false;
      }
      creditAuditInfo.addCompletedCredit(rs.getCreditAuditInfo().getCompleted());
      // 若不审核全部学期，此处将重新计算要求总学分
      if (!fullTerm) {
        creditAuditInfo.addRequiredCredit(rs.getCreditAuditInfo().getRequired());
      }
    }
    if (!flag || Boolean.FALSE.equals(this.getCreditAuditInfo().getIsPass())) {
  ...
  public List getOrderCourseGroupAuditResults() {
    Collections.sort(courseGroupAuditResults, new Comparator() {
      public int compare(Object arg0, Object arg1) {
        ...
        int compusoryRs = o1.getCourseType().getIsCompulsory().equals(o0.getCourseType().getIsCompulsory()) ? 0
            : (o1.getCourseType().getIsCompulsory().booleanValue() ? 1 : -1);
        if (compusoryRs != 0) return compusoryRs;
        else {
          return o0.getCourseType().getPriority().compareTo(o1.getCourseType().getPriority());
        }
      }
    });
    return courseGroupAuditResults;
  ...
  public void disauditCourseType(CourseGroup courseGroup) {
    if (this.creditAuditInfo.getRequired() != null) {
      float required = this.creditAuditInfo.getRequired().floatValue()
          - (courseGroup.getCredit() == null ? 0 : courseGroup.getCredit().floatValue());
      this.creditAuditInfo.setRequired(new Float(required < 0f ? 0f : required));
    }
  }
