...
public class ForeignerListener extends TransferConvertListener {
  ...
  public void tranferField(String field, String value) throws Exception {
    ...
    if (StringUtils.contains(field, '.') && field.endsWith("code") && StringUtils.isNotEmpty(value)) {
      Object nestedForeigner = PropertyUtils.getProperty(CSVDataTransfer.getCurTransfered(),
          StringUtils.substring(field, 0, field.lastIndexOf(".")));

      if (nestedForeigner instanceof Entity) {
        if (!ValidEntityPredicate.INSTANCE.evaluate(nestedForeigner)) {

          Map values = (Map) foreigers.get(nestedForeigner.getClass());
          if (null == values) {
            Map valueMap = new HashMap();
            foreigers.put(nestedForeigner.getClass(), valueMap);
            Object Id = valueMap.get(value);
            if (Id == null) {
              List foreigners = utilService.load(nestedForeigner.getClass(), "code", value);

              if (!foreigners.isEmpty()) {
                Id = PropertyUtils.getProperty(foreigners.iterator().next(), "id");
                valueMap.put(value, Id);
              } else {
                throw new PojoNotExistException("wrong code :" + field + " with value:" + value);
              }
            }
            PropertyUtils.setProperty(nestedForeigner, ((Entity) nestedForeigner).key(), Id);
          } else {
