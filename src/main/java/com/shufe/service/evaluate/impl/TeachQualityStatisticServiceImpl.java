...
public class TeachQualityStatisticServiceImpl extends BasicService implements TeachQualityStatisticService {
  ...
  public Map statEvaluatePageByNum(List floatSegments, List calendars, String stdTypeIdSeq,
    ...
    for (Iterator iter = markAndyears.iterator(); iter.hasNext();) {
      ...
      for (int i = 0; i < floatSegments.size(); i++) {
        ...
        if (score.floatValue() <= floatSegment.getMax() && score.floatValue() >= floatSegment.getMin()) {
      ...
      if (score.floatValue() >= excellentMark) {
        StatUtils.setValueToMap(calendarId + "excellent", new Integer(1), "integer", requestMap);
