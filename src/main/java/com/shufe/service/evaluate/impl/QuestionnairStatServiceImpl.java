...
public class QuestionnairStatServiceImpl implements QuestionnairStatService {
  ...
  public List evaluateDepartmentScorePlace(TeachCalendar calendar) {
    ...
    orders.add(new Order("evaluateTeacher.teacher.department.id, evaluateTeacher.sumScore desc"));
    ...
    for (Iterator it = evaluateTeachers.iterator(); it.hasNext();) {
      ...
      if (!ObjectUtils.equals(beforeDepartId, evaluateTeacher.getTeacher().getDepartment().getId())) {
  ...
  public List getEvaluateTeachers(TeachCalendar calendar, boolean isAllCalendar) {
    if (isAllCalendar) {
      EntityQuery query = new EntityQuery(EvaluateTeacherStat.class, "evaluateTeacher");
      query.add(new Condition("evaluateTeacher.calendar.start <= :calendarStart", calendar.getStart()));
  ...
  public EvaluateTeacherStat getEvaluateTeacherStat(TeachCalendar calendar, Course course, Teacher teacher) {
    if (null == calendar || null == course || null == teacher) { return null; }
    EntityQuery query = new EntityQuery(EvaluateTeacherStat.class, "evaluateTeacher");
    query.add(new Condition("evaluateTeacher.calendar = :teachCalendar", calendar));
  ...
  public EvaluateDepartmentStat getEvaluateDepartmentStat(TeachCalendar calendar, Department department) {
    if (null == calendar || null == department) { return null; }
    EntityQuery query = new EntityQuery(EvaluateDepartmentStat.class, "evaluateDepartment");
    query.add(new Condition("evaluateDepartment.calendar = :teachCalendar", calendar));
   ...
   * @return
   */
  public List buildTeacherStatResult(Long[] stdTypeIds, Long[] departmentIds, TeachCalendar calendar) {
    List evaluateResults = questionEvaluateTeacherList(stdTypeIds, departmentIds, calendar, true);
    evaluateResults.addAll(questionEvaluateTeacherList(stdTypeIds, departmentIds, calendar, false));
    ...
    for (Iterator it = evaluateResults.iterator(); it.hasNext();) {
      ...
      Question question = (Question) utilDao.load(Question.class, new Long(obj[2].toString()));
      ...
      if (teacherStatMap.containsKey(key)) {
        ...
        evaluateTeacher.getQuestionsStat().add(new QuestionTeacherStat(evaluateTeacher, question, avgPoints));
      } else {
        // 否则就新建一个EvaluateTeacherStat对象，把问题统计结果加入
        evaluateTeacher = getEvaluateTeacherStat(calendar, course, teacher);
        if (null != evaluateTeacher && !CollectionUtils.isEmpty(evaluateTeacher.getQuestionsStat())) {
    ...
    for (Iterator it = teacherStatMap.entrySet().iterator(); it.hasNext();) {
      ...
      EvaluateTeacherStat evaluateTeacher = (EvaluateTeacherStat) teacherStatMap.get(entry.getKey());
   ...
   * @return
   */
  private List questionEvaluateTeacherList(Long[] stdTypeIds, Long[] departmentIds, TeachCalendar calendar,
      boolean evaluateByTeacher) {
    EntityQuery query = new EntityQuery(QuestionResult.class, "questionResult");
    query
        .add(new Condition("questionResult.result.teachCalendar.year = (:calendarYear)", calendar.getYear()));
    query
        .add(new Condition("questionResult.result.teachCalendar.term = (:calendarTerm)", calendar.getTerm()));
    if (null != stdTypeIds) {
      query.add(new Condition("questionResult.result.stdType.id in (:stdTypeIds)", stdTypeIds));
    }
    if (null != departmentIds) {
      query.add(new Condition("questionResult.result.department.id in (:departmentIds)", departmentIds));
    }
    query.add(new Condition("questionResult.result.statState = true"));
    String groupBy = null;
    if (evaluateByTeacher) {
      query.add(new Condition("questionResult.result.task.requirement.evaluateByTeacher = true"));
      groupBy = "questionResult.result.teacher.id, questionResult.result.task.course.id, questionResult.question.id";
    } else {
      query.add(new Condition("questionResult.result.task.requirement.evaluateByTeacher = false"));
      query.join("questionResult.result.task.arrangeInfo.teachers", "teacher");
  ...
  public List buildDepartmentStatResult(Long[] departmentIds, TeachCalendar calendar) {
    ...
    for (Iterator it = evaluateResults.iterator(); it.hasNext();) {
      Object[] obj = (Object[]) it.next();
      Department department = (Department) ((Teacher) utilDao
          .load(Teacher.class, new Long(obj[0].toString()))).getDepartment();
      Question question = (Question) utilDao.load(Question.class, new Long(obj[2].toString()));
      Double avgPoints = new Double(obj[3].toString());
      ...
      if (deparmentStatMap.containsKey(key)) {
        ...
        evaluateDepartment.getQuestionsStat().add(new QuestionStat(evaluateDepartment, question, avgPoints));
      } else {
        // 否则就新建一个EvaluateDepartmentStat对象，把问题统计结果加入
        evaluateDepartment = getEvaluateDepartmentStat(calendar, department);
        if (null != evaluateDepartment && !CollectionUtils.isEmpty(evaluateDepartment.getQuestionsStat())) {
    ...
    for (Iterator it = deparmentStatMap.entrySet().iterator(); it.hasNext();) {
      Entry entry = (Entry) it.next();
      EvaluateDepartmentStat evaluateDepartment = (EvaluateDepartmentStat) deparmentStatMap.get(entry
          .getKey());
  ...
  public List buildCollegeStatResult(TeachCalendar calendar) {
    ...
    for (Iterator it = evaluateResults.iterator(); it.hasNext();) {
      ...
      if (collegeStatMap.containsKey(key)) {
        ...
        evaluateCollege.getQuestionsStat().add(new QuestionStat(evaluateCollege, question, avgPoints));
      } else {
        evaluateCollege = getEvaluateCollegeStat(calendar);
        if (null != evaluateCollege && !CollectionUtils.isEmpty(evaluateCollege.getQuestionsStat())) {
    ...
    for (Iterator it = collegeStatMap.entrySet().iterator(); it.hasNext();) {
      Entry entry = (Entry) it.next();
      EvaluateCollegeStat evaluateCollege = (EvaluateCollegeStat) collegeStatMap.get(entry.getKey());
  ...
  public Object[] getCollegeResultList(List departmentResults) {
    ...
    for (Iterator it = departmentResults.iterator(); it.hasNext();) {
      ...
      if (null == collegeResult) {
        ...
        collegeResult[0] = getEvaluateCollegeStat(((EvaluateDepartmentStat) obj[0]).getCalendar());
      }
      for (int i = 1; i < obj.length; i++) {
        ...
        if (null == collegeResult[i]) {
          ...
          collegeResult[i] = String.valueOf(Integer.parseInt(collegeResult[i].toString()) + count);
        }
      }
    }
  ...
  public List getCollegeResultList(TeachCalendar calendar, List criteriaItems) {
    ...
    for (Iterator it = criteriaItems.iterator(); it.hasNext();) {
      ...
      caseWhens[scoreLevel++] = " when evaluateTeacher.sumScore >= " + criteriaItem.getMin()
          + " and evaluateTeacher.sumScore <= " + criteriaItem.getMax() + " then " + scoreLevel;
    }
  ...
  public List getDepartResultList(List criteriaItems, TeachCalendar calendar) {
    ...
    for (Iterator it = queryList.iterator(); it.hasNext();) {
      Object[] obj = (Object[]) it.next();
      Department department = (Department) utilDao.load(Department.class, new Long(obj[0].toString()));
      if (!ObjectUtils.equals(beforeDepartment, department)) {
        newObj = new Object[criteriaItems.size() + 1];
        EvaluateDepartmentStat evaluateDepartment = getEvaluateDepartmentStat(calendar, department);
        if (null == evaluateDepartment) {
          continue;
        }
  ...
  public List getDepartResultList(List criteriaItems, Department department) {
    ...
    for (Iterator it = queryList.iterator(); it.hasNext();) {
      Object[] obj = (Object[]) it.next();
      TeachCalendar calendar = (TeachCalendar) utilDao.load(TeachCalendar.class, new Long(obj[0].toString()));
  ...
  public double[] getDepartmentResults(Collection teacherResults) {
    if (CollectionUtils.isEmpty(teacherResults)) { throw new RuntimeException(
        "The parameter width teacherResults is empty or null."); }
    int n = 0;
  ...
  public EvaluateDepartmentStat getEvaluateDepartmentStat(Long evaluateDepartmentId) {
    return (EvaluateDepartmentStat) utilDao.get(EvaluateDepartmentStat.class, evaluateDepartmentId);
  }
  ...
  public Boolean isEmptyOfEvaluateTeacherStat(TeachCalendar calendar, Course course, Teacher teacher) {
    return new Boolean(null == course || null == getEvaluateTeacherStat(calendar, course, teacher));
  }
}
