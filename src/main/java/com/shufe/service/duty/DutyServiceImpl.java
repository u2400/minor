...
public class DutyServiceImpl extends BasicService implements DutyService {
  ...
  public void batchUpdateDutyRecord(Long teachTaskId, Long[] studentIdArray, Map dutyStatusMap,
    ...
    List dutyRecordList = utilService.load(DutyRecord.class, new String[] { "student.id", "teachTask.id" },
        new Object[] { studentIdArray, teachTaskId });
    for (Iterator ite = dutyRecordList.iterator(); ite.hasNext();) {
      ...
      dutyRecord.setTotalCount(dutyRecord.getTotalCount() != null ? new Integer(dutyRecord.getTotalCount()
    ...
    map.put("convertAbsenteeism", Double.valueOf(String.valueOf(AttendanceType.convertAbsenteeism)));
    ...
    for (Iterator ite = dutyRecordList.iterator(); ite.hasNext();) {
      ...
      try {
        ...
        if (dutyStatusObj instanceof AttendanceType) {
          ...
          cloneObj.setDutyStatus(dutyStatus);
          if (dutyStatus.getId().equals(AttendanceType.presence)) {
            dutyRecord.setDutyCount(dutyRecord.getDutyCount() != null ? new Integer(dutyRecord.getDutyCount()
                .intValue() + 1) : new Integer(1));
          } else if (dutyStatus.getId().equals(AttendanceType.absenteeism)) {
            dutyRecord.setAbsenteeismCount(dutyRecord.getAbsenteeismCount() != null ? new Integer(dutyRecord
                .getAbsenteeismCount().intValue() + 1) : new Integer(1));
          } else if (dutyStatus.getId().equals(AttendanceType.late)) {
            dutyRecord.setLateCount(dutyRecord.getLateCount() != null ? new Integer(dutyRecord.getLateCount()
                .intValue() + 1) : new Integer(1));
          } else if (dutyStatus.getId().equals(AttendanceType.leaveEarly)) {
            dutyRecord.setLeaveEarlyCount(dutyRecord.getLeaveEarlyCount() != null ? new Integer(dutyRecord
                .getLeaveEarlyCount().intValue() + 1) : new Integer(1));
          } else if (dutyStatus.getId().equals(AttendanceType.askedForLeave)) {
            dutyRecord.setAskedForLeaveCount(dutyRecord.getAskedForLeaveCount() != null ? new Integer(
                dutyRecord.getAskedForLeaveCount().intValue() + 1) : new Integer(1));
          }
        } else {
          cloneObj.setDutyStatus(new AttendanceType(AttendanceType.absenteeism));
          dutyRecord.setAbsenteeismCount(dutyRecord.getAbsenteeismCount() != null ? new Integer(dutyRecord
  ...
  public void batchUpdateDutyRecordDetail(Long recordId, String[] recordDetailIds) {
    ...
    utilService.update(RecordDetail.class, "id", recordDetailIds, new String[] { "dutyStatus" },
        new Object[] { new Boolean(true) });
    Integer dutyCount = new Integer(recordDetailIds.length);
    utilService.update(DutyRecord.class, "id", new Object[] { recordId }, new String[] { "dutyCount",
        "dutyRatio" }, new Object[] { dutyCount, dutyCount + "/totalCount" });
  }
  ...
  public List loadStudentDutyRecordDetails(Long teachTaskId, String dutyDate, Long beginUnitId, Long endUnitId) {
    String hql = "SELECT recordDetail" + " FROM RecordDetail as recordDetail "
        + "join recordDetail.dutyRecord as dutyRecord " + "join dutyRecord.teachTask as teachTask "
        + "WHERE teachTask.id=(:teachTaskId)" + "and recordDetail.dutyDate=TO_DATE((:dutyDate),'YYYY-MM-DD')"
        + "and recordDetail.beginUnit.id=(:beginUnitId)" + "and recordDetail.endUnit.id=(:endUnitId)";
    Map params = new HashMap();
    params.put("teachTaskId", teachTaskId);
  ...
  public boolean checkDutyRecordsIfExists(String stdIds, Long teachTaskId, String dutyDate,
    ...
    if (ArrayUtils.isEmpty(stdIdArray) || !NotZeroNumberPredicate.getInstance().evaluate(teachTaskId)
        || StringUtils.isEmpty(dutyDate) || StringUtils.isEmpty(beginUnitIdString)
        || StringUtils.isEmpty(endUnitIdString)) { return false; }
    EntityQuery entityQuery = new EntityQuery(RecordDetail.class, "recordDetail");
    entityQuery.add(new Condition("recordDetail.dutyRecord.student.id in (:stdIds)", stdIdArray));
    entityQuery.add(new Condition("recordDetail.dutyRecord.teachTask.id = :teachTaskId", teachTaskId));
    entityQuery.add(new Condition("recordDetail.dutyDate = :dutyDate", Date.valueOf(dutyDate)));
    entityQuery.add(new Condition("recordDetail.beginUnit.id = :beginUnitId", new Long(beginUnitIdString)));
    entityQuery.add(new Condition("recordDetail.endUnit.id = :endUnitId", new Long(endUnitIdString)));
  ...
  public Long getDutyRecordIdIfExists(String stdIds, Long teachTaskId, String dutyDate, String timeBegin,
      String timeEnd) {
    Long[] stdIdArray = SeqStringUtil.transformToLong(stdIds);
    for (int i = 0; i < stdIdArray.length; i++) {
      if (utilService.exist(RecordDetail.class, new String[] { "dutyRecord.student.id",
          "dutyRecord.teachTask.id", "dutyDate", "timeBegin", "timeEnd" }, new Object[] { stdIdArray[i],
          teachTaskId, Date.valueOf(dutyDate), timeBegin, timeEnd })) {
        Long recordId = ((RecordDetail) utilService.load(
            RecordDetail.class,
            new String[] { "dutyRecord.student.id", "dutyRecord.teachTask.id", "dutyDate", "timeBegin",
                "timeEnd" },
            new Object[] { stdIdArray[i], teachTaskId, Date.valueOf(dutyDate), timeBegin, timeEnd }).get(0))
            .getDutyRecord().getId();
        return recordId;
      }
    }
  ...
  public void batchUpdateDutyRecordDetail(Map dutyStatusMap) {
    ...
    List studentDutyRecordDetailsList = utilService.load(RecordDetail.class, "id", DetailKeySet);
    ...
    for (Iterator iter = studentDutyRecordDetailsList.iterator(); iter.hasNext();) {
      ...
      if (NullPredicate.getInstance().evaluate(dutyStatusMap.get(recordDetailId))
        ...
        if (dutyStatus.getId().equals(AttendanceType.presence)) {
          ...
          dutyRecord.setAbsenteeismCount(new Integer(dutyRecord.getAbsenteeismCount().intValue() - 1));
        } else if (dutyStatus.getId().equals(AttendanceType.late)) {
          dutyRecord.setLateCount(new Integer(dutyRecord.getLateCount().intValue() - 1));
        } else if (dutyStatus.getId().equals(AttendanceType.leaveEarly)) {
          dutyRecord.setLeaveEarlyCount(new Integer(dutyRecord.getLeaveEarlyCount().intValue() - 1));
        } else if (dutyStatus.getId().equals(AttendanceType.askedForLeave)) {
          dutyRecord.setAskedForLeaveCount(new Integer(dutyRecord.getAskedForLeaveCount().intValue() - 1));
        }
        if (dutyStatusMap.get(recordDetailId) instanceof Long) {
          ...
          if (dutyStatusId.equals(AttendanceType.presence)) {
            ...
            dutyRecord.setAskedForLeaveCount(dutyRecord.getAskedForLeaveCount() != null ? new Integer(
                dutyRecord.getAskedForLeaveCount().intValue() + 1) : new Integer(1));
            recordDetail.setDutyStatus(new AttendanceType(AttendanceType.askedForLeave));
          }
        }

        dutyRecord.setDutyRatio(new Float((dutyRecord.getDutyCount() != null ? dutyRecord.getDutyCount()
            .floatValue() : 0.0f)
            / ((dutyRecord.getTotalCount() != null ? (dutyRecord.getTotalCount().floatValue() == 0.0f ? (-1f)
                : dutyRecord.getTotalCount().floatValue()) : (-1f)))));
        int convertAbsenteeismCount = ((dutyRecord.getLateCount() != null ? dutyRecord.getLateCount()
            .intValue() : 0) + (dutyRecord.getLeaveEarlyCount() != null ? dutyRecord.getLeaveEarlyCount()
            .intValue() : 0))
            / AttendanceType.convertAbsenteeism;
        dutyRecord
            .setAbsenteeismRatio(new Float(
                (dutyRecord.getTotalCount() == null || dutyRecord.getTotalCount().floatValue() == 0.0f) ? 0.0f
                    : ((dutyRecord.getAbsenteeismCount() != null ? dutyRecord.getAbsenteeismCount()
                        .floatValue() : 0.0f) + convertAbsenteeismCount)
                        / ((dutyRecord.getTotalCount() != null ? (dutyRecord.getTotalCount().floatValue() == 0.0f ? (-1f)
  ...
  public Map dutyRecordStatistics(Long[] studentIdArray, List calendarList, String dateBegin, String dateEnd,
      boolean isCancelStat, Predicate predicate) {
    ...
    if (ArrayUtils.isEmpty(studentIdArray) || CollectionUtils.isEmpty(calendarList)) {
      ...
      for (int i = 0; i < studentIdArray.length; i++) {
        ...
        // 生成学生考勤统计对象
        DutyRecordStatisticsStd dutyRecordStatisticsStd = new DutyRecordStatisticsStd(String.valueOf(student
            .getId()), student.getCode(), student.getName(), student.getEngName());
        stdMap.put(String.valueOf(dutyRecordStatisticsStd.getId()), dutyRecordStatisticsStd);
        Map studentStatisticsMap = new HashMap();
        Map studentCourseTakeTypeMap = new HashMap();
        // 获取学生考勤记录集合
        Collection studentDutyRecordList = CollectionUtils.select(dutyRecordList, new BeanPredicate(
            "student.id", new EqualPredicate(student.getId())));
        for (Iterator iter = studentDutyRecordList.iterator(); iter.hasNext();) {
          ...
          DutyRecordStatisticsCourse dutyRecordStatisticsCourse = new DutyRecordStatisticsCourse(
              dutyRecordElement);
          courseMap.put(dutyRecordStatisticsCourse.getTeachTaskId(), dutyRecordStatisticsCourse);
          CourseTakeType courseTakeType = dutyRecordElement.getCourseTakeType(Boolean.TRUE);
          studentCourseTakeTypeMap.put(dutyRecordStatisticsCourse.getTeachTaskId(), courseTakeType);
          // 获取指定日期范围内的考勤详情
          List recordCountList = dutyRecordElement.getRecordDetailList(StringUtils.isEmpty(dateBegin) ? null
              : Date.valueOf(dateBegin), StringUtils.isEmpty(dateEnd) ? null : Date.valueOf(dateEnd));
          String dutyRecordStatisticsCourseId = dutyRecordStatisticsCourse.getTeachTaskId();
          countStatus(recordCountList, studentStatisticsMap, dutyRecordStatisticsCourseId, "presence");
          countStatus(recordCountList, studentStatisticsMap, dutyRecordStatisticsCourseId, "absenteeism");
          countStatus(recordCountList, studentStatisticsMap, dutyRecordStatisticsCourseId, "late");
          countStatus(recordCountList, studentStatisticsMap, dutyRecordStatisticsCourseId, "leaveEarly");
          countStatus(recordCountList, studentStatisticsMap, dutyRecordStatisticsCourseId, "askedForLeave");
          countStatus(recordCountList, studentStatisticsMap, dutyRecordStatisticsCourseId, null);
          ...
          if (studentStatisticsMap.get(dutyRecordStatisticsCourse.getTeachTaskId()) instanceof DutyRecordStatisticsStatus) {
            ...
            Object courseTotalObject = studentStatisticsMap.get(DutyRecordStatisticsCourse.totalId);
            if ((courseTakeType != null)
                && (courseTakeType.getId().equals(CourseTakeType.RESTUDY) || courseTakeType.getId().equals(
                    CourseTakeType.REEXAM))) {
              ...
              if (courseTotalObject == null) {
                ...
                try {
                  ...
                  courseTotal.setPresenceCount(mapStatus.getPresenceCount(Boolean.TRUE));
                  courseTotal.setAbsenteeismCount(mapStatus.getAbsenteeismCount(Boolean.TRUE));
                } catch (Exception e) {
                  e.printStackTrace();
                  throw new RuntimeException("PropertyUtils.copyProperties( courseTotal, mapStatus );");
                }
                courseMap.put(DutyRecordStatisticsCourse.totalId, new DutyRecordStatisticsCourse(
                    DutyRecordStatisticsCourse.totalId, DutyRecordStatisticsCourse.totalName,
                    DutyRecordStatisticsCourse.totalEngName));
                studentStatisticsMap.put(DutyRecordStatisticsCourse.totalId, courseTotal);
              } else {
                DutyRecordStatisticsStatus courseTotal = (DutyRecordStatisticsStatus) courseTotalObject;
                courseTotal.setPresenceCount(new Integer((courseTotal.getPresenceCount() == null ? 0
                    : courseTotal.getPresenceCount().intValue())
                    + (mapStatus.getPresenceCount(Boolean.TRUE) == null ? 0 : mapStatus.getPresenceCount(
                        Boolean.TRUE).intValue())));
                courseTotal.setAbsenteeismCount(new Integer((courseTotal.getAbsenteeismCount() == null ? 0
                    : courseTotal.getAbsenteeismCount().intValue())
                    + (mapStatus.getAbsenteeismCount(Boolean.TRUE) == null ? 0 : mapStatus
                        .getAbsenteeismCount(Boolean.TRUE).intValue())));
                courseTotal.setLateCount(new Integer((courseTotal.getLateCount() == null ? 0 : courseTotal
                    .getLateCount().intValue())
                    + (mapStatus.getLateCount() == null ? 0 : mapStatus.getLateCount().intValue())));
                courseTotal.setLeaveEarlyCount(new Integer(
                    (courseTotal.getLeaveEarlyCount() == null ? 0 : courseTotal.getLeaveEarlyCount()
                        .intValue())
                        + (mapStatus.getLeaveEarlyCount() == null ? 0 : mapStatus.getLeaveEarlyCount()
                            .intValue())));
                courseTotal.setAskedForLeaveCount(new Integer(
                    (courseTotal.getAskedForLeaveCount() == null ? 0 : courseTotal.getAskedForLeaveCount()
                        .intValue())
                        + (mapStatus.getAskedForLeaveCount() == null ? 0 : mapStatus.getAskedForLeaveCount()
                            .intValue())));
                courseTotal.setTotalCount(new Integer((courseTotal.getTotalCount() == null ? 0 : courseTotal
                    .getTotalCount().intValue())
                    + (mapStatus.getTotalCount() == null ? 0 : mapStatus.getTotalCount().intValue())));
              }

              Object totalMapStatusObject = (Object) totalStatisticsMap.get(dutyRecordStatisticsCourse
                  .getTeachTaskId());
              if (totalMapStatusObject == null) {
                ...
                totalStatisticsMap.put(dutyRecordStatisticsCourse.getTeachTaskId(), courseTotal);
              } else {
                DutyRecordStatisticsStatus totalMapStatus = (DutyRecordStatisticsStatus) totalMapStatusObject;
                totalMapStatus.setPresenceCount(new Integer((totalMapStatus.getPresenceCount() == null ? 0
                    : totalMapStatus.getPresenceCount().intValue())
                    + (mapStatus.getPresenceCount(Boolean.TRUE) == null ? 0 : mapStatus.getPresenceCount(
                        Boolean.TRUE).intValue())));
                totalMapStatus.setAbsenteeismCount(new Integer(
                    (totalMapStatus.getAbsenteeismCount() == null ? 0 : totalMapStatus.getAbsenteeismCount()
                        .intValue())
                        + (mapStatus.getAbsenteeismCount(Boolean.TRUE) == null ? 0 : mapStatus
                            .getAbsenteeismCount(Boolean.TRUE).intValue())));
                totalMapStatus.setLateCount(new Integer((totalMapStatus.getLateCount() == null ? 0
                    : totalMapStatus.getLateCount().intValue())
                    + (mapStatus.getLateCount() == null ? 0 : mapStatus.getLateCount().intValue())));
                totalMapStatus.setLeaveEarlyCount(new Integer(
                    (totalMapStatus.getLeaveEarlyCount() == null ? 0 : totalMapStatus.getLeaveEarlyCount()
                        .intValue())
                        + (mapStatus.getLeaveEarlyCount() == null ? 0 : mapStatus.getLeaveEarlyCount()
                            .intValue())));
                totalMapStatus.setAskedForLeaveCount(new Integer(
                    (totalMapStatus.getAskedForLeaveCount() == null ? 0 : totalMapStatus
                        .getAskedForLeaveCount().intValue())
                        + (mapStatus.getAskedForLeaveCount() == null ? 0 : mapStatus.getAskedForLeaveCount()
                            .intValue())));
                totalMapStatus.setTotalCount(new Integer((totalMapStatus.getTotalCount() == null ? 0
                    : totalMapStatus.getTotalCount().intValue())
                    + (mapStatus.getTotalCount() == null ? 0 : mapStatus.getTotalCount().intValue())));
              }
            }
        ...
        studentStatisticsMap.put(DutyRecordStatisticsCourse.courseTakeType, studentCourseTakeTypeMap);
        ...
        if (studentStatisticsMap.get(DutyRecordStatisticsCourse.totalId) instanceof DutyRecordStatisticsStatus) {
          ...
          if (totalObject == null) {
            ...
            try {
              ...
              e.printStackTrace();
              throw new RuntimeException("PropertyUtils.copyProperties( stdTotal, mapStatus );");
            }
            stdMap.put(DutyRecordStatisticsCourse.totalId, new DutyRecordStatisticsCourse(
                DutyRecordStatisticsCourse.totalId, DutyRecordStatisticsCourse.totalName,
                DutyRecordStatisticsCourse.totalEngName));
            ...
            stdTotal.setPresenceCount(new Integer((stdTotal.getPresenceCount() == null ? 0 : stdTotal
                .getPresenceCount().intValue())
                + (mapStatus.getPresenceCount() == null ? 0 : mapStatus.getPresenceCount().intValue())));
            stdTotal
                .setAbsenteeismCount(new Integer((stdTotal.getAbsenteeismCount() == null ? 0 : stdTotal
                    .getAbsenteeismCount().intValue())
                    + (mapStatus.getAbsenteeismCount() == null ? 0 : mapStatus.getAbsenteeismCount()
                        .intValue())));
            stdTotal.setLateCount(new Integer((stdTotal.getLateCount() == null ? 0 : stdTotal.getLateCount()
                .intValue()) + (mapStatus.getLateCount() == null ? 0 : mapStatus.getLateCount().intValue())));
            stdTotal.setLeaveEarlyCount(new Integer((stdTotal.getLeaveEarlyCount() == null ? 0 : stdTotal
                .getLeaveEarlyCount().intValue())
                + (mapStatus.getLeaveEarlyCount() == null ? 0 : mapStatus.getLeaveEarlyCount().intValue())));
            stdTotal.setAskedForLeaveCount(new Integer((stdTotal.getAskedForLeaveCount() == null ? 0
                : stdTotal.getAskedForLeaveCount().intValue())
                + (mapStatus.getAskedForLeaveCount() == null ? 0 : mapStatus.getAskedForLeaveCount()
                    .intValue())));
            stdTotal.setTotalCount(new Integer((stdTotal.getTotalCount() == null ? 0 : stdTotal
                .getTotalCount().intValue())
                + (mapStatus.getTotalCount() == null ? 0 : mapStatus.getTotalCount().intValue())));
          }
        }
    ...
    stdMap.put(DutyRecordStatisticsStd.totalNo, new DutyRecordStatisticsStd(DutyRecordStatisticsStd.totalNo,
        DutyRecordStatisticsStd.totalNo, DutyRecordStatisticsStd.totalName,
        DutyRecordStatisticsStd.totalEngName));
   ...
   * @param studentStatisticsMap 考勤统计<code>Map</code>,key:dutyRecordStatisticsCourseId;value:
   *          <code>DutyRecordStatisticsStatus</code>
  ...
  private void countStatus(List recordCountList, Map studentStatisticsMap,
    ...
    if (StringUtils.isEmpty(status)) {
      ...
      if (statusCount == null) {
        ...
        dutyRecordStatisticsStatus.setTotalCount(new Integer(
            ((dutyRecordStatisticsStatus.getTotalCount() == null) ? 0 : dutyRecordStatisticsStatus
                .getTotalCount().intValue()) + recordCountList.size()));
      }
      return;
    }
    ...
    if (statusCount == null) {
      DutyRecordStatisticsStatus dutyRecordStatisticsStatus = new DutyRecordStatisticsStatus();
      try {
        PropertyUtils.setProperty(dutyRecordStatisticsStatus, status + "Count", new Integer(count));
      } catch (Exception e) {
        e.printStackTrace();
       ...
       * dutyRecordStatisticsStatus.setTotalCount(dutyRecordStatisticsStatus.getTotalCount()==null?(
       * Integer)s[1]:new
      ...
      try {
        PropertyUtils.setProperty(dutyRecordStatisticsStatus, status + "Count", new Integer(
            ((oldCount == null) ? 0 : ((Integer) oldCount).intValue()) + count));
      } catch (Exception e) {
        e.printStackTrace();
        throw new RuntimeException("dutyRecordStatisticsStatus NoSuchMethodException");
      }
  ...
  public List getDutyRecordOfStd(Long[] studentIdArray, List calendarList) {
    if (calendarList == null) {
      return null;
    } else if (calendarList.isEmpty()) return utilService
        .load(DutyRecord.class, "student.id", studentIdArray);
    else {
  ...
  public Map getCourseUnitStatusMap(Long teachTaskId, java.util.Date beginDate, java.util.Date endDate) {
    ...
    for (Iterator iter = recordDetailList.iterator(); iter.hasNext();) {
      ...
      Object stdCourseUnitStatusMapObject = stdCourseUnitStatusMap.get(String.valueOf(recordDetailElement
          .getDutyRecord().getStudent().getId()));
      if (stdCourseUnitStatusMapObject instanceof Map) {
        ...
        if (courseUnitStatusMapObject instanceof Map) {
          ((Map) courseUnitStatusMapObject).put(
              String.valueOf(recordDetailElement.getBeginUnit().getId() + "-"
                  + recordDetailElement.getEndUnit().getId()), recordDetailElement.getDutyStatus());
        } else {
        ...
        courseUnitStatusMap.put(dutyDate, unitTempMap);
        stdCourseUnitStatusMap.put(String.valueOf(recordDetailElement.getDutyRecord().getStudent().getId()),
            courseUnitStatusMap);
      }
    }
  ...
  public void saveRecordDetail(Long studentId, Long teachTaskId, Date day, Long beginUnitId, Long endUnitId,
      Long dutyStatusId) {

  }

  public void updateOrSaveRecordDetail(Long studentId, Long teachTaskId, Date day, Long beginUnitId,
      Long endUnitId, Long dutyStatusId) {
    List recordDetailList = utilService.load(RecordDetail.class, new String[] { "dutyRecord.student.id",
        "dutyRecord.teachTask.id", "dutyDate", "beginUnit.id", "endUnit.id" }, new Object[] { studentId,
        teachTaskId, day, beginUnitId, endUnitId });
    if (!recordDetailList.isEmpty()) {
      ...
      List dutyRecordList = utilService.load(DutyRecord.class, new String[] { "student.id", "teachTask.id" },
          new Object[] { studentId, teachTaskId });
      if (!dutyRecordList.isEmpty()) {
        if (dutyRecordList.size() > 1) {
          ...
          if (dutyStatusId != null) {
            if (dutyStatusId.equals(AttendanceType.presence)) this.setDutyRecordProperty(dutyRecord,
                "dutyCount", 1);
            else if (dutyStatusId.equals(AttendanceType.absenteeism)) this.setDutyRecordProperty(dutyRecord,
                "absenteeismCount", 1);
            else if (dutyStatusId.equals(AttendanceType.askedForLeave)) this.setDutyRecordProperty(
                dutyRecord, "askedForLeaveCount", 1);
            else if (dutyStatusId.equals(AttendanceType.late)) this.setDutyRecordProperty(dutyRecord,
                "lateCount", 1);
            else if (dutyStatusId.equals(AttendanceType.leaveEarly)) this.setDutyRecordProperty(dutyRecord,
                "leaveEarlyCount", 1);
          }
          utilService.saveOrUpdate(dutyRecord);
          utilService.saveOrUpdate(recordDetail);
        ...
        if (dutyStatusId != null) {
          if (dutyStatusId.equals(AttendanceType.presence)) this.setDutyRecordProperty(dutyRecord,
              ...
              "absenteeismCount", 1);
          else if (dutyStatusId.equals(AttendanceType.askedForLeave)) this.setDutyRecordProperty(dutyRecord,
              "askedForLeaveCount", 1);
          else if (dutyStatusId.equals(AttendanceType.late)) this.setDutyRecordProperty(dutyRecord,
              "lateCount", 1);
          else if (dutyStatusId.equals(AttendanceType.leaveEarly)) this.setDutyRecordProperty(dutyRecord,
              "leaveEarlyCount", 1);
  ...
  public void updateRecordDetail(Long recordDetailId, Long dutyStatusId) {
    RecordDetail recordDetail = (RecordDetail) utilService.get(RecordDetail.class, recordDetailId);
    updateRecordDetail(recordDetail, dutyStatusId);
  }
  ...
  private void updateRecordDetail(RecordDetail recordDetail, Long dutyStatusId) {
    if (recordDetail == null) { throw new RuntimeException("recordDetail==null\n"); }
    DutyRecord dutyRecord = recordDetail.getDutyRecord();
    AttendanceType oldDutyStatus = recordDetail.getDutyStatus();
    if (oldDutyStatus != null) {
      if (oldDutyStatus.getId().equals(AttendanceType.presence)) this.setDutyRecordProperty(dutyRecord,
          "dutyCount", -1);
      else if (oldDutyStatus.getId().equals(AttendanceType.absenteeism)) this.setDutyRecordProperty(
          dutyRecord, "absenteeismCount", -1);
      else if (oldDutyStatus.getId().equals(AttendanceType.askedForLeave)) this.setDutyRecordProperty(
          dutyRecord, "askedForLeaveCount", -1);
      else if (oldDutyStatus.getId().equals(AttendanceType.late)) this.setDutyRecordProperty(dutyRecord,
          "lateCount", -1);
      else if (oldDutyStatus.getId().equals(AttendanceType.leaveEarly)) this.setDutyRecordProperty(
          dutyRecord, "leaveEarlyCount", -1);
    }
    if (dutyStatusId != null) {
      if (dutyStatusId.equals(AttendanceType.presence)) this
          .setDutyRecordProperty(dutyRecord, "dutyCount", 1);
      else if (dutyStatusId.equals(AttendanceType.absenteeism)) this.setDutyRecordProperty(dutyRecord,
          "absenteeismCount", 1);
      else if (dutyStatusId.equals(AttendanceType.askedForLeave)) this.setDutyRecordProperty(dutyRecord,
          "askedForLeaveCount", 1);
      else if (dutyStatusId.equals(AttendanceType.late)) this.setDutyRecordProperty(dutyRecord, "lateCount",
          1);
      else if (dutyStatusId.equals(AttendanceType.leaveEarly)) this.setDutyRecordProperty(dutyRecord,
          "leaveEarlyCount", 1);
    }
  ...
  public void deleteRecordDetail(Long studentId, Long teachTaskId, Date day, Long beginUnitId, Long endUnitId) {
  ...
  private void deleteRecordDetail(RecordDetail recordDetail) {
    ...
    AttendanceType dutyStatus = recordDetail.getDutyStatus();
    if (dutyStatus != null) {
      if (dutyStatus.getId().equals(AttendanceType.presence)) this.setDutyRecordProperty(dutyRecord,
          "dutyCount", -1);
      else if (dutyStatus.getId().equals(AttendanceType.absenteeism)) this.setDutyRecordProperty(dutyRecord,
          "absenteeismCount", -1);
      else if (dutyStatus.getId().equals(AttendanceType.askedForLeave)) this.setDutyRecordProperty(
          dutyRecord, "askedForLeaveCount", -1);
      else if (dutyStatus.getId().equals(AttendanceType.late)) this.setDutyRecordProperty(dutyRecord,
          "lateCount", -1);
      else if (dutyStatus.getId().equals(AttendanceType.leaveEarly)) this.setDutyRecordProperty(dutyRecord,
          "leaveEarlyCount", -1);
    }
    utilService.saveOrUpdate(dutyRecord);
  ...
  private void setDutyRecordProperty(DutyRecord dutyRecord, String propertyName, int number) {
    ...
    if (propertyObject == null) {
      ...
      try {
        PropertyUtils.setProperty(dutyRecord, propertyName, new Integer(propertyValue.intValue() + number));
      } catch (Exception e) {
        throw new RuntimeException("dutyRecord set property exception");
      }
    }
    dutyRecord.setTotalCount(dutyRecord.getTotalCount() == null ? new Integer(number) : new Integer(
        dutyRecord.getTotalCount().intValue() + number));
  }
