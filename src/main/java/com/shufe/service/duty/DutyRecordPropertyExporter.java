...
public class DutyRecordPropertyExporter implements PropertyExtractor {
  ...
  public Object getPropertyValue(Object target, String property) throws Exception {
    ...
    for (int i = 0; i < subProperties.length - 1; i++) {
      if (null != PropertyUtils.getProperty(record, passedProperty.toString())) passedProperty.append(".")
          .append(subProperties[i + 1]);
      else return "";
    }
    try {
      Object value = PropertyUtils.getProperty(target, property);
      if (value instanceof Boolean && null != locale && null != resourses) {
        if (value.equals(Boolean.TRUE)) return resourses.getMessage(locale, "common.yes");
        else return resourses.getMessage(locale, "common.no");
      } else return value;
    } catch (Exception e) {
