...
public class Speciality2ndSignUpServiceImpl extends BasicService implements Speciality2ndSignUpService {
  ...
  public void autoMatriculate(SignUpSetting setting, MatriculateParams params) throws IOException {
    ...
    while (!signUpStds.isEmpty()) {
      SignUpStudent signUpStudent = (SignUpStudent) Collections.max(signUpStds, new BeanComparator(
          "matriculateGPA"));
    ...
    if (Boolean.TRUE.equals(params.getAdjustable())) {
      if (!toAdjustList.isEmpty()) {
        for (Iterator iter = setting.getSpecialitySettings().iterator(); iter.hasNext();) {
          ...
          if (toAdjustList.isEmpty()) break;
        }
      }
    }
    if (logger.isInfoEnabled()) {
      logger.info("matriculate finish! ready to synchronize to db:[Matriculate:" + matriculatedRecords.size()
          + ",adjust:" + adjustSignUpStds.size() + "]");
    }
  ...
  public void statStdCountOf(Collection specialitySettings) {
    if (CollectionUtils.isEmpty(specialitySettings)) return;
    // 查询报名人数和录取人数
  ...
  public List statSignUpAndMatriculate(SignUpSetting setting) {
    ...
    for (int i = 0; i < setting.getChoiceCount().intValue(); i++) {
      sb.append(",(select count(*) from SignUpStudentRecord r where r.specialitySetting.id =setting.id and r.rank="
          + (i + 1) + ")");
      sb.append(",(select count(*) from SignUpStudentRecord r where r.specialitySetting.id =setting.id and r.status=true and r.rank="
          + (i + 1) + ")");
    }
    // 总报名人数
    sb.append(",(select count(*) from SignUpStudentRecord r where r.specialitySetting.id =setting.id)");
    // 总录取人数
    sb.append(",(select count(*) from SignUpStudent r where r.matriculated.id =setting.id)");
    // 调剂人数
    sb.append(",(select count(*) from SignUpStudent r where r.matriculated.id =setting.id and isAdjustMatriculated=true)");

    sb.append(" from SignUpSpecialitySetting setting where setting.setting.id=" + setting.getId());
  ...
  public List getCorrespondingAdminClasses(SignUpSpecialitySetting specialitySetting) {
    ...
    query.add(new Condition("adminClass.enrollYear=:enrollYear", specialitySetting.getSetting()
        .getEnrollTurn()));
    query.add(new Condition("adminClass.speciality=:speciality", specialitySetting.getSpeciality()));
  ...
  public List getUnsaturatedSpecialitySettings(SignUpSetting setting) {
    ...
    CollectionUtils.filter(specialitySettings,
        new BeanPredicate("isSaturated", EqualPredicate.getInstance(Boolean.FALSE)));
  ...
  public void cancelMatriculate(SignUpStudent signUpStd) {
    ...
    if (null != adminClass) {
      ...
      // 如果在校，减少班级的在校人数
      if (std.isInSchool()) {
        adminClass.setActualStdCount(new Integer(adminClass.getActualStdCount().intValue() - 1));
      }
      // 如果有效，降低班级的学籍有效人数
