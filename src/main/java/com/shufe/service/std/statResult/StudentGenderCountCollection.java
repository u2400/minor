...
public class StudentGenderCountCollection {
    ...
    if (studentGenderCount != null) {
      ...
      for (Iterator iter = this.data.iterator(); iter.hasNext();) {
        ...
        if (data.equals(element.get(dataName))) {
          studentGenderCount.add((Gender) element.get("student_basicInfo_gender"),
  ...
  public List getList(List propertyList, Integer index, String sort) {
    ...
    BeanComparator beanComparator = new BeanComparator(sort, comparator) {
      ...
      public int compare(Object o1, Object o2) {
        try {
          ...
          if (getProperty() == null) { return getComparator().compare(o1, o2); }
          Object value1 = null;
          Object value2 = null;
          try {
    ...
    if (index.intValue() == propertyList.size()) { return new ArrayList(); }
    MultiValueMap resultMap = new MultiValueMap();
    for (Iterator iter = this.data.iterator(); iter.hasNext();) {
      Map element = (Map) iter.next();
    ...
    for (Iterator iter = keySet.iterator(); iter.hasNext();) {
      Object keyTemp = (Object) iter.next();
      resultList.add(new StudentGenderCountCollection(keyTemp, ((BaseValueBean) propertyList.get(index
          .intValue())).getDataName(), index, (List) resultMap.get(keyTemp)));
    }
  ...
  public Integer getSize(List propertyList, Integer fromIndex, Integer toIndex) {
    if (this.sizeShow != null) { return this.sizeShow; }
    if (fromIndex == null) {
      return new Integer(0);
    } else {
      if (toIndex != null && fromIndex.compareTo(toIndex) >= 0) { return new Integer(1); }
    }
    int countTemp = 0;
   ...
   * }else{
   * studentGenderCount = new StudentGenderCount();
   * for (Iterator iter = this.getList(propertyList, new
   * Integer(this.index.intValue()+1)).iterator(); iter.hasNext();) {
   * StudentGenderCountCollection element = (StudentGenderCountCollection) iter.next();
  ...
  public boolean equals(Object object) {
    if (!(object instanceof StudentGenderCountCollection)) { return false; }
