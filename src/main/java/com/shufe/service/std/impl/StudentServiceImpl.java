...
public class StudentServiceImpl extends BasicService implements StudentService {
  ...
  public Student updateStudent(Student student, StudentStatusInfo statusInfo, BasicInfo basicInfo,
      AbroadStudentInfo abroadStudentInfo) {
    EntityUtils.evictEmptyProperty(student);
  ...
  public Pagination searchStudent(Student std, int pageNo, int pageSize) {
    return studentDAO.dynaSearch(studentDAO.constructStudentCriteria(std, getDefaultSortList()), pageNo,
        pageSize);
  }

  private Pagination searchStudent(Student std, int pageNo, int pageSize, String studentGraduateAuditStatus) {
    Criteria criteria = constructStudentCriteriaWithGraduateAuditStatus(std, studentGraduateAuditStatus);
  ...
  private Criteria constructStudentCriteriaWithGraduateAuditStatusArray(Student std,
      String[] studentGraduateAuditStatus) {
    if (studentGraduateAuditStatus.length < 1) return studentDAO.constructStudentCriteria(std,
        getDefaultSortList());
    String[] studentGraduateAuditStatusArray = StringUtils.split(studentGraduateAuditStatus[0], ",");
    Criteria criteria = studentDAO.constructStudentCriteria(std, getDefaultSortList());
    addAuditStatus(studentGraduateAuditStatusArray, criteria, "graduateAuditStatus");
    if (studentGraduateAuditStatus.length < 2) return criteria;
    ...
    if (!ArrayUtils.isEmpty(statusArray)) {
      ...
      for (int i = 0; i < statusArray.length; i++) {
        if (ArrayUtils.contains(str, statusArray[i])) {
          Criterion criterion = Restrictions.eq(statusName,
              ConvertUtils.convert(statusArray[i], Boolean.class));
  ...
  public Pagination searchStudent(Student student, int pageNo, int pageSize, String studentTypeIds,
  ...
  public Pagination searchSecondSpecialityStudent(Student student, int pageNo, int pageSize,
      String studentTypeIds, String departmentIds, String[] secondSpecialityStudentGraduateAuditStatus) {
    this.filterAuthorityByDepartment(departmentIds);
    this.filterAuthorityByStudentType(studentTypeIds);
  ...
  public void batchUpdateGraduateAuditStatus(Long[] studentIdArray, Boolean status) {
    ...
    utilService.update(Student.class, "id", studentIdArray, valuesMap);
  }
  ...
  public void batchUpdateGraduateAuditStatus(Collection stdIds, Boolean status) {
    Map valuesMap = new HashMap(1);
    valuesMap.put("graduateAuditStatus", status);
    utilService.update(Student.class, "id", stdIds.toArray(), valuesMap);
  }

  public void batchUpdateSecondGraduateAuditStatus(Collection stdId, Boolean status) {
    Map valuesMap = new HashMap(1);
    valuesMap.put("secondGraduateAuditStatus", status);
    utilService.update(Student.class, "id", stdId.toArray(), valuesMap);
  }
  ...
  public Pagination searchStudentClass(AdminClass adminClass, int pageNo, int pageSize,
    ...
    this.filterAuthorityByDepartment(departmentIds);
    this.filterAuthorityByStudentType(studentTypeIds);
    return studentDAO.dynaSearch(studentDAO.constructStudentClassCriteria(adminClass), pageNo, pageSize);
  }
  ...
  public List searchStudent(Student student, String studentTypeIds, String departmentIds,
    ...
    this.filterAuthorityByDepartment(departmentIds);
    this.filterAuthorityByStudentType(studentTypeIds);
    Criteria criteria = constructStudentCriteriaWithGraduateAuditStatus(student, studentGraduateAuditStatus);
    return criteria.list();
  }

  public List searchSecondSpecialityStudent(Student student, String studentTypeIds, String departmentIds,
      String[] studentGraduateAuditStatus) {
    this.filterAuthorityByDepartment(departmentIds);
    this.filterAuthorityByStudentType(studentTypeIds);
    studentDAO.enbleFilter("filterSecondMajorStudent", null, null);
  ...
  public void batchCountActualStdCount(Long[] adminClassIds) {
    ...
    for (Iterator iter = list.iterator(); iter.hasNext();) {
      ...
      utilService.update(AdminClass.class, "id", new Object[] { element.get("adminClassId") },
          new String[] { "actualStdCount" }, new Object[] { element.get("studentSetSize") });
    }
  }
  ...
  public List getAlterationReasonList(Long alterationTypeId) {
    List alterationReasonList = utilService.load(AlterReason.class, "alterMode.id", alterationTypeId);
  ...
  public void updateStudentAdminClass(Long stdId, Long[] adminClassIdArray, Boolean is2ndMajor) {
  ...
  public Collection search2ndSpecialityStudent(Student student, PageLimit pageLimit, String studentTypeIds,
      String firstDepartmentIds, String secondDepartmentIds, String andOr) {
    this.filterAuthorityByStudentType(studentTypeIds);
    ...
    if (CollectionUtils.isNotEmpty(student.getAdminClasses())) {
      ...
      studentQuery.add(new Condition("adminClass in (:adminClasses)", student.getAdminClasses()));
      if (student.getAdminClasses().size() == 2) {
        ...
        studentQuery.add(new Condition("exists (from AdminClass class"
            ...
            + ((AdminClass) adminClasses.get(0)).getId() + ".students std where std.id=student.id and class"
            + ((AdminClass) adminClasses.get(0)).getId() + ".id = :classId"
            + ((AdminClass) adminClasses.get(0)).getId() + ")" + " and exists (from AdminClass class"
            + ((AdminClass) adminClasses.get(1)).getId() + " join class"
            + ((AdminClass) adminClasses.get(1)).getId() + ".students std where std.id=student.id and class"
            + ((AdminClass) adminClasses.get(1)).getId() + ".id = :classId"
            + ((AdminClass) adminClasses.get(1)).getId() + ")", ((AdminClass) adminClasses.get(0)).getId(),
    ...
    if (!ArrayUtils.isEmpty(firstDepartmentIdArray) && !ArrayUtils.isEmpty(secondDepartmentIdArray)) {
      studentQuery.add(new Condition("(student.firstMajor.department.id in (:firstMajor_department_id)) "
          + andOr + " (student.secondMajor.department.id in (:secondMajor_department_id))",
      ...
      if (!ArrayUtils.isEmpty(secondDepartmentIdArray)) {
        studentQuery.add(new Condition("student.secondMajor.department.id in (:secondMajor_department_id)",
  ...
  private List getDefaultSortList() {
    ...
    sortList.add(new com.ekingstar.commons.query.Order("enrollYear", com.ekingstar.commons.query.Order.DESC));
