...
public class DegreeCourseChecker implements RuleExecutor {
  ...
  public boolean execute(Context context) {
    ...
    for (Iterator iter = degreePlanCourse.iterator(); iter.hasNext();) {
      ...
      List gradeList = gradeService.getCourseGradeOfDegree(result.getStd().getId(), planCourse.getCourse()
          .getId());
      // 判断对应学位课程的成绩是否 为空 或为 多个
      if (null == gradeList || gradeList.size() != 1) {
        info.setDescription(info.getDescription() + " 该学位课程成绩不存在或多次修读(" + planCourse.getCourse().getName()
            + ")");
        info.setPass(false);
        ...
        if (courseGrade.getIsPass() == Boolean.TRUE) {
          if (null != oneOff && oneOff == Boolean.TRUE) {
            ...
            // 补考成绩
            ExamGrade makeup = courseGrade.getExamGrade(new GradeType(GradeType.MAKEUP));
          ...
          if ("51".equals(result.getStd().getType().getId())) {
            if (null != degreeMap.get("11009")) {
              ...
              if (null != degreeMap.get("11010") && null != degreeMap.get("11011")) {
                List degreeCourse1 = gradeService.getCourseGradeOfDegree(result.getStd().getId(),
                    Long.valueOf((String) degreeMap.get("11010")));
                List degreeCourse2 = gradeService.getCourseGradeOfDegree(result.getStd().getId(),
                    Long.valueOf((String) degreeMap.get("11011")));
                if (degreeCourse1.size() != 1 || degreeCourse2.size() != 1) {
                  info.setDescription(info.getDescription() + " 该学位课程中级财务会计1和2成绩不存在或多次修读");
                  info.setPass(false);
                  result.getDegreeAuditInfos().put(degreeAuditContext.getRuleConfig().getId(), info);
                  return false;
                } else {
                  ...
                  if (course1.getGA().floatValue() + course2.getGA().floatValue() < point.floatValue()) {
                    info.setDescription(info.getDescription() + " 该学位课程中级财务会计1和2成绩之和小于:" + point.floatValue());
                    info.setPass(false);
                    result.getDegreeAuditInfos().put(degreeAuditContext.getRuleConfig().getId(), info);
                    return false;
                  }
          ...
          if ("53".equals(result.getStd().getType().getId())) {
            if (null != degreeMap.get("11068") && null != degreeMap.get("11069")) {
              List degreeCourse1 = gradeService.getCourseGradeOfDegree(result.getStd().getId(),
                  Long.valueOf((String) degreeMap.get("11068")));
              List degreeCourse2 = gradeService.getCourseGradeOfDegree(result.getStd().getId(),
                  Long.valueOf((String) degreeMap.get("11069")));
              if (degreeCourse1.size() != 1 || degreeCourse2.size() != 1) {
                info.setDescription(info.getDescription() + " 该学位课程中级财务会计1和2成绩不存在或多次修读");
                info.setPass(false);
                result.getDegreeAuditInfos().put(degreeAuditContext.getRuleConfig().getId(), info);
