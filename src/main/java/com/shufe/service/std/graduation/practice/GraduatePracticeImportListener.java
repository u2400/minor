...
public class GraduatePracticeImportListener extends ItemImporterListener {
  ...
  public GraduatePracticeImportListener(UtilDao utilDao, TeachCalendarDAO teachCalendarDAO) {
    super();
  ...
  public void startTransferItem(TransferResult tr) {
    ...
    if (StringUtils.isNotBlank(String.valueOf(data[0]))) {
      ...
      entityQuery.add(Condition.eq("graduatePractice.student.code", String.valueOf(data[0])));
  ...
  public void endTransferItem(TransferResult tr) {
    GraduatePractice graduatePractice = (GraduatePractice) importer.getCurrent();
    int errors = tr.errors();
    if (!ValidEntityPredicate.INSTANCE.evaluate(graduatePractice.getStudent())) {
      tr.addFailure("error.parameters.illegal", importer.curDataMap().get("std.code"));
    } else {
      // 查找教学日历
      String calendarMapId = graduatePractice.getStudent().getType().getId() + "_"
          + graduatePractice.getTeachCalendar().getYear() + "_"
          + graduatePractice.getTeachCalendar().getTerm();

      TeachCalendar calendar = (TeachCalendar) calendarMap.get(calendarMapId);
      if (null == calendar) {
        calendar = teachCalendarDAO.getTeachCalendar(graduatePractice.getStudent().getType().getId(),
            graduatePractice.getTeachCalendar().getYear(), graduatePractice.getTeachCalendar().getTerm());
        if (null == calendar) {
          tr.addFailure("error.parameters.illegal", importer.curDataMap().get("calendar.year") + " "
      ...
      if (null != graduatePractice.getPracticeTeacher()) {
        if (!ValidEntityPredicate.INSTANCE.evaluate(graduatePractice.getPracticeTeacher())) {
          if (StringUtils.isNotEmpty(graduatePractice.getPracticeTeacher().getName())) {
            nameCondition.setValues(Collections.singletonList(graduatePractice.getPracticeTeacher().getName()
                .trim()));
            List teachers = (List) utilDao.search(teacherQuery);
            if (teachers.size() == 1) {
              graduatePractice.setPracticeTeacher((Teacher) teachers.get(0));

              // 如果学生的导师也没有，则更新学生的导师信息
              Teacher stdTeacher = null;
              if (MajorType.FIRST.equals(graduatePractice.getMajorType().getId())) {
                stdTeacher = graduatePractice.getStudent().getTeacher();
              } else {
                stdTeacher = graduatePractice.getStudent().getTutor();
              }
              if (null == stdTeacher) {
                if (MajorType.FIRST.equals(graduatePractice.getMajorType().getId())) {
                  graduatePractice.getStudent().setTeacher(graduatePractice.getPracticeTeacher());
                } else {
                  graduatePractice.getStudent().setTutor(graduatePractice.getPracticeTeacher());
                }
                utilDao.saveOrUpdate(graduatePractice.getStudent());
              }
            }
          }
          if (!ValidEntityPredicate.INSTANCE.evaluate(graduatePractice.getPracticeTeacher())) {
    ...
    if (tr.errors() == errors) {
      graduatePractice.setPracticeTeacher(graduatePractice.getStudent().getTeacher());
      if (null == graduatePractice.getPracticeSource()) {
        graduatePractice.setPracticeSource(new PracticeSource(PracticeSource.SELF_DECIDE));
      }
