...
public class PunishmentChecker implements RuleExecutor {
  ...
  public boolean execute(Context context) {
    ...
    if (null != getLowestPunishType()) {
      PunishmentType punishmentType = (PunishmentType) utilService.load(PunishmentType.class,
          lowestPunishType.getEntityId());
      if ((null != result.getPunishmentType())
          && (result.getPunishmentType().equals(punishmentType) || result.getPunishmentType().isSeriousThan(
              punishmentType))) {
        info.setPass(false);
