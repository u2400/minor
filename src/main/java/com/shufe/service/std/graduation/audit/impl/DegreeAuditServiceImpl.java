...
public class DegreeAuditServiceImpl extends BasicService implements DegreeAuditService {
  ...
  protected Float stateCourseCredits(AuditResult result) {
    List resultList = graduateAuditService.getStudentTeachPlanAuditDetail(result.getStd(), null, null,
        new ArrayList(), Boolean.TRUE, Boolean.TRUE);
    TeachPlanAuditResult rs = (TeachPlanAuditResult) resultList.get(0);
    Float completed = rs.getCreditAuditInfo().getCompleted();
    return completed == null ? 0.0f : completed;
  }
  ...
  private void populateAuditResult(AuditResult result, DegreeAuditStandard standard) {
    ...
    if (null != standard.getGPA()) {
      result
          .setGPA(gradePointService.statStdGPA(result.getStd(), null, standard.getMajorType(), Boolean.TRUE));
    }
    // 查找处分
    ...
    if (Boolean.TRUE.equals(standard.getIsPassDoctorComprehensiveExam())) {
      result.setIsPassDoctorComprehensiveExam(Boolean.valueOf(otherGradeService.isPass(result.getStd(),
  ...
  private AuditResult getAuditResult(Student std, MajorType majorType) {
    for (Iterator iterator = std.getAuditResults().iterator(); iterator.hasNext();) {
      ...
      if (one.getMajorType().equals(majorType)) { return one; }
    }
    return null;
  }
