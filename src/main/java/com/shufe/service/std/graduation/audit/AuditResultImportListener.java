...
public class AuditResultImportListener extends ItemImporterListener {
  ...
  public void startTransferItem(TransferResult tr) {
    ...
    query.add(new Condition("audit.majorType.code=:majorTypeCode", datas.get("majorType.code").toString()));
  ...
  public void endTransferItem(TransferResult tr) {
    ...
    String graduateOn = (String) importer.curDataMap().get("std.degreeInfo.graduateOn");
    if (StringUtils.isNotBlank(graduateOn)) {
      if (auditResult.getStd().getDegreeInfo() == null) {
        auditResult.getStd().setDegreeInfo(new DegreeInfo());
      }
      auditResult.getStd().getDegreeInfo().setGraduateOn(graduateOn);
    }
