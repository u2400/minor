...
public class FeeDefaultServiceImpl extends BasicService implements FeeDefaultService {
  ...
  public FeeDefault getFeeDefault(Student std1, FeeType type) {
    ...
    List feeDefaults = feeDefaultDAO.getFeeDefaults(type,
        studentTypeService.getStdTypesUp(std.getType().getId()), std.getDepartment(), std.getFirstMajor());
    if (feeDefaults.isEmpty()) return null;
