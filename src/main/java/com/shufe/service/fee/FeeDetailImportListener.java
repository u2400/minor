...
public class FeeDetailImportListener extends ItemImporterListener {
  ...
  public void endTransferItem(TransferResult tr) {
    ...
    if (feeDetail.getToRMB() == null) {
      Float value = new Float(feeDetail.getPayed().floatValue() * feeDetail.getRate().intValue());
  ...
  private void processTeachCalendar(TransferResult tr, FeeDetail feeDetail) {
    String calendarMapId = feeDetail.getStd().getType().getId() + "_" + feeDetail.getCalendar().getYear()
        + "_" + feeDetail.getCalendar().getTerm();

    TeachCalendar calendar = (TeachCalendar) calendarMap.get(calendarMapId);
    if (null == calendar) {
      calendar = teachCalendarDAO.getTeachCalendar(feeDetail.getStd().getType().getId(), feeDetail
          .getCalendar().getYear(), feeDetail.getCalendar().getTerm());
      if (null == calendar) {
    ...
    if (null == calendar) {
      tr.addFailure("can't findt teachCalendar with std code", feeDetail.getStd().getType().getId() + ":"
  ...
  protected boolean isValidInvoiceCode(TransferResult tr, String invoiceCode, int flag) {
    ...
    if (checkEmpty(tr, invoiceCode, "invoiceCode is null")) { return false; }
    ...
    for (int i = 0; i < invoiceCode.length(); i++) {
      ...
      case FIRSTLETTERCHAIN:
        if (i == 0 && (invoiceCode.charAt(i) >= 'A' && invoiceCode.charAt(i) <= 'Z') == false) { return false; }
      case LETTERNUMBERCHAIN:
        ...
        if (isLetter == false && isNumber == false) { return false; }
        break;
