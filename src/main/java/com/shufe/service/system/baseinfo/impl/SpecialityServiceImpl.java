...
public class SpecialityServiceImpl extends BasicService implements SpecialityService {
  ...
  public Speciality getSpeciality(String code) {
    ...
    else return (Speciality) rs.get(0);
  }
  ...
  public Pagination getSpecialities(Speciality speciality, String stdTypeIdSeq, String departIdSeq,
      int pageNo, int pageSize) {
    if (StringUtils.isEmpty(stdTypeIdSeq) || StringUtils.isEmpty(departIdSeq) || pageNo < 1 || pageSize < 1) return new Pagination(
        new Result(0, Collections.EMPTY_LIST));
    else return specialityDAO.getSpecialities(speciality, SeqStringUtil.transformToLong(stdTypeIdSeq),
  ...
  public List getSpecialities(Speciality speciality, String stdTypeIdSeq, String departIdSeq) {
    if (StringUtils.isEmpty(stdTypeIdSeq) || StringUtils.isEmpty(departIdSeq)) return Collections.EMPTY_LIST;
    else return specialityDAO.getSpecialities(speciality, SeqStringUtil.transformToLong(stdTypeIdSeq),
  ...
  public List getSpecialitiesByDepartmentIds(String departmentIds) {
    if (StringUtils.isEmpty(departmentIds)) return null;
    else return specialityDAO.getSpecialitiesByDepartmentIds(SeqStringUtil.transformToLong(departmentIds));
  }
  ...
  public Collection getSpecialities(Boolean state, MajorType majorType) {
    ...
    query.add(new Condition("speciality.majorType.id = (:majorTypeId)",
        (null == majorType || null == majorType.getId()) ? MajorType.FIRST : majorType.getId()));
