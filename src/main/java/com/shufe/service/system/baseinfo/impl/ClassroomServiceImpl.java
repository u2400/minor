...
public class ClassroomServiceImpl extends BasicService implements ClassroomService {
   ...
   */
  public Pagination getClassrooms(Classroom classroom, String departIdSeq, int pageNo, int pageSize) {
    if (pageNo > 0 && pageSize > 0) return classroomDAO.getClassrooms(classroom,
        SeqStringUtil.transformToLong(departIdSeq), pageNo, pageSize);
   ...
   *      java.lang.String, int, int)
   */
  public Pagination getAllClassrooms(Classroom classroom, String departIdSeq, int pageNo, int pageSize) {
    if (pageNo > 0 && pageSize > 0) return classroomDAO.getAllClassrooms(classroom,
        SeqStringUtil.transformToLong(departIdSeq), pageNo, pageSize);
    else return new Pagination(new Result(0, Collections.EMPTY_LIST));
  }
  ...
  public List getClassrooms(Classroom classroom, String departIdSeq) {
    if (StringUtils.isNotEmpty(departIdSeq)) return classroomDAO.getClassrooms(classroom,
  ...
  public List getClassrooms(Collection configTypeList, String departIdSeq) {
    if (StringUtils.isNotEmpty(departIdSeq)) return classroomDAO.getClassrooms(configTypeList,
        SeqStringUtil.transformToLong(departIdSeq));
    return Collections.EMPTY_LIST;
  }
  ...
  public List getClassrooms(String departIdSeq) {
    return StringUtils.isNotEmpty(departIdSeq) ? classroomDAO.getClassrooms(SeqStringUtil
        .transformToLong(departIdSeq)) : Collections.EMPTY_LIST;
  }
  ...
  public List getClassrooms(Collection roomIds) {
    if (!roomIds.isEmpty()) return classroomDAO.getClassrooms(roomIds);
  ...
  public void saveOrUpdate(Classroom classroom) throws PojoExistException {
    try {
      ...
      throw new PojoExistException("classroom already exits:" + classroom);
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.service.system.baseinfo.ClassroomService#getClassroomIdsByClassroom(com.shufe.model
   * .system.baseinfo.Classroom,
   * java.lang.String)
   */
