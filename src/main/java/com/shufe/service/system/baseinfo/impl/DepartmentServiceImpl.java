...
public class DepartmentServiceImpl extends BasicService implements DepartmentService {
  ...
  public List getDepartments(Long[] ids) {
    ...
    else return departmentDAO.getDepartments(ids);
  }
  ...
  public List getTeachDeparts(String idSeq) {
    if (StringUtils.isEmpty(idSeq)) return Collections.EMPTY_LIST;
    else {
      return departmentDAO.getTeachDeparts(SeqStringUtil.transformToLong(idSeq));
    }
  }
  ...
  public Pagination getAllDepartments(Department department, int pageNo, int pageSize) {
    return departmentDAO.getAllDepartments(department, pageNo, pageSize);
  }
  ...
  public Collection getRelatedDeparts(String stdTypeIds) {
    ...
    entityQuery.join("depart.stdTypes", "stdType").add(
        new Condition("stdType.id in (:stdTypeIds)", SeqStringUtil.transformToLong(stdTypeIds)));
  ...
  public void saveOrUpdate(Department department) throws PojoExistException {
    try {
      ...
      throw new PojoExistException("department already exits:" + department);
  ...
  public List getTeachDeparts() {
    ...
    query.add(new Condition("department.isTeaching=:teaching", Boolean.TRUE));
