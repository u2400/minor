...
public class TeacherServiceImpl extends BasicService implements TeacherService {
  ...
  public Teacher getTeacherByNO(String teacherNO) {
    if (StringUtils.isEmpty(teacherNO)) return null;
    else return teacherDAO.getTeacherByNO(teacherNO);
  }
  ...
  public List getTeachersByDepartment(String departIds) {
    if (StringUtils.isEmpty(departIds)) return Collections.EMPTY_LIST;
    else return getTeachersByDepartment(SeqStringUtil.transformToLong(departIds));
  }
  ...
  public List getTeachersByDepartment(Long[] departIds) {
    if (null == departIds || departIds.length < 1) return Collections.EMPTY_LIST;
    else return teacherDAO.getTeachersByDepartment(departIds);
  }
  ...
  public List getTeachersById(Long[] teacherIds) {
    if (null == teacherIds || teacherIds.length < 1) return Collections.EMPTY_LIST;
    else return teacherDAO.getTeachersById(teacherIds);
  }
  ...
  public List getTeachersById(Collection teacherIds) {
    if (!teacherIds.isEmpty()) return teacherDAO.getTeachersById(teacherIds);
  ...
  public List getTeachersByNO(String[] teacherNOs) {
    if (null == teacherNOs || teacherNOs.length < 1) return Collections.EMPTY_LIST;
    else return teacherDAO.getTeachersByNO(teacherNOs);
  }
  ...
  public List getTeacherNames(String teacherIdSeq) {
    if (StringUtils.isEmpty(teacherIdSeq)) return Collections.EMPTY_LIST;
    else return teacherDAO.getTeacherNames(SeqStringUtil.transformToLong(teacherIdSeq));
  }
   ...
   */
  public Pagination getTeachers(Teacher teacher, Long[] departIds, int pageNo, int pageSize) {
    if (pageNo < 1 || pageSize < 1) return new Pagination(new Result(0, Collections.EMPTY_LIST));
    else return teacherDAO.getTeachers(teacher, departIds, pageNo, pageSize);
  }
   ...
   *      java.lang.String, int, int)
   */
  public Pagination getTeachers(Teacher teacher, String departIdSeq, int pageNo, int pageSize) {
    if (StringUtils.isEmpty(departIdSeq) || pageNo < 1 || pageSize < 1) return new Pagination(new Result(0,
        Collections.EMPTY_LIST));
    return teacherDAO.getTeachers(teacher, SeqStringUtil.transformToLong(departIdSeq), pageNo, pageSize);
  }
  ...
  public List getTeachers(Teacher teacher, String departIdSeq) {
    ...
    else return teacherDAO.getTeachers(teacher, SeqStringUtil.transformToLong(departIdSeq));
  }
