...
public class SeqCodeGenerator extends ScriptCodeGenerator {
  ...
  public String gen(Entity entity, CodeScript codeScript) {
    ...
    if (null == codeScript) {
      ...
      codeGenScriptQuery.add(new Condition("codeScript.codeClassName=:codeClassName", EntityUtils
          .getEntityClassName(entity.getClass())));
    ...
    if (StringUtils.contains(getScript(), SEQ)) {
      seqLength = NumberUtils.toInt(StringUtils.substringBetween(getScript(), SEQ + "[", "]"));

      setScript(StringUtils.replace(getScript(),
          SEQ + "[" + StringUtils.substringBetween(getScript(), SEQ + "[", "]") + "]", SEQ));
    }
    String code = super.gen(entity, codeScript);
    List seqs = new ArrayList();
    if (-1 != seqLength) {
      try {
        EntityQuery query = new EntityQuery(Class.forName(codeScript.getCodeClassName()), "entity");
        query.setSelect("select substr(entity." + codeScript.getAttr() + "," + (code.indexOf(SEQ) + 1) + ","
            + seqLength + ")");
        query.add(new Condition(" entity." + codeScript.getAttr() + " like :codeExample", StringUtils
            .replace(code, SEQ, "%")));
