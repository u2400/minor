...
public class NoticeServiceImpl extends BasicService implements NoticeService {
  ...
  public SinglePage getStudentNotices(Student std, PageLimit limit) {
    StudentNotice notice = new StudentNotice();
    List orderList = new ArrayList();
    orderList.add(new Order("notice.isUp", Order.DESC));
    orderList.add(new Order("notice.modifyAt", Order.DESC));
    return noticeDAO.getStudentNotices(notice, std, orderList, limit);
  }
