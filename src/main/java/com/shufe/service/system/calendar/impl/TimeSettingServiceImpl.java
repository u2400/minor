...
public class TimeSettingServiceImpl extends BasicService implements TimeSettingService {

  private TimeSettingDAO settingDAO;

  public TimeSetting getDefaultSetting() {
    return (TimeSetting) utilService.get(TimeSetting.class, TimeSetting.DEFAULT_ID);
  }

  public List getTimeSettingOf(String stdTypeSeq) {
    if (StringUtils.isEmpty(stdTypeSeq)) return Collections.EMPTY_LIST;
    else return getTimeSettingOf(SeqStringUtil.transformToLong(stdTypeSeq));
  }

  public List getTimeSettingOf(Long[] stdTypes) {
    if (stdTypes.length == 0) return Collections.EMPTY_LIST;
    else {
  ...
  public void saveTimeSetting(TimeSetting setting) {
    if (null != setting) settingDAO.saveTimeSetting(setting);
  }
