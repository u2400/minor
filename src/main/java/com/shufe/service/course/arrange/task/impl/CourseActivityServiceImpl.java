 ...
 */
public class CourseActivityServiceImpl extends BasicService implements CourseActivityService {

  private CourseActivityDAO courseActivityDAO;

  public void saveActivities(Collection tasks) {
    if (!tasks.isEmpty()) courseActivityDAO.saveActivities(tasks);
  }

  public void removeActivities(Collection tasks) {
    if (!tasks.isEmpty()) courseActivityDAO.removeActivities(tasks);
  }

  public void removeActivities(Long[] taskIds) {
    if (taskIds.length > 0) courseActivityDAO.removeActivities(taskIds);
  }
  ...
  public List getCourseUnits(Long taskId, Date date) {
    ...
    if (null == task) {
      ...
      if (!calendar.contains(date)) { return Collections.EMPTY_LIST; }
      boolean units[] = new boolean[calendar.getTimeSetting().getCourseUnits().size()];
      ...
      for (Iterator iter = calendar.getTimeSetting().getCourseUnits().iterator(); iter.hasNext();) {
        CourseUnit courseUnit = (CourseUnit) iter.next();
        if (units[courseUnit.getIndex().intValue() - 1]) courseUnits.add(courseUnit);
      }
