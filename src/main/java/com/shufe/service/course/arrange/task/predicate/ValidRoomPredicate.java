...
public class ValidRoomPredicate implements Predicate {
  ...
  public boolean evaluate(Object arg0) {
    ...
    if (null != room.getConfigType() && null != type && room.getConfigType().getId().equals(type.getId())) {
      // 检查容纳听课人数
      if (room.getCapacityOfCourse().intValue() < this.capacityOfCourse) { return false; }
      if (!considerAvailTime) { return true; }
      /*------如果教室没有设置可用时间，则默认所时间都可用--------------*/
      AvailableTime time = room.getAvailableTime();
      if (null == time || StringUtils.isEmpty(time.getAvailable())) { return true; }
      for (int i = 0; i < timeUnits.length; i++) {
        int startIndex = (timeUnits[i].getWeekId().intValue() - 1) * TeachCalendar.MAXUNITS;
        // 取出所要匹配的那一天
        String dayTime = time.getAvailable().substring(startIndex, startIndex + TeachCalendar.MAXUNITS);
        int startUnitIndex = timeUnits[i].getStartUnit().intValue() - 1;
        if (startUnitIndex + timeUnits[i].getUnitCount() > TeachCalendar.MAXUNITS) { return true; }
        if (!dayTime.substring(startUnitIndex, startUnitIndex + timeUnits[i].getUnitCount()).equals(
            StringUtils.repeat("1", timeUnits[i].getUnitCount()))) { return false; }
      }
      return true;
    } else return false;
  }
