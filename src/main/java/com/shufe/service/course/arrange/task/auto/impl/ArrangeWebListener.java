...
public class ArrangeWebListener implements ArrangeListener {
  ...
  public void endArrange(Arrange arrange) {
    try {
      ...
      if (null != arrange.getCurTask()) {
        buf.append(CourseActivityDigestor.digest(arrange.getCurTask(), resourses, locale));
      }
      writer.println(buf.toString());
      writer.flush();
    } catch (Exception e) {
  ...
  public void addFailure(Arrange arrange, GeneralArrangeFailure g) {
    try {
      StringBuffer buf = new StringBuffer(50);
      buf.append(messageOf("common.failure")).append("<br>").append(messageOf("common.cause")).append(":")
          .append(messageOf(g.getFailureKey()));
      writer.println(buf.toString());
      writer.flush();
    } catch (Exception e) {
      logger.debug("[ArrangeWebListener]system error occured");
  ...
  public String messageOf(String key) {
    if (StringUtils.isNotEmpty(key)) return resourses.getMessage(locale, key);
    else return "";
  }
}
