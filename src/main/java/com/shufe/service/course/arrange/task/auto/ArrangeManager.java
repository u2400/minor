...
public class ArrangeManager {
   ...
   * @return
   */
  public ArrangeResult doArrange(Arrange arrange, ArrangeListener optionListener) {
    ArrangeListener[] listeners = new ArrangeListener[] { persistence, optionListener };
  ...
  public Arrange makeArrange(TaskGroup group, ArrangeParams params) {
    ...
    if (Boolean.TRUE.equals(group.getIsSameTime())) {
      ...
      if (Boolean.TRUE.equals(group.getIsClassChange())) {
        ...
        AvailableTime parentTime = (null == group.getParent()) ? null : group.getParent().getSuggest()
            .getTime();
        suite.setAvailableTime(mergeAvailableTime(group.getSuggest().getTime(), parentTime));
        suite.setClassrooms(group.getSuggest().getRooms());
        suite.setFixture(fixture);
        suite.setPriority(group.getPriority().intValue());
        // 添加单独课程
        if (null != group.getDirectTasks() && !group.getDirectTasks().isEmpty()) for (Iterator iter = group
            .getDirectTasks().iterator(); iter.hasNext();) {
          TeachTask task = (TeachTask) iter.next();
          // 如果已经排了，就不再排课了
          if (Boolean.TRUE.equals(task.getArrangeInfo().getIsArrangeComplete())) continue;
          ArrangeCase arrangeCase = new ArrangeCase(task);
          arrangeCase.setSuite(suite);
          // 设置任务排课的建议可用时间
          if (params.getBySuggest()[ArrangeParams.TIME]) {
            if (null == task.getArrangeInfo().getSuggest().getTime()) arrangeCase.setAvailableTime(suite
                .getAvailableTime());
            else {
              arrangeCase.setAvailableTime(task.getArrangeInfo().getSuggest().getTime());
            }
          } else {
        ...
        // 添加嵌套课程组
        if (null != group.getGroups() && !group.getGroups().isEmpty()) for (Iterator iter = group.getGroups()
   ...
   * @return
   */
  private AvailableTime mergeAvailableTime(AvailableTime child, AvailableTime parent) {
    if (null == parent) return child;
    if (null == child) return parent;
    else {
      child.setAvailable(BitStringUtil.andWith(child.getAvailable(), parent.getAvailable()));
      return child;
    }
  }
