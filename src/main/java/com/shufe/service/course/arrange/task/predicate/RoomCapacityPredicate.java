...
public class RoomCapacityPredicate implements Predicate, EvaluateValueRemember {
  ...
  public boolean evaluate(Object object) {
    ...
    for (Iterator iter = task.getArrangeInfo().getSuggest().getRooms().iterator(); iter.hasNext();) {
      ...
      if (room.getCapacityOfCourse().intValue() < task.getTeachClass().getPlanStdCount().intValue()) { return false; }
    }
    return true;
  }
  ...
  public Object getEvaluateValue(Object obj) {
    ...
    for (Iterator iter = task.getArrangeInfo().getSuggest().getRooms().iterator(); iter.hasNext();) {
      Classroom room = (Classroom) iter.next();
      if (room.getCapacityOfCourse().intValue() < task.getTeachClass().getPlanStdCount().intValue()) {
        buf.append(room.getName()).append("(").append(room.getCapacityOfCourse()).append(") ");
      }
    }
    return buf;
  }
