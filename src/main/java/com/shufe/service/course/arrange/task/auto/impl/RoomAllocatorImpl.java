...
public class RoomAllocatorImpl implements RoomAllocator {
   ...
   */
  public Classroom alloc(TimeUnit time, Collection roomScope, Classroom example, boolean considerAvailTime) {
    if (null == time) return null;
    Collection validRooms = CollectionUtils.select(roomScope,
        new ValidRoomPredicate(time, example.getConfigType(), example.getCapacityOfCourse().intValue(),
            considerAvailTime));
    if (validRooms.isEmpty()) return null;
    else {
      return teachResourceDAO.getFreeRoomIn(EntityUtils.extractIds(validRooms), new TimeUnit[] { time },
          example, CourseActivity.class);
    }
  }
   ...
   *      java.util.Collection)
   */
  public Classroom[] alloc(TimeUnit[] times, Collection roomScope, Classroom example,
      boolean considerAvailTime) {
    Classroom[] allocated = new Classroom[times.length];
    Collection validRooms = CollectionUtils.select(roomScope,
        new ValidRoomPredicate(times, example.getConfigType(), example.getCapacityOfCourse().intValue(),
            considerAvailTime));

    if (validRooms.isEmpty()) { return allocated; }
    Classroom firstTryRs = teachResourceDAO.getFreeRoomIn(EntityUtils.extractIds(validRooms), times, example,
    ...
    if (null != firstTryRs) {
      ...
      for (int i = 0; i < times.length; i++)
        allocated[i] = alloc(times[i], roomScope, example, considerAvailTime);
    }
    return allocated;
  }
   ...
   *      Long[],Classroom)
   */
  public Classroom[] alloc(TimeUnit[] times, Long[] roomIds, Classroom example, boolean considerAvailTime) {
   ...
   */
  public Classroom[] allocIgnoreOccupy(TimeUnit[] times, Collection rooms, Classroom room,
      boolean considerAvailTime) {
    Classroom[] allocated = new Classroom[times.length];
    for (int i = 0; i < times.length; i++)
      allocated[i] = allocIgnoreOccupy(times[i], rooms, room, considerAvailTime);
    return allocated;
  }
   ...
   *      java.util.Collection, com.shufe.model.system.baseinfo.Classroom)
   */
  public Classroom allocIgnoreOccupy(TimeUnit time, Collection rooms, Classroom example,
      boolean considerAvailTime) {
    if (null == time) return null;
  ...
  public boolean isOccupied(TimeUnit[] times, Serializable roomId) {
    for (int i = 0; i < times.length; i++)
      if (teachResourceDAO.isRoomOccupied(times[i], roomId)) return true;
    return false;
  }
