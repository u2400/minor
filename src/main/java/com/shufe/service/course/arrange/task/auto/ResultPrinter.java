...
public class ResultPrinter {
  ...
  protected void printHeader(long runTime) {
    getWriter().println();
    getWriter().println("Time: " + ArrangeManager.elapsedTimeAsString(runTime));
  }
  ...
  protected void printDefectTrace(ArrangeFailure booBoo) {
    getWriter().print("[" + booBoo.failedArrange().getName() + "]" + booBoo.thrownException().getMessage());
  }

  protected void printDefects(Enumeration booBoos, int count, String type) {
    if (count == 0) return;
    if (count == 1) getWriter().println("There was " + count + " " + type + ":");
    else getWriter().println("There were " + count + " " + type + "s:");
  ...
  protected void printFooter(ArrangeResult result) {
    if (result.wasSuccessful()) {
      getWriter().println();
      getWriter().print("OK");
      getWriter().println(" (" + result.runCount() + " Arrange" + (result.runCount() == 1 ? "" : "s") + ")");

    } else {
      getWriter().println();
      getWriter().println("FAILURES!!!");
      getWriter().println("Arranges run: " + result.runCount() + ",  Failures: " + result.failureCount());
    }
    getWriter().println();
  }
