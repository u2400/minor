...
public class ArrangeSuite implements Arrange {
  ...
  public Classroom allocRoom(TimeUnit time, Classroom room) {
    ...
    while (classrooms == null || classrooms.isEmpty()) {
      if (null == parentSuite) throw new RoomAllocFailure("info.arrange.noClassroom.avaliable");
      classrooms = parentSuite.getClassrooms();
      parentSuite = parentSuite.getSuite();
    }
    if (fixture.getParams().getDetectCollision()[ArrangeParams.ROOM]) return fixture.getRoomAlloc().alloc(
        time, classrooms, room, getParams().getConsiderAvailTime()[ArrangeParams.ROOM]);
    else return fixture.getRoomAlloc().allocIgnoreOccupy(time, classrooms, room,
        getParams().getConsiderAvailTime()[ArrangeParams.ROOM]);
  }
