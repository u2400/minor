...
public class ExamClassroomComparator implements Comparator {
  ...
  public int compare(Object arg0, Object arg1) {
    ...
    int compare = new CompareToBuilder().append(room0.getSchoolDistrict(), room1.getSchoolDistrict())
        .toComparison();
    if (compare != 0) return compare;
    compare = new CompareToBuilder().append(room0.getBuilding(), room1.getBuilding()).toComparison();
    if (compare != 0) return compare;
    ...
    if (null != room0.getFloor()) {
      if (room0.getFloor().intValue() % 2 == 1) {
        return new CompareToBuilder().append(room0.getName(), room1.getName()).toComparison();
      } else {
        return new CompareToBuilder().append(room1.getName(), room0.getName()).toComparison();
      }
    } else {
      return 0;
    }
