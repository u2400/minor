...
public class RoomApplyServiceImpl extends BasicService implements RoomApplyService {
  ...
  public List getRoomApply(Classroom room, ApplyTime applyTime) {
    ...
    query.add(new Condition(
        "roomApply.applyTime.dateBegin <= :dateEnd  and :dateBeign <= roomApply.applyTime.dateEnd", applyTime
            .getDateEnd(), applyTime.getDateBegin()));
    query.add(new Condition(
        "roomApply.applyTime.timeBegin <= :timeEnd  and :timeBeign <= roomApply.applyTime.timeEnd", applyTime
            .getTimeEnd(), applyTime.getTimeBegin()));
    query.add(new Condition("roomApply.applyTime.cycleCount = :cycleCount", applyTime.getCycleCount()));
    query.add(new Condition("roomApply.applyTime.cycleType = :cycleType", applyTime.getCycleType()));
   ...
   * @param roomApply
   */
  public boolean approve(RoomApply roomApply, Set<Classroom> classrooms) {
    roomApply.setIsApproved(Boolean.TRUE);
    roomApply.setApproveAt(roomApply.getApplyAt());
    roomApply.setApproveBy(roomApply.getDepartApproveBy());
    float all = 0;

    if (!roomApply.getIsFree().booleanValue()) {
    ...
    Integer startTime = new Integer(StringUtils.replace(roomApply.getApplyTime().getTimeBegin(), ":", ""));
    Integer endTime = new Integer(StringUtils.replace(roomApply.getApplyTime().getTimeEnd(), ":", ""));
    ...
    for (int i = 0; i < units.length; i++) {
      ...
      query.add(new Condition("bitand (activity.time.validWeeksNum, :validWeeksNum) > 0", units[i]
          .getValidWeeksNum()));
      ...
      if (CollectionUtils.isNotEmpty(utilService.search(query))) { return false; }
    }
    for (int i = 0; i < units.length; i++) {
      for (Classroom classroom : classrooms) {
        ...
        roomApply.addActivity(activity);
      }
    }
  ...
  public void departApprove(RoomApply roomApply, User approveBy) {
    ...
    roomApply.setDepartApproveAt(new Date());
