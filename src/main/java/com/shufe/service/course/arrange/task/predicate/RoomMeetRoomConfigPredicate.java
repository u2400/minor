...
public class RoomMeetRoomConfigPredicate implements Predicate, EvaluateValueRemember {
  ...
  public boolean evaluate(Object object) {
    ...
    if (task.getArrangeInfo().getSuggest().getRooms().isEmpty()) {
      ...
      for (Iterator iter = task.getArrangeInfo().getSuggest().getRooms().iterator(); iter.hasNext();) {
        ...
        if (!room.getConfigType().equals(task.getRequirement().getRoomConfigType())) { return false; }
      }
    }
    return true;
  }
  ...
  public Object getEvaluateValue(Object obj) {
    ...
    for (Iterator iter = task.getArrangeInfo().getSuggest().getRooms().iterator(); iter.hasNext();) {
      Classroom room = (Classroom) iter.next();
      if (buf.indexOf(room.getConfigType().getName()) == -1) buf.append(room.getConfigType().getName())
          .append(" ");
    }
    return buf;
  }
