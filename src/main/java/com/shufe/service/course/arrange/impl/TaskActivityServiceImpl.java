...
public class TaskActivityServiceImpl extends BasicService implements TaskActivityService {
  ...
  public void adjustTimeUnit(TeachCalendar from, TeachCalendar to) {
    ...
    Collection keeped = shift(activities, fromYear, fromWeekStart, fromWeekEnd, toYear, toWeekStart,
        toWeekEnd, timeSetting, to);
  ...
  public void shift(TeachTask task, int offset) {
    ...
    int fromWeekEnd = from.getWeekStart().intValue() + from.getWeekLength() - 1;
    int fromYear = from.getStartYear();
    Collection shifted = shift(task.getArrangeInfo().getActivities(), fromYear, fromWeekStart, fromWeekEnd,
        fromYear, fromWeekStart + offset, fromWeekEnd + offset, null, from);
    utilDao.saveOrUpdate(shifted);
    // 更新任务
    task.getArrangeInfo().setWeekStart(new Integer(task.getArrangeInfo().getWeekStart().intValue() + offset));
    // 如果开始周调整后小于或等于0,则置为1
    if (task.getArrangeInfo().getWeekStart().intValue() <= 0) {
      task.getArrangeInfo().setWeekStart(new Integer(1));
    }
    if (task.getArrangeInfo().getWeekStart().intValue() > 53) {
      task.getArrangeInfo().setWeekStart(new Integer(53));
    }
   ...
   * @return
   */
  private Collection shift(Collection activities, int fromYear, int fromWeekStart, int fromWeekEnd,
      int toYear, int toWeekStart, int toWeekEnd, TimeSetting timeSetting, TeachCalendar to) {
    ...
    for (Iterator iter = activities.iterator(); iter.hasNext();) {
      ...
      if (activity.getTime().getYear().intValue() == fromYear) {
        ...
        validWeeks = activity.getTime().getValidWeeks() + activity.getTime().getValidWeeks();
      }
      // 截取"有效"的时间占用周
      ...
      if (!StringUtils.contains(toWeeks, '1')) {
        ...
        activity.getTime().setValidWeeks(StringUtils.repeat("0", TeachCalendar.OVERALLWEEKS));
        keeped.add(activity);
      } else {
        if (toWeeks.length() <= TeachCalendar.OVERALLWEEKS) {
          activity.getTime().setValidWeeks(
              toWeeks + StringUtils.repeat("0", TeachCalendar.OVERALLWEEKS - toWeeks.length()));
          if (null != timeSetting) {
          ...
          activity.getTime().setValidWeeks(toWeeks.substring(0, TeachCalendar.OVERALLWEEKS));
          if (null != timeSetting) {
            activity.getTime().setTimeByTimeSetting(timeSetting);
          }
          activity.getTime().setYear(new Integer(toYear));
          ...
          if (-1 != toWeeks.indexOf('1', TeachCalendar.OVERALLWEEKS)) {
            ...
            newActivity.getTime().setValidWeeks(
                toWeeks.substring(TeachCalendar.OVERALLWEEKS)
                    + StringUtils.repeat("0", 2 * TeachCalendar.OVERALLWEEKS - toWeeks.length()));
            ...
            if (!newActivity.isPO()) keeped.add(newActivity);
          }
        }
      }
    }
  ...
  public List detectClassCollision(TeachCalendar calendar, Class activityClass) {
    ...
    Condition timeCondition = new Condition("activity.time.year=:year"
        + " and bitand(activity.time.validWeeksNum,:validWeeksNum)>0" + " and activity.time.weekId=:weekId "
        + " and activity.time.endUnit>=:startUnit" + " and activity.time.startUnit<=:endUnit "
  ...
  public List detectTeacherCollision(TeachCalendar calendar, Class activityClass) {
    ...
    collisionQuery.setGroups(Collections.singletonList("activity.teacher.id having count(*)>1"));
  ...
  public List collisionCourseTakes(TeachCalendar calendar) {
    ...
    Condition timeCondition = new Condition("activity.calendar.id=" + calendar.getId()
        + " and activity.time.year=:year and " + " bitand(activity.time.validWeeksNum,:validWeeksNum)>0"
  ...
  private Collection getActivityTimes(List calendars, Class activityClass) {
    ...
    timeQuery.setSelect("select  distinct new com.ekingstar.eams.system.time.TimeUnit("
        ...
        + "activity.time.endUnit," + "activity.time.validWeeks," + "activity.time.validWeeksNum)");
    timeQuery
        .addOrder(OrderUtils
    ...
    for (int i = 0; i < times.size(); i++) {
      ...
      for (int j = start; j <= end; j++) {
        TimeUnit time = new TimeUnit(timeUnit.getYear(), timeUnit.getWeekId(), new Integer(j),
            new Integer(j), timeUnit.getValidWeeks(), timeUnit.getValidWeeksNum());
        ...
        for (Iterator iter = timeList.iterator(); iter.hasNext();) {
          ...
          if (unit.getYear().equals(time.getYear())) {
            // 冲突时间段
            long collisionWeekNum = unit.getValidWeeksNum().longValue() & time.getValidWeeksNum().longValue();
            if (collisionWeekNum > 0) {
              // 组内时间点 差异段
              long unitWeekNum = unit.getValidWeeksNum().longValue() ^ collisionWeekNum;
              if (0 == unitWeekNum) {
              ...
              // 要加入的时间点
              long timeWeekNum = time.getValidWeeksNum().longValue() ^ collisionWeekNum;
              if (0 == timeWeekNum) {
                if (0 != unitWeekNum) {
              ...
              if (0 != unitWeekNum && 0 != timeWeekNum && 0 != collisionWeekNum) {
                addTimeUnit(newTimeUnits, new TimeUnit(time.getYear(), time.getWeekId(), time.getStartUnit(),
                    time.getEndUnit(), TimeUnit.weekOccupyString(collisionWeekNum),
                    new Long(collisionWeekNum)));
              }
              // 是否还有利用价值
  ...
  private Map getCollisionInfos(Collection activityTimes, AbstractQuery collisionQuery, Collection resourceIds) {
  ...
  private List fillCollisionEntities(Map collisionInfos, Class entityClass) {
    Collection collisionResources = utilService.load(entityClass, "id", collisionInfos.keySet());
  ...
  public boolean isBeenTaskForTeacher(Long calendarId, Long teacherId, Long thisTaskId) {
    ...
    if (task.getArrangeInfo().getTeachers().contains(teacher)) { return false; }
