...
public class SuggestTimePredicate implements EvaluateValueRemember, Predicate {
  ...
  public boolean evaluate(Object object) {
    ...
    if (null != task.getArrangeInfo().getSuggest().getTime()) {
      ...
      if (StringUtil.countChar(time.getAvailable(), '0') > (WeekInfo.MAX * TeachCalendar.MAXUNITS - task
          .getArrangeInfo().getWeekUnits().intValue())) return false;
      int hasUnits = StringUtil.countStr(time.getAvailable(),
          StringUtils.repeat("1", task.getArrangeInfo().getCourseUnits().intValue()));
      int needUnits = task.getArrangeInfo().getWeekUnits().intValue()
          / task.getArrangeInfo().getCourseUnits().intValue();
      if (hasUnits < needUnits) return false;
    }
    return true;
  }
}
