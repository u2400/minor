...
public class CourseTableCheckServiceImpl extends BasicService implements CourseTableCheckService {
  ...
  public List statCheckBy(TeachCalendar calendar, DataRealm dataRealm, String attr, Class clazz) {
    return statCheckBy(calendar, dataRealm, new String[] { "check.std.type.id", "check.std.department.id" },
        attr, clazz);
  }

  private List statCheckBy(TeachCalendar calendar, DataRealm dataRealm, String[] dataRealmAttrs, String attr,
      Class clazz) {
  ...
  public List statCheckByDepart(TeachCalendar calendar, DataRealm dataRealm, MajorType majorType) {
    ...
    DataRealmUtils.addDataRealm(query, new String[] { "check.std.type.id", "check.std.department.id" },
        dataRealm);
    ...
    query.groupBy("check.std.type.id").groupBy("check.std.department.id").groupBy("check.std.enrollYear");
    List datas = (List) utilDao.search(query);
    new StatHelper(utilService).replaceIdWith(datas, new Class[] { StudentType.class, Department.class });
