...
public class ArrangeTaskValidatorImpl implements ArrangeTaskValidator {
  ...
  static {
    ...
    taskPredicates.put("error.teachTask.course.needed",
        new BeanPredicateWrapper("course", NotNullPredicate.getInstance()));
    // 验证起始周[1..20]
    taskPredicates.put("error.teachTask.arrangeInfo.weekStart.notInScope", new BeanPredicateWrapper(
        "arrangeInfo.weekStart", new ValidNumScopePredicate(1, 20)));
    // 验证教学周占用周期[1..3]
    taskPredicates.put("error.teachTask.arrangeInfo.weekCycle.random", new BeanPredicateWrapper(
        "arrangeInfo.weekCycle", new ValidNumScopePredicate(TimeUnit.CONTINUELY, TimeUnit.EVEN)));
    // 验证周课时[1..7*12]
    taskPredicates.put("error.teachTask.arrangeInfo.weekUnits.notInScope", new BeanPredicateWrapper(
        "arrangeInfo.weekUnits", new ValidNumScopePredicate(1, WeekInfo.MAX * TeachCalendar.MAXUNITS)));
    // 验证小节数[1..4]
    taskPredicates.put("error.teachTask.arrangeInfo.courseUnits.notInScope", new BeanPredicateWrapper(
        "arrangeInfo.courseUnits", new ValidNumScopePredicate(CourseUnit.MINUNITS, CourseUnit.MAXUNITS)));
    // 验证周课时是否为小节的整倍数
    taskPredicates.put("error.teachTask.arrangeInfo.weekUnits.notRegular",
        new TaskWithRegularWeekUnitsPredicate());

    // 验证上课周数[1..53]
    taskPredicates.put("error.teachTask.arrangeInfo.weeks.notInScope", new BeanPredicateWrapper(
        "arrangeInfo.weeks", new ValidNumScopePredicate(1, TeachCalendar.OVERALLWEEKS)));

    // 验证教室设备配置-not null
    taskPredicates.put("error.teachTask.requirement.roomConfigType.needed", new BeanPredicateWrapper(
        "requirement.roomConfigType.id", NotNullPredicate.getInstance()));

    // 验证教学日历
    taskPredicates.put("error.teachTask.calendar.needed", new BeanPredicateWrapper("calendar.id",
        NotNullPredicate.getInstance()));
    // 验证教学日历中的学年度
    taskPredicates.put("error.teachTask.calendar.year.needed", new BeanPredicateWrapper("calendar.year",
        NotNullPredicate.getInstance()));
    // 验证教学日历中的起始周
    taskPredicates.put("error.teachTask.calendar.weekStart.needed", new BeanPredicateWrapper(
        "calendar.weekStart", NotNullPredicate.getInstance()));
    // 验证教学班中的计划人数
    taskPredicates.put("error.teachTask.teachClass.planStdCount.notInScope", new BeanPredicateWrapper(
        "teachClass.planStdCount", new ValidNumScopePredicate(0, 1000)));
    ...
    // 验证教室设备和建议教室相符合
    suggestRoomPredicates.put("error.teachTask.arrangeInfo.rooms.notMeetRequiement",
        new RoomMeetRoomConfigPredicate());
    // 验证建议教室的容量是否符合计划人数
    suggestRoomPredicates.put("error.teachTask.arrangeInfo.room.notEnoughCapacity",
        new RoomCapacityPredicate());
    // 验证建议时间
    suggestTimePredicate.put("error.teachTask.arrangeInfo.suggestTime.notSuitable",
   ...
   * 验证教学任务是否满足自动排课的要求
   */
  public ArrangeValidateMessages validateTask(TeachTask task, ArrangeParams params) {
  ...
  public ArrangeValidateMessages validateTaskGroup(TaskGroup group, ArrangeParams params) {
    ArrangeValidateMessages msgs = new ArrangeValidateMessages();
    ...
    if (group.getIsSameTime().equals(Boolean.TRUE)) {
      ...
      for (; iter.hasNext();) {
        ...
        if (!one.getArrangeInfo().getWeekUnits().equals(weekUnits)
            || !one.getArrangeInfo().getCourseUnits().equals(courseUnits)
            || !one.getCalendar().equals(calendar)) {
          msgs.add(ArrangeValidateMessages.TASKGROUP,
              new ArrangeValidateMessage(group.getId(), group.getName(), "error.taskGroup.notSameTime", ""));
          break;
        }
      }
    }
  ...
  private void validateTask(TeachTask task, ArrangeValidateMessages msgs, ArrangeParams params) {
  ...
  private void validateTaskUsing(HashMap predicateMap, TeachTask task, ArrangeValidateMessages msgs) {
    ...
    for (Iterator iter = keys.iterator(); iter.hasNext();) {
      ...
      if (!predicate.evaluate(task)) msgs.add(ArrangeValidateMessages.TASK,
          new ArrangeValidateMessage(task.getSeqNo(), task.getCourse().getName(), errorKey,
              ((EvaluateValueRemember) predicate).getEvaluateValue(task).toString()));
