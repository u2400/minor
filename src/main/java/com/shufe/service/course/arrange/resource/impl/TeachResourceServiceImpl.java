...
public class TeachResourceServiceImpl extends BasicService implements TeachResourceService {
  ...
  public boolean isStdsOccupied(TimeUnit time, Collection stdIds) {
    if (stdIds.isEmpty()) return false;
    else return teachResourceDAO.isStdsOccupied(time, stdIds);
  }

  public boolean isStdsOccupied(TimeUnit time, Collection stdIds, TeachTask expect) {
    if (stdIds.isEmpty()) return false;
    else return teachResourceDAO.isStdsOccupied(time, stdIds, expect);
  }
  ...
  public boolean isAdminClassesOccupied(TimeUnit time, Collection adminClasses) {
    return (!adminClasses.isEmpty()) ? teachResourceDAO.isAdminClassesOccupied(time, adminClasses) : true;
  }
  ...
  public boolean isAdminClassOccupied(TimeUnit time, Long adminClassId) {
    return (null != adminClassId) ? teachResourceDAO.isAdminClassOccupied(time, adminClassId) : true;
  ...
  public Pagination getFreeRoomsOf(Long[] departIds, TimeUnit[] times, Classroom room, Class activityClass,
    ...
    return teachResourceDAO.getFreeRoomsOf(departIds, times, room, activityClass, pageNo, pageSize);
  }
  ...
  public List getAdminClassActivities(Serializable adminClassId, TimeUnit time, Class activityClass) {
    return (null != adminClassId && StringUtils.isNotEmpty(time.getValidWeeks())) ? teachResourceDAO
        .getAdminClassActivities(adminClassId, time, activityClass) : Collections.EMPTY_LIST;
  }
  ...
  public List getTeacherActivities(Serializable teacherId, TimeUnit time, Class activityClass,
    ...
    return (null != teacherId && StringUtils.isNotEmpty(time.getValidWeeks())) ? teachResourceDAO
        .getTeacherActivities(teacherId, time, activityClass, calendar) : Collections.EMPTY_LIST;
  }
  ...
  public Collection getRoomUtilizationsOfExam(TeachCalendar calendar, ExamType examType,
    ...
    Collection rs = teachResourceDAO.getExamRoomUtilizations(calendar, examType, limit, freeRatio);
   ...
   * 查询上课教室利用率
   */
  public Collection getRoomUtilizationOfCourse(TeachCalendar calendar, DataRealmLimit limit, List orders,
      Float ratio) {
    ...
    entityQuery.add(new Condition("task.arrangeInfo.teachDepart.id in(:departIds)", SeqStringUtil
    ...
    String ratioStr = "(case when room.capacityOfCourse >0 " + " then case when task.teachClass.stdCount!=0 "
        + "   then (task.teachClass.stdCount/room.capacityOfCourse*1.0)"
        + "   else  (task.teachClass.planStdCount/room.capacityOfCourse*1.0)" + "end else 0.0 end)";
  ...
  public List getClassrooms(Long[] roomIds) {
    if (null != roomIds || roomIds.length > 0) return teachResourceDAO.getClassrooms(roomIds);
  ...
  public List getClassrooms(Collection roomIds) {
    return (!roomIds.isEmpty()) ? teachResourceDAO.getClassrooms((Long[]) roomIds.toArray(new Long[roomIds
        .size()])) : Collections.EMPTY_LIST;
  }
  ...
  public List getTeachers(Collection teacherIds) {
    return (!teacherIds.isEmpty()) ? teachResourceDAO.getTeachers(teacherIds) : Collections.EMPTY_LIST;
  }
