...
public class ArrangeCase implements Arrange {
  ...
  public void arrangeBare(ArrangeResult rs) throws GeneralArrangeFailure {
    availUnitBitMap = BitStringUtil.convertToBoolStr(availableTime.getAvailable());
    /**
     * donot forget set task's requirement although it's CapacityOfCourse is
     * wasn't stored in db.
     */
    task.getRequirement().setCapacityOfCourse(task.getTeachClass().getPlanStdCount());
  ...
  public String getAvailUnitBitMap() {
    if (null == availUnitBitMap) availUnitBitMap = BitStringUtil.convertToBoolStr(availableTime
        .getAvailable());
