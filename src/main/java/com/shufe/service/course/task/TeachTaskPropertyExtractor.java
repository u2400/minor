...
public class TeachTaskPropertyExtractor extends DefaultPropertyExtractor {
  ...
  public TeachTaskPropertyExtractor(Locale locale, MessageResources resources) {
    super();
    this.locale = locale;
  ...
  public Object getPropertyValue(Object target, String property) throws Exception {
    ...
    if (property.equals("arrangeInfo.teachers")) {
      ...
      return teacherNames;
    } else if (property.equals("arrangeInfo.teachers.gender")) {
      String rs = "";
      for (Iterator iter = task.getArrangeInfo().getTeachers().iterator(); iter.hasNext();) {
        Teacher teacher = (Teacher) iter.next();
        if (rs != "") rs += " ";
        if (teacher.getGender() != null) rs += teacher.getGender().getName();
      }
      return rs;
    } else if (property.equals("arrangeInfo.teachers.title")) {
      String rs = "";
      for (Iterator iter = task.getArrangeInfo().getTeachers().iterator(); iter.hasNext();) {
        Teacher teacher = (Teacher) iter.next();
        if (rs != "") rs += " ";
        if (teacher.getTitle() != null) rs += teacher.getTitle().getName();
      }
      return rs;
    } else if (property.equals("arrangeInfo.teachers.titleGrade")) {
      String rs = "";
      for (Iterator iter = task.getArrangeInfo().getTeachers().iterator(); iter.hasNext();) {
        Teacher teacher = (Teacher) iter.next();
        if (rs != "") rs += " ";
        if (teacher.getTitleLevel() != null) rs += teacher.getTitleLevel().getName();
      }
      return rs;
    ...
    // 建议教室
      ...
      return CourseActivityDigestor.digest(task.getCalendar(), task.getArrangeInfo(), resources, locale,
      ...
      StringBuffer arrangeTime = new StringBuffer(CourseActivityDigestor.digest(task.getCalendar(),
          task.getArrangeInfo(), task.getArrangeInfo().getActivities(), resources, locale, ":day{:units}"));
      if (arrangeTime.length() == 0) { return ""; }
      ...
      for (Iterator it = weekValues.keySet().iterator(); it.hasNext() && countChar > 0;) {
        ...
        if (StringUtils.indexOf(arrangeTime.toString(), key) >= 0) {
          arrangeTime = new StringBuffer(StringUtils.replace(arrangeTime.toString(), key, weekValues.get(key)
              .toString()));
          countChar--;
        }
      }
      for (int i = 0, startChar = 0, week = 0; i < arrangeTime.toString().length(); i++) {
        ...
        if (arrangeTime.toString().charAt(i) == '}') {
          ...
          arrangeTime = new StringBuffer(StringUtils.replace(arrangeTime.toString(), week + "{" + str + "} ",
              week + units[0] + "-" + week + units[1]));
        }
      }
      return arrangeTime;
    }
    // 教室可容纳人数
      ...
      if (examActivities.isEmpty()) return "";
      else {
    ...
    } else if (property.equals("requirement.languageAbilityNames")) {
      return task.getRequirement().getLanguageAbilitiyNames();
    } else {
