...
import com.shufe.model.course.arrange.task.CourseTakeClose;
...
public class TeachTaskServiceImpl extends BasicService implements TeachTaskService {
  ...
  public List getTeachTasksByCategory(Serializable id, TeachTaskFilterCategory category,
    ...
    if (null == id || null == calendar.getId()) {
      ...
      return taskDAO.getTeachTasksByCategory(id, category, Collections.singletonList(calendar));
    }
  }
  ...
  public List getTeachTasksByIds(Long[] taskIds) {
    ...
    else return taskDAO.getTeachTasksByIds(taskIds);
  }
  ...
  public Pagination getTeachTasksOfTeachDepart(TeachTask task, String stdTypeIdSeq, String teachDepartIdSeq,
    ...
    if (StringUtils.isEmpty(teachDepartIdSeq) || StringUtils.isEmpty(stdTypeIdSeq) || pageNo < 1
      ...
      return taskDAO.getTeachTasksOfTeachDepart(task, SeqStringUtil.transformToLong(stdTypeIdSeq),
  ...
  public Pagination getTeachTasksOfTeachPlan(String teachPlanIdSeq, TeachTask task, int pageNo, int pageSize) {
    return taskDAO.getTeachTasksOfTeachPlan(SeqStringUtil.transformToLong(teachPlanIdSeq), task, pageNo,
        pageSize);
  }
  ...
  public void teachTasksConfirm(String taskIds, boolean isConfirm) {
    ...
    try {
      taskDAO.updateTeachTaskByIds("isConfirm", new Boolean(isConfirm),
  ...
  public void delByAddProjectClosePlan(TeachTask task) {
    ...
    plan.setArrangeInfo(task.getArrangeInfo().digest(task.getCalendar()));
    plan.setIsArrangeComplete(task.getArrangeInfo().getIsArrangeComplete());
    plan.setTeacher(task.getArrangeInfo().getTeacherNames());
    plan.setCalendar(task.getCalendar());
    utilService.saveOrUpdate(plan);
    saveStudentMenu(task.getTeachClass().getCourseTakes(), plan);

  }

  public void saveStudentMenu(Set<CourseTake> courseTakes, ProjectClosePlan plan) {
    for (Iterator iterator = courseTakes.iterator(); iterator.hasNext();) {
      CourseTake courseTake = (CourseTake) iterator.next();
      CourseTakeClose courseTakeClose = new CourseTakeClose();
      courseTakeClose.setCourseGrade(courseTake.getCourseGrade());
      courseTakeClose.setCourseTakeType(courseTake.getCourseTakeType());
      courseTakeClose.setIsCourseEvaluated(courseTake.getIsCourseEvaluated());
      courseTakeClose.setProjectClosePlan(plan);
      courseTakeClose.setRemark(courseTake.getRemark());
      courseTakeClose.setStudent(courseTake.getStudent());
      utilService.saveOrUpdate(courseTakeClose);
    }
  }
  ...
  public List getCourseTypesOfTask(String stdTypeIdSeq, String departIdSeq, TeachCalendar calendar) {
    if (StringUtils.isNotEmpty(stdTypeIdSeq) && StringUtils.isNotEmpty(departIdSeq) && calendar.checkId()) return taskDAO
  ...
  public List getTeachTasksByCalendar(String stdTypeIds, String departIds, TeachCalendar calendar) {
    if (StringUtils.isEmpty(stdTypeIds) || StringUtils.isEmpty(departIds)
      ...
      return taskDAO.getTeachTasksByCalendar(calendar, SeqStringUtil.transformToLong(stdTypeIds),
  ...
  public Map getTeachTaskWithCalendarDWR(String taskSeqNo, String calendarId, String teacherId) {
    if (!StringUtils.isEmpty(taskSeqNo) && !StringUtils.isEmpty(calendarId)) {
      EntityQuery query = new EntityQuery(TeachTask.class, "task");
      query.add(new Condition("task.seqNo = (:seqNo)", taskSeqNo));
      query.add(new Condition("task.calendar.id = (:calendarId)", new Long(calendarId)));
      if (StringUtils.isNotBlank(teacherId)) {
        query.add(new Condition(
            "exists (from task.arrangeInfo.teachers teacher where teacher.id = :teacherId)", new Long(
                teacherId)));
      ...
      taskMap.put("course.code", task.getCourse().getCode());
      taskMap.put("course.name", task.getCourse().getName());
      taskMap.put("department.name", task.getArrangeInfo().getTeachDepart().getName());
      taskMap.put("task.gradeState.precision", task.getGradeState().getPrecision());
      taskMap.put("task.gradeState.markStyle.id", task.getGradeState().getMarkStyle().getId());
      taskMap.put("task.gradeState.markStyle.name", task.getGradeState().getMarkStyle().getName());
      taskMap.put("activityInfo", task.getArrangeInfo().digest(task.getCalendar()));
      return taskMap;
    }
    return null;
  }
