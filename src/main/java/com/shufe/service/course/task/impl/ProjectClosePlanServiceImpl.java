...
import com.shufe.dao.course.task.ProjectClosePlanDAO;
...
import com.shufe.web.action.course.task.ProjectClosePlanAction;
 ...
 * @author chaostone
 */
public class ProjectClosePlanServiceImpl extends BasicService implements ProjectClosePlanService {

  protected ProjectClosePlanDAO projectClosePlanDAO;

  protected TaskSeqNoGenerator taskSeqNoGenerator;
  ...
  public TeachTask getTeachTask(final Long taskId) {
    if (null == taskId || taskId.intValue() == 0) {
      ...
      return projectClosePlanDAO.getTeachTask(taskId);
    }
  }
  ...
  public TeachTask loadTeachTask(final Long taskId) {
    return projectClosePlanDAO.loadTeachTask(taskId);
  }

  public Collection getTeachTasks(final List conditions, final PageLimit limit, final List orders) {
    return projectClosePlanDAO.getTeachTasks(conditions, limit, orders);
  }
  ...
  public Pagination getTeachTasksByCategory(Serializable id, TeachTaskFilterCategory category,
    ...
    if (null == id || null == calendar.getId() || pageNo < 1 || pageSize < 1) {
      ...
      return projectClosePlanDAO.getTeachTasksByCategory(id, category, calendar, pageNo, pageSize);
    }
  }
  ...
  public List getTeachTasksByCategory(Serializable id, TeachTaskFilterCategory category,
    ...
    if (null == id || null == calendar.getId()) {
      ...
      return projectClosePlanDAO.getTeachTasksByCategory(id, category, Collections.singletonList(calendar));
    }
  }

  public List getTeachTasksByCategory(Serializable id, TeachTaskFilterCategory category, Collection calendars) {
    if (null == id || calendars.isEmpty()) {
      return Collections.EMPTY_LIST;
    } else {
      return projectClosePlanDAO.getTeachTasksByCategory(id, category, calendars);
    }
  }

  public List getTeachTasksOfStd(Serializable stdId, List calendars) {
    if (!calendars.isEmpty()) {
      return projectClosePlanDAO.getTeachTasksOfStd(stdId, calendars);
    } else {
      return Collections.EMPTY_LIST;
    }
  }
  ...
  public List getTeachTasks(TeachTask task) {
    return projectClosePlanDAO.getTeachTasks(task);
  }
  ...
  public List getTeachTasksByIds(Long[] taskIds) {
    if (null == taskIds || taskIds.length < 1) return Collections.EMPTY_LIST;
    else return projectClosePlanDAO.getTeachTasksByIds(taskIds);
  }
  ...
  public List getTeachTasksByIds(String taskIds) {
    if (StringUtils.isEmpty(taskIds)) {
      ...
      return projectClosePlanDAO.getTeachTasksByIds(SeqStringUtil.transformToLong(taskIds));
    }
  }
  ...
  public Pagination getTeachTasks(TeachTask task, int pageNo, int pageSize) {
    if (pageNo < 1 || pageSize < 1) {
      ...
      return projectClosePlanDAO.getTeachTasks(task, pageNo, pageSize);
    }
  }
   ...
   * @return
   */
  public Pagination getTeachTasksOfTeachDepart(TeachTask task, String stdTypeIdSeq, String teachDepartIdSeq,
      int pageNo, int pageSize) {
    if (StringUtils.isEmpty(teachDepartIdSeq) || StringUtils.isEmpty(stdTypeIdSeq) || pageNo < 1
        || pageSize < 1) {
      return new Pagination(new Result(0, Collections.EMPTY_LIST));
    } else {
      return projectClosePlanDAO.getTeachTasksOfTeachDepart(task,
          SeqStringUtil.transformToLong(stdTypeIdSeq), SeqStringUtil.transformToLong(teachDepartIdSeq),
          pageNo, pageSize);
    }
  }

  public Pagination getTeachTasksOfLonely(TeachTask task, Long[] stdTypeIds, Long[] teachDepartIds,
      int pageNo, int pageSize) {
    return projectClosePlanDAO.getTeachTasksOfLonely(task, stdTypeIds, teachDepartIds, pageNo, pageSize);
  }
   ...
   */
  public Pagination getTeachTasksOfTeachPlan(String teachPlanIdSeq, TeachTask task, int pageNo, int pageSize) {
    return projectClosePlanDAO.getTeachTasksOfTeachPlan(SeqStringUtil.transformToLong(teachPlanIdSeq), task,
        pageNo, pageSize);
  }
  ...
  public void teachTasksConfirm(Long[] taskIds, boolean isConfirm) {
    if (null == taskIds || taskIds.length < 1) return;
    projectClosePlanDAO.updateTeachTaskByIds("isConfirm", new Boolean(isConfirm), taskIds);
  }
  ...
  public void teachTasksConfirm(String taskIds, boolean isConfirm) {
    if (StringUtils.isEmpty(taskIds)) { return; }
    try {
      projectClosePlanDAO.updateTeachTaskByIds("isConfirm", new Boolean(isConfirm),
  ...
  public void teachTasksConfirmOfDepart(Long departId, boolean isConfirm, TeachCalendar calendar) {
    if (null == departId) {
      ...
      projectClosePlanDAO.updateTeachTaskByCategory("isConfirm", new Boolean(isConfirm), departId,
  ...
  public void teachTasksConfirm(TeachTask task, boolean isConfirm) {
    try {
      projectClosePlanDAO.updateTeachTaskByCriteria("isConfirm", new Boolean(isConfirm), task);
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
  ...
  private TeachTask merge(Long[] taskIds, int target) {
    if (null == taskIds || taskIds.length < 1 || target >= taskIds.length || target < 0) { return null; }
    List taskList = projectClosePlanDAO.getTeachTasksByIds(taskIds);
  ...
  public TeachTask merge(Long[] taskIds, Long reservedId) {
    if (null == taskIds || taskIds.length < 1 || null == reservedId) {
      ...
      for (int i = 0; i < taskIds.length; i++) {
        if (reservedId.equals(taskIds[i])) { return merge(taskIds, i); }
      }
      return null;
    }
  }
  ...
  private TeachTask merge(TeachTask[] tasks, int target) {
    ...
    projectClosePlanDAO.saveMergeResult(tasks, target);
    return tasks[target];
  }
   ...
   */
  public TeachTask[] split(TeachTask task, int num, TeachClassSplitMode mode, Integer[] splitUnitNums) {
    if (num < 2) {
      return new TeachTask[] { task };
    } else {
      List<String> newSeqNos = taskSeqNoGenerator.genTaskSeqNos(task.getCalendar(), num - 1);
      TeachTask tasks[] = new TeachTask[num];
      tasks[0] = task;
      for (int i = 1; i < tasks.length; i++) {
        tasks[i] = (TeachTask) task.clone();
        tasks[i].setSeqNo(newSeqNos.get(i - 1));
      ...
      for (int j = 1; j < tasks.length; j++) {
        ...
        if (null != tasks[j].getTeachClass().getCourseTakes()) {
          for (Iterator iter = tasks[j].getTeachClass().getCourseTakes().iterator(); iter.hasNext();) {
      ...
      projectClosePlanDAO.saveOrUpdate(tasks);
      return tasks;
    }
  }
   ...
   *      com.shufe.model.system.calendar.TeachCalendar)
   */
  public int[] statTeachTask(String[] ids, TeachTaskFilterCategory category, TeachCalendar calendar) {
    return null;
  }
  ...
  public void removeTeachTask(Long taskId) {
    ...
    TeachTask task = projectClosePlanDAO.getTeachTask(taskId);
    projectClosePlanDAO.removeTeachTask(task);
  }
  ...
  public void removeTeachTask(TeachTask task) {
    ...
    // projectClosePlanDAO.removeTeachTask(task);
    delByAddProjectClosePlan(task);
  }

  /**
   * 删除一条教学任务，添加一条不开课记录
   */
  public void delByAddProjectClosePlan(TeachTask task) {
    ProjectClosePlan plan = new ProjectClosePlan();
    plan.setCourse(task.getCourse());
    plan.setCourseType(task.getCourseType());

    plan.setName(task.getTeachClass().getName());
    plan.setPlanStdCount(task.getTeachClass().getPlanStdCount());
    plan.setGender(task.getTeachClass().getGender());

    plan.setWeekUnits(task.getArrangeInfo().getWeekUnits());
    plan.setWeeks(task.getArrangeInfo().getWeeks());
    plan.setOverallUnits(task.getArrangeInfo().getOverallUnits());
    plan.setIsArrangeComplete(task.getArrangeInfo().getIsArrangeComplete());
    List<Teacher> list = task.getArrangeInfo().getTeachers();
    plan.setTeacher(list.get(0).getName());
    plan.setCalendar(task.getCalendar());
  ...
  public void batchUpdateTeachTask(Long[] taskIds, String attr, Object value) {
    if (null == taskIds && taskIds.length < 1 || StringUtils.isBlank(attr)) { return; }
    projectClosePlanDAO.updateTeachTaskByIds(attr, value, taskIds);
  }
  ...
  public int genTeachTasks(Long[] planIds, TeachCalendar calendar, TaskGenObserver observer,
    ...
    for (int i = 0; i < planIds.length; i++) {
      ...
      if (term == 0) {
        term = termCalc.getTerm(plan.getStdType(), plan.getEnrollTurn(), params.getIsOmitSmallTerm());
      }
      ...
      try {
        ...
        e.printStackTrace();
        observer.outputNotify(OutputWebObserver.error, new OutputMessage("", e.getMessage()), true);
   ...
   * @return
   */
  protected int genTeachTasksForTerm(TeachPlan plan, TeachCalendar calendar, TaskGenObserver observer,
      int term, TaskGenParams params) throws Exception {
    /*--------------------在必要时，删除已经生成的任务------------------*/
    if (StringUtils.isNotEmpty(plan.getTaskTerms())) {
      if (StringUtils.contains(plan.getTaskTerms(), String.valueOf(term))) {
        if (params.isRemoveGenerated()) {
          observer.outputNotify(term, plan, "info.plan.removeGenTask", false);
          try {
            projectClosePlanDAO.removeTeachTaskOfPlan(plan, calendar);
          } catch (Exception e) {
    ...
    exampleClass.getSpeciality().setId((null == plan.getSpeciality()) ? null : plan.getSpeciality().getId());
    exampleClass.getAspect().setId((null == plan.getAspect()) ? null : plan.getAspect().getId());
    ...
    if (params.getIgnoreClass().equals(Boolean.FALSE) && adminClasses.isEmpty()) {
      info("Generate 0 task for No AdminClass correspond Plan[" + plan.getId() + "] of term [" + term + "]");
      return 0;
    }
    ResettableIterator adminClassIterator = new ArrayListIterator(adminClasses.toArray());
    /*---------------------------指定要生成的课程-----------------------------*/
    Set courseCodeSet = new HashSet(utilService.load(Course.class, "code",
        StringUtils.split(params.getCourseCodeSeq(), ",")));
    /*-------------------------指定要生成的课程类别---------------------------*/
    ...
    for (Iterator iter = planCourses.iterator(); iter.hasNext();) {
      ...
      // 检查是否该课程可以不生成
      if (StringUtils
          .contains(params.getOmmitedCourseCodeSeq(), "," + planCourse.getCourse().getCode() + ",")) {
        continue;
      }
      if (StringUtils.isNotBlank(params.getOmmitedCourseName())) {
        if (StringUtils.contains(planCourse.getCourse().getName(), params.getOmmitedCourseName())) {
          continue;
        }
      }
      if (null == planCourse.getTeachDepart()) {
        debug("planCourse id:" + planCourse.getId() + " course name:" + planCourse.getCourse().getName()
            + " in group:" + planCourse.getCourseGroup().getId()
            + " has no department.It will be  Ingored in generating TeachTask");
        continue;
      }
      if (CollectionUtils.isNotEmpty(courseCodeSet) && !courseCodeSet.contains(planCourse.getCourse())) {
        continue;
      }
    ...
    // 批量存储教学任务和更新培养计划
    if (!tasks.isEmpty()) {
      String existTerms = plan.getTaskTerms();
      if (StringUtils.isEmpty(existTerms)) {
        plan.setTaskTerms(new Integer(term).toString());
      } else {
        if (!StringUtils.contains(plan.getTaskTerms(), String.valueOf(term))) {
          plan.setTaskTerms(existTerms + "," + new Integer(term).toString());
        }
      }
      projectClosePlanDAO.saveGenResult(plan, tasks);
    }
    return count;
  }
   ...
   * @return
   */
  public TeachTask genTeachTaskDetail(PlanCourse planCourse, TeachCalendar calendar, TeachPlan plan,
      AdminClass adminClass, TaskGenParams params) {
    ...
    if (params.getUnitStatus().equals(new Integer(1))) {
      // 2.5学分算3节
      task.getArrangeInfo().setCourseUnits(
          new Integer((int) Math.round(planCourse.getCourse().getCredits().doubleValue())));
    } else {
    ...
    if (null == arrangeInfo.getWeekUnits()) arrangeInfo.setWeekUnits(new Float(arrangeInfo.getOverallUnits()
        .intValue() / calendar.getWeeks().intValue()));
    /*------------------设置教学班信息----------------------------------*/
    TeachClass teachClass = task.getTeachClass();
    ...
    teachClass.processTaskForClass();
    /*-------------------设置选课信息------------------------------*/
    task.getElectInfo().setHSKDegree(planCourse.getHSKDegree());
    task.getElectInfo().getPrerequisteCourses().addAll(planCourse.getPreCourses());
    /*-------------------设置生成和修改时间------------------------------*/
  ...
  public List getDepartsOfTask(String stdTypeIdSeq, String departIdSeq, TeachCalendar calendar) {
    if (StringUtils.isNotEmpty(stdTypeIdSeq) && StringUtils.isNotEmpty(departIdSeq) && calendar.checkId()) {
      return projectClosePlanDAO.getDepartsOfTask(SeqStringUtil.transformToLong(stdTypeIdSeq),
   ...
   *      java.lang.String, com.shufe.model.system.calendar.TeachCalendar)
   */
  public List getTeachDepartsOfTask(String stdTypeIdSeq, String departIdSeq, TeachCalendar calendar) {
    if (StringUtils.isNotEmpty(stdTypeIdSeq) && StringUtils.isNotEmpty(departIdSeq) && calendar.checkId()) {
      return projectClosePlanDAO.getTeachDepartsOfTask(SeqStringUtil.transformToLong(stdTypeIdSeq),
          SeqStringUtil.transformToLong(departIdSeq), calendar);
    } else {
      return Collections.EMPTY_LIST;
    }
  ...
  public List getTeachDepartsOfTask(String stdTypeIdSeq, String departIdSeq, Collection calendars) {
    if (StringUtils.isNotEmpty(stdTypeIdSeq) && StringUtils.isNotEmpty(departIdSeq) && calendars.isEmpty()) {
      return projectClosePlanDAO.getTeachDepartsOfTask(SeqStringUtil.transformToLong(stdTypeIdSeq),
  ...
  public List getCourseTypesOfTask(String stdTypeIdSeq, String departIdSeq, TeachCalendar calendar) {
    if (StringUtils.isNotEmpty(stdTypeIdSeq) && StringUtils.isNotEmpty(departIdSeq) && calendar.checkId()) return projectClosePlanDAO
        .getCourseTypesOfTask(SeqStringUtil.transformToLong(stdTypeIdSeq),
            SeqStringUtil.transformToLong(departIdSeq), calendar);
  ...
  public List getTeachersOfTask(String stdTypeIdSeq, String departIdSeq, TeachCalendar calendar) {
    if (StringUtils.isEmpty(stdTypeIdSeq) || StringUtils.isEmpty(departIdSeq) || !calendar.checkId()) {
      return Collections.EMPTY_LIST;
    } else {
      return projectClosePlanDAO.getTeachersOfTask(SeqStringUtil.transformToLong(stdTypeIdSeq),
          SeqStringUtil.transformToLong(departIdSeq), calendar);
    }
  }
  ...
  public List getStdTypesForTeacher(Teacher teacher) {
    if (null == teacher) {
      ...
      return projectClosePlanDAO.getStdTypesForTeacher(teacher);
    }
  }
  ...
  public List getTeachTasksByCalendar(String stdTypeIds, String departIds, TeachCalendar calendar) {
    if (StringUtils.isEmpty(stdTypeIds) || StringUtils.isEmpty(departIds)
      ...
      return projectClosePlanDAO.getTeachTasksByCalendar(calendar, SeqStringUtil.transformToLong(stdTypeIds),
  ...
  public List getTaskOfCourseTypes(TeachCalendar calendar, String stdTypeIds, String departIds,
    ...
    Collection plans = teachPlanService.getTeachPlansOfActive(calendar, stdTypeIds, departIds, Boolean.TRUE);
    TermCalculator termCalc = new TermCalculator(calendarService, calendar);
    ...
    for (Iterator iter = plans.iterator(); iter.hasNext();) {
      ...
      AdminClass exampleClass = new AdminClass();
      exampleClass.setEnrollYear(plan.getEnrollTurn());
      exampleClass.setStdType((StudentType) EntityUtils.getEntity(StudentType.class, plan.getStdType()
          .getId()));
      exampleClass.getDepartment().setId(plan.getDepartment().getId());
      exampleClass.getSpeciality()
          .setId((null == plan.getSpeciality()) ? null : plan.getSpeciality().getId());
      exampleClass.getAspect().setId((null == plan.getAspect()) ? null : plan.getAspect().getId());
      List adminClasses = adminClassDAO.getAdminClassesExactly(exampleClass);
      // 小学期忽略掉了
      int term = 0;
      ...
      for (Iterator iterator = plan.getCourseGroups().iterator(); iterator.hasNext();) {
        ...
        for (Iterator adminIter = adminClasses.iterator(); adminIter.hasNext();) {
          ...
          taskOfCourseTypes.add(new TaskOfCourseType(group.getCourseType(), adminClass, Float
              .valueOf(creditPerTerm[term - 1])));
        }
      }
    }
  ...
  public Collection copy(Collection tasks, TaskCopyParams params) {
    ...
    for (Iterator iter = tasks.iterator(); iter.hasNext();) {
      ...
      if (Boolean.TRUE.equals(params.getCopySeqNo())) {
        ...
        List exsits = utilDao.searchHQLQuery("from TeachTask where seqNo='" + task.getSeqNo()
            + "' and calendar.id=" + params.getCalendar().getId());
        if (!exsits.isEmpty()) {
          needAdd = false;
    ...
    projectClosePlanDAO.saveOrUpdate(copied);
    return copied;
  }

  public Map getTeachTaskDWR(String taskSeqNo, String stdTypeId, String year, String term) {
    if (!StringUtils.isEmpty(taskSeqNo) && !StringUtils.isEmpty(taskSeqNo) && !StringUtils.isEmpty(year)
      ...
      taskMap.put("task.gradeState.markStyle.id", task.getGradeState().getMarkStyle().getId());
      taskMap.put("task.gradeState.markStyle.name", task.getGradeState().getMarkStyle().getName());
      return taskMap;
    }
    return null;
  }

  public Collection getTeachTasksWithMultiClass(TeachTask task, String stdTypeIds, String departIds) {
    return projectClosePlanDAO.getTeachTasksWithMultiClass(task, stdTypeIds, departIds);
  }

  public int statStdCount(TeachCalendar calendar) {
    return projectClosePlanDAO.statStdCount(calendar);
  }

  public int statStdCount(String taskIdSeq) {
    return projectClosePlanDAO.statStdCount(taskIdSeq);
  }

  /**
   * @param projectClosePlanDAO
   *          The projectClosePlanDAO to set.
   */
  public void setProjectClosePlanDAO(ProjectClosePlanDAO projectClosePlanDAO) {
    this.projectClosePlanDAO = projectClosePlanDAO;
  }
  ...
  public void saveOrUpdateTask(TeachTask task) {
    projectClosePlanDAO.saveOrUpdateTask(prepareTask(task));
  }
  ...
  public void saveOrUpdateTasks(Collection tasks) {
    for (Iterator it = tasks.iterator(); it.hasNext();) {
      ...
      task.getTeachClass().processTaskForClass();
      projectClosePlanDAO.saveOrUpdateTask(task);
    }
  }
  ...
  public void setTaskSeqNoGenerator(TaskSeqNoGenerator taskSeqNoGenerator) {
    this.taskSeqNoGenerator = taskSeqNoGenerator;
  }
