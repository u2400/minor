...
public class TeachTaskStatServiceImpl extends BasicService implements TeachTaskStatService {
  ...
  public List statClassTask(TeachCalendar calendar, DataRealm dataRealm) {
    ...
    for (Iterator iter = gpTasks.iterator(); iter.hasNext();) {
      ...
      if (null == stat) {
        statMap.put(pgStat.getWhat(), new StatItem(pgStat.getWhat(), new Integer(1), pgStat.getCountors()[1],
  ...
  public List statTeachDepartConfirm(TeachCalendar calendar, DataRealm dataRealm) {
    return statTaskConfirm(calendar, dataRealm, "task.arrangeInfo.teachDepart.id", Department.class);
  }

  private List statTaskConfirm(TeachCalendar calendar, DataRealm dataRealm, String item, Class entityClass) {
    EntityQuery entityQuery = new EntityQuery(TeachTask.class, "task");
    entityQuery.setSelect("select new  com.shufe.service.util.stat.StatItem(" + item + ",count(*),0)");
    entityQuery.add(new Condition("task.calendar=:calendar", calendar));
    addDataRealm(entityQuery, dataRealm);
  ...
  public StatGroup statRoomConfigType(TeachCalendar calendar, DataRealm dataRealm) {
    ...
    query.setSelect("activity.task.arrangeInfo.teachDepart.id,activity.room.configType.id,count(*)");
    ...
    new StatHelper(utilService).replaceIdWith(stats, new Class[] { Department.class, ClassroomType.class });
  ...
  private void addDataRealm(EntityQuery entityQuery, DataRealm dataRealm) {
    if (null != dataRealm) {
      if (StringUtils.isNotBlank(dataRealm.getStudentTypeIdSeq())) {
        entityQuery.add(new Condition("task.teachClass.stdType.id in(:stdTypeIds)", SeqStringUtil
            .transformToLong(dataRealm.getStudentTypeIdSeq())));
      }
      if (StringUtils.isNotBlank(dataRealm.getDepartmentIdSeq())) {
        entityQuery.add(new Condition("task.arrangeInfo.teachDepart.id in(:departIds)", SeqStringUtil
            .transformToLong(dataRealm.getDepartmentIdSeq())));
      }
    }
  }
  ...
  public List statTeacherTitle(List calendars) {
    ...
    new StatHelper(utilService).replaceIdWith(stats, new Class[] { TeachCalendar.class, TeacherTitle.class });
    return stats;
  }
