...
public class OtherExamSignUpServiceImpl extends BasicService implements OtherExamSignUpService {
   ...
   * 校外考试报名
   */
  public String signUp(final Student std, final TeachCalendar calendar, final OtherExamSignUpSetting setting) {
    if (!std.isActive() || !std.isInSchool()) { return "otherExam.failure.isInSchoolOrIsInValidLimit"; }
    // 判断时间
    if (Boolean.FALSE.equals(setting.getIsOpen()) || !setting.inSignUpTime()) return notInTime;
    // 前提考试类别
  ...
  public OtherExamSignUp getOtherExamSignUp(Student std, OtherExamSignUpSetting setting) {
    ...
    query.add(new Condition("signUp.signUpAt between :start and :end", setting.getStartAt(), setting
        .getEndAt()));
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.service.course.grade.other.OtherExamSignUpService#getOtherExamSignUps(com.shufe.model
   * .std.Student,
