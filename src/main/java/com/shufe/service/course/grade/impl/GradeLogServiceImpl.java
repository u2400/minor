...
public class GradeLogServiceImpl extends BasicService implements GradeLogService {
   ...
   * @return
   */
  public Collection buildGradeCatalogInfo(final User user, final List grades, final Map inputGradeMap,
      final Map scoreMap) {
    Collection catalogs = new HashSet();
    if (null == inputGradeMap || null == scoreMap) { return catalogs; }
    ...
    for (Iterator it1 = grades.iterator(); it1.hasNext();) {
      ...
      if (null != status) {
        ...
        catalogs.add(new GradeLog(student, grade.getCalendar(), task, status, user,
            context.length() > 0 ? "当初：" + keyMap.get(gradeId) + "<br>" + context.toString() : "成绩还未录入。"));
      }
    }
    return catalogs;
  }
  ...
  public Collection buildGradeCatalogInfo(final User user, final List grades, final Map inputGradeMap,
      final Map scoreMap, String content, boolean isRemove) {
  ...
  public Collection buildGradeCatalogInfo(final User user, final List grades, final Map inputGradeMap,
      final Map scoreMap, boolean isRemove) {
    if (!isRemove) { return buildGradeCatalogInfo(user, grades, inputGradeMap, scoreMap); }
    ...
    for (Iterator it1 = grades.iterator(); it1.hasNext();) {
      ...
      Student student = (Student) utilService.load(Student.class, grade.getStd().getId());
      catalogs
          .add(new GradeLog(student, grade.getCalendar(), task, GradeLog.REMOVE, user, context.toString()));
    }
    return catalogs;
  }
