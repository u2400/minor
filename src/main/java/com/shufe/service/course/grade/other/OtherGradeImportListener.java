...
public class OtherGradeImportListener extends ItemImporterListener {
  ...
  public void startTransferItem(TransferResult tr) {
    ...
    if (null != markStyle) {
      values[scoreIndex] = ConverterFactory.getConverter().convert(values[scoreIndex].toString(), markStyle);
    }
  }
  ...
  public void endTransferItem(TransferResult tr) {
    ...
    if (null == otherGrade.getStd()) {
      ...
      String calendarMapId = otherGrade.getStd().getType().getId() + "_" + otherGrade.getCalendar().getYear()
          + "_" + otherGrade.getCalendar().getTerm();

      TeachCalendar calendar = (TeachCalendar) calendarMap.get(calendarMapId);
      if (null == calendar) {
        calendar = teachCalendarDAO.getTeachCalendar(otherGrade.getStd().getType().getId(), otherGrade
            .getCalendar().getYear(), otherGrade.getCalendar().getTerm());
        if (null == calendar) {
    ...
    if (tr.errors() == errors) {
      ...
      try {
        ...
        query.add(new Condition("grade.calendar.id=:calendarId and grade.category.id=:categoryId", otherGrade
            .getCalendar().getId(), otherGrade.getCategory().getId()));
