...
public class GradeImportListener extends ItemImporterListener {
  ...
  /** 考试成绩 */
  private Map examGradeMap = new HashMap();
  ...
  public GradeImportListener(UtilDao utilDao, GradePointRuleService gradePointRuleService) {
    ...
    for (Iterator it = calendars.iterator(); it.hasNext();) {
      ...
      calendarMap.put(
          calendar.getStudentType().getId() + "_" + calendar.getYear() + "_" + calendar.getTerm(), calendar);
    }
  ...
  public void startTransferItem(TransferResult tr) {
    ...
    if (StringUtils.isEmpty(year) || StringUtils.isEmpty(term)) {
      tr.addFailure("error.parameters.illegal", "教学日历[" + importer.curDataMap().get("calendar.year") + " "
          + importer.curDataMap().get("calendar.term") + "]");
    }
    String courseTakeTypeCode = String.valueOf(importer.curDataMap().get("courseTakeType.code"));
    if (StringUtils.isEmpty(courseTakeTypeCode)) {
      tr.addFailure("error.parameters.illegal", "修读类别没填写");
    }

    String endScoreValue = (String) importer.curDataMap().get("endScore");
    try {
      if (StringUtils.isNotBlank(endScoreValue)) {
        new Float(endScoreValue);
      }
    } catch (Exception e) {
      tr.addFailure("error.parameters.illegal", "无效期末成绩[" + endScoreValue + "]");
    }

    String usualScoreValue = (String) importer.curDataMap().get("usualScore");
    try {
      if (StringUtils.isNotBlank(usualScoreValue)) {
        new Float(usualScoreValue);
      }
    } catch (NumberFormatException e) {
      tr.addFailure("error.parameters.illegal", "无效期末成绩[" + usualScoreValue + "]");
    }

    if (tr.errors() == 0) {
      if (StringUtils.isNotEmpty(scoreValue)) {
        importer.curDataMap().put("markStyle.code",
            ConverterFactory.getConverter().convert(scoreValue, (MarkStyle) markStyles.get(markStyleCode)));
      }
      // 如果出现已有的则进行更新
  ...
  public void endTransferItem(TransferResult tr) {
    if (tr.errors() == 0) {
      ...
      if (courseGrade.isVO()) {
        ...
        String calendarMapId = studentType.getId() + "_" + courseGrade.getCalendar().getYear() + "_"
        ...
        if (null == teachCalendar) {
          while (null != studentType.getSuperType()) {
            ...
            calendarMapId = courseGrade.getStd().getType().getSuperType().getId() + "_"
                + courseGrade.getCalendar().getYear() + "_" + courseGrade.getCalendar().getTerm();
            teachCalendar = (TeachCalendar) calendarMap.get(calendarMapId);
          }
        }
        if (null == courseGrade.getCalendar()) {
          tr.addFailure("error.parameters.illegal", "该学生对应日历[" + importer.curDataMap().get("calendar.year")
              + " " + importer.curDataMap().get("calendar.term") + "]");
        }
      ...
      String endScoreValue = (String) importer.curDataMap().get("endScore");
      if (StringUtils.isNotBlank(endScoreValue)) {
        addExamGrade(courseGrade, new GradeType(GradeType.END), new Float(endScoreValue));
      }

      String usualScoreValue = (String) importer.curDataMap().get("usualScore");
      Float usualScore = null;
      if (StringUtils.isNotBlank(usualScoreValue)) {
        addExamGrade(courseGrade, new GradeType(GradeType.USUAL), new Float(usualScoreValue));
      }
      ...
      if (StringUtils.isNotEmpty(courseGrade.getTaskSeqNo())) {
        ...
        query.add(new Condition("task.calendar.id = :calendarId", courseGrade.getCalendar().getId()));
        List results = (List) utilDao.search(query);
        if (CollectionUtils.isNotEmpty(results)) {
  ...
  protected void addExamGrade(CourseGrade courseGrade, GradeType gradeType, Float score) {
    ExamGrade examGrade = courseGrade.getExamGrade(gradeType);
    if (null == examGrade) {
      if (null == courseGrade.getScore()) {
        examGrade = new ExamGrade(gradeType, (ExamStatus) examStatusMap.get(ExamStatus.CHEAT), null);
        courseGrade.addExamGrade(examGrade);
      } else {
        examGrade = new ExamGrade(gradeType, (ExamStatus) examStatusMap.get(ExamStatus.NORMAL), score);
        courseGrade.addExamGrade(examGrade);
      }
    } else {
      if (null == courseGrade.getScore()) {
