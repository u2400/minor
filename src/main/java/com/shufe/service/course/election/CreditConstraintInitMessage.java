...
public class CreditConstraintInitMessage extends OutputMessage {
  ...
  public String getMessage(MessageResources resourses, Locale locale) {
    ...
    if (null != credit) msgBuf.append(" ").append(credit).append(" ");
    msgBuf.append("[").append(common.getEnrollTurn());
    if (locale.getLanguage().indexOf("en") != -1) {
      if (StringUtils.isEmpty(common.getStdType().getEngName())) msgBuf.append(common.getStdType().getName());
      else msgBuf.append(common.getStdType().getEngName());
      msgBuf.append("    ");
      if (StringUtils.isEmpty(common.getDepart().getEngName())) msgBuf.append(common.getDepart().getName());
      else msgBuf.append(common.getDepart().getEngName());
      msgBuf.append("    ");
      if (null != common.getSpeciality()) {
        ...
        else msgBuf.append(common.getSpeciality().getEngName());
      }
      msgBuf.append("    ");
      if (null != common.getAspect()) {
        if (StringUtils.isEmpty(common.getAspect().getEngName())) msgBuf.append(common.getAspect().getName());
        else msgBuf.append(common.getAspect().getEngName());
      }
    } else {
      msgBuf.append(common.getStdType().getName()).append("    ");
      msgBuf.append(common.getDepart().getName()).append("    ");
      if (null != common.getSpeciality()) msgBuf.append(common.getSpeciality().getName()).append("    ");
      if (null != common.getAspect()) msgBuf.append(common.getAspect().getName());
    }
    msgBuf.append("]");
