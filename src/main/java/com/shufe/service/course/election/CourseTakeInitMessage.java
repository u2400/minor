...
public class CourseTakeInitMessage extends OutputMessage {
  ...
  public String getMessage(MessageResources resourses, Locale locale) {
    ...
    msgBuf.append("[").append(resourses.getMessage(locale, "attr.taskNo")).append(":")
        .append(task.getSeqNo()).append("]").append(task.getTeachClass().getName()).append(":");
    if (StringUtils.isNotEmpty(getMessage())) msgBuf.append(getMessage());
