...
public class ValidElectStdScopePredicate implements Predicate {
  ...
  public boolean evaluate(Object arg0) {
    ...
    if ((null == scope.getStartNo()) ^ (null == scope.getEndNo())) return false;
    if (null == scope.getStartNo() && null == scope.getEndNo()) {
      if (StringUtils.isBlank(scope.getStdTypeIds())) return false;
      if (null == scope.getTask().getId()) return false;
    }
    return true;
  }
