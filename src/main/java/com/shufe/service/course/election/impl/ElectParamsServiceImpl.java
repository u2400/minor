...
public class ElectParamsServiceImpl extends BasicService implements ElectParamsService {
  ...
  public void saveElectParams(ElectParams params) throws PojoExistException {
    ...
    if (!params.isPO()) {
      ...
      for (Iterator iter = existed.iterator(); iter.hasNext();) {
        ...
        if (!CollectionUtils.intersection(params.getEnrollTurns(), oneParams.getEnrollTurns()).isEmpty()
            && !CollectionUtils.intersection(params.getDeparts(), oneParams.getDeparts()).isEmpty()
            && !CollectionUtils.intersection(params.getStdTypes(), oneParams.getStdTypes()).isEmpty()
            && oneParams.isTimeCollision(params)) { throw new PojoExistException("error.electParams.existed"); }
      }
    }
  ...
  public ElectParams getAvailElectParams(Student std) {
    List<ElectParams> existed = electParamsDAO.getAvailElectParams(std);
    List<ElectParams> suitable = new ArrayList();
    for (ElectParams params : existed) {
      if (params.isStudentSuitable(std)) suitable.add(params);
    }
    // 取时间最近的选择参数
    PropertyComparator pc = new PropertyComparator("startAt");
    Collections.sort(suitable, pc);
    return suitable.isEmpty() ? null : (ElectParams) suitable.get(0);
  }
  ...
  public List getElectParams(String stdTypeIds, TeachCalendar calendar) {
    if (StringUtils.isNotEmpty(stdTypeIds) && null != calendar.getId()) {
      return electParamsDAO.getElectParams(SeqStringUtil.transformToLong(stdTypeIds), calendar);
