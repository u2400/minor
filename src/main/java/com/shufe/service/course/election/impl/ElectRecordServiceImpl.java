 ...
 * @author chaostone
 */
public class ElectRecordServiceImpl extends BasicService implements ElectRecordService {
  private ElectRecordDAO electRecordDAO;

  public Pagination getElectRecords(ElectRecord record, int pageNo, int pageSize) {
    if (pageNo > 0 && pageSize > 0 && null != record) return electRecordDAO.getElectRecords(record, pageNo,
        pageSize);
   ...
   */
  public void setCourseTakeFor(TeachTask task, String pitchOnStdIds, String allStdIds) {
    ...
    for (Iterator iter = courseTakeSet.iterator(); iter.hasNext();)
      courseTakeMap.put(((CourseTake) iter.next()).getStudent(), Boolean.TRUE);

    for (Iterator iter = task.getElectInfo().getElectResults().iterator(); iter.hasNext();) {
      ElectRecord record = (ElectRecord) iter.next();
      if (allStdIds.indexOf(record.getStudent().getId().toString()) == -1) continue;
      if (pitchOnStdIds.indexOf(record.getStudent().getId().toString()) != -1) {
        if (null == courseTakeMap.get(record.getStudent())) tobeAdded.add(new CourseTake(record));
        if (record.getIsPitchOn().equals(Boolean.FALSE)) record.setIsPitchOn(Boolean.TRUE);
      } else {
        if (null != courseTakeMap.get(record.getStudent())) tobeRemoved.add(new CourseTake(record));
        if (record.getIsPitchOn().equals(Boolean.TRUE)) record.setIsPitchOn(Boolean.FALSE);
      }
    }
  ...
  public List getElectRecords(ElectRecord record) {
    if (null != record) return electRecordDAO.getElectRecords(record);
