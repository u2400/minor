...
public class CreditConstraintServiceImpl extends BasicService implements CreditConstraintService {
  ...
  public Collection getStdCreditConstraints(List conditions, DataRealmLimit limit, List sortList) {
    if (null != limit && null != limit.getPageLimit() && limit.getPageLimit().isValid()) {
      ...
      if (null != limit && null != limit.getPageLimit()) {
        return new SinglePage(limit.getPageLimit().getPageNo(), limit.getPageLimit().getPageSize(), 0,
  ...
  public StdCreditConstraint getPersonCreditConstraints(Long constraintId) {
    StdCreditConstraint constraint = (StdCreditConstraint) utilDao.get(StdCreditConstraint.class,
        constraintId);
  ...
  public List getStdCreditConstraints(Student std, Collection calendars) {
    if (calendars.isEmpty()) return Collections.EMPTY_LIST;
    else return creditDAO.getStdCreditConstraints(std, calendars);
  }
  ...
  private void updateCreditConstraints(Collection constraints, Float value, boolean isCeil) {
    for (Iterator iter = constraints.iterator(); iter.hasNext();) {
      ...
      if (isCeil) constraint.setMaxCredit(value);
      else constraint.setMinCredit(value);
   ...
   * 选课专业学分初始化
   */
  public void initCreditConstraint(TeachCommon common, TeachCalendar calendar, OutputProcessObserver observer) {
    doInitCreditConstraint(teachPlanService.getTeachPlans(new TeachPlan(common), null, null, null, null),
        calendar, observer);
  }
  ...
  public void initCreditConstraint(String stdTypes, String departs, TeachCalendar calendar,
    ...
    Collection plans = teachPlanService.getTeachPlans(plan, limit, sortList, Boolean.TRUE, null);
   ...
   * @param observer
   */
  private void doInitCreditConstraint(Collection plans, TeachCalendar calendar, OutputProcessObserver observer) {
    ...
    for (Iterator iter = plans.iterator(); iter.hasNext();) {
      ...
      if (!constraints.isEmpty()) {
        constraint = (SpecialityCreditConstraint) creditDAO.getSpecialityCreditConstraints(common).get(0);
      }
      if (null != constraint) {
        if (null != observer) {
          observer.outputNotify(OutputObserver.warnning, new CreditConstraintInitMessage(
              "error.specialityCreditConstraint.existed", common, constraint.getMaxCredit()), true);
          continue;
        }
      }
      // 计算学期数
      ...
      try {
        ...
        observer.outputNotify(OutputObserver.error, new OutputMessage("", e.getMessage()), true);
        continue;
      }
      if (!termRange.containsInteger(term)) {
        if (null != observer) observer.outputNotify(OutputObserver.warnning, new CreditConstraintInitMessage(
            "error.specialityCreditConstraint.expiry", new TeachCommon(plan), null), true);
        continue;
      }
      // 计算计划对应学期的学分
      Float credit = teachPlanService.getCreditByTerm(plan, term);
      if (null == credit) credit = new Float(0);
      if (null != observer) observer.outputNotify(OutputObserver.good, new CreditConstraintInitMessage(
  ...
  public void initCreditConstraint(Long[] specialityConstraintIds, TeachCalendar calendar) {
    ...
    for (Iterator iter = constraints.iterator(); iter.hasNext();) {
      ...
      Collection plans = teachPlanService.getTeachPlans(new TeachPlan(common), null, null, null, null);
      if (plans.isEmpty()) {
  ...
  public void statStdCreditConstraint(Long[] stdCreditConstraintIds, CreditInitParams params) {
    List creditConstraints = creditDAO
        .getCreditConstraints(stdCreditConstraintIds, StdCreditConstraint.class);
  ...
  public void statStdCreditConstraint(StdCreditConstraint constraint, CreditInitParams params,
    ...
    for (Iterator iter = stdIds.iterator(); iter.hasNext();) {
      ...
      observer.outputNotify(OutputObserver.good, new OutputMessage("", buffer.toString()), true);
    }
  }
