...
public class ElectStdScopeServiceImpl extends BasicService implements ElectStdScopeService {
  ...
  public void batchUpdateTasksElectInfo(Collection tasks, ElectInitParams setting,
    ...
    if (null != observer) {
      observer.notifyStart(observer.messageOf("info.scope.setting", new Integer(tasks.size())), tasks.size(),
          null);
    }
    ...
    for (Iterator iter = tasks.iterator(); iter.hasNext();) {
      ...
      if (0 != setting.getAddScopeByAdminClass()) {
        ...
        if ((shareCondition & ElectStdScope.ENROLLYEAR_MARK) > 0) {
          if (StringUtils.isEmpty(task.getTeachClass().getEnrollTurn())) {
            ...
            cons.add(new Condition("task.teachClass.enrollTurn='" + task.getTeachClass().getEnrollTurn()
                + "'"));
        ...
        if ((shareCondition & ElectStdScope.STDTYPE_MARK) > 0) {
          if (null == task.getTeachClass().getStdType()) {
            ...
            cons.add(new Condition("task.teachClass.stdType.id=" + task.getTeachClass().getStdType().getId()));
        ...
        if ((shareCondition & ElectStdScope.DEPART_MARK) > 0) {
          if (null == task.getTeachClass().getDepart()) {
            ...
            cons.add(new Condition("task.teachClass.depart.id=" + task.getTeachClass().getDepart().getId()));
        ...
        if ((shareCondition & ElectStdScope.SPECIALITY_MARK) > 0) {
          if (null == task.getTeachClass().getSpeciality()) {
            ...
            reserved.setSpecialityIds("," + task.getTeachClass().getSpeciality().getId() + ",");
          }
        }
        if ((shareCondition & ElectStdScope.ASPECT_MARK) > 0) {
          if (null == task.getTeachClass().getAspect()) {
            ...
            cons.add(new Condition("task.teachClass.aspect.id=" + task.getTeachClass().getAspect().getId()));
        ...
        if (scopeMap.containsKey(sb.toString())) {
          ...
          scope = new ElectStdScope(utilService.search(query), setting.getAddScopeByAdminClass());
        }
      ...
      if (setting.isInitByTeachClass() != 0) {
        ElectStdScope scope = new ElectStdScope(task.getTeachClass(), setting.isInitByTeachClass());
        addScope(scope, task);
      }
      ...
      if (StringUtils.equals(setting.getBatchAction(), ElectInitParams.MERGE)) {
        for (Iterator iterator = task.getElectInfo().getElectScopes().iterator(); iterator.hasNext();) {
      ...
      if (setting.isUpdateMinWithRoomConfigAndPlanCount()) {
        ...
        task.getElectInfo().setMaxStdCount(
            new Integer(Math.min(policy1.getStdCountLimited().intValue(), policy2.getStdCountLimited()
                .intValue())));
      }
        ...
        case 0:
          ...
          if (StringUtils.contains("," + setting.getStdCountSwitchs() + ",", ",0,")) {
            for (Iterator it = rooms.iterator(); it.hasNext();) {
              ...
              minCount = (minCount != -1 ? Math.min(minCount, room.getCapacityOfCourse().intValue()) : room
                  .getCapacityOfCourse().intValue());
            }
          }
          if (StringUtils.contains("," + setting.getStdCountSwitchs() + ",", ",1,")) {
            minCount = (minCount != -1 ? Math
                .min(minCount, task.getTeachClass().getPlanStdCount().intValue()) : task.getTeachClass()
                .getPlanStdCount().intValue());
          }
          if (StringUtils.contains("," + setting.getStdCountSwitchs() + ",", ",2,")) {
            for (Iterator it = rooms.iterator(); it.hasNext();) {
              Classroom room = (Classroom) it.next();
              minCount = (minCount != -1 ? Math.min(minCount, room.getCapacity().intValue()) : room
      ...
      if (null != observer) {
        observer.outputNotify(OutputObserver.good, new OutputMessage("", "[" + task.getSeqNo() + "]"
            + task.getCourse().getName()), true);
      }
    }
