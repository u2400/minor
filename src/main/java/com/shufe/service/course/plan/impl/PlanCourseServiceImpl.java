...
public class PlanCourseServiceImpl extends BasicService implements PlanCourseService {
  ...
  public void addPlanCourse(PlanCourse planCourse, TeachPlan plan, boolean needReCalculateCredit) {
    CourseGroup myGroup = courseGroupService.getCourseGroup(planCourse.getCourseGroup().getId());
    ...
    if (null == firstAspectId) {
      degreeCourseList = utilService
          .searchHQLQuery("from PlanCourse pc where pc.isDegreeCourse = 1 and exists (from CourseGroup cg where cg.id=pc.courseGroup.id "
              + "and exists (from TeachPlan tp join tp.courseGroups as tcg where cg.id=tcg.id and tp.enrollTurn = '"
              + EnrollYear
              + "' and tp.department.id = "
              + deparmentId
              + " and tp.speciality.id = "
              + firstMajorId + "))");
    } else {
      degreeCourseList = utilService
          .searchHQLQuery("from PlanCourse pc where pc.isDegreeCourse = 1 and exists (from CourseGroup cg where cg.id=pc.courseGroup.id "
              ...
              + deparmentId
              + " and tp.speciality.id = "
              + firstMajorId + " and tp.aspect.id =" + firstAspectId + "))");
    }
   ...
   *      com.shufe.model.course.plan.TeachPlan, boolean)
   */
  public void removePlanCourse(PlanCourse planCourse, TeachPlan plan, boolean needReCalculateCredit) {
   ...
   *      com.shufe.model.course.plan.TeachPlan, boolean)
   */
  public void updatePlanCourse(PlanCourse planCourse, TeachPlan plan, boolean needReCalculateCredit) {
    PlanCourse savedPlanCourse = (PlanCourse) utilService.load(PlanCourse.class, planCourse.getId());
    ...
    savedPlanCourse.setPreCourses(planCourse.getPreCourses());
    ...
    if (null != planCourse1 && null != planCourse2) {
      try {
        ...
        for (Iterator it = map1.keySet().iterator(); it.hasNext();) {
          ...
          if (!ObjectUtils.equals(map1.get(key), map2.get(key))) { return false; }
        }
      } catch (Exception e) {
        return false;
      }
