...
public class TeachPlanServiceImpl extends BasicService implements TeachPlanService {
  ...
  public boolean isExist(String enrollTurn, Long stdTypeId, Long departmentId, Long specialityId,
    ...
    return isTeachPlanExists(new TeachPlan(enrollTurn, stdTypeId, departmentId, specialityId, aspectId, stdId));
  }
   ...
   */
  public Collection getTeachPlansOfTaskGeneration(TeachPlan plan, TeachCalendar calendar,
      String stdTypeIdSeq, String departIdSeq, Boolean generated, boolean omitSmallTerm) {
    if (null == calendar) {
      return Collections.EMPTY_LIST;
    } else {
  ...
  public Collection getTeachPlansOfActive(TeachCalendar calendar, String stdTypeIdSeq, String departIdSeq,
    ...
    if (StringUtils.isEmpty(stdTypeIdSeq) || StringUtils.isEmpty(departIdSeq)) {
      ...
      planQuery.add(new Condition("plan.stdType.id in (:stdTypeIds)", SeqStringUtil
          .transformToLong(stdTypeIdSeq)));
      planQuery.add(new Condition("plan.department.id in (:departIds)", SeqStringUtil
  ...
  public CourseType getCourseTypeOfStd(String stdNo, Course course, CourseType courseType, MajorType majorType) {
    if (null == majorType) { return null; }
    Student std = (Student) utilService.load(Student.class, "code", stdNo).get(0);
    TeachPlan plan = getTeachPlan(std, ((majorType.getId().equals(MajorType.FIRST)) ? Boolean.TRUE
        : Boolean.FALSE), null);
    ...
    if (null == plan) {
      ...
      if (planCourseType == null) {
        for (Iterator iter = plan.getCourseGroups().iterator(); iter.hasNext();) {
          ...
          if (courseType.getName().contains("通识类") && group.getCourseType().getName().contains("通识类")) {
            planCourseType = courseType;
            break;
          }
        }
  ...
  public PlanCourse getPlanCourse(Student std, Course course, Boolean isFirstSpeciality) {
    ...
    if (null == planCourse) {
      ...
      planCourse.getCourseGroup().setCourseType(
          (CourseType) utilDao.get(CourseType.class, CourseType.PUBLIC_COURSID));
    }
    return planCourse;
  }
  ...
  public List getTeachCalendarsOf(TeachPlan plan, List termList) {
    ...
    if (null == onCampusTime) { throw new OnCampusTimeNotFoundException(plan.getStdType().getName() + "  "
        + plan.getEnrollTurn()); }
    TeachCalendar calendar = onCampusTime.getEnrollCalendar();
    Collections.sort(termList);
    int maxTerm = ((Number) termList.get(termList.size() - 1)).intValue();
    if (maxTerm > plan.getTermsCount().intValue()) { throw new IllegalArgumentException("plan has max term:"
        + plan.getTermsCount() + ". but need:" + termList); }
