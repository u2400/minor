...
public class StdTeachPlanServiceImpl implements StdTeachPlanService {
   ...
   * 如果没有个人计划,则生成
   */
  public TeachPlan getTeachPlan(Student std, MajorType majorType, Boolean forceGen) {
    if (null == majorType) majorType = new MajorType(MajorType.FIRST);
    TeachPlan plan = teachPlanService.getTeachPlan(std,
        new Boolean(MajorType.FIRST.equals(majorType.getId())), null);
    if (null == plan) return null;
    if (plan.isStdTeachPlan()) return plan;
    else {
      if (null == forceGen) forceGen = Boolean.FALSE;
      if (Boolean.FALSE.equals(forceGen)) {
        return plan;
      } else {
        List newPlans = teachPlanService.genTeachPlanForStd(Collections.singleton(std), plan);
        if (!newPlans.isEmpty()) return (TeachPlan) newPlans.get(0);
        else return null;
      }
    }
  }
