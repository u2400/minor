...
public class BookRequireStatServiceImpl extends BasicService implements BookRequireStatService {

  public List statRequireBy(TeachCalendar calendar, DataRealm dataRealm, String attr, Class clazz) {
    EntityQuery entityQuery = new EntityQuery(BookRequirement.class, "require");
    entityQuery.setSelect("select new  com.shufe.service.util.stat.StatItem(" + attr
        + ",count(*),sum(require.countForStd+require.countForTeacher))");
    entityQuery.add(new Condition("require.task.calendar=:calendar", calendar));
    DataRealmUtils.addDataRealm(entityQuery, new String[] { "require.task.teachClass.stdType.id",
