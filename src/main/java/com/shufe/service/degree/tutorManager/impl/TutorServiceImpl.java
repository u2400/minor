...
public class TutorServiceImpl extends BasicService implements TutorService {
  ...
  public void batchModifyTeacherOfStd(String studentIds, Long tutorId) {
    List studentList = utilService.load(Student.class, "id", SeqStringUtil.transformToLong(studentIds));
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.service.degree.tutorManager.TutorService#getPropertyOfTutor(com.shufe.model.system
   * .baseinfo.Tutor,
   * java.lang.String, java.lang.String, java.lang.Boolean)
   */
  public List getPropertyOfTutor(Tutor tutor, String departIdSeq, String propertys, Boolean isCount) {
    return tutorDAO.getPropertyOfTutor(tutor, departIdSeq, propertys, isCount);
  }

  /*
   * (non-Javadoc)
   * @see
   * com.shufe.service.degree.tutorManager.TutorService#getStatisticMap(com.shufe.model.system.baseinfo
   * .Tutor,
   * java.lang.String, java.lang.String)
   */
  public Map getStatisticMap(Tutor tutor, String departIdSeq, String propertys) {
    Map returnMap = new HashMap();
    if (StringUtils.split(propertys, ",").length != 2) { return returnMap; }
    List results = tutorDAO.getPropertyOfTutor(tutor, departIdSeq, propertys, Boolean.TRUE);
    for (Iterator iter = results.iterator(); iter.hasNext();) {
      ...
      StatUtils.setValueToMap(null == baseCode ? "0-null" : "0-" + baseCode.getId(), element[2], "integer",
          returnMap);
      StatUtils.setValueToMap(null == department ? "null-0" : department.getId() + "-0", element[2],
          "integer", returnMap);
  ...
  public Map getSpecialityTutorMap(Tutor tutor, String departIdSeq) {
    ...
    for (Iterator iter = tutorList.iterator(); iter.hasNext();) {
      ...
      String keyValue = null == speciality ? "null" : speciality.getId().toString();
      if (null != element.getTeacherType()) {
        if (TeacherType.REGISTERTYPEID.equals(element.getTeacherType().getId())) {
          StatUtils.setValueToMap(keyValue + "ZS", element, "list", returnMap);
        } else if (TeacherType.TEMPTYPEID.equals(element.getTeacherType().getId())) {
          StatUtils.setValueToMap(keyValue + "JZ", element, "list", returnMap);
        } else if (TeacherType.BACKTYPEID.equals(element.getTeacherType().getId())) {
          StatUtils.setValueToMap(keyValue + "FP", element, "list", returnMap);
        }
      }
    }
    return returnMap;
  ...
  public Map getTutorTypeMap(Tutor tutor, String departIdSeq, List operatorList) {
    ...
    for (Iterator iter = tutorList.iterator(); iter.hasNext();) {
      ...
      int teacherAge = Calendar.getInstance().get(Calendar.YEAR) - teacherBirthday.get(Calendar.YEAR);
      ...
      if (null == gender) {
        ...
        if (null == tutorType) {
          ...
          keyValue = gender.getId() + "-" + tutorType.getId() + scoreMin;
        }
      }
      StatUtils.setValueToMap(keyValue, new Integer(1), "integer", returnMap);
      if (null != tutorType) {
        StatUtils.setValueToMap(
            null == gender ? "null-0" + scoreMin : "null-" + tutorType.getId() + scoreMin, new Integer(1),
            "integer", returnMap);
      }
      if (null != gender) {
        StatUtils.setValueToMap(null == tutorType ? "0-null" + scoreMin : gender.getId() + "-0" + scoreMin,
            new Integer(1), "integer", returnMap);
      }
      StatUtils.setValueToMap("0-0" + scoreMin, new Integer(1), "integer", returnMap);
    }
    return returnMap;
  }
