...
public class TopicOpenServiceImpl extends BasicService implements TopicOpenService {
  ...
  public boolean checkDepartAndTutorValidate(Long studentId) {
    ...
    if (null != thesisTopicOpen
        && (thesisTopicOpen.getDepartmentAffirm().booleanValue() || thesisTopicOpen.getTutorAffirm()
  ...
  public boolean checkDepartmentValidate(Student student) {
    ...
    if (null != thesisTopicOpen && thesisTopicOpen.getDepartmentAffirm().booleanValue()) return true;
    return false;
  }
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.service.thesisTopicOpen.ThesisTopicOpenService#checkThesisOpenAffirmByCode(java.lang
   * .String)
   */
  ...
  public TopicOpen getTopicOpen(Long studentId) {
    ...
    if (null != thesisManage) { return thesisManage.getTopicOpen(); }
    return null;
  }
  ...
  public void batchAffirmTimeAndAddress(Long[] selectIds, String openTime, String openAddress) {
    if (selectIds == null || selectIds.length == 0 || StringUtils.isEmpty(openTime)
        || StringUtils.isEmpty(openAddress)) { return; }
   ...
   */
  public Pagination getstdsNoOpened(Student student, String departmentIdSeq, String stdTypeIdSeq, int pageNo,
      int pageSize) {
    return topicOpenDAO.getStdsNoOpened(student, departmentIdSeq, stdTypeIdSeq, pageNo, pageSize);
  }
