...
public class ThesisStatisticServiceImpl extends BasicService implements ThesisStatisticService {
  ...
  public List getstdGroupInfo(Student student, String departmentIdSeq, String stdTypeIdSeq) {
    return thesisStatisticDAO.getstdGroupInfo(student, departmentIdSeq, stdTypeIdSeq);
  }

  public List getTOpicOpenGroupInfo(ThesisManage thesisManage, String departmentIdSeq, String stdTypeIdSeq) {
    return thesisStatisticDAO.getTOpicOpenGroupInfo(thesisManage, departmentIdSeq, stdTypeIdSeq);
  }

  /*
   * (non-Javadoc)
   * @see
   * com.shufe.service.degree.thesis.thesisStatistic.ThesisStatisticService#getThesisAndTeacherMap
   * (java.lang.String, java.util.Set, java.lang.String)
   */
  public Map getThesisAndTeacherMap(String stdTypeIdSeq, Set stdTypes, String requireYear) {
    Map requestMap = new HashMap();
    Collection requireMents = thesisStatisticDAO.getThesisAndTeachers(stdTypeIdSeq, stdTypes, requireYear);
    ...
    for (Iterator iter = requireMents.iterator(); iter.hasNext();) {
      ...
      titles.add(title);
      StatUtils.setValueToMap(enrollYear + "-0", thesisNum, "integer", requestMap);
      StatUtils.setValueToMap("0-0", thesisNum, "integer", requestMap);
      if (null == title) {
        StatUtils.setValueToMap(enrollYear + "-teacher-null", teacherTitls, "integer", requestMap);
        StatUtils.setValueToMap("0-teacher-null", teacherTitls, "integer", requestMap);
      } else {
        StatUtils
            .setValueToMap(enrollYear + "-teacher-" + title.getId(), teacherTitls, "integer", requestMap);
        StatUtils.setValueToMap("0-teacher-" + title.getId(), teacherTitls, "integer", requestMap);
      }
      StatUtils.setValueToMap(enrollYear + "-teacher-0", teacherTitls, "integer", requestMap);
      StatUtils.setValueToMap("0-teacher-0", teacherTitls, "integer", requestMap);
    }
