...
public class StudyProductServiceImpl extends BasicService implements StudyProductService {
  ...
  public Map getStdPropertyOfProduct(Map studyValueMap, String departIdSeq, String stdTypeIdSeq) {
    ...
    List statisThesises = studyProductDAO.getStdPropertyProduct(thesis, departIdSeq, stdTypeIdSeq,
        "student.department,student.type,publicationLevel", true);
    setValueToMap(statisThesises, returnMap, "thesis");

    // 项目
    Project project = new Project();
    EntityUtils.populate(studyValueMap, project);
    List statisticProject = studyProductDAO.getStdPropertyProduct(project, departIdSeq, stdTypeIdSeq,
        "student.department,student.type,projectType", false);
    setValueToMap(statisticProject, returnMap, "project");

    // 著作
    Literature literature = new Literature();
    EntityUtils.populate(studyValueMap, literature);
    List statisticLiterature = studyProductDAO.getStdPropertyProduct(literature, departIdSeq, stdTypeIdSeq,
        "student.department,student.type,literatureType", false);
  ...
  public void setValueToMap(List studyProducts, Map value, String diffStr) {
    for (Iterator iter = studyProducts.iterator(); iter.hasNext();) {
      ...
      StatUtils.setValueToMap("0_0_" + diffStr + entity.getEntityId(), productNum, "integer", value);
    }
  }
   ...
   */
  public Pagination getStudyProductPagi(Object studyProduct, String departIdSeq, String stdTypeSeq,
      int pageNo, int pageSize) {
    return studyProductDAO.getStudyProductPagi(studyProduct, departIdSeq, stdTypeSeq, pageNo, pageSize);
  }
  ...
  public Map getStudyProducts(Long stdId, Boolean isNeedList, Boolean isPassed) {
    ...
    for (Iterator iter = awardSet.iterator(); iter.hasNext();) {
      ...
      StatUtils.setValueToMap("awards" + studyAward.getType().getId(), new Integer(1), "integer",
          studyResults);
    }
    // 学生科研论文
    studyMap.clear();
    ...
    for (Iterator iter = studyThesiss.iterator(); iter.hasNext();) {
      ...
      for (Iterator iterator = studyThesis.getIndexes().iterator(); iterator.hasNext();) {
        ...
        StatUtils.setValueToMap("index" + thesisIndex.getThesisIndexType().getId(), new Integer(1),
    ...
    for (Iterator iter = studyMeetings.iterator(); iter.hasNext();) {
      ...
      StatUtils.setValueToMap("meeting" + studyMeeting.getMeetingType().getId(), new Integer(1), "integer",
          studyResults);
    }
    // 学生科研著作
    studyMap.clear();
    ...
    for (Iterator iter = literatures.iterator(); iter.hasNext();) {
      ...
      StatUtils.setValueToMap("literature" + literature.getLiteratureType().getId(), new Integer(1),
          "integer", studyResults);
    }
