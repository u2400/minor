...
public class DegreeServiceImpl extends BasicService implements DegreeService {
   ...
   *      Long[], Long[], int, int)
   */
  public Pagination getPaginas(DegreeApply degreeApply, Long[] departmentSeq, Long[] stdTypeSeq, int pageNo,
      int pageSize) {
  ...
  public String returnDegreeApplyDataPage(StudentType stdType) {
    StudentType studentType = studentTypeService.getStudentType(StudentType.PROFESSIONAL_MASTER);
    ...
    } else if (Degree.EQUIVALENTDOCTOR.equals(stdType.getDegree().getId())) { return "degreeApplyEquivalentMaster"; }
    return null;
  }
}
