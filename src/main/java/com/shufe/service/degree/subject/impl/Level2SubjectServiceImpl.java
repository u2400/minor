...
public class Level2SubjectServiceImpl extends BasicService implements Level2SubjectService {
  ...
  public Pagination getLevel2Subjects(Level2Subject subject, int pageSize, int pageNo) {
    ...
    if (pageSize < 0 || pageNo < 0) return new Pagination(new Result(0, Collections.EMPTY_LIST));
    else return level2SubjectDAO.getLevel2Subjects(subject, pageSize, pageNo);
  }
  ...
  public Map getAllStatisticDataByCollege(Level2Subject level2Subject) {
    ...
    for (Iterator iter = level2Subjects.iterator(); iter.hasNext();) {
      ...
      if (subject2.getForDoctor().booleanValue()) {
        StatUtils.setValueToMap(department.getId() + "-doctor", subject2, "list", returnMap);
        countDoctor++;
      }
      if (subject2.getForMaster().booleanValue()) {
        StatUtils.setValueToMap(department.getId() + "-master", subject2, "list", returnMap);
        countMaster++;
      }
    }
  ...
  /**
   * @see com.shufe.service.degree.subject.Level2SubjectService#getPropertyOfSubject(com.shufe.model.degree.subject.Level2Subject,
