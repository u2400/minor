...
import com.shufe.service.quality.evaluate.EvaluateResultLdService;

public class EvaluateResultLdServiceImpl extends BasicService implements EvaluateResultLdService {

  public List getTaskIdAndTeacherIdOfResult(Student student, Collection teachCalendars) {
    List results = new ArrayList();
    if (null == student || null == teachCalendars) { return Collections.EMPTY_LIST; }
    EntityQuery entityQuery = new EntityQuery(EvaluateResultLd.class, "result");
    ...
    entityQuery.setSelect("select result.task.id,result.teacher.id,task.requirement.evaluateByTeacher");
    results.addAll(utilService.search(entityQuery));
    return results;
  }

  public List getUserIdAndTeacherIdOfResult(User user, Collection teachCalendars) {
    List results = new ArrayList();
    if (null == user || null == teachCalendars) { return Collections.EMPTY_LIST; }
    EntityQuery entityQuery = new EntityQuery(EvaluateResultLd.class, "result");
    entityQuery.join("result.task", "task");
    entityQuery.add(new Condition("result.user=:user", user));
    entityQuery.add(new Condition("result.teachCalendar in(:calendars)", teachCalendars));
    entityQuery.setSelect("select result.task.id,result.teacher.id,task.requirement.evaluateByTeacher");
    results.addAll(utilService.search(entityQuery));
    return results;
   ...
   *      java.lang.Long, Long)
   */
  public EvaluateResultLd getResultByStdIdAndTaskId(Long stdId, Long taskId, Long teacherId) {
    EntityQuery query = new EntityQuery(EvaluateResultLd.class, "result");
    ...
    return results.size() > 0 ? (EvaluateResultLd) results.iterator().next() : null;
  }

  public EvaluateResultLd getResultByUserIdAndTaskId(Long userId, Long taskId, Long teacherId) {
    EntityQuery query = new EntityQuery(EvaluateResultLd.class, "result");
    query.add(new Condition("result.user.id=:userId", userId));
    query.add(new Condition("result.task.id=:taskId", taskId));
    if (null != teacherId) {
      query.add(new Condition("result.teacher.id=:teacherId", teacherId));
    } else {
