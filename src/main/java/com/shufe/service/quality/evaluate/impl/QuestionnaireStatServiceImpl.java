...
public class QuestionnaireStatServiceImpl extends BasicService implements QuestionnaireStatService {
  ...
  public void statEvaluateResult(String stdTypeIdSeq, String departIdSeq, String calendarIdSeq, User user) {
    ...
    if (beforStat(stdTypeIdSeq, departIdSeq, calendarIdSeq)) {
      ...
      Iterator resultIterator = getQuestionTypeStats(stdTypeIdSeq, departIdSeq, calendarIdSeq).iterator();
      ...
      while (resultIterator.hasNext()) {
        ...
        if (!ObjectUtils.equals(teacherId, newTeacherId) || !taskId.equals(newTaskId)) {
          ...
          questionnaireStat = new QuestionnaireStat(teachTask, newTeacherId == null ? null : new Teacher(
              newTeacherId), vaildTicket);
          savingEvaluateResults.add(questionnaireStat);
        }
        teacherId = newTeacherId;
        taskId = newTaskId;
        questionnaireStat.addQuestionTypeStat(new QuestionTypeStat(new QuestionType(questionTypeId),
            (totleMark <= 0) ? new Float(0) : new Float((totleScore / totleMark) * 100)));
      }
      if (null != questionnaireStat) {
        questionnaireStat.calcScore();
      }
      ...
      for (Iterator it1 = savingEvaluateResults.iterator(); it1.hasNext();) {
        ...
        if (null == stat.getTeacher()) {
          if (Boolean.FALSE.equals(stat.getTask().getRequirement().getEvaluateByTeacher())) {
            for (Iterator it2 = stat.getTask().getArrangeInfo().getTeachers().iterator(); it2.hasNext();) {
      ...
      info(user.getName() + " has evaluated '" + departIdSeq + "''s studentEvaluate.  StudentTypeId is '"
          + stdTypeIdSeq + "' teachCalendarIdSeq" + calendarIdSeq + "statistic about "
  ...
  public int batchDeleteQuestionnaireStat(String stdTypeIdSeq, String departIdSeq, String calendarIdSeq) {
    return questionnaireStatDAO.batchDeleteQuestionnaireStat(stdTypeIdSeq, departIdSeq, calendarIdSeq);
  ...
  public List getQuestionTypeStats(String stdTypeIdSeq, String departIdSeq, String calendarIdSeq) {
    return questionnaireStatDAO.getQuestionTypeStats(stdTypeIdSeq, departIdSeq, calendarIdSeq, Boolean.TRUE);
  }
   ...
   */
  public Map getDataByDepartAndRelated(String stdTypeIdSeq, String departIdSeq, Long calendarIdSeq,
      Map optionNameMap) {
    List tempList = questionnaireStatDAO.getDataByDepartAndRelated(stdTypeIdSeq, departIdSeq, calendarIdSeq);
    ...
    for (Iterator iter = tempList.iterator(); iter.hasNext();) {
      ...
      StatUtils.setValueToMap(department.getId() + "-all", new Integer(1), "integer", returnMap);
   ...
   */
  public Map getDatasOfPieChar(String stdTypeIdSeq, Long departId, Long calendarId, Map optionNameMap) {
    List chartPieList = questionnaireStatDAO.getDatasForPieChart(stdTypeIdSeq, departId, calendarId);
    float excellentvalue = ((Float) optionNameMap.get("A")).floatValue();
    float bettervalue = ((Float) optionNameMap.get("B")).floatValue();
    float ordinaryvalue = ((Float) optionNameMap.get("C")).floatValue();
    Map returnMap = new HashMap();
    for (Iterator iter = chartPieList.iterator(); iter.hasNext();) {
      ...
      } else StatUtils.setValueToMap("D", new Integer(1), "integer", returnMap);
