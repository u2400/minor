...
public class InstructWorkloadServiceImpl extends BasicService implements InstructWorkloadService {
  ...
  public InstructModulus getInstructModulus(Long studentTypeId, Long departmentId) {
    if (null == studentTypeId || null == departmentId) { return null; }
    Modulus modulus = new Modulus();
    modulus.getStudentType().setId(studentTypeId);
    modulus.getDepartment().setId(departmentId);
    return (InstructModulus) instructModulusService.getUniqueModulus(modulus, String.valueOf(studentTypeId));
  }

  /*
   * (non-Javadoc)
   * @see
   * com.shufe.service.workload.impl.InstructWorkloadService#findStatisticObjects(org.hibernate.
   * Criteria,
   * int, int)
   */
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.service.workload.impl.InstructWorkloadService#getStatisticsObjects(com.shufe.model
   * .workload.InstructWorkload,
   * java.lang.String)
   */
  public Criteria getStatisticsObjects(InstructWorkload instructWorkload, String departmentIds, String age1,
      String age2) {
    return instructWorkloadDAO.getExampleInstructWorkloads(instructWorkload, departmentIds, age1, age2);
  }
  ...
  public void affirmType(String affirmType, String teachWorkloadIds, boolean b) {
    if (StringUtils.isNotEmpty(teachWorkloadIds)) instructWorkloadDAO.affirmType(affirmType,
        teachWorkloadIds, b);
  }
  ...
  public Collection getCurrentCalendarByStdType(String studentTypeIds) {
    ...
    for (Iterator iter = teachCalendars.iterator(); iter.hasNext();) {
      ...
      if (element.getStart().before(calendar.getTime()) && element.getFinish().after(calendar.getTime())) {
  ...
  public void statWorkload(TeachCalendar teachCalendar, DataRealm realm, String modulusType) {
    ...
    List instructModuluses = instructModulusService.getModulus(modulusExample, realm.getStudentTypeIdSeq());
    ...
    if (modulusType.equals(InstructModulus.PARCTICE)) {
      ...
      stdTypeTeachers = graduatePracticeService.getPropertyOfPractices(graduatePractice,
          realm.getDepartmentIdSeq(), realm.getStudentTypeIdSeq(), "student.type,practiceTeacher",
          Boolean.TRUE);
      ...
      stdTypeTeachers = instructionDAO.statStdTypeTeacher(teachCalendar, realm, Boolean.FALSE);
    }
