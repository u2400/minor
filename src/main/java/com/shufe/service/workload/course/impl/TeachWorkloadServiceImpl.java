...
public class TeachWorkloadServiceImpl extends BasicService implements TeachWorkloadService {
  ...
  public Set getCurrenTeachCalendars(String stdTypeSeq) {
    if (StringUtils.isBlank(stdTypeSeq)) { return Collections.EMPTY_SET; }
  ...
  public Collection getHistoryCalendarByStdList(Collection stdTypeList) {
    if (null != stdTypeList && stdTypeList.size() < 1) { return Collections.EMPTY_LIST; }
  ...
  public List getTeachWorkloadAffirmTaskIds(TeachWorkload teachWorkload, String stdTypeIdSeq,
    ...
    return teachWorkloadDAO.getTeachWorkloadAffirmedTaskIds(teachWorkload, stdTypeIdSeq, departIdSeq,
        calendarIdSeq);
  }
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.service.workload.TeachWorkloadService#getStatisticsObjectList(com.shufe.model.workload
   * .TeachWorkload,
   * java.lang.String, java.lang.String, java.util.Collection, java.lang.String,
   * java.lang.String)
   */
  public List getStatisticsObjectList(TeachWorkload teachWorkload, String stdTypeIdSeq, String departIdSeq,
      String calendarIdSeq, String age1, String age2) {
    if (StringUtils.isBlank(stdTypeIdSeq) || StringUtils.isBlank(departIdSeq)
        || StringUtils.isBlank(calendarIdSeq)) { return Collections.EMPTY_LIST; }
    return teachWorkloadDAO.getExampleTeachWorkloads(teachWorkload, stdTypeIdSeq, departIdSeq, calendarIdSeq,
        age1, age2).list();
  }
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.service.workload.TeachWorkloadService#getStatisticsObjectPagi(com.shufe.model.workload
   * .TeachWorkload,
   * java.lang.String, java.lang.String, java.util.Collection, java.lang.String,
   * java.lang.String, int, int)
   */
  public Pagination getStatisticsObjectPagi(TeachWorkload teachWorkload, String stdTypeIdSeq,
      String departIdSeq, String calendarIdSeq, String age1, String age2, int pageNo, int pageSize) {
    if (StringUtils.isBlank(stdTypeIdSeq) || StringUtils.isBlank(departIdSeq)
        || StringUtils.isBlank(calendarIdSeq)) {
      Result result = new Result(0, new ArrayList());
      return new Pagination(pageNo, pageSize, result);
    }
    return teachWorkloadDAO.dynaSearch(teachWorkloadDAO.getExampleTeachWorkloads(teachWorkload, stdTypeIdSeq,
        departIdSeq, calendarIdSeq, age1, age2), pageNo, pageSize);
  }
  ...
  public Map statTeachWorkload(String stdTypeIdSeq, String departIdSeq, String calendarIdSeq) {
    ...
    List affirmTaskIds = getTeachWorkloadAffirmTaskIds(new TeachWorkload(), stdTypeIdSeq, departIdSeq,
        calendarIdSeq);
    // 用户权限内所能查询到的教学任务
    List teachTaskList = teachWorkloadDAO.getTasksOfTeacherByChargeDepart(stdTypeIdSeq, departIdSeq,
        calendarIdSeq);
    int successNum = 0;
    ...
    for (Iterator taskIter = teachTaskList.iterator(); taskIter.hasNext();) {
      ...
      for (Iterator iter = teachers.iterator(); iter.hasNext();) {
        ...
        try {
          ...
          e.printStackTrace();
          info("NO:" + teacher.getCode() + "teachTaskNo:" + teachTask.getId() + ":baseCode errors");
          noArrangeInfos += "课程序号:" + teachTask.getSeqNo() + "&课程名称:" + teachTask.getCourse().getName() + ",";
          noArrangeInfoNum++;
          continue;
        }
        if (teachWorkload.getTeachModulus() == null) {
          noTeachModulus += "课程序号:" + teachTask.getSeqNo() + "&课程名称:" + teachTask.getCourse().getName() + ",";
          noModuleNum++;
          continue;
        }
        successNum++;
   ...
   * @return
   */
  public static TeachModulus getModulu(StudentType stdType, Long courseCategroyId, Integer stdNumber,
      List modules) {
    TeachModulus teachModulus = null;
    while (null != stdType) {
      for (Iterator iter = modules.iterator(); iter.hasNext();) {
        TeachModulus element = (TeachModulus) iter.next();
        if (null != courseCategroyId && courseCategroyId.equals(element.getCourseCategory().getId())
   ...
   * @return
   */
  public static TeachCategory getCategory(StudentType stdType, Long courseTypeId, List teachCategorys) {
    ...
    while (null != stdType) {
      for (Iterator iter = teachCategorys.iterator(); iter.hasNext();) {
        ...
        if (null != courseTypeId && courseTypeId.equals(element.getCourseType().getId()) && null != stdType
            && element.getStudentType().getId().equals(stdType.getId())) {
      ...
      if (null != stdType) stdType = (StudentType) stdType.getSuperType();
    }
