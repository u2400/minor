...
public class TeachCategoryServiceImpl extends BasicService implements TeachCategoryService {
  ...
  public Pagination getPaginationOfValue(int pageNo, int pageSize) {
    return teachCategoryDAO.dynaSearch(teachCategoryDAO.getExampleOfCategory(false), pageNo, pageSize);
  }

  /*
   * (non-Javadoc)
   * @see
   * com.shufe.service.workload.TeachCategoryService#getCategoryByStdTypeAndCourseType(com.shufe
   * .model.system.baseinfo.StudentType,
   * CourseType)
   */
  public TeachCategory getCategoryByStdTypeAndCourseType(StudentType studentType, CourseType courseType) {
    TeachCategory teachCategory = null;
    if (courseType != null) {
      teachCategory = teachCategoryDAO.getTeachCategoryBystdTypeAndcourseType(studentType.getId(),
          courseType.getId());
    }
