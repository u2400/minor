...
public class TeachModulusServiceImpl extends BasicService implements ModulusService {
  ...
  /**
   * @see com.shufe.service.workload.ModulusService#getUniqueModulus(com.shufe.model.workload.course.TeachModulus,
   *      java.lang.String)
   */
  ...
  /**
   * @see com.shufe.service.workload.ModulusService#getModulusByConditions(TeachModulus,
   *      java.lang.Integer)
   */
  public Boolean getModulusByConditions(Modulus modulus, Integer personNum) {
    Boolean flag = Boolean.FALSE;
    if (null == modulus || null == personNum) { return flag; }
    String stdTypeIdSeq = null;
    if (null != modulus.getStudentType() && null != modulus.getStudentType().getId()) {
    ...
    for (Iterator iter = tempModulus.iterator(); iter.hasNext();) {
      ...
      if (element.getMinPeople().compareTo(personNum) <= 0 && element.getMaxPeople().compareTo(personNum) > 0) {
        flag = Boolean.TRUE;
        break;
      }
    }
  ...
  public Modulus getUniqueModulus(Modulus modulus, String studentTypeId) {
    ...
    return (teachModulus.size() > 0) ? (TeachModulus) teachModulus.get(0) : null;
  }
