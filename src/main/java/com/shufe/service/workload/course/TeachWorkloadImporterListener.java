...
public class TeachWorkloadImporterListener extends ItemImporterListener {
  ...
  public void endTransferItem(TransferResult tr) {
    ...
    if (checkEmpty(tr, teacherNo, "teacher-NO is not pointed.") == false) {
      Teacher teacher = (Teacher) getDBExistObject(tr, Teacher.class, "teacher", "teacher.code = (:NO)",
          teacherNo);
      if (teacher == null) {
    ...
    // 课程种类
    try {
      if (checkEmpty(workload.getCourseCategory()) || checkEmpty(workload.getCourseCategory().getCode())) {
        workload.setCourseCategory(courseCategory);
      } else {
        checkEmpty(tr, workload.getCourseCategory().getCode(), "course category is not pointed.");
      }
    } catch (Exception e) {
    ...
    // 课程序号
    if (!existsCalendar) {
      tr.addFailure("can't get the value of the teach-task " + "because calendar is invalid.", "");
    } else {
      String seqNo = workload.getCourseSeq();
      if (checkEmpty(seqNo) == false && checkEmpty(tr, seqNo, "task NO can't empty.") == false) {
        TeachTask task = (TeachTask) getDBExistObject(tr, TeachTask.class, "task",
            "task.seqNo = (:no) and task.calendar.id = (:id)", seqNo, workload.getTeachCalendar().getId(),
            false);
        if (task != null) {
    ...
    // 开课院系代码
    try {
      checkEmpty(tr, workload.getCollege().getCode(), "the code of department of college is not pointed.");
    } catch (Exception e) {
    ...
    // 支付报酬
    checkEmpty(tr, workload.getPayReward(), "whether or not pay reward is empty value or invalid value.",
        dataMap.get("payReward"));
    // 计工作量
    checkEmpty(tr, workload.getCalcWorkload(),
        "whether or not calc-workload is empty value or invalid value.", dataMap.get("calcWorkload"));
    // 教师确认
    checkEmpty(tr, workload.getTeacherAffirm(),
        "whether or not need teacher-affirm is empty value or invalid value.", dataMap.get("teacherAffirm"));
    // 院系确认
    checkEmpty(tr, workload.getCollegeAffirm(),
        "whether or not need college-affirm is empty value or invalid value.", dataMap.get("collegeAffirm"));
    // 是否授课工作量
    checkEmpty(tr, workload.getIsTeachWorkload(),
        "whether or not need teach-workload is empty value or invalid value.", dataMap.get("isTeachWorkload"));
    // 上课周数
    checkEmpty(tr, workload.getWeeks(), "the number of weeks in class are not pointed.", dataMap.get("weeks"));
    // 总课时
    checkEmpty(tr, workload.getTotleCourses(), "total courses are not pointed.", dataMap.get("totleCourses"));
    // 周课时
    checkEmpty(tr, workload.getClassNumberOfWeek(), "the total number of weeks in class are not pointed.",
        dataMap.get("classNumberOfWeek"));
    // 授课工作量
    checkEmpty(tr, workload.getTotleWorkload(), "the total number of workloads are not pointed.",
        dataMap.get("totleWorkload"));
    // 备注
    /* 允许为空值 */
   ...
   * @return
   */
  private Object getDBExistObject(TransferResult tr, Class clazz, String field, String whereStr, Object obj1,
      Object obj2, boolean isOutputErrorMessages) {
   ...
   * @return
   */
  private Object getDBExistObject(TransferResult tr, Class clazz, String field, String whereStr, Object obj) {
   ...
   * @return
   */
  private Object getObject(TransferResult tr, Object obj, boolean isOutputErrorMessages, EntityQuery query) {
  ...
  private boolean checkEmpty(Object obj) {
    if (obj == null || (obj instanceof String) && StringUtils.isEmpty(((String) obj))) { return true; }
    return false;
  }
  ...
  private boolean checkTeachCalendar(TransferResult tr, TeachWorkload workload, Object value) {
    ...
    if (checkEmpty(tr, year, "the academic year is empty.")) { return false; }
    String term = workload.getTeachCalendar().getTerm();
    if (checkEmpty(tr, term, "the academic term is empty.")) { return false; }
