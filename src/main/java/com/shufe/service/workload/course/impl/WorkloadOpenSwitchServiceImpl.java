...
public class WorkloadOpenSwitchServiceImpl extends BasicService implements WorkloadOpenSwitchService {

  private WorkloadOpenSwitchDAO workloadOpenSwitchDAO;

  private TeachCalendarService teachCalendarService;

  public void setWorkloadOpenSwitchDAO(WorkloadOpenSwitchDAO workloadOpenSwitchDAO) {
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.service.workload.WorkloadButtonService#getCalendarsOfCurrentAndHistory(java.lang.
   * String)
   */
  public List getCalendarsOfCurrentAndHistory(String stdTypeIdSeq) {
    List conditionList = new ArrayList();
    List teachCalendars = teachCalendarService.getTeachCalendars(stdTypeIdSeq);
    Date date = new Date(System.currentTimeMillis());
    for (Iterator iter = teachCalendars.iterator(); iter.hasNext();) {
      TeachCalendar teachCalendar = (TeachCalendar) iter.next();
      if (teachCalendar.getStart().before(date)) conditionList.add(teachCalendar);
    }
  ...
  public WorkloadOpenSwitch getWorkloadByCalendarId(Long teachCalendarId) {
    List workloadButtons = workloadOpenSwitchDAO.getWorkloadButtonByCalendarSeq(String
        .valueOf(teachCalendarId));
    return workloadButtons.size() > 0 ? (WorkloadOpenSwitch) workloadButtons.get(0)
  ...
  public List getWorkloadsByCalendarIdSeq(String teachCalendarIdSeq) {
    return workloadOpenSwitchDAO.getWorkloadButtonByCalendarSeq(teachCalendarIdSeq);
  }
