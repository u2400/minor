...
public class DataRealmUtils {
  ...
  public static void addDataRealms(EntityQuery query, String[] attrs, List dataRealms) {
    if (CollectionUtils.isEmpty(dataRealms)) return;
    ...
    for (int i = 0; i < dataRealms.size(); i++) {
      ...
      if (attrs.length > 1) {
        if (StringUtils.isNotEmpty(dataRealm.getDepartmentIdSeq()) && StringUtils.isNotEmpty(attrs[1])) {
          ...
          buffer.append(" exists (from Department mydepart where mydepart.id =" + attrs[1]);
          buffer.append(" and mydepart.id in(:myDepartIds" + RandomUtils.nextInt() + "))");
