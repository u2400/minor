  ...
  public boolean evaluate(Object arg0) {
    try {
      ...
      if (StringUtils.isNotEmpty(stdTypeDataRealm)) {
        ...
        if ((null != stdTypeId) && !StringUtils.contains(stdTypeDataRealm, stdTypeId.toString())) return false;
      }
      if (StringUtils.isNotEmpty(departDataRealm)) {
        Long departId = (Long) PropertyUtils.getProperty(arg0, departAttrName + ".id");
        if ((null != departId) && !StringUtils.contains(departDataRealm, departId.toString())) return false;
      }
      return true;
    } catch (Exception e) {
