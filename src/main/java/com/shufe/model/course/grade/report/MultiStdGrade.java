...
public class MultiStdGrade {
  ...
  public MultiStdGrade(TeachCalendar calendar, List grades, Float ratio) {
    ...
    for (Iterator iter = grades.iterator(); iter.hasNext();) {
      ...
      StdGrade stdGrade = (StdGrade) gradesMap.get(grade.getStd().getId());
      if (null == stdGrade) {
    ...
    int maxStdCount = ((CourseStdNum) (courseStdNums.get(0))).getCount().intValue();
    ...
    for (Iterator iter = stdGrades.iterator(); iter.hasNext();) {
      ...
      for (Iterator iterator = stdGrade.getGrades().iterator(); iterator.hasNext();) {
...
class CourseStdNum implements Comparable {
  ...
  public int compareTo(Object object) {
    ...
    return new CompareToBuilder().append(myClass.count, this.count).toComparison();
  }
}
