...
public class TeachClassGrade {
  ...
  public TeachClassGrade(List gradeTypes, TeachTask task, List courseGrades) {
    ...
    if (!CollectionUtils.isEmpty(courseGrades)) {
      Collections.sort(courseGrades, new PropertyComparator("std.code"));
    }
  }

  public static List buildTaskClassGrade(List gradeTypes, TeachTask task, List courseGrades,
      Integer stdPerClass) {
    List classGrades = new ArrayList();
    TeachClassGrade teachClassGrade = new TeachClassGrade(gradeTypes, task, courseGrades);
    int begin = 0;
    while (teachClassGrade.getCourseGrades().size() - begin > stdPerClass.intValue()) {
      ...
      other.setCourseGrades(new ArrayList(teachClassGrade.getCourseGrades().subList(begin, end)));
