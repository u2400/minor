...
public class TimeTable implements Serializable {
  ...
  public void addActivities(Collection activities) {
    for (Iterator iter = activities.iterator(); iter.hasNext();) {
      ...
      for (int i = 0; i < time.getUnitCount(); i++) {
        Integer unitIndex = new Integer(time.getWeekId().intValue() * TeachCalendar.MAXUNITS
            + time.getStartUnit().intValue() + i);
        Long weeksNum = (Long) occupyMap.get(unitIndex);
        // add it
        if (null == weeksNum) {
          occupyMap.put(unitIndex, time.getValidWeeksNum());
        } else {
          occupyMap.put(unitIndex, new Long(time.getValidWeeksNum().longValue() | weeksNum.longValue()));
        }
      }
    }
  }
  ...
  public void removeArrangement(Arrangement arrangement) {
    for (Iterator iter = arrangement.getArrangeInfo().getActivities().iterator(); iter.hasNext();) {
      Activity activity = (Activity) iter.next();
      TimeUnit time = activity.getTime();
      Map occupyMap = (Map) yearOccupyMap.get(activity.getTime().getYear());
      if (null == occupyMap) {
    ...
    for (Iterator iter = activities.iterator(); iter.hasNext();) {
      ...
      for (int i = 0; i < time.getUnitCount(); i++) {
        ...
        if (null == weeksNum) {
          ...
          if ((weeksNum.longValue() & activity.getTime().getValidWeeksNum().longValue()) > 0) { return true; }
        }
      }
    }
    return false;
