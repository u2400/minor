...
public class OccupyUnit implements Comparable {
  ...
  public OccupyUnit(int year, long validWeeksNum, int weekId, int startUnit, int endUnit) {
    this.year = year;
  ...
  public String getOccupyString(int from, int begin, int end, boolean occupy) {
    ...
    if (occupy && validWeeksNum == 0) return "";
    if (!occupy && validWeeksNum == TimeUnit.FULL_OCCUPY_WEEKNUM) return "";
    ...
    String validWeeks = StringUtils.repeat("0", TeachCalendar.OVERALLWEEKS - binaryStr.length()) + binaryStr;

    String weekDigest = TimeUnitUtil.digest(validWeeks, from, begin, end, null, null, " -  ");
    // if no week was occupied then return empty string
    if (StringUtils.isEmpty(weekDigest)) return "";
  ...
  public boolean canMergerWith(OccupyUnit other) {
    if (weekId == other.weekId) {
      if ((startUnit == other.startUnit) && (endUnit == other.endUnit)) return true;
      else {
        if (validWeeksNum == other.validWeeksNum) return (startUnit == other.endUnit + 1)
  ...
  public int compareTo(Object object) {
    ...
    return new CompareToBuilder().append(this.weekId, myClass.weekId)
        .append(this.startUnit, myClass.startUnit).append(this.endUnit, myClass.endUnit).toComparison();
  }
  ...
  public String toString() {
    return new ToStringBuilder(this, ToStringStyle.MULTI_LINE_STYLE)
        .append("validWeeks", TimeUnit.weekOccupyString(this.validWeeksNum)).append("weekId", this.weekId)
        .append("endUnit", this.endUnit).append("startUnit", this.startUnit).append("year", this.year)
        .toString();
  }
}
