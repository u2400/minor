...
public class TaskGroup extends LongIdObject {
  ...
  public List getTaskList() {
    ...
    if (null != directTasks && !directTasks.isEmpty()) taskList.addAll(directTasks);
  ...
  private void mergeTaskOfSameTeacher() {
    ...
    HashMap<List, TeachTask> taskMap = new HashMap<List, TeachTask>();
    for (Iterator iterator = sortedTasks.iterator(); iterator.hasNext();) {
      TeachTask task = (TeachTask) iterator.next();// 获取所有的教学任务

      if (taskMap.containsKey(task.getArrangeInfo().getTeachers())) {
        TeachTask tobeMergedTask = (TeachTask) taskMap.get(task.getArrangeInfo().getTeachers());
    ...
    // 去除空老师的教学任务
    for (List object : taskMap.keySet()) {
      if (object.size() == 0) {
        keys.add(object);
      }
    }
  ...
  public int getStdCountInClass() {
    ...
    Set<AdminClass> adminClasses = getAdminClasses();
    for (AdminClass adminClass : adminClasses) {
      for (Student student : (Set<Student>) adminClass.getStudents()) {
        if (student.isActive() && student.isInSchool()) {
