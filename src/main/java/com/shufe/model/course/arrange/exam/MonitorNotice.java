...
public class MonitorNotice {
  ...
  public void sortActivities() {
    class ActivityComparator implements Comparator {
      public int compare(Object arg0, Object arg1) {
        ...
        if (rs == 0) {
          return activity0.getTask().getCourse().compareTo(activity1.getTask().getCourse());
        } else return rs;
      }
    }
  ...
  public static List builidNotices(List examActivities, Collection departs) {
    ...
    for (Iterator iter = examActivities.iterator(); iter.hasNext();) {
      ...
      // 打印主考老师
      if (departs.contains(activity.getTask().getArrangeInfo().getTeachDepart())) {
        if (null != activity.getExamMonitor().getExaminer()) {
          MonitorNotice notice = (MonitorNotice) monitorMap.get(activity.getExamMonitor().getExaminer()
              .getId());
          if (null == notice) {
            notice = new MonitorNotice(activity.getExamMonitor().getExaminer());
            monitorMap.put(activity.getExamMonitor().getExaminer().getId(), notice);
          }
          notice.add(activity);
        }
        // 自定主考老师
        if (StringUtils.isNotBlank(activity.getExamMonitor().getExaminerName())) {
          MonitorNotice notice = (MonitorNotice) monitorMap.get(activity.getExamMonitor().getExaminerName());
          if (null == notice) {
            notice = new MonitorNotice(activity.getExamMonitor().getExaminerName());
            monitorMap.put(activity.getExamMonitor().getExaminerName(), notice);
          }
          notice.add(activity);
        }
      }
      ...
      if (departs.contains(activity.getExamMonitor().getDepart())) {
        if (null != activity.getExamMonitor().getInvigilator()) {
          MonitorNotice notice = (MonitorNotice) monitorMap.get(activity.getExamMonitor().getInvigilator()
              .getId());
          if (null == notice) {
            notice = new MonitorNotice(activity.getExamMonitor().getInvigilator());
            monitorMap.put(activity.getExamMonitor().getInvigilator().getId(), notice);
          }
          notice.add(activity);
        }
        // 自定监考老师
        if (StringUtils.isNotBlank(activity.getExamMonitor().getInvigilatorName())) {
          MonitorNotice notice = (MonitorNotice) monitorMap.get(activity.getExamMonitor()
              .getInvigilatorName());
          if (null == notice) {
            notice = new MonitorNotice(activity.getExamMonitor().getInvigilatorName());
            monitorMap.put(activity.getExamMonitor().getInvigilatorName(), notice);
          }
          notice.add(activity);
        }
      }
