 ...
 */
public class CourseTakeClose extends LongIdObject implements Cloneable, Comparable {

  private static final long serialVersionUID = -430500660708769149L;
  /** 教学任务 */
  private ProjectClosePlan projectClosePlan;
  ...
  public ExamTake getExamTake(ExamType examType) {
    if (null == getExamTakes()) return null;
    else {
      if (examType.getId().equals(ExamType.DELAY_AGAIN)) {
        ExamTake take = (ExamTake) getExamTakes().get(ExamType.AGAIN);
        if (null == take) {
  ...
  public CourseTakeClose() {
    projectClosePlan = new ProjectClosePlan();
    student = new Student();
  }

  public CourseTakeClose(Long takeId) {
    setId(takeId);
  }

  public CourseTakeClose(ProjectClosePlan projectClosePlan, Student student, CourseTakeType courseTakeType) {
    this.projectClosePlan = projectClosePlan;
   ...
   */
  public Object clone() {
    CourseTakeClose take = new CourseTakeClose();
  ...
  /**
   * @return Returns the projectClosePlan.
   */
  public ProjectClosePlan getTask() {
    return projectClosePlan;
  }

  public ProjectClosePlan getProjectClosePlan() {
    return projectClosePlan;
  }

  public void setProjectClosePlan(ProjectClosePlan projectClosePlan) {
    this.projectClosePlan = projectClosePlan;
  }
  ...
  public boolean equals(Object object) {
    if (!(object instanceof CourseTakeClose)) { return false; }
    // FIXME 去掉教学任务的判断，实际上像一个新的教学任务添加学生时会报错
    CourseTakeClose rhs = (CourseTakeClose) object;
    return new EqualsBuilder().append(getTask(), rhs.getTask())
        .append(getStudent().getId(), rhs.getStudent().getId()).isEquals();
  }
   ...
   */
  public int hashCode() {
    return new HashCodeBuilder(-1219136969, -1215718665).append(getStudent().getId()).toHashCode();
  }
  ...
  public int compareTo(Object object) {
    CourseTakeClose myClass = (CourseTakeClose) object;
    return new CompareToBuilder().append(this.student.getCode(), myClass.student.getCode()).toComparison();
  }
