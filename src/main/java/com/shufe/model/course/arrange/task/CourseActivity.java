...
public class CourseActivity extends TaskActivity implements Comparable, Serializable {
  ...
  public boolean isSameActivityExcept(CourseActivity other, boolean teacher, boolean room) {
    if (teacher) {
      if (null == getTeacher() && null != other.getTeacher()) { return false; }
      if (null != getTeacher() && null == other.getTeacher()) { return false; }
      if (null != getTeacher() && null != other.getTeacher() && !getTeacher().equals(other.getTeacher())) { return false; }
    }

    if (room) {
      if (null == getRoom() && null != other.getRoom() || null != getRoom() && null == other.getRoom()
          || null != getRoom() && null != other.getRoom() && !getRoom().equals(other.getRoom())) { return false; }
    }
    if (!getTime().getStartUnit().equals(other.getTime().getStartUnit())
        || !getTime().getEndUnit().equals(other.getTime().getEndUnit())
        || !getTime().getWeekId().equals(other.getTime().getWeekId())) { return false; }
    return true;
  }
  ...
  public boolean canMergerWith(CourseActivity activity) {
    if (null == getRoom() && null != activity.getRoom()) return false;
    else if (null != getRoom() && null == activity.getRoom()) return false;
    else if (null != getRoom() && null != activity.getRoom() && !getRoom().equals(activity.getRoom())) return false;
    if (null == getTeacher() && null != activity.getTeacher()) return false;
    else if (null != getTeacher() && null == activity.getTeacher()) return false;
    else if (null != getTeacher() && null != activity.getTeacher()
        && !getTeacher().equals(activity.getTeacher())) return false;
  ...
  public static List mergeActivites(List tobeMerged) {
    ...
    if (CollectionUtils.isEmpty(tobeMerged)) { return mergedActivityList; }
  ...
  public Date getFirstActivityTime(TeachCalendar calendar) {
    if (null != time) {
      ...
      gc.setTime(date);
      CourseUnit unit = calendar.getTimeSetting().getCourseUnit(time.getStartUnit().intValue());
      if (null != unit) {
  ...
  public String toString() {
    return new ToStringBuilder(this, ToStringStyle.MULTI_LINE_STYLE).append("room", this.room)
        .append("time", this.time).append("teacher", this.teacher).append("id", this.id).toString();
  }

  public String digest(MessageResources resourses, Locale locale, String format) {
    Collection activities = new ArrayList();
    activities.add(this);
    return CourseActivityDigestor.digest(this.calendar, this.task.getArrangeInfo(), activities, resourses,
        locale, format);
  }
