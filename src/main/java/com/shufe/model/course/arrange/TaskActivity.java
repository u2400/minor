...
public class TaskActivity extends Activity implements Serializable {
  ...
  public String getWeekState() {
    String weekState = TimeUnitUtil.digest(time.getValidWeeks(), calendar.getWeekStart().intValue(), task
        .getArrangeInfo().getWeekStart().intValue(), task.getArrangeInfo().getEndWeek(), null, null);
  ...
  public String getRoomPlace() {
    ...
    activities.add(this);
    return CourseActivityDigestor.digest(calendar, task.getArrangeInfo(), activities, null, null,
        CourseActivityDigestor.room);
  }
  ...
  public int getActivityFirstWeek() {
    if (StringUtils.isEmpty(this.time.getValidWeeks())) { return -1; }
    for (int i = calendar.getWeekStart().intValue() - 1; i < this.time.getValidWeeks().length(); i++) {
      if (this.time.getValidWeeks().charAt(i) == '1') { return i - calendar.getWeekStart().intValue() + 2; }
    }
    return -1;
  }

  public int getActivityLastWeek() {
    int start = getActivityFirstWeek();
    if (start == -1) {
      start = 1;
    }
    for (int i = start - 1, n = 0; n < 53; i = (i + 1) % 53, n++) {
      if (this.time.getValidWeeks().charAt(i) == '1') {
        start = i >= calendar.getWeekStart().intValue() ? i : i + 53;
      }
    }
    return start - calendar.getWeekStart().intValue() + 2;
  }
}
