...
public class ExamActivity extends TaskActivity {
  ...
  public boolean isSameActivityExcept(ExamActivity other, boolean teacher, boolean room) {
    ...
    if (!getTime().getStartUnit().equals(other.getTime().getStartUnit())) return false;
    if (!getTime().getEndUnit().equals(other.getTime().getEndUnit())) return false;
    if (!getTime().getWeekId().equals(other.getTime().getWeekId())) return false;
    return true;
  }
  ...
  public String digest(Locale locale, MessageResources resources, String format) {
    return ExamActivityDigestor.digest(calendar, Collections.singletonList(this), resources, locale, format);
  }
