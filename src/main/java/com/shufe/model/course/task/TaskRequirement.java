...
public class TaskRequirement implements Component {
  ...
  /** 语种熟练程度（语言能力） */
  private Set<LanguageAbility> languageAbilities = new HashSet<LanguageAbility>();
  ...
  public Object clone() {
    ...
    require.setCases(cases);
    require.setTeachLangType(teachLangType);
    require.setIsGuaPai(isGuaPai);
    require.setReferenceBooks(referenceBooks);
    require.getTextbooks().addAll(textbooks);
    require.setRoomConfigType(roomConfigType);
    require.setRequireRemark(requireRemark);
    require.setEvaluateByTeacher(evaluateByTeacher);
    require.getLanguageAbilities().addAll(languageAbilities);
    return require;
  }
  ...
  /**
   * @param referenceBooks
   *          The referenceBooks to set.
   */
  ...
  /**
   * @param roomConfigType
   *          The roomConfigType to set.
   */
  ...
  /**
   * @param capacityOfCourse
   *          The capacityOfCourse to set.
   */
  ...
  public Set<LanguageAbility> getLanguageAbilities() {
    return languageAbilities;
  }

  public void setLanguageAbilities(Set<LanguageAbility> languageAbilities) {
    this.languageAbilities = languageAbilities;
  }

  public String getLanguageAbilitiyNames() {
    StringBuilder names = new StringBuilder();
    if (CollectionUtils.isNotEmpty(languageAbilities)) {
      for (LanguageAbility ability : languageAbilities) {
        if (names.length() != 0) {
          names.append(",");
        }
        names.append(ability.getName());
      }
    }
