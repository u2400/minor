...
public class ReservedStudent extends LongIdObject {
  ...
  public boolean contains(Student student) {
    if (null != grade) {
      if (!grade.equals(student.getGrade())) return false;
    }
    if (null != major) {
      if (!major.equals(student.getFirstMajor())) { return false; }
    }
    if (null != majorField) {
      if (!majorField.equals(student.getFirstAspect())) { return false; }
    }
    return true;
  }
