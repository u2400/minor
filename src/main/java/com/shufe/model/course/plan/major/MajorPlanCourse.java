 ...
 * @author chaostone
 */
public class MajorPlanCourse extends LongIdObject implements Comparable, Cloneable {
   ...
   * 学时<br>
   * 这里的学分和课程的学时可能不一样.<br>
   * 因为不同的课程类别限制了它的学时取值<br>
   */

  private Integer creditHour = new Integer(0);

  /** 周课时 */
  private Float weekHour = new Float(0);
  ...
  /** 所属课程组 */
  private MajorCourseGroup courseGroup = new MajorCourseGroup();
  ...
  public boolean equals(Object object) {
    ...
    return new EqualsBuilder().append(getPreCourses(), rhs.getPreCourses())
        ...
        .append(getTeachDepart().getId(), rhs.getTeachDepart().getId())
        .append(getCourse().getId(), rhs.getCourse().getId()).append(getId(), rhs.getId())
        .append(getCredit(), rhs.getCredit()).append(getCreditHour(), rhs.getCreditHour())
        .append(getWeekHour(), rhs.getWeekHour()).isEquals();
  }
  ...
  public int compareTo(Object object) {
    MajorPlanCourse myClass = (MajorPlanCourse) object;
  ...
  public MajorCourseGroup getCourseGroup() {
    return courseGroup;
  }

  public void setCourseGroup(MajorCourseGroup courseGroup) {
