 ...
 * @author chaostone
 */
public class MajorCourseGroup extends LongIdObject implements Comparable, Cloneable {
  ...
  public void addPlanCourse(MajorPlanCourse course) {
    course.setCourseGroup(this);
    getPlanCourses().add(course);
  }

  public void removePlanCourse(MajorPlanCourse course) {
   ...
   */
  public Object clone() {
    MajorCourseGroup group = new MajorCourseGroup();
    try {
      ...
      for (Iterator iter = this.getPlanCourses().iterator(); iter.hasNext();)
        group.addPlanCourse((MajorPlanCourse) ((MajorPlanCourse) iter.next()).clone());
    } catch (Exception e) {
  ...
  public void removePlanCourse(Course course) {
    for (Iterator iter = getPlanCourses().iterator(); iter.hasNext();) {
      MajorPlanCourse planCourse = (MajorPlanCourse) iter.next();
    ...
    for (Iterator iter = getPlanCourses().iterator(); iter.hasNext();) {
      ...
      while (terms.hasMoreTokens()) {
        ...
        if (termIndex >= 1 && termIndex <= termsCount) {
          credit[termIndex - 1] += planCourse.getCredit().floatValue();
          creditHour[termIndex - 1] += planCourse.getCreditHour().intValue();
          weekHour[termIndex - 1] += planCourse.getWeekHour().floatValue();
        }
      }
    }
    /**
