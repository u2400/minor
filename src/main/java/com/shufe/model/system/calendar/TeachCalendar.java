...
public class TeachCalendar extends LongIdObject implements Comparable,
  ...
  public static int MAXUNITS = SystemConfigLoader.getConfig().getIntParam("arrange.maxCourseUnit");
  ...
  public String getYearTerm() {
    return ((null == getYear()) ? "" : getYear()) + "        " + ((null == getTerm()) ? "" : getTerm());
  }
  ...
  public String toString() {
    return new ToStringBuilder(this, ToStringStyle.MULTI_LINE_STYLE)
        .append("studentType", this.studentType.getId()).append("year", this.year).append("term", this.term)
        .append("start", this.getStart()).append("finish", this.getFinish()).append("weeks", this.getWeeks())
        .toString();
  }
  ...
  public int compareTo(Object object) {
    ...
    return new CompareToBuilder().append(this.studentType, myClass.studentType)
        .append(this.year, myClass.year).append(this.start, myClass.start).toComparison();
  }
  ...
  public java.util.Date[] getWeekTime(int weekIndex) {
    ...
    if (!weekRange.containsInteger(weekIndex)) throw new IllegalArgumentException("weekIndex");
    else {
      ...
      if (dates[1].after(getFinish())) dates[1] = getFinish();
      return dates;
    }
  }
  ...
  public TimeUnit[] buildTimeUnits(int beginWeek, int endWeek) {
    if (beginWeek < 1 || endWeek > getWeekSpan().intValue()) { throw new RuntimeException(
        "out of week range:[" + beginWeek + ":" + endWeek + "]"); }
    return TimeUnitUtil.buildTimeUnits(getStartYear(), getWeekStart().intValue(), beginWeek, endWeek,
        TimeUnit.CONTINUELY);
  }

  public TimeUnit[] buildTimeUnits() {
    return TimeUnitUtil.buildTimeUnits(getStartYear(), getWeekStart().intValue(), 1, getWeeks().intValue(),
        TimeUnit.CONTINUELY);
  }
  ...
  public boolean contains(Date date) {
    if (date.before(getStart()) || date.after(getFinish())) return false;
    else return true;
  }
