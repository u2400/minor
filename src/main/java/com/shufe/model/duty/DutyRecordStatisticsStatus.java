...
public class DutyRecordStatisticsStatus {
  ...
  public Integer getPresenceCount(Boolean convert) {
    if (Boolean.TRUE.equals(convert)) {
      return new Integer(((this.presenceCount == null ? 0 : this.presenceCount.intValue())
          + (this.lateCount == null ? 0 : this.lateCount.intValue()) + (this.leaveEarlyCount == null ? 0
            : this.leaveEarlyCount.intValue()))
          - ((this.lateCount == null ? 0 : this.lateCount.intValue()) + (this.leaveEarlyCount == null ? 0
              : this.leaveEarlyCount.intValue()))
          / Integer.parseInt(SystemConfigLoader.getConfig().getConfigItemValue("system.dutyRuleInf")
              .toString()));
    } else if (Boolean.FALSE.equals(convert)) {
      return new Integer((this.presenceCount == null ? 0 : this.presenceCount.intValue())
          + (this.lateCount == null ? 0 : this.lateCount.intValue())
          + (this.leaveEarlyCount == null ? 0 : this.leaveEarlyCount.intValue()));
    } else {
      return presenceCount;
    }
  }
  ...
  public String toString() {
    return new ToStringBuilder(this).append("TotalCount", this.getTotalCount())
        .append("PresenceCount", this.getPresenceCount())
        .append("AbsenteeismCount", this.getAbsenteeismCount()).append("LateCount", this.getLateCount())
        .append("LeaveEarlyCount", this.getLeaveEarlyCount())
        .append("AskedForLeaveCount", this.getAskedForLeaveCount()).toString();
  }
   ...
   * this.setConvertAbsenteeismCount(totalConvert==null?new Integer(0):new
   * Integer(totalConvert.intValue()/AttendanceType.convertAbsenteeism));
   * }
   */
  ...
  public Float getAbsenteeismRatio(Boolean convert) {
    if (new Integer(0).compareTo(totalCount) < 0) {
      return new Float((this.getAbsenteeismCount(convert) == null ? 0f : this.getAbsenteeismCount(convert)
          .floatValue()) / this.totalCount.floatValue());
    } else {
      return new Float(0);
    }
  ...
  public Float getPresenceRatio(Boolean convert) {
    if (new Integer(0).compareTo(totalCount) < 0) {
      return new Float((this.getPresenceCount(convert) == null ? 0f : this.getPresenceCount(convert)
          .floatValue()) / totalCount.floatValue());
    } else {
      return new Float(0);
    }
  }
