...
public class StudyThesisAward extends StudyAward {
  ...
  public String getThesisIndexInfo() {
    ...
    for (Iterator iter = studyThesis.getIndexes().iterator(); iter.hasNext();) {
      ThesisIndex element = (ThesisIndex) iter.next();
      info.append(element.getThesisIndexType().getName()).append("收录,检索号:")
          .append(element.getThesisIndexNo());
      if (iter.hasNext()) {
        info.append(",");
      }
    }
