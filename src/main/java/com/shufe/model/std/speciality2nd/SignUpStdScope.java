...
public class SignUpStdScope extends LongIdObject {
  ...
  public boolean inScope(Student std) {
    if (null != getStdType()) {
      if (!(getStdType().getId()).equals(std.getType().getId())) { return false; }
    }
    if (null != getEnrollTurn()) {
      if (!getEnrollTurn().equals(std.getEnrollYear())) { return false; }
    }
    if (null != getDepartment()) {
      if (!getDepartment().equals(std.getDepartment())) { return false; }
    }
    if (null != getSpeciality()) {
      if (!ObjectUtils.equals(getSpeciality(), std.getFirstMajor())) { return false; }
    }
    if (null != getAspect()) {
      if (!ObjectUtils.equals(getAspect(), std.getFirstAspect())) { return false; }
    }
    return true;
  }
  ...
  public boolean isSame(Object object) {
    if (!(object instanceof SignUpStdScope)) { return false; }
    SignUpStdScope rhs = (SignUpStdScope) object;
    return new EqualsBuilder().append(getAspect(), rhs.getAspect())
        .append(getEnrollTurn(), rhs.getEnrollTurn()).append(getSpeciality(), rhs.getSpeciality())
        .append(getDepartment(), rhs.getDepartment()).append(getStdType(), rhs.getStdType()).isEquals();
  }
