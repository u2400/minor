...
public class SignUpSetting extends LongIdObject {
  ...
  public boolean isDateSuitable() {
    if (null != getBeginOn() && null != getEndOn()) {
      java.util.Date now = DateUtils.truncate(new GregorianCalendar(), Calendar.DATE).getTime();
      if (now.before(DateUtils.truncate(getBeginOn(), Calendar.DATE))
          || now.after(DateUtils.truncate(getEndOn(), Calendar.DATE))) { return false; }
    }
    return true;
  }
  ...
  public boolean canSignUp(Student std, Float GPA) {
    ...
    if (can) {
      if (getMinGPA().compareTo(GPA) < 1) return true;
      else return false;
    } else {
      return false;
    }
