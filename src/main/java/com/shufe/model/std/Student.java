...
public class Student extends LongIdObject implements Cloneable, com.ekingstar.eams.std.info.Student {
  ...
  /** 学生英语默认等级为C（不推荐这样编码） */
  @Deprecated
  public static final Long ENGLISH_ABILITY_DEFAULT = 3L;
  ...
  /** 毕业审核结果 */
  private Set auditResults = new HashSet();
  ...
  /** 双专业毕业审核状态(是否审核过) */
  private Boolean secondGraduateAuditStatus;
  ...
  public AdminClass getMajorClass(MajorType majorType) {
    ...
    if (null != getAdminClasses()) {
      if (getAdminClasses().isEmpty()) return null;
      else {
        for (Iterator iter = getAdminClasses().iterator(); iter.hasNext();) {
          ...
          if (null != adminClass.getSpeciality()
              && adminClass.getSpeciality().getMajorType().getId().equals(majorTypeId)) { return adminClass; }
        }
        return null;
      }
    } else {
  ...
  public Set getAdminClassSet(final Boolean is2ndMajor) {
    if (is2ndMajor == null) {
      ...
      if (getAdminClasses() == null) { return getAdminClasses(); }
        ...
        public boolean evaluate(Object object) {
          ...
          if (adminClass.getSpeciality() == null) {
            ...
            if (Boolean.TRUE.equals(is2ndMajor)) {
              return adminClass.getSpeciality().getMajorType().getId().equals(MajorType.SECOND);
            } else {
              return adminClass.getSpeciality().getMajorType().getId().equals(MajorType.FIRST);
            }
          }
        }
      }));
  ...
  public Float getScoreInLanguage() {
    return scoreInLanguage;
  }

  public void setScoreInLanguage(Float scoreInLanguage) {
    this.scoreInLanguage = scoreInLanguage;
  }
}
