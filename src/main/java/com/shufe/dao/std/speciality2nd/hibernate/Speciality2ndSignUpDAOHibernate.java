...
public class Speciality2ndSignUpDAOHibernate extends BasicHibernateDAO implements Speciality2ndSignUpDAO {
  ...
  public void batchMatriculateSignUpStudent(List signUpStds) {
    if (CollectionUtils.isEmpty(signUpStds)) return;
    utilDao.saveOrUpdate(signUpStds);
    ArrayList recordIds = new ArrayList(CollectionUtils.collect(signUpStds, new PropertyTransformer("id")));
  ...
  public void batchMatriculateSignUpStudentRecords(SignUpSetting setting, List records) {
    ...
    ArrayList recordIds = new ArrayList(CollectionUtils.collect(records, new PropertyTransformer("id")));
