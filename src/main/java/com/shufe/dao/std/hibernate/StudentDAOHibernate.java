...
public class StudentDAOHibernate extends BasicHibernateDAO implements StudentDAO {
  ...
  public Criteria constructStudentCriteria(Student std, List sortList) {
    Criteria criteria = this.constructStudentCriteriaWithOutOrder(std, StudentDAO.DEFAULTEXCLUDEDATTRS);
    return criteria;
  }
  ...
  private Criteria constructStudentCriteriaWithOutOrder(Student std, String[] excludedAttrs) {
    ...
    if (std != null) {
      ...
      for (int i = 0; i < excludedAttrs.length; i++) {
        if (!excludedAttrs[i].equals("type")) {
          ...
          if (ValidEntityPredicate.INSTANCE.evaluate(std.getType())) {
            StudentType stdType = (StudentType) utilDao.get(StudentType.class, std.getType().getId());
            ...
            criteria.add(Restrictions.in("type.id", SeqStringUtil.transformToLong(sb.toString())));
          }
        }
      }
      if (null != std.getAdminClasses() && !std.getAdminClasses().isEmpty()) {
        ...
        List adminClassCriterions = CriterionUtils.getEntityCriterions("adminClass.", adminClass);
  ...
  public Criteria constructStdCountCriteria(Student std) {
    ...
    Criteria criteria = this.getSession().createCriteria(Student.class).addOrder(Order.desc("enrollYear"))
        .addOrder(Order.asc("code"));
    String[] excludedAttrs = new String[] { "department", "firstMajor", "firstAspect", "studentStatusInfo",
        "basicInfo" };
    List criterions = CriterionUtils.getEntityCriterions(std, excludedAttrs);
    for (Iterator iter = criterions.iterator(); iter.hasNext();) {
      Criterion criterion = (Criterion) iter.next();
  ...
  /*
   * (non-Javadoc)
   * @see com.shufe.dao.std.StudentDAO#generateStudentClassCriteria(com.shufe.model.system.baseinfo.
   * AdminClass)
   */
  public Criteria constructStudentClassCriteria(AdminClass adminClass) {
    ...
    List criterions = CriterionUtils.getEntityCriterions("", adminClass, new String[] { "stdType" });
    for (Iterator iter = criterions.iterator(); iter.hasNext();) {
    ...
    if (ValidEntityPredicate.INSTANCE.evaluate(adminClass.getStdType())) {
      ...
      criteria.add(Restrictions.in("stdType.id", SeqStringUtil.transformToLong(sb.toString())));
    }
    return criteria;
  }
    ...
    for (int i = 0; i < ids.length; i++) {
      ...
      for (int j = 0; j < studentIdArray.length; j++) {
        ...
        if (studentSet.contains(student)) {
          ...
          if (student.isActive()) {
            adminClass.setStdCount(adminClass.getStdCount() == null ? new Integer(0) : new Integer(adminClass
                .getStdCount().intValue() - 1));
          } else {

          }
          if (student.isInSchool()) {
            adminClass.setActualStdCount(adminClass.getActualStdCount() == null ? new Integer(0)
  ...
  public AdminClass loadAdminClassById(Long id) {
    AdminClass adminClass = (AdminClass) getSession()
        .createQuery("from AdminClass adminClass where adminClass.id = :id").setLong("id", id.longValue())
        .uniqueResult();
    return adminClass;
  }
    ...
    for (int i = 0; i < ids.length; i++) {
      ...
      for (int j = 0; j < studentIdArray.length; j++) {
        ...
        if (studentSet.contains(student)) {
          ...
          if (student.isActive()) {
            adminClass.setStdCount(adminClass.getStdCount() == null ? new Integer(1) : new Integer(adminClass
                .getStdCount().intValue() + 1));
          } else {

          }
          if (student.isInSchool()) {
            adminClass.setActualStdCount(adminClass.getActualStdCount() == null ? new Integer(1)
                : new Integer(adminClass.getActualStdCount().intValue() + 1));
          }
        }
      }
  ...
  public Map getBasicInfoName(String stdCode) {
    Criteria criteria = getSession().createCriteria(Student.class).addOrder(Order.desc("enrollYear"))
        .addOrder(Order.asc("code"));
    ...
    if ((null == temp.getAbroadStudentInfo()) || (null == temp.getAbroadStudentInfo().getHSKDegree())) {
      stdMap.put("abroadStudentInfo.HSKDegree.name", "");
    } else {
      stdMap.put("abroadStudentInfo.HSKDegree.name", temp.getAbroadStudentInfo().getHSKDegree().getName());
    }

    stdMap.put("studentStatusInfo.feeOrigin.name", (null == temp.getStudentStatusInfo().getFeeOrigin()) ? ""
        : temp.getStudentStatusInfo().getFeeOrigin().getName());
    return stdMap;
  ...
  public Collection studentInfoStat(Student student, DataRealmLimit limit, String[] groupArray,
    ...
    if (Boolean.TRUE.equals(isExactly)) {
      criteria = getStudentInfoStatCriteria(student, groupArray, new String[] { "abroadStudentInfo",
          "studentStatusInfo", "basicInfo", "firstMajor", "firstAspect", "secondMajor", "secondAspect",
          "graduateAuditStatus", "secondGraduateAuditStatus" });
    } else {
    ...
    if (null != limit) {
      if (StringUtils.isNotEmpty(limit.getDataRealm().getStudentTypeIdSeq())) {
        criteria.add(Restrictions.in("type.id",
  ...
  private Criteria getStudentInfoStatCriteria(Student student, String[] groupArray, String[] excludedAttrs) {
    Criteria criteria = this.constructStudentCriteriaWithOutOrder(student, StudentDAO.DEFAULTEXCLUDEDATTRS);
    if (!ArrayUtils.isEmpty(excludedAttrs)) {
      for (int i = 0; i < excludedAttrs.length; i++) {
        try {
          if (!ValidEntityPredicate.INSTANCE.evaluate(PropertyUtils.getProperty(student, excludedAttrs[i]))) {
    ...
    if (null != groupArray) {
      for (int i = 0; i < groupArray.length; i++) {
        ...
        projectionList.add(Projections
            .groupProperty(temp.length > 1 ? (temp[temp.length - 2] + "." + temp[temp.length - 1])
                : (temp[temp.length - 1]))/* ,groupArray[i] */);
        /*
