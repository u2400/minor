...
public class Level1SubjectDAOHibernate extends BasicHibernateDAO implements Level1SubjectDAO {

  public Pagination getLevel1Subject(Level1Subject subject, int pageNo, int pageSize) {

    Criteria criteria = getSession().createCriteria(Level1Subject.class);
    if (null != subject && null != subject.getCategory()) {
      List categoryCriterions = CriterionUtils.getEntityCriterions("category.", subject.getCategory());
      if (!categoryCriterions.isEmpty()) {
        Criteria categoryCriteria = criteria.createCriteria("category", "category");
