...
public class FormalAnswerHibernateDAO extends BasicHibernateDAO implements FormalAnswerDAO {
  ...
  private Criteria getExampleOfFormalAnswer(FormalAnswer formalAnswer, String departmentIdSeq,
    ...
    if (null != formalAnswer.getThesisManage()) {
      ...
      criteriions.addAll(CriterionUtils.getEntityCriterions("thesisManage.", formalAnswer.getThesisManage()));
    }
    if (null != formalAnswer.getStudent()) {
      criteriions.addAll(CriterionUtils.getEntityCriterions("student.", formalAnswer.getStudent()));
    }
    ...
    criteria.add(Restrictions.in("student.department.id", SeqStringUtil.transformToLong(departmentIdSeq)));
  ...
  public List getFormalAnswers(FormalAnswer formalAnswer, String departmentIdSeq, String stdTypeIdSeq) {
    if (StringUtils.isBlank(departmentIdSeq) && StringUtils.isBlank(stdTypeIdSeq)) { return Collections.EMPTY_LIST; }
  ...
  public void batchUpdate(String[] propertys, Object[] values, String formalAnswerIdSeq) {
    ...
    while (formalAnswerIdSeq.split(",").length >= 1000) {
      String tempseq = formalAnswerIdSeq.substring(0, formalAnswerIdSeq.indexOf(",", maxTop * 500));
      updateFormalAnswer(propertys, values, tempseq);
      formalAnswerIdSeq = formalAnswerIdSeq.substring(formalAnswerIdSeq.indexOf(",", maxTop * 500) + 1);
    }
