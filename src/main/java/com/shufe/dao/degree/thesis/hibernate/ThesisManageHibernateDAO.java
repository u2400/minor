...
public class ThesisManageHibernateDAO extends BasicHibernateDAO implements ThesisManageDAO {
  ...
  public Criteria getThesisManageCriteria(ThesisManage thesisManage, String deparmentSeq, String stdTypeSeq) {
    ...
    if (null != thesisManage.getStudent()) {
      List criterionStds = CriterionUtils.getEntityCriterions("student.", thesisManage.getStudent(),
          new String[] { "type" });
      ...
      if (StringUtils.isNotEmpty(stdTypeSeq)) {
        criteria.add(Restrictions.in("student.type.id", SeqStringUtil.transformToLong(stdTypeSeq)));
      }
    ...
    if (null != thesisManage.getTopicOpen()) {
      ...
      } else if (null == topicOpen.getIsPassed() || new Integer(0).equals(topicOpen.getIsPassed())) {
      } else {
    ...
    if (null != thesisManage.getAnnotate()) {
      List criteriaAnswers = CriterionUtils.getEntityCriterions("annotate.", thesisManage.getAnnotate());
      if (null != thesisManage.getAnnotate().getAnnotateBooks()
          && thesisManage.getAnnotate().getAnnotateBooks().size() > 0) {
        ThesisAnnotateBook annotateBook = (ThesisAnnotateBook) thesisManage.getAnnotate().getAnnotateBooks()
            .iterator().next();
        criteria.createAlias("annotate.annotateBooks", "book");
        criteriaAnswers.add(Restrictions.like("book.serial", annotateBook.getSerial(), MatchMode.ANYWHERE));
      }
    ...
    if (StringUtils.isNotEmpty(notNulls)) {
      ...
      for (int i = 0; i < notNull.length; i++) {
        ...
        while (count != -1) {
          ...
          try {
            if (!PropertyUtils.getPropertyType(thesisManage, temp).equals(Component.class)) {
  ...
  public List getThesisManagesNotNull(ThesisManage thesisManage, String deparmentSeq, String stdTypeSeq,
      String notNulls, String nulls) {
    return getThesisManageAttributeNotNull(thesisManage, deparmentSeq, stdTypeSeq, notNulls, nulls).list();
  }
   ...
   *      int, int)
   */
  public Pagination getThesisManagePagis(ThesisManage thesisManage, String deparmentSeq, String stdTypeSeq,
      int pageNo, int pageSize) {
    return dynaSearch(getThesisManageCriteria(thesisManage, deparmentSeq, stdTypeSeq), pageNo, pageSize);
  }
   ...
   * @return
   */
  public Pagination getThesisManagePaginaForNotNull(ThesisManage thesisManage, String deparmentSeq,
      String stdTypeSeq, int pageNo, int pageSize, String notNulls, String nulls, String orderString) {

    Criteria criteria = getThesisManageAttributeNotNull(thesisManage, deparmentSeq, stdTypeSeq, notNulls,
        nulls);
    ...
    if (StringUtils.isNotBlank(deparmentSeq) && StringUtils.isNotBlank(stdTypeSeq)
      ...
      for (int i = 0; i < orders.size(); i++) {
        com.ekingstar.commons.query.Order o = (com.ekingstar.commons.query.Order) orders.get(i);
   ...
   */
  public List getProjectionConditions(ThesisManage thesisManage, String departmentSeq, String stdTypeSeq,
