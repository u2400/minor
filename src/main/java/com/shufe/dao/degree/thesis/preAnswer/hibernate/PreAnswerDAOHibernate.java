...
public class PreAnswerDAOHibernate extends BasicHibernateDAO implements PreAnswerDAO {
   ...
   */
  public void batchUpdateTimeAndAddress(String preAnswerIdSeq, String answerTime, String anwerAddress) {
    int maxTop = 0;
    ...
    while (preAnswerIdSeq.split(",").length > 1000) {
      ...
      preAnswerIdSeq = preAnswerIdSeq.substring(preAnswerIdSeq.indexOf(",", maxTop * 500) + 1);
    }
  ...
  public void updateTutorAffirm(String preAnswerIdSeq, Boolean flag) {
    String hqlString = "update PreAnswer set isTutorAffirm=" + flag + " where id in(" + preAnswerIdSeq + ")";
    if (StringUtils.isNotEmpty(preAnswerIdSeq)) {
      Query query = getSession().createQuery(hqlString);
      query.executeUpdate();
    }
   ...
   * @return
   */
  public Criteria getExampleOfPreAnsWer(PreAnswer preAnswer, String departIdSeq, String stdTypeIdSeq) {
    Criteria criteria = getSession().createCriteria(PreAnswer.class);
    List criterions = CriterionUtils.getEntityCriterions(preAnswer, new String[] { "isPassed" });
    ...
    if (null != preAnswer.getThesisManage() && null != preAnswer.getThesisManage().getStudent()) {
      ...
      stdTypeIdSeq = intersectStdTypeIdSeq(preAnswer.getThesisManage().getStudent().getType(), stdTypeIdSeq);
      ...
      List criterionStds = CriterionUtils.getEntityCriterions("student.", preAnswer.getThesisManage()
          .getStudent(), new String[] { "type" });
      criterions.addAll(criterionStds);
    }
    if (null != preAnswer.getThesisManage()
        && ValidEntityPredicate.getInstance().evaluate(preAnswer.getThesisManage().getTutor())) {
      criteria.add(Restrictions.eq("thesisManage.tutor.id", preAnswer.getThesisManage().getTutor().getId()));
    }
  ...
  public List getPreAnswers(PreAnswer preAnswer, String departIdSeq, String stdTypeIdSeq) {
    if (StringUtils.isBlank(departIdSeq) || StringUtils.isBlank(stdTypeIdSeq)) { return Collections.EMPTY_LIST; }
   ...
   * @return
   */
  public List getPropertyList(PreAnswer preAnswer, String departIdSeq, String stdTypeIdSeq, String property) {
    if (StringUtils.isBlank(property)) { return Collections.EMPTY_LIST; }
   ...
   *      java.lang.String, java.lang.String)
   */
  public Criteria getPreAnswerNoApply(ThesisManage thesisManage, String departIdSeq, String stdTypeIdSeq) {
  ...
  public Pagination getPaginationOfNoApply(ThesisManage thesisManage, String departIdSeq,
    ...
    return dynaSearch(getPreAnswerNoApply(thesisManage, departIdSeq, stdTypeIdSeq), pageNo, pageSize);
  }
