...
public class ThesisStatisticDAOHibernate extends BasicHibernateDAO implements ThesisStatisticDAO {
  ...
  public List getstdGroupInfo(Student student, String departmentIdSeq, String stdTypeIdSeq) {
    ...
    if (StringUtils.isNotBlank(departmentIdSeq)) {
      criteria.add(Restrictions.in("department.id", SeqStringUtil.transformToLong(departmentIdSeq)));
    }
    ...
    criteria.setProjection(Projections.projectionList().add(Projections.rowCount())
        .add(Projections.groupProperty("department")).add(Projections.groupProperty("type")));
    return criteria.list();
  }

  public List getTOpicOpenGroupInfo(ThesisManage thesisManage, String departmentIdSeq, String stdTypeIdSeq) {
    Criteria criteria = thesisManageDAO.getThesisManageCriteria(thesisManage, departmentIdSeq, stdTypeIdSeq);
   ...
   * 统计
   * @see
   * com.shufe.dao.degree.thesis.thesisStatistic.ThesisStatisticDAO#getThesisAndTeachers(java.lang
   * .String,
   * java.util.Set)
   */
  public List getThesisAndTeachers(String stdTypeIdSeq, Set stdTypes, String requireYear) {
    ...
    criteria
        .setProjection(Projections.projectionList().add(Projections.rowCount())
            .add(Projections.countDistinct("student.teacher"))
            .add(Projections.groupProperty("student.enrollYear"))
            .add(Projections.groupProperty("teacher.title")));
    return criteria.list();
  }
