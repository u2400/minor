...
public class TutorHibernateDAO extends BasicHibernateDAO implements TutorDAO {
  ...
  private Criteria getExampleTutor(Tutor tutor, String departIdSeq) {
    ...
    if (StringUtils.isNotBlank(departIdSeq)) {
      criteria.add(Restrictions.in("department.id", SeqStringUtil.transformToLong(departIdSeq)));
    }
  ...
  public List getTutorList(Tutor tutor, String departmentIdSeq) {
    if (null == tutor) { return Collections.EMPTY_LIST; }
   ...
   * @return
   */
  public Pagination getTutorPagi(Tutor tutor, String departmentIdSeq, int pageNo, int pageSize) {
    if (null == tutor) {
   ...
   * 仅仅指对象的直接属性集合
   * 
   * @see com.shufe.dao.degree.tutorManager.TutorDAO#getPropertyOfTutor(com.shufe.model.system.baseinfo.Tutor,
