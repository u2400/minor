...
public class TutorApplyHibernateDAO extends BasicHibernateDAO implements TutorApplyDAO {
   ...
   * @return
   */
  private Criteria getExampleTutorApply(TutorApply tutorApply, String departIdSeq) {
    ...
    if (null != tutorApply.getTeacher()) {
      criterions.addAll(CriterionUtils.getEntityCriterions("teacher.", tutorApply.getTeacher()));
    }
    criteria.createAlias("teacher", "teacher");
    criteria.add(Restrictions.in("teacher.department.id", SeqStringUtil.transformToLong(departIdSeq)));
  ...
  public List getTutorApplyList(TutorApply tutorApply, String departIdSeq) {
    if (null == tutorApply || StringUtils.isBlank(departIdSeq)) { return Collections.EMPTY_LIST; }
