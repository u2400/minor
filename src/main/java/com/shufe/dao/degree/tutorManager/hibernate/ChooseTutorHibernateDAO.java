...
public class ChooseTutorHibernateDAO extends BasicHibernateDAO implements ChooseTutorDAO {
  ...
  public Criteria constructStudentCriteria(ChooseTutor ct, String studentTypeIds, String departmentIds,
    ...
    if (ct.getStd().getDepartment().isVO()) criteria.add(Restrictions.in("std.department.id",
        SeqStringUtil.transformToLong(departmentIds)));
    if (ct.getStd().getType().isVO()) {
      criteria.add(Restrictions.in("std.type.id", SeqStringUtil.transformToLong(studentTypeIds)));
    }
    return criteria;
  }
