...
public class DegreeDAOHibernate extends BasicHibernateDAO implements DegreeDAO {
  ...
  private Criteria getDegreeApplys(DegreeApply degreeApply, Long[] departmentId, Long[] stdTypeId) {
    ...
    if (null != degreeApply.getStudent()
        && (null == degreeApply.getStudent().getId() || new Long(0).equals(degreeApply.getStudent().getId()))) {
      List stdCriterions = CriterionUtils.getEntityCriterions("student.", degreeApply.getStudent());
      if (null != departmentId && departmentId.length > 0) {
        criteria.add(Restrictions.in("student.department.id", departmentId));
      }
      if (null != stdTypeId && stdTypeId.length > 0) {
        criteria.add(Restrictions.in("student.type.id", stdTypeId));
      }
      if ((null != stdTypeId && stdTypeId.length > 0) || null != departmentId && departmentId.length > 0
   ...
   * int, int)
   */
  public Pagination getPaginas(DegreeApply degreeApply, Long[] stdTypeIdSeq, Long[] departmentSeq,
      int pageNo, int pageSize) {
    return dynaSearch(getDegreeApplys(degreeApply, departmentSeq, stdTypeIdSeq), pageNo, pageSize);
  }
  ...
  public void affirmById(String idSeq, Boolean affirm) {
    String hqlString = "update DegreeApply set isAgree=" + affirm + " where id in(" + idSeq + ")";
