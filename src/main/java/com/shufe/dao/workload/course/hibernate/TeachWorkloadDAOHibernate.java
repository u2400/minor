 ...
 */
public class TeachWorkloadDAOHibernate extends BasicHibernateDAO implements TeachWorkloadDAO {
  ...
  public List getTeachWorkloads(Long teacherId, Long teachTaskId) {
    ...
    if (teacherId != null) {
      criteria.add(Restrictions.eq("teacherInfo.teacher.id", teacherId));
    }
    if (teachTaskId != null) {
   ...
   * @return
   */
  private Criteria getTeachWorkloads(TeachWorkload teachWorkload, String stdTypeIdSeq, String departIdSeq,
    ...
    if (StringUtils.isNotBlank(calendarIdSeq)) {
      criteria.add(Restrictions.in("teachCalendar.id", SeqStringUtil.transformToLong(calendarIdSeq)));
    }
   ...
   * @return
   */
  public List getTeachWorkloadAffirmedTaskIds(TeachWorkload teachWorkload, String stdTypeIdSeq,
      String departIdSeq, String calendarIdSeq) {
    Criteria criteria = getTeachWorkloads(teachWorkload, stdTypeIdSeq, departIdSeq, calendarIdSeq);
    criteria.add(Restrictions.or(Restrictions.eq("teacherAffirm", Boolean.TRUE),
        Restrictions.eq("collegeAffirm", Boolean.TRUE)));
   ...
   * 根据条件得到id串
   * 
   * @see com.shufe.dao.workload.course.TeachWorkloadDAO#getIdListBycondition(com.shufe.model.workload.course.TeachWorkload,
   *      java.lang.String, java.lang.String)
   */
  public List getIdListBycondition(TeachWorkload teachWorkload, String departmentSeq, String stdTypeSeq) {
    Criteria criteria = getTeachWorkloads(teachWorkload, stdTypeSeq, departmentSeq, null);
   ...
   */
  public Criteria getExampleTeachWorkloads(TeachWorkload teachWorkload, String stdTypeIdSeq,
      String departmentSeq, String calendarIdSeq, String age1, String age2) {
    Criteria criteria = getTeachWorkloads(teachWorkload, stdTypeIdSeq, departmentSeq, calendarIdSeq);
    if (StringUtils.isNotBlank(teachWorkload.getTeacherInfo().getTeacher().getCode())) {
      criteria.createAlias("teacherInfo.teacher", "teacher");
      criteria.add(Restrictions.like("teacher.code", teachWorkload.getTeacherInfo().getTeacher().getCode(),
          MatchMode.ANYWHERE));
    }
    ...
    if (teachWorkload.getTeachCalendar() != null) {
      if (StringUtils.isNotEmpty(teachWorkload.getTeachCalendar().getYear())) {
        criteria.add(Restrictions.eq("teachCalendar.year", teachWorkload.getTeachCalendar().getYear()));
      }
      if (StringUtils.isNotEmpty(teachWorkload.getTeachCalendar().getTerm())) {
        criteria.add(Restrictions.eq("teachCalendar.term", teachWorkload.getTeachCalendar().getTerm()));
      }
    }
    criteria.createAlias("college", "college");
    criteria.addOrder(Order.desc("teachCalendar.year")).addOrder(Order.asc("teachCalendar.term"))
        .addOrder(Order.asc("college.name")).addOrder(Order.asc("teacherInfo.teacherName"))
        .addOrder(Order.asc("teacherInfo.teacherAge"));
    return criteria;
  }
   ...
   */
  public List getTeachWorkloads(Long teacherId, Long studentTypeId, String year, String term) {
    Criteria criteria = this.getSession().createCriteria(TeachWorkload.class);
    if (teacherId != null) {
      criteria.add(Restrictions.eq("teacherInfo.teacher.id", teacherId));
    }
    ...
    if (StringUtils.isNotEmpty(year)) {
      criteria.add(Restrictions.eq("teacherInfo.teachCalendar.year", year));
    }
    if (StringUtils.isNotEmpty(term)) {
      criteria.add(Restrictions.eq("teacherInfo.teachCalendar.term", term));
    }
    return criteria.list();
  }

  /**
   * @see com.shufe.dao.workload.TeachWorkloadDAO12#affirmType(java.lang.String, java.lang.String,
   *      boolean)
   *      3885由来：
  ...
  public void affirmType(String affirmType, String teachWorkloadIds, boolean b) {
    ...
    while (teachWorkloadIds.split(",").length >= 1000) {
      String tempseq = teachWorkloadIds.substring(0, teachWorkloadIds.indexOf(",", maxTop * 500));
      batchForAffirmType(affirmType, tempseq, b);
      teachWorkloadIds = teachWorkloadIds.substring(teachWorkloadIds.indexOf(",", maxTop * 500) + 1);
    }
    batchForAffirmType(affirmType, teachWorkloadIds, b);
  }

  public void batchForAffirmType(String affirmType, String teachWorkloadIds, boolean b) {
    String strHql = "update TeachWorkload set " + affirmType + "=" + b + " where id in (" + teachWorkloadIds
        + ")";
  ...
  public void updateAffirmAll(String departmentIds, String typeName, boolean b) {
    ...
    if (StringUtils.isNotEmpty(departmentIds)) {
      updateHql += " where teacherInfo.teachDepart.id in(" + departmentIds + ")";
   ...
   *      java.lang.String)
   */
  public List getTasksOfTeacherByChargeDepart(String stdTypeSeq, String departIdSeq, String teachCalendarIdSeq) {
    if (StringUtils.isBlank(stdTypeSeq) || StringUtils.isBlank(departIdSeq)
        || StringUtils.isBlank(teachCalendarIdSeq)) { return Collections.EMPTY_LIST; }
    Criteria criteria = getSession().createCriteria(TeachTask.class);
    criteria.add(Restrictions.in("calendar.id", SeqStringUtil.transformToLong(teachCalendarIdSeq)));
    criteria.add(Restrictions.in("arrangeInfo.teachDepart.id", SeqStringUtil.transformToLong(departIdSeq)));
    criteria.add(Restrictions.in("teachClass.stdType.id", SeqStringUtil.transformToLong(stdTypeSeq)));
    return criteria.list();
  }
  ...
  public void batchDeleteTeachWorkload(String departmentSeq, String stdTypeSeq, String teachCalendarIds) {
    ...
    if (StringUtils.isNotEmpty(departmentSeq) && StringUtils.isNotEmpty(stdTypeSeq)) {
      query.executeUpdate();
    }
  }
  ...
  public void batchDeleteBySeq(Map conditionsSeq) {
    ...
    for (Iterator iter = keySet.iterator(); iter.hasNext();) {
      ...
      if (iter.hasNext()) stringhql += element + " in(" + conditionsSeq.get(element) + ") and ";
      else stringhql += element + " in(" + conditionsSeq.get(element) + ")";
    }
    if (flag) {
    ...
    if (null != taskCollection && taskCollection.size() > 0) {
      ...
      if (taskIds.size() > 500) {
        while (from < taskIds.size()) {
          int end = from + 500;
          if (end > taskIds.size()) end = taskIds.size();
        ...
        query.setParameterList("taskIds", EntityUtils.extractIds(taskCollection));
        query.executeUpdate();
      }
