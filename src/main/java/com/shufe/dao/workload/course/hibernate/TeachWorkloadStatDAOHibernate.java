...
public class TeachWorkloadStatDAOHibernate extends BasicHibernateDAO implements TeachWorkloadStatDAO {
   ...
   */
  public List getEduDegreeAndTitle(String deaprtmentIds, String propertyName, Long calendarId, Integer age,
      String stdTypeIdSeq) {
    if (StringUtils.isBlank(deaprtmentIds) || StringUtils.isBlank(stdTypeIdSeq) || null == calendarId
        || new Long(0).equals(calendarId)) { return Collections.EMPTY_LIST; }
    Criteria criteria = getSession().createCriteria(TeachWorkload.class);
    criteria.add(Restrictions.eq("teachCalendar.id", calendarId));
    criteria.add(Restrictions.in("teacherInfo.teachDepart.id", SeqStringUtil.transformToLong(deaprtmentIds)));
    criteria.add(Restrictions.in("studentType.id", SeqStringUtil.transformToLong(stdTypeIdSeq)));
    if (null != age && age.intValue() > 0) {
      criteria.add(Restrictions.le("teacherInfo.teacherAge", age));
    }
    criteria.setProjection(Projections.projectionList()
        .add(Projections.countDistinct("teacherInfo.teacher.id"))
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.dao.workload.TeachWorkloadStatDAO12#getEduDegreeAndTitleInTeacher(java.util.List,
  ...
  public List getEduDegreeAndTitleInTeacher(String propertyName, String departmentIds, Integer age) {
    ...
    if (StringUtils.isBlank(departmentIds) || StringUtils.isEmpty(propertyName)
        || propertyName.indexOf(",") > -1) { return Collections.EMPTY_LIST; }
    if (propertyName.indexOf("degreeInfo") > -1) {
      criteria.createCriteria("degreeInfo", "degreeInfo");
    }
    criteria.add(Restrictions.in("department.id", SeqStringUtil.transformToLong(departmentIds)));
   ...
   * (non-Javadoc)
   */
  public List getTeacherNumberByConditions(String departIdSeq, String stdTypeIdSeq, Long teacherTypeId,
      TeachCalendar teachCalendar) {
    if (StringUtils.isBlank(departIdSeq) || StringUtils.isBlank(stdTypeIdSeq) || null == teacherTypeId
        || null == teachCalendar) { return Collections.EMPTY_LIST; }
    ...
    criteria
        .setProjection(Projections.projectionList().add(Projections.rowCount())
            ...
            .add(Projections.groupProperty("teacherInfo.teacherTitle")));
    return criteria.list();
  }
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.dao.workload.TeachWorkloadStatDAO12#getTeacherNumberOfDepartTeacherTypeTimeOfWorkload
   * (java.util.List,
   * java.lang.Long, java.util.List, java.util.List, java.lang.Long)
   */
  public List getTotalWorkloadByConditions(String departIdSeq, String stdTypeIdSeq, Long teacherTypeId,
      TeachCalendar teachCalendar) {
    if (StringUtils.isBlank(departIdSeq) || StringUtils.isBlank(stdTypeIdSeq) || null == teachCalendar
        || null == teacherTypeId) { return Collections.EMPTY_LIST; }
    List calendars = new ArrayList();
    calendars.add(teachCalendar);
    Criteria criteria = getExampleWorkloadOfCalendars(departIdSeq, stdTypeIdSeq, calendars);
    criteria.add(Restrictions.eq("teacherInfo.teacherType.id", teacherTypeId));
    criteria
        .setProjection(Projections.projectionList().add(Projections.sum("totleWorkload"))
            .add(Projections.groupProperty("college"))
            .add(Projections.groupProperty("teacherInfo.teacherTitle")));
    return criteria.list();
  }
  ...
  public List getNumberAndWorkloadOfTitle(TeachCalendar teachCalendar, String stdTypeIdSeq) {
    ...
    criteria.setProjection(Projections.projectionList()
        .add(Projections.countDistinct("teacherInfo.teacher").as("teacherNumber"))
        .add(Projections.sum("totleWorkload").as("totleWorkload"))
        .add(Projections.groupProperty("teacherInfo.teacherTitle").as("teacherTitle")));
  ...
  public List getWorkloadOfDepartment(TeachCalendar teachCalendar, String studentTypeIds, String departmentIds) {
    if (null == teachCalendar || StringUtils.isBlank(studentTypeIds) || StringUtils.isBlank(departmentIds)) { return Collections.EMPTY_LIST; }
    ...
    criteria.setProjection(Projections.projectionList().add(Projections.countDistinct("teacherInfo.teacher"))
        .add(Projections.sum("totleWorkload")).add(Projections.groupProperty("teacherInfo.teacherType"))
        .add(Projections.groupProperty("teacherInfo.teachDepart")));
  ...
  public List getRegisterPeopleOfDepartment(String departmentIds) {
    if (StringUtils.isBlank(departmentIds)) { return Collections.EMPTY_LIST; }
    Criteria criteria = getSession().createCriteria(Teacher.class);
    criteria.add(Restrictions.in("department.id", SeqStringUtil.transformToLong(departmentIds)));
    criteria.setProjection(Projections.projectionList().add(Projections.rowCount())
        .add(Projections.groupProperty("teacherType")).add(Projections.groupProperty("department")));
    return criteria.list();
  }
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.dao.workload.TeachWorkloadStatDAO12#getCourseNumberOfDepartment(com.shufe.model.system
   * .calendar.TeachCalendar,
  ...
  public List getCourseNumberOfDepartment(TeachCalendar teachCalendar, String studentTypeIds,
      String collegeIdSeq) {
    if (null == teachCalendar || StringUtils.isBlank(studentTypeIds) || StringUtils.isBlank(collegeIdSeq)) { return Collections.EMPTY_LIST; }
    Criteria criteria = transMethod(teachCalendar, studentTypeIds, Boolean.TRUE);
    criteria.add(Restrictions.in("college.id", SeqStringUtil.transformToLong(collegeIdSeq)));
    criteria.setProjection(Projections.projectionList().add(Projections.rowCount())
        .add(Projections.groupProperty("college")).add(Projections.groupProperty("courseType")));
   ...
   */
  public List getTitleAndStdTypes(TeachCalendar teachCalendar, String deprtIdSeq, String stdTypeIdSeq) {
    if (StringUtils.isBlank(deprtIdSeq) || StringUtils.isBlank(stdTypeIdSeq) || null == teachCalendar) { return Collections.EMPTY_LIST; }
    Criteria criteria = transMethod(teachCalendar, stdTypeIdSeq, Boolean.TRUE);
    criteria.add(Restrictions.in("college.id", SeqStringUtil.transformToLong(deprtIdSeq)));
    criteria.setProjection(Projections.projectionList().add(Projections.rowCount())
        .add(Projections.groupProperty("studentType"))
        .add(Projections.groupProperty("teacherInfo.teacherTitle")));
    return criteria.list();
  }
  ...
  public List getWorkloadOfTeacherTypeAndDepart(TeachCalendar teachCalendar, String studentTypeIds,
    ...
    if (StringUtils.isBlank(studentTypeIds) || StringUtils.isBlank(departIdSeq) || null == teachCalendar) { return Collections.EMPTY_LIST; }
    ...
    criteria.setProjection(Projections.projectionList().add(Projections.sum("totleWorkload"))
        .add(Projections.groupProperty("college")).add(Projections.groupProperty("teacherInfo.teacherType")));
    return criteria.list();
  }
   ...
   */
  public List getAvgCourseNoOfWokload(String departIdSeq, String stdTypeIdSeq, TeachCalendar teachCalendar) {
    if (StringUtils.isBlank(stdTypeIdSeq) || StringUtils.isBlank(departIdSeq) || null == teachCalendar) { return Collections.EMPTY_LIST; }
    Criteria criteria = transMethod(teachCalendar, stdTypeIdSeq, Boolean.TRUE);
    criteria.add(Restrictions.in("college.id", SeqStringUtil.transformToLong(departIdSeq)));
    criteria.setProjection(Projections.projectionList().add(Projections.countDistinct("teacherInfo.teacher"))
        .add(Projections.sum("classNumberOfWeek")).add(Projections.groupProperty("college"))
  ...
  public List getWorkloadByDataRealm(String departmentIdSeq, String studentTypeIdSeq,
    ...
    criteria.setProjection(Projections.projectionList().add(Projections.sum("totleWorkload"))
        .add(Projections.groupProperty("college")).add(Projections.groupProperty("studentType")));
    return criteria.list();
  }
  ...
  public List getWorkloadByDataRealm(String departmentIdSeq, String studentTypeIdSeq,
    ...
    if (StringUtils.isBlank(studentTypeIdSeq) || StringUtils.isBlank(departmentIdSeq)
        || null == teachCalendars || teachCalendars.size() < 1) { return Collections.EMPTY_LIST; }
    Criteria criteria = getExampleWorkloadOfCalendars(departmentIdSeq, studentTypeIdSeq, teachCalendars);
    criteria.setProjection(Projections.projectionList().add(Projections.sum("totleWorkload"))
        .add(Projections.groupProperty("college")).add(Projections.groupProperty("studentType")));
    return criteria.list();
  }
  ...
  private Criteria getExampleWorkloadOfCalendars(String departmentIdSeq, String studentTypeIdSeq,
    ...
    Criteria criteria = getSession().createCriteria(TeachWorkload.class);
    criteria.add(Restrictions.in("college.id", SeqStringUtil.transformToLong(departmentIdSeq)));
    criteria.add(Restrictions.in("studentType.id", SeqStringUtil.transformToLong(studentTypeIdSeq)));
  ...
  public List getWorkloadOfTeacherAndCalendar(String departmentIdSeq, String studentTypeIdSeq,
    ...
    criteria.setProjection(Projections.projectionList().add(Projections.sum("totleWorkload"))
        .add(Projections.groupProperty("teacherInfo.teacher"))
        .add(Projections.groupProperty("teachCalendar")));
    return criteria.list();
  }
  ...
  public List getTotalAndRegisterWorkload(List collegeList, Long teachCalendarId) {
    ...
    String sqlString = "sum(zgzl) as yxzgzl, SUM (case when jzglb_T_id =" + TEACHERTYPEID
        ...
        + " then zgzl else 0 end) as bbmzczgzl, SUM (case when jzglb_T_id =" + TEACHERTYPEID
        + " then 1 else 0 end) as zczrs";
    String[] alases = new String[] { "yxzgzl", "yxzrs", "bbmzczgzl", "zczrs" };
    Type[] types = new Type[] { Hibernate.FLOAT, Hibernate.INTEGER, Hibernate.FLOAT, Hibernate.INTEGER };
    criteria.add(Restrictions.eq("calcWorkload", Boolean.TRUE));
    criteria.setProjection(Projections.projectionList()
        .add(Projections.sqlProjection(sqlString, alases, types)).add(Projections.groupProperty("college")));
    criteria.addOrder(Order.asc("college.id"));
    return criteria.list();
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.dao.workload.TeachWorkloadStatDAO12#getTitleOfBasicWorkload(com.shufe.model.system
   * .calendar.TeachCalendar,
   * java.lang.String, java.lang.Long)
   */
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.dao.workload.TeachWorkloadStatDAO12#getPeopleOfDepartment(com.shufe.model.system.
   * calendar.TeachCalendar,
   * java.lang.String, java.lang.String)
   */
  public List getPeopleOfDepartment(TeachCalendar teachCalendar, String studentTypeIds, String departmentIds) {
    Criteria criteria = transMethod(teachCalendar, studentTypeIds, Boolean.TRUE);
    ...
    criteria.add(Restrictions.eq("calcWorkload", Boolean.TRUE));
    criteria.setProjection(Projections.projectionList().add(Projections.property("teacherInfo.teachDepart"))
        .add(Projections.sqlGroupProjection(strSql, "jsssyx", strAs, types)));
    criteria.addOrder(Order.asc("teacherInfo.teachDepart.id"));
    return criteria.list();
  }
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.dao.workload.TeachWorkloadStatDAO12#getWorkloadOfDepartment(com.shufe.model.system
   * .calendar.TeachCalendar,
   * java.lang.String, java.lang.String)
   */
  ...
  /*
   * (non-Javadoc)
   * @see com.shufe.dao.workload.TeachWorkloadStatDAO12#transMethod(com.shufe.model.system.calendar.
   * TeachCalendar,
   * java.lang.String)
   */
  public Criteria transMethod(TeachCalendar teachCalendar, String studentTypeIds, Boolean isCaculate) {
    Criteria criteria = getSession().createCriteria(TeachWorkload.class);
    ...
    if (StringUtils.isNotBlank(studentTypeIds)) {
      criteria.add(Restrictions.in("studentType.id", SeqStringUtil.transformToLong(studentTypeIds)));
    } else {
  ...
  public List getPropertyOfTeachWorkload(String propertyOfTeachWorkload, Long teachCalendarId) {
    if (null == teachCalendarId || new Long(0).equals(teachCalendarId)) { return Collections.EMPTY_LIST; }
    if (StringUtils.isBlank(propertyOfTeachWorkload)) { return Collections.EMPTY_LIST; }
    Criteria criteria = getSession().createCriteria(TeachWorkload.class);
    criteria.add(Restrictions.eq("teachCalendar.id", teachCalendarId));
