...
public class InstructWorkloadDAOHibernate extends BasicHibernateDAO implements InstructWorkloadDAO {

  /*
   * (non-Javadoc)
   * @see
   * com.shufe.dao.workload.InstructWorkloadDAO12#getExampleInstructWorkloads(com.shufe.model.workload
   * .InstructWorkload,
   * java.lang.String, java.lang.String, java.lang.String)
   */
  public Criteria getExampleInstructWorkloads(InstructWorkload instructWorkload, String departmentIds,
      String age1, String age2) {
    Criteria criteria = this.getSession().createCriteria(InstructWorkload.class);
    ...
    if (instructWorkload.getTeachCalendar() != null) {
      if (StringUtils.isNotEmpty(instructWorkload.getTeachCalendar().getYear())) {
        criteria.add(Restrictions.eq("teachCalendar.year", instructWorkload.getTeachCalendar().getYear()));
      }
      if (StringUtils.isNotEmpty(instructWorkload.getTeachCalendar().getTerm())) {
        criteria.add(Restrictions.eq("teachCalendar.term", instructWorkload.getTeachCalendar().getTerm()));
      }
    }
    // 排序
    criteria.addOrder(Order.desc("teachCalendar.year")).addOrder(Order.asc("teacherInfo.teachDepart.id"))
        .addOrder(Order.asc("teacherInfo.teacherName"));
    return criteria;
  }
  ...
  public void affirmType(String affirmType, String teachWorkloadIds, boolean b) {
    String strHql = "update InstructWorkload set " + affirmType + "=" + b + " where id in ("
        + teachWorkloadIds + ")";
  ...
  public void updateAffirmAll(String stdTypeIds, String typeName, boolean b) {
    String updateHql = "update InstructWorkload set " + typeName + "=" + b + " where studentType.id in("
        + stdTypeIds + ")";
  ...
  public int batchRemove(TeachCalendar calendar, DataRealm realm, String modulusType) {
    ...
    query.setParameterList("stdTypeIds", SeqStringUtil.transformToLong(realm.getStudentTypeIdSeq()));
    query.setParameterList("departIds", SeqStringUtil.transformToLong(realm.getDepartmentIdSeq()));
