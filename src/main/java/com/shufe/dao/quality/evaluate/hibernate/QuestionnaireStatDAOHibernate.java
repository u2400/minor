...
public class QuestionnaireStatDAOHibernate extends BasicHibernateDAO implements QuestionnaireStatDAO {

  public int batchDeleteQuestionnaireStat(String stdTypeIdSeq, String departIdSeq, String calendarIdSeq) {
    // 删除问卷统计的子项目前先删除它的子项目
    String condition = " where depart.id in(" + departIdSeq + ") and stdType.id in(" + stdTypeIdSeq
    ...
    if (StringUtils.isNotBlank(departIdSeq) && StringUtils.isNotBlank(stdTypeIdSeq)
  ...
  public List getQuestionTypes(String stdTypeIdSeq, String departIdSeq, Collection teachCalendars) {
    ...
    criteria
        .add(Restrictions.in("questionnaireStat.stdType.id", SeqStringUtil.transformToLong(stdTypeIdSeq)));
    criteria.add(Restrictions.in("questionnaireStat.depart.id", SeqStringUtil.transformToLong(departIdSeq)));
    if (null != teachCalendars && !teachCalendars.isEmpty()) {
      criteria.add(Restrictions.in("questionnaireStat.calendar", teachCalendars));
    }
    criteria.setProjection(Projections.groupProperty("type"));
    return criteria.list();
  }

  private Criteria getQuestionnaireStats(String stdTypeIdSeq, String departIdSeq, String calendarIdSeq,
      QuestionnaireStat questionnaireStat) {
    Criteria criteria = getSession().createCriteria(QuestionnaireStat.class);
    ...
    if (StringUtils.isNotBlank(departIdSeq)) {
      criteria.add(Restrictions.in("depart.id", SeqStringUtil.transformToLong(departIdSeq)));
    }
    ...
    if (StringUtils.isNotBlank(calendarIdSeq)) {
      criteria.add(Restrictions.in("calendar.id", SeqStringUtil.transformToLong(calendarIdSeq)));
    }
    return criteria;
  }
   ...
   */
  public List getDataByDepartAndRelated(String stdTypeIdSeq, String departIdSeq, Long calendarIdSeq) {
    Criteria criteria = getQuestionnaireStats(stdTypeIdSeq, departIdSeq, String.valueOf(calendarIdSeq), null);
    criteria.setProjection(Projections.projectionList().add(Projections.groupProperty("score"))
        .add(Projections.groupProperty("depart")).add(Projections.groupProperty("teacher.id"))
        .add(Projections.groupProperty("task.id")));
  ...
  public List getDatasForPieChart(String stdTypeIdSeq, Long departId, Long calendarId) {
    Criteria criteria = getQuestionnaireStats(stdTypeIdSeq, String.valueOf(departId),
        String.valueOf(calendarId), null);
    criteria.setProjection(Projections.projectionList().add(Projections.groupProperty("score"))
        .add(Projections.groupProperty("teacher")).add(Projections.groupProperty("task")));
    return criteria.list();
  }
