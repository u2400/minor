...
public class FeeDetailDAOHibernate extends BasicHibernateDAO implements FeeDetailDAO {
   ...
   * @return
   */
  private Criteria getFeeDetailCriteria(FeeDetail feeDetail, String departIdSeq, String orderByName) {
    Criteria criteria = getSession().createCriteria(FeeDetail.class);
    // 添加学生条件
    ...
    if (StringUtils.isNotEmpty(orderByName)) {
      ...
      for (int i = 0; i < orderName.length; i++) {
        ...
        if (orderName[i].indexOf(" ") != -1) {
          if (orderName[i].substring(orderName[i].indexOf(" ") + 1).equals("desc")) criteria.addOrder(Order
              .desc(orderName[i].substring(0, orderName[i].indexOf(" "))));
   ...
   */
  public Pagination getFeeDetails(FeeDetail feeDetail, String departIdSeq, String orderbyName, int pageNo,
      int pageSize) {
   ...
   * @return
   */
  private Criteria getFeeDetailOfTypesCriteria(FeeDetail feeDetail, List feeTypeList, String orderByName) {
    Criteria criteria = getSession().createCriteria(FeeDetail.class);
    List criterions = CriterionUtils.getEntityCriterions(feeDetail);
    for (Iterator iter = criterions.iterator(); iter.hasNext();)
      criteria.add((Criterion) iter.next());
    ...
    projectionList.add(Projections.groupProperty("year")).add(Projections.groupProperty("term"));
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.dao.fee.hibernate.FeeDetailDAO#getFeeDetailOfTypes(com.shufe.model.fee.FeeDetail,
   * java.util.List, java.lang.String, int, int)
   */
  public Pagination getFeeDetailOfTypes(FeeDetail feeDetail, List feeTypeList, String orderByName,
      int pageNo, int pageSize) {
    return dynaSearch(getFeeDetailOfTypesCriteria(feeDetail, feeTypeList, orderByName), pageNo, pageSize);
  }
   ...
   */
  public Criteria getStdSelectCourses(String departmentIds, CourseTake courseTake, TeachCalendar teachCalenar) {
    ...
    if (courseTake != null) {
      List criterionOfStudent = CriterionUtils.getEntityCriterions("student.", courseTake.getStudent());
  ...
  public List getDetailInfos(Long stdId, Long teachCalendarId) {
    ...
    criteria.setProjection(Projections.projectionList().add(Projections.sum("payed"))
        .add(Projections.sum("toRMB")).add(Projections.groupProperty("type.id"))
        .add(Projections.groupProperty("currencyCategory.id")));
    return criteria.list();
  }
}
