...
public class QuestionnaireRecycleRateDAOHibernate extends BasicHibernateDAO implements
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.dao.evaluate.QuestionnaireRecycleRateDAO12#getTotlePersonAmount(java.lang.String,
  ...
  public List getTotlePersonAmount(String collegeIds, Collection teachCalendarCollection,
    ...
    if (StringUtils.isEmpty(collegeIds) || StringUtils.isEmpty(studentTypeIds)
        || null == teachCalendarCollection) { return Collections.EMPTY_LIST; }
    ...
    criteria.add(Restrictions.in("student.department.id", SeqStringUtil.transformToLong(collegeIds)));
    criteria.add(Restrictions.in("student.type.id", SeqStringUtil.transformToLong(studentTypeIds)));
    criteria.add(Restrictions.in("task.calendar", teachCalendarCollection));
    criteria.setProjection(Projections.projectionList().add(Projections.rowCount())
        .add(Projections.groupProperty("student.department.id"))
        .add(Projections.groupProperty("student.type.id")));

    criteria.addOrder(Order.asc("student.department.id")).addOrder(Order.asc("student.type.id"));
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.dao.evaluate.QuestionnaireRecycleRateDAO12#getHasEvlautaeAmount(java.lang.String,
  ...
  public List getHasEvlautaeData(String collegeIds, String studentTypeIds, Collection teachCalendars) {
    if (StringUtils.isBlank(collegeIds) || StringUtils.isBlank(studentTypeIds) || null == teachCalendars
        || teachCalendars.size() < 1) { return Collections.EMPTY_LIST; }
    ...
    criteria.setProjection(Projections.projectionList().add(Projections.rowCount())
        .add(Projections.groupProperty("stdDepart.id")).add(Projections.groupProperty("stdType.id"))
        .add(Projections.groupProperty("student")));
    return criteria.list();
  }

  public Criteria getRecycleRateExample(String deparmtentIds, String studentTypeIds, Collection teachCalenders) {
    Criteria criteria = getSession().createCriteria(QuestionnaireRecycleRate.class);
    criteria.add(Restrictions.in("department.id", SeqStringUtil.transformToLong(deparmtentIds)));
   ...
   *      java.lang.String)
   */
  public List getRecycleRateObjects(String deparmtentIds, String studentTypeIds, Collection teachCalenders) {
    if (StringUtils.isEmpty(deparmtentIds) || StringUtils.isEmpty(studentTypeIds) || null == teachCalenders
        || teachCalenders.size() < 1) { return Collections.EMPTY_LIST; }
  ...
  public List getStatisticResults(String collegeIdSeq, String stdTypeIdSeq, Collection teachCalendars) {
    if (StringUtils.isBlank(collegeIdSeq) || StringUtils.isBlank(stdTypeIdSeq) || null == teachCalendars
        || teachCalendars.size() < 1) { return Collections.EMPTY_LIST; }
    Criteria criteria = getSession().createCriteria(EvaluateResult.class);
    ...
    criteria.add(Restrictions.in("teachCalendar", teachCalendars));
    criteria.setProjection(Projections.projectionList().add(Projections.countDistinct("student"))
        .add(Projections.groupProperty("stdDepart.id")).add(Projections.groupProperty("stdType.id")));
    return criteria.list();
  }
  ...
  public List getStudentTypeLists(String departmentIds) {
    if (StringUtils.isEmpty(departmentIds)) { return Collections.EMPTY_LIST; }
    Criteria criteria = getSession().createCriteria(QuestionnaireRecycleRate.class);
  ...
  public List getAllRateByCourseTask(String departmentIds, String studentTypeIds, Collection teachCalendarSet) {
    if (StringUtils.isBlank(departmentIds) || StringUtils.isBlank(studentTypeIds) || null == teachCalendarSet
        || teachCalendarSet.size() < 1) { return Collections.EMPTY_LIST; }
    Criteria criteria = getSession().createCriteria(CourseTake.class);
    criteria.createAlias("task", "task");
    criteria.createAlias("student", "student");
    criteria.add(Restrictions.isNotNull("task.questionnaire"));
    criteria.add(Restrictions.in("task.arrangeInfo.teachDepart.id",
        SeqStringUtil.transformToLong(departmentIds)));
    criteria.add(Restrictions.in("student.type.id", SeqStringUtil.transformToLong(studentTypeIds)));
            ...
            .add(
                Projections
                    .sqlProjection(
                        ...
                        new String[] { "pjrc", "pjzrc", "bl" }, new Type[] { Hibernate.INTEGER,
                            Hibernate.INTEGER, Hibernate.FLOAT }))
            .add(
                Projections
                    .sqlProjection(
                        ...
                        new String[] { "sprs", "spzrs", "spbl" }, new Type[] { Hibernate.INTEGER,
                            Hibernate.INTEGER, Hibernate.FLOAT }))
            .add(Projections.groupProperty("task.arrangeInfo.teachDepart"))
            .add(Projections.groupProperty("student.type")).add(Projections.groupProperty("task.calendar")));
    return criteria.list();
  }
   ...
   */
  public List getDetailInfoOfTask(String departmentIds, String departmentId, String studentTypeIds,
      Collection teachCalendarSet, String studentOrTeacher) {
    if (StringUtils.isEmpty(departmentIds) || StringUtils.isEmpty(studentTypeIds) || null == teachCalendarSet) { return Collections.EMPTY_LIST; }
    Criteria criteria = getSession().createCriteria(CourseTake.class);
    criteria.createAlias("task", "task");
    ...
    criteria.setProjection(Projections
        .projectionList()
        .add(
            Projections.sqlProjection("count (distinct(case when sfpjwc = 1 then xsid end)) as sprs,"
                + "count(distinct xsid) as spzrc,"
                + "count (distinct(case when sfpjwc = 1 then xsid end))/count(distinct xsid)*100 as spbl",
                new String[] { "sprs", "spzrc", "spbl" }, new Type[] { Hibernate.INTEGER, Hibernate.INTEGER,
                    Hibernate.FLOAT })).add(Projections.groupProperty("task"))
        .add(Projections.groupProperty(propertyString))
        .add(Projections.groupProperty("task.teachClass.stdType")));
    criteria.addOrder(Order.asc(propertyString)).addOrder(Order.asc("task.teachClass.stdType.id"));
    return criteria.list();
  }
}
