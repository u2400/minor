...
public class TeacherDAOHibernate extends BasicHibernateDAO implements TeacherDAO {
  ...
  public Teacher getTeacherByNO(String TeacherNO) {
    List teachers = getExampleCriteria(getSession(), null).add(Restrictions.eq("code", TeacherNO)).list();
    if (teachers.isEmpty()) return null;
    else return (Teacher) teachers.get(0);
  }
  ...
  public List getTeacherNamesByDepart(Long departId, Boolean isTeaching) {
    ...
    String hql = "select teacher.id,teacher.name,teacher.code from Teacher as teacher"
        + " where teacher.department.id =" + departId + teachingCondition + " order by teacher.name";
    Query query = getSession().createQuery(hql);
    return query.list();
  }

  public Collection<Teacher> getValidTeachersInDepart(String taskGroupIdValue, String departmentIdValue,
      String teacherIdSeq) {
    ...
    if (StringUtils.isBlank(taskGroupIdValue)) {
      query.add(new Condition("teacher is null"));
    } else {
      query
          .add(new Condition(
              "exists (from "
                  + TaskGroup.class.getName()
                  + " taskGroup where exists (from taskGroup.directTasks task where exists (from task.arrangeInfo.teachers teacherInTask where teacherInTask != teacher)))"));
    }
    if (null == departmentIdValue) {
      query.add(new Condition("teacher is null"));
    } else {
      query.add(new Condition("teacher.department.id = :departmentId", new Long(departmentIdValue)));
    }
    if (null != teacherIds) {
      query.add(new Condition("teacher.id not in (:teacherIds)", teacherIds));
    }
    query.addOrder(OrderUtils.parser("teacher.name"));
  ...
  public List getTeachersByDepartment(Long[] departIds) {
    return getExampleCriteria(getSession(), null).add(Restrictions.in("department.id", departIds)).list();
  ...
  public List getTeachersByNO(String[] teacherNOs) {
    return getExampleCriteria(getSession(), null).add(Restrictions.in("code", teacherNOs)).list();
  }
   ...
   */
  public Pagination getTeachers(final Teacher teacher, Long[] departIds, int pageNo, int pageSize) {
  ...
  private Criteria genTeacherInDepartCriteria(final Teacher teacher, Long[] departIds) {
    ...
    if (null != departIds) criterions.add(Restrictions.in("department.id", departIds));
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.dao.system.baseinfo.TeacherDAO#getTeacherIdsByTeacher(com.shufe.model.system.baseinfo
