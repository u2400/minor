...
public class AdminClassDAOHibernate extends BasicHibernateDAO implements AdminClassDAO {
  ...
  public List getAdminClasses(AdminClass adminClass) {
    ...
    if (null != adminClass && ValidEntityPredicate.INSTANCE.evaluate(adminClass.getStdType())) {
      StudentType stdType = (StudentType) utilDao.get(StudentType.class, adminClass.getStdType().getId());
  ...
  public List getAdminClassesById(Long[] classIds) {
    return buildExampleCriteria(null, null, null).add(Restrictions.in("id", classIds)).list();
  }

  /*
   * (non-Javadoc)
   * @see
   * com.shufe.dao.system.baseinfo.AdminClassDAODWRFacade#getAdminClassNamesExactly(java.lang.String
   * ,
   * java.lang.Long, java.lang.Long, java.lang.Long, java.lang.Long)
   */
  public List getAdminClassNamesExactly(String enrollTurn, Long stdTypeId, Long departId, Long specialityId,
      Long aspectId) {
    HashMap params = new HashMap();
    String hql = "select d.id, d.name from AdminClass as d" + "where d.state=true";
    ...
    if (null != stdTypeId) {
      ...
      hql += "and d.stdType=:stdTypeId";
    }
    if (null != departId) {
      params.put("departId", departId);
      hql += "and d.department=:departId";
    }
    if (null != specialityId) {
      params.put("specialityId", specialityId);
      hql += "and d.speciality=:specialityId";
    } else {
  ...
  public List getAdminClassesExactly(AdminClass adminClass) {
    ...
    if (!ValidEntityPredicate.INSTANCE.evaluate(adminClass.getSpeciality())) criteria.add(Restrictions
        .isNull("speciality.id"));
    if (!ValidEntityPredicate.INSTANCE.evaluate(adminClass.getAspect())) criteria.add(Restrictions
        .isNull("aspect.id"));
   ...
   */
  public Pagination getAdminClasses(AdminClass adminClass, Long[] stdTypeIds, Long[] departIds, int pageNo,
      int pageSize) {
    Criteria criteria = buildExampleCriteria(adminClass, stdTypeIds, departIds);
    return dynaSearch(criteria, pageNo, pageSize);
  }

  public List getAdminClasses(AdminClass adminClass, Long[] stdTypeIds, Long[] departIds) {
   ...
   * @return
   */
  private Criteria buildExampleCriteria(AdminClass adminClass, Long[] stdTypeIds, Long[] departIds) {
    Criteria criteria = getSession().createCriteria(AdminClass.class);
    List criterions = CriterionUtils
        .getEntityCriterions(adminClass, new String[] { "speciality", "stdType" });
    ...
    if (null != departIds) criteria.add(Restrictions.in("department.id", departIds));
    if (null != stdTypeIds && stdTypeIds.length != 0) criteria.add(Restrictions.in("stdType.id", stdTypeIds));

    // 主要用于查询二专业的班级
    if (null != adminClass && null != adminClass.getSpeciality()) {
      List specialityCriterions = CriterionUtils.getEntityCriterions("speciality.",
          adminClass.getSpeciality());
    ...
    getSession().enableFilter("validAdminClass");
    return criteria;
  }
  ...
  public int updateActualStdCount(Long adminClassId) {
    ...
    try {
      ...
      try {
        ...
        info("execproduct is failed" + "in update_classactualstdcount" + ExceptionUtils.getStackTrace(e));
  ...
  public int updateStdCount(Long adminClassId) {
    ...
    try {
      ...
      try {
        ...
        info("execproduct is failed" + "in update_classstdcount" + ExceptionUtils.getStackTrace(e));
        throw new RuntimeException(e1.getMessage());
      }
    }
    return stdCount;
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.dao.system.baseinfo.AdminClassDAO#getAdminClassIdsByAdminClass(com.shufe.model.system
   * .baseinfo.AdminClass,
   * java.lang.String, java.lang.String)
   */
  public List getAdminClassIds(AdminClass adminClass, String departmentIds, String stdTypeIds) {
    ...
    Criteria criteria = buildExampleCriteria(adminClass, stdTypeId, departmentId);
    criteria.setProjection(Projections.property("id"));
    return criteria.list();
  }

  public Long[] getAdminClassIds(String enrollYear, Long stdTypeId, Long departmentId, Long specialityId,
      Long aspectId) {
    EntityQuery query = new EntityQuery(AdminClass.class, "adminClass");
    if (StringUtils.isEmpty(enrollYear)) { return null; }
    query.add(new Condition("adminClass.enrollYear = (:enrollYear)", enrollYear));
    if (null == stdTypeId || stdTypeId.intValue() == 0) { return null; }
    query.add(new Condition("adminClass.stdType.id = (:stdTypeId)", stdTypeId));
    if (null == departmentId || departmentId.intValue() == 0) { return null; }
    query.add(new Condition("adminClass.department.id = (:departmentId)", departmentId));
    if (null == specialityId || specialityId.intValue() == 0) {
      query.add(new Condition("adminClass.speciality.id is null"));
    } else {
      query.add(new Condition("adminClass.speciality.id = (:specialityId)", specialityId));
    }
    if (null == aspectId || aspectId.intValue() == 0) {
      query.add(new Condition("adminClass.aspect.id is null"));
    } else {
      query.add(new Condition("adminClass.aspect.id = (:aspectId)", aspectId));
    }
