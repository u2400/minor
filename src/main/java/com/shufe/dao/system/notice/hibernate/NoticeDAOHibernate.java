...
public class NoticeDAOHibernate extends BasicHibernateDAO implements NoticeDAO {
  ...
  public Collection getNotices(Notice notice, DataRealmLimit limit, List sortList) {
    Criteria criteria = getSession().createCriteria((null == notice) ? Notice.class : notice.getClass());
  ...
  public SinglePage getStudentNotices(Notice notice, Student std, List sortList, PageLimit limit) {
    ...
    entityQuery.add(new Condition("department.id = (:departmentId)", std.getDepartment().getId()));
    entityQuery.setLimit(limit);
    entityQuery.setCacheable(true);
    return (SinglePage) utilDao.search(entityQuery);
  }
