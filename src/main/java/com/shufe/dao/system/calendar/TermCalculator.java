...
public class TermCalculator {
  ...
  public int getTerm(StudentType stdType, String enrollTurn, Boolean omitSmallTerm) {
    // 获取培养计划中学生的入学日历
    OnCampusTime onCampusTime = (OnCampusTime) onCampusTimeMap.get(stdType.getId() + enrollTurn);
    ...
    if (null == onCampusTime) {
      OnCampusTimeNotFoundException ex = new OnCampusTimeNotFoundException(
          "error.onCampusTime.notExists:stdType is " + stdType.getId() + " for enroll turn " + enrollTurn);
    ...
    if (null == term) {
      if (logger.isDebugEnabled()) {
        logger.debug("calculate a term [stdtypeid:" + stdType.getName() + ", enrollTurn:" + enrollTurn + "]");
      }
      term = calendarDAO.getTermsBetween(onCampusTime.getEnrollCalendar(), calendar, omitSmallTerm);
