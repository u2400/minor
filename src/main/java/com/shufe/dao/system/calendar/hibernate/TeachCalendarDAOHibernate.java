...
public class TeachCalendarDAOHibernate extends BasicHibernateDAO implements TeachCalendarDAO {
  ...
  public TeachCalendar getTeachCalendar(Long id) {
    return (TeachCalendar) get(TeachCalendar.class, id);
  }
  ...
  private OnCampusTime getOnCampusTimeOf(StudentType stdType, String enrollTurn) {
    ...
    if (enrollCalendars.isEmpty()) return null;
    else return (OnCampusTime) enrollCalendars.get(0);
  }
  ...
  public OnCampusTime getOnCampusTime(StudentType stdType, String enrollTurn) {
    ...
    while (null == time) {
      ...
      if (null == stdType) return null;
      else time = getOnCampusTimeOf(stdType, enrollTurn);
    }
    return time;
  }
  ...
  public Map getWeekTimeMap(Long stdTypeId, String year, String term) {
    ...
    if (null == calendar) return Collections.EMPTY_MAP;
    else {
  ...
  private List getTeachCalendarDirectly(Long stdTypeId, String year, String term) {
    StringBuffer buf = new StringBuffer("from TeachCalendar as calendar where calendar.studentType.id=");
    buf.append(stdTypeId).append(" and calendar.year ='").append(year).append("' and calendar.term='")
        .append(term).append("'");
  ...
  public List getTeachCalendarNames(Long schemeId) {
    TeachCalendarScheme scheme = (TeachCalendarScheme) utilDao.get(TeachCalendarScheme.class, schemeId);
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.dao.system.calendar.TeachCalendarDAO#gerCalendarsByStdTypeIdSeq(java.lang.String)
   */
