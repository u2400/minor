...
public class GradeDAOHibernate extends BasicHibernateDAO implements GradeDAO {
  ...
  public boolean needReStudy(Student std, Course course) {
    ...
    if (rs.isEmpty()) return false;
      ...
      for (Iterator iter = rs.iterator(); iter.hasNext();) {
        ...
        if (Boolean.TRUE.equals(isPass)) return false;
      }
      return true;
    }
  }
  ...
  public boolean isPass(Student std, Course course) {
    ...
    if (rs.isEmpty()) return false;
      ...
      for (Iterator iter = rs.iterator(); iter.hasNext();) {
        Boolean isPass = (Boolean) iter.next();
        if (Boolean.TRUE.equals(isPass)) return true;
      }
      return false;
    }
  }
  ...
  public void getGradeCourseMap(ElectState state) {
    ...
    query.add(new Condition("cg.std.id = :stdId", state.getStd().getId()));
    query.setSelect("cg.course.id,cg.course.name, cg.isPass");
    ...
    for (Iterator iter = rs.iterator(); iter.hasNext();) {
      ...
      if (null != obj[2]) {
        result = (Boolean) obj[2];
      }
      Boolean existed = state.getHisCourses().get(courseId);
      if (null == existed || Boolean.TRUE.equals(result)) {
        state.getHisCourses().put(courseId, result);
      }
      state.getHisCourseNames().add(obj[1].toString());
    }
  }
  ...
  public void publishCourseGrade(TeachTask task, Boolean isPublished) {
    String hql = "update CourseGrade set status="
        + (Boolean.TRUE.equals(isPublished) ? Grade.PUBLISHED : Grade.CONFIRMED) + " where task.id="
        + task.getId();
  ...
  public void publishExamGrade(TeachTask task, GradeType gradeType, Boolean isPublished) {
    String hql = "update ExamGrade set status="
        + (Boolean.TRUE.equals(isPublished) ? Grade.PUBLISHED : Grade.CONFIRMED) + "where gradeType.id="
        + gradeType.getId() + " exists(from CourseGrade cg where cg.task.id=" + task.getId()
  ...
  public void publishExamGrade(TeachTask task, List gradeTypes, Boolean isPublished) {
    ...
    String hql = "update ExamGrade set status="
        ...
        + " where id in (select id from ExamGrade where instr('" + sb + "',','||gradeType.id||',')>0 "
        + " and courseGrade.task.id=" + task.getId() + ")";
    getSession().createQuery(hql).executeUpdate();
  }

  public Long getGradeMaxCalendarId(Student std) {
    String hql = "select Max(cg.calendar.id) from CourseGrade cg where cg.std.id=" + std.getId();
    Query query = getSession().createQuery(hql);
