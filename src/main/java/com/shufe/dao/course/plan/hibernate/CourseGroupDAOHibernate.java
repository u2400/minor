...
public class CourseGroupDAOHibernate extends BasicHibernateDAO implements CourseGroupDAO {
  ...
  public List getCourseGroupsOfStd(CourseGroup group, TeachPlan specialityPlan) {
    ...
    planCriteria.add(Restrictions.eq("enrollTurn", specialityPlan.getEnrollTurn()))
        .add(Restrictions.eq("stdType", specialityPlan.getStdType()))
        .add(Restrictions.eq("department", specialityPlan.getDepartment()));

    if (null != specialityPlan.getSpeciality() && specialityPlan.getSpeciality().isPO()) planCriteria
        .add(Restrictions.eq("speciality", specialityPlan.getSpeciality()));
    else planCriteria.add(Restrictions.isNull("speciality"));
    if (null != specialityPlan.getAspect() && specialityPlan.getAspect().isPO()) planCriteria
        .add(Restrictions.eq("aspect", specialityPlan.getAspect()));
    else planCriteria.add(Restrictions.isNull("aspect"));
  ...
  public CourseType getCourseType(Long planId, Long courseId) {
    String hql = "select courseGroup.courseType" + " from CourseGroup as courseGroup "
        + " join courseGroup.planCourses as planCourse " + " join courseGroup.teachPlans as plan "
        + " where plan.id=:teachPlanId and planCourse.course.id=:courseId";
