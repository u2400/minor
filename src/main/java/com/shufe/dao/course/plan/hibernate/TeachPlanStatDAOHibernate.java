...
public class TeachPlanStatDAOHibernate extends BasicHibernateDAO implements TeachPlanStatDAO {
  ...
  public List statByDepart(DataRealm realm, String enrollTurn) {
    String statHql = "select new com.shufe.service.util.stat.CountItem(count(*), plan.department) "
        + " from TeachPlan as plan" + " where plan.enrollTurn=  :enrollTurn "
        + " and plan.department.id in (:departIds)" + " and plan.stdType.id in (:stdTypeIds)"
  ...
  public List getEnrollTurns(DataRealm realm) {
    String statHql = "select distinct plan.enrollTurn from TeachPlan as plan "
        + " where plan.department.id in (:departIds) " + " and plan.stdType.id in(:stdTypeIds)"
