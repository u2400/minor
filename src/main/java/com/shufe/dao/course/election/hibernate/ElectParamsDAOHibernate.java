...
public class ElectParamsDAOHibernate extends BasicHibernateDAO implements ElectParamsDAO {
  ...
  public List getElectParams(ElectParams params) {
    ...
    // filter other params
    if (null != params.getStdTypes() && !params.getStdTypes().isEmpty()) for (Iterator iter = rsParams
        .iterator(); iter.hasNext();) {
      ElectParams oneParams = (ElectParams) iter.next();
      if (!oneParams.getStdTypes().containsAll(params.getStdTypes())) iter.remove();
    }
    if (null != params.getDeparts() && !params.getDeparts().isEmpty()) for (Iterator iter = rsParams
        .iterator(); iter.hasNext();) {
      ElectParams oneParams = (ElectParams) iter.next();
      if (!oneParams.getDeparts().containsAll(params.getDeparts())) iter.remove();
    }
    if (null != params.getEnrollTurns() && !params.getEnrollTurns().isEmpty()) for (Iterator iter = rsParams
        .iterator(); iter.hasNext();) {
      ElectParams oneParams = (ElectParams) iter.next();
      if (!oneParams.getEnrollTurns().containsAll(params.getEnrollTurns())) iter.remove();
    }
    return rsParams;
  }
