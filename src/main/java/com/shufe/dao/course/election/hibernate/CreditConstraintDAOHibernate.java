...
public class CreditConstraintDAOHibernate extends BasicHibernateDAO implements CreditConstraintDAO {
  ...
  public CreditConstraint getCreditConstraint(Serializable id, Class constraintClass) {
    CreditConstraint constraint = (CreditConstraint) getHibernateTemplate().get(constraintClass, id);
    if (null == constraint) constraint = (CreditConstraint) load(constraintClass, id);
    return constraint;
  }
  ...
  public List getCreditConstraints(Serializable[] ids, Class constraintClass) {
    String hql = "";
    if (constraintClass.equals(SpecialityCreditConstraint.class)) hql = "from SpecialityCreditConstraint where id in (:ids)";
    else hql = "from StdCreditConstraint where id in (:ids)";
  ...
  public Collection getStdCreditConstraints(List conditions, DataRealmLimit limit, List orders) {
    ...
    if (null != limit) {
      if (null != limit.getDataRealm()) {
        if (StringUtils.isNotEmpty(limit.getDataRealm().getStudentTypeIdSeq())) {
          conditions.add(new Condition("std.type.id in (:stdTypeIds) ", SeqStringUtil.transformToLong(limit
              .getDataRealm().getStudentTypeIdSeq())));
        }
        if (StringUtils.isNotEmpty(limit.getDataRealm().getDepartmentIdSeq())) {
          conditions.add(new Condition("std.department.id in (:departIds) ", SeqStringUtil
    ...
    if (null != limit && null != limit.getPageLimit()) return this.search(query,
        ConditionUtils.getParamMap(conditions), limit.getPageLimit());
  ...
  private Criteria buildCriteria(StdCreditConstraint creditConstrant) {
    ...
    List criterions = CriterionUtils.getEntityCriterions(creditConstrant, new String[] { "std" });
    for (Iterator iter = criterions.iterator(); iter.hasNext();)
      criteria.add((Criterion) iter.next());
    if (null != creditConstrant) {
      List stdCriterions = CriterionUtils.getEntityCriterions("std.", creditConstrant.getStd());
  ...
  public List getSpecialityCreditConstraints(SpecialityCreditConstraint creditConstrant) {
    ...
    List criterions = CriterionUtils.getEntityCriterions(creditConstrant, new String[] { "speciality",
        "aspect" });
    for (Iterator iter = criterions.iterator(); iter.hasNext();)
      criteria.add((Criterion) iter.next());
    if (null == creditConstrant.getSpeciality()) criteria.add(Restrictions.isNull("speciality"));
    if (null == creditConstrant.getAspect()) criteria.add(Restrictions.isNull("aspect"));
  ...
  public List getSpecialityCreditConstraints(TeachCommon common) {
    ...
    for (Iterator iter = criterions.iterator(); iter.hasNext();)
      criteria.add((Criterion) iter.next());
    if (null == common.getSpeciality()) criteria.add(Restrictions.isNull("speciality"));
    if (null == common.getAspect()) criteria.add(Restrictions.isNull("aspect"));
    return criteria.list();
  }
  ...
  public StdCreditConstraint getStdCreditConstraint(Long stdId, TeachCalendar calendar) {
    ...
    if (constraints.size() > 0) return (StdCreditConstraint) constraints.get(0);
    else return null;
  }
  ...
  public void initCreditConstraint(TeachCommon common, TeachCalendar calendar, Float credit) {
    ...
    try {
      ...
      if (null != common.getSpeciality()) cstmt.setLong(8, common.getSpeciality().getId().longValue());
      else cstmt.setLong(8, 0);
      if (null != common.getAspect()) cstmt.setLong(9, common.getAspect().getId().longValue());
      else cstmt.setLong(9, 0);
      cstmt.executeUpdate();
      con.commit();
      cstmt.close();
  ...
  public StdCreditConstraint statStdCreditConstraint(Long stdId, CreditInitParams params) {
    ...
    Connection con = getSession().connection();
    try {
      if (null != params.getInitElectedCredit() && params.getInitElectedCredit().equals(Boolean.TRUE)) {
  ...
  public Collection getStdWithoutCredit(Student std, TeachCalendar calendar, DataRealmLimit limit,
      List orderList) {
   ...
   * @return
   */
  private Criteria buildNoCreditStdCriteria(Student std, TeachCalendar calendar, DataRealmLimit limit,
      List orderList) {
    Criteria criteria = getSession().createCriteria(Student.class);
    List criterions = CriterionUtils.getEntityCriterions("", std, new String[] { "active", "inSchool" },
    ...
    criteria.add(Restrictions.sqlRestriction(
        "{alias}.id not in(select xsid from xk_xsxf_t where xsid={alias}.id and jxrlid=" + calendar.getId()
            + ")", new Object[] {}, new Type[] {}));
