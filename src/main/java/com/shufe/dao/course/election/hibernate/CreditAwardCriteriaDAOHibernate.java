...
public class CreditAwardCriteriaDAOHibernate extends BasicHibernateDAO implements CreditAwardCriteriaDAO {

  /**
   * @see com.shufe.dao.course.election.CreditAwardCriteriaDAO#getAwardCredit(float)
   */
  public Float getAwardCredit(Float avgGrade) {
    List awardCritrias = find("getCreditAwardCriteria", new Object[] { avgGrade, avgGrade });
    if (awardCritrias.isEmpty()) return new Float(0);
    else return ((CreditAwardCriteria) awardCritrias.iterator().next()).getAwardCredits();
  }
