 ...
 * @author yangdong
 */
public class TaskAlterRequestDAOHibernate extends BasicHibernateDAO implements TaskAlterRequestDAO {
  /**
  ...
  public List getTaskAlterRequests(TeachCalendar calendar, Teacher teacher) {
    String hql = "select request from TaskAlterRequest request where request.task.calendar.id="
        + calendar.getId() + " and request.teacher.id=" + teacher.getId();
    Query query = getSession().createQuery(hql);
    return query.list();
  }

  public TaskAlterRequest getTaskAlterRequest(Long taskAlterRequestId) {
    return (TaskAlterRequest) get(TaskAlterRequest.class, taskAlterRequestId);
  }
