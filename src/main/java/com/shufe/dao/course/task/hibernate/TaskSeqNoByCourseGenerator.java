...
public class TaskSeqNoByCourseGenerator extends TaskSeqNoGeneratorImpl {
  ...
  public void genTaskSeqNo(TeachTask task) {
    ...
    synchronized (this) {
      String hql = "select seqNo from TeachTask  where calendar.id=" + task.getCalendar().getId()
          + " and course.id=" + task.getCourse().getId() + " and seqNo is not null order by seqNo";
      ...
      newSeqNo++;
      task.setSeqNo(courseCode + "." + StringUtils.repeat("0", 2 - String.valueOf(newSeqNo).length())
          + newSeqNo);
    }
  }
