...
public class RequirePreferDAOHibernate extends BasicHibernateDAO implements RequirePreferDAO {
  ...
  public RequirePrefer getPreference(Long preferenceId) {
    RequirePrefer preference = (RequirePrefer) getHibernateTemplate().get(RequirePrefer.class, preferenceId);
    if (null == preference) preference = (RequirePrefer) load(RequirePrefer.class, preferenceId);
    return preference;
  }
  ...
  public List getPreferences(RequirePrefer preference) {
    Criteria criteria = getSession().createCriteria(RequirePrefer.class);
