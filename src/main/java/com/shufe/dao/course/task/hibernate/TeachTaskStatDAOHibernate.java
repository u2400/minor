 ...
 * @author chaostone
 */
public class TeachTaskStatDAOHibernate extends BasicHibernateDAO implements TeachTaskStatDAO {
  ...
  public List statTeacherTitle(List calendars) {
    String queryString = "select jxrlid,jszcid,count(*) as num from ("
        ...
        + "				(teachtask0_.JXRLID in (:calendarIds))" + "				)group by jxrlid,jszcid";
    SQLQuery query = getSession().createSQLQuery(queryString);
    query.setParameterList("calendarIds", EntityUtils.extractIds(calendars));
