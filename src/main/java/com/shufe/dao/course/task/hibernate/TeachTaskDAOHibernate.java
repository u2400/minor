...
public class TeachTaskDAOHibernate extends BasicHibernateDAO implements TeachTaskDAO {
  ...
  public TeachTask getTeachTask(Long taskId) {
    ...
    if (null == task) task = (TeachTask) load(TeachTask.class, taskId);
    return task;
  }
  ...
  public TeachTask loadTeachTask(Long taskId) {
    ...
    else return (TeachTask) tasks.get(0);
  }

  public TeachTask getTeachTask(Teacher teacher, Course course, AdminClass adminClass, TeachCalendar calendar) {
    String taskHql = "select task from TeachTask as task " + " join task.arrangeInfo.teachers as teacher"
        + " join task.teachClass.adminClasses as adminClass " + " where task.course.id =" + course.getId()
        + " and task.calendar.id=" + calendar.getId() + " and adminClass.id=" + adminClass.getId()
        + " and teacher.id=" + teacher.getId();
    ;
  ...
  public Pagination getTeachTasksByCategory(Serializable id, TeachTaskFilterCategory category,
    ...
    if (category.getCategoryName().equals("teacher")) id = "%" + id + "%";
    params.put("id", id);
  ...
  public List getTeachTasksOfStd(Serializable stdId, List calendars) {
    String hql = "select take.task.id from CourseTake take"
        + " where take.student.id= :id  and take.task.calendar in (:calendars) ";
  ...
  public Pagination getTeachTasksOfTeachDepart(TeachTask task, Long[] stdTypeIds, Long[] teachDepartIds,
  ...
  public Pagination getTeachTasksOfLonely(TeachTask task, Long[] stdTypeIds, Long[] teachDepartIds,
      int pageNo, int pageSize) {
    Criteria criteria = getExampleCriteria(getSession(), task, new String[] { "arrangeInfo.teachDepart",
        "taskGroup", "requirement.courseCategory" }, true);
   ...
   */
  public Pagination getTeachTasksOfTeachPlan(Long[] teachPlanIds, TeachTask task, int pageNo, int pageSize) {
    Criteria criteria = getExampleCriteria(getSession(), task);
   ...
   */
  public Pagination getTeachTasksOfElectable(TeachTask task, ElectState state, Collection courseIds,
      TimeUnit time, boolean isScopeConstraint, int pageNo, int pageSize, Integer op) {
    if (null == task) {
      task = new TeachTask();
    }
    task.getElectInfo().setIsElectable(Boolean.TRUE);
    Criteria criteria = getExampleCriteria(getSession(), task, null, false);
    criteria.add(Restrictions.eq("calendar", state.getParams().getCalendar()));
    if (!CollectionUtils.isEmpty(courseIds)) {
      criteria.add(Restrictions.in("course.id", courseIds));
    }
    // 如果已选等于（0）或者小于（-1）或大于（1）
    if (op != null) {
      if (op == 0) {
        criteria.add(Expression.eqProperty("teachClass.stdCount", "electInfo.maxStdCount"));
      } else if (op == -1) {
        criteria.add(Expression.ltProperty("teachClass.stdCount", "electInfo.maxStdCount"));
      } else if (op == 1) {
        criteria.add(Expression.gtProperty("teachClass.stdCount", "electInfo.maxStdCount"));
      }
    }
    ElectParams params = (ElectParams) utilDao.get(ElectParams.class, state.getParams().getId());

    if (null == params.getIsLimitedInTeachClass() || params.getIsLimitedInTeachClass().booleanValue()) {
      // 增加男女生限制
      if (null != state.std.getGenderId()) {
        criteria.add(Restrictions.or(Restrictions.isNull("teachClass.gender.id"),
            Restrictions.eq("teachClass.gender.id", state.std.getGenderId())));
      }
      // time condition
      if (isScopeConstraint) {
        addScopeCriterion2(state.std, criteria);
      }
    }
    ...
    for (Iterator iter = taskIds.getItems().iterator(); iter.hasNext();) {
      ...
      TeachTask tt = (TeachTask) load(TeachTask.class, (Long) taskIdAndStdCount[0]);
      rs.add(tt);
      tt.getTeachClass().setStdCount((Integer) taskIdAndStdCount[1]);
    }
    return new Pagination(taskIds.getPageNumber(), taskIds.getPageSize(), new Result(taskIds.getResult()
        .getItemCount(), rs));
  }

  public Pagination getTeachTasksOfElectable(TeachTask task, TimeUnit time, Collection calendars, int pageNo,
      int pageSize) {
    if (null == task) task = new TeachTask();
    task.getElectInfo().setIsElectable(Boolean.TRUE);
    Criteria criteria = getExampleCriteria(getSession(), task);
   ...
   * @param criteria
   */
  private void addScopeCriterion2(SimpleStdInfo std, Criteria criteria) {
    if (null == std.getAdminClassIds() || std.getAdminClassIds().isEmpty()) {
      StringBuffer scopseRestriction = new StringBuffer(
          "(not exists(select xzb.jxrwid from jxrw_xzb_t xzb where xzb.jxrwid={alias}.id)");
      Criterion scopeCriterion = Restrictions.sqlRestriction(scopseRestriction.toString(), new Object[0],
          new Type[0]);
      criteria.add(scopeCriterion);
    } else {
      StringBuffer scopseRestriction = new StringBuffer(
          "(not exists(select xzb.jxrwid from jxrw_xzb_t xzb where xzb.jxrwid={alias}.id)"
              + " or exists(select xzb.jxrwid from jxrw_xzb_t xzb where xzb.jxrwid={alias}.id and xzb.bjid in(?"
              + StringUtils.repeat(",?", std.getAdminClassIds().size() - 1) + ")))");
      ...
      Criterion scopeCriterion = Restrictions.sqlRestriction(scopseRestriction.toString(), params.toArray(),
          (Type[]) types.toArray(typeArray));
      criteria.add(scopeCriterion);
    }
  }
  ...
  private void addScopeCriterion(SimpleStdInfo std, Criteria criteria) {
    ...
    List params = new ArrayList();
    StringBuffer scopseRestriction = new StringBuffer(
        "exists( select *  from XK_FW_T  where {alias}.id=jxrwid and ( ( (BJIDC is not null and ");
    if (null == std.getAdminClassIds() || std.getAdminClassIds().isEmpty()) scopseRestriction
        .append(" instr(BJIDC,(',,'))>0 ");
    else {
    ...
    scopseRestriction.append(" or ( BJIDC is null and BMIDC=',1,' and"
        + " (RXPCC is null or instr(RXPCC,(?))>0) and" + "(XSLBIDC is null or instr(XSLBIDC,(?))>0) ) ");

    scopseRestriction.append(" or ( BJIDC is null and (XSLBIDC is null or instr(XSLBIDC,(?))>0) "
        + "and (RXPCC is null or instr(RXPCC,(?))>0)" + "and (BMIDC is null or instr(BMIDC,(?))>0)"
        + "and (ZYIDC is null or instr(ZYIDC,(?))>0)" + "and (ZYFXIDC is null or instr(ZYFXIDC,(?))>0)");
    scopseRestriction.append(" ))))");
    ...
    if (null != time && null != time.getWeekId() && !zero.equals(time.getWeekId())
      ...
      Criterion timeCriterion = Restrictions
          .sqlRestriction(
              ...
              new Object[] { time.getWeekId(), time.getStartUnit(), time.getStartUnit() }, new Type[] {
   ...
   *      com.shufe.model.system.calendar.TeachCalendar)
   */
  public int countTeachTask(Serializable id, TeachTaskFilterCategory category, TeachCalendar calendar) {
   ...
   */
  public int updateTeachTaskByCategory(String attr, Object value, Long id, TeachTaskFilterCategory category,
    ...
    return utilDao.executeUpdateHql(category.getQueryString(), new Object[] { value, calendar.getId() });
  }
   ...
   */
  public int updateTeachTaskByCriteria(String attr, Object value, TeachTask task) throws Exception {
    if (attr.indexOf('.') != -1) return updateTeachTaskOneByOne(attr, value, task);
    else {
  ...
  private int updateTeachTaskOneByOne(String attr, Object value, TeachTask task) throws Exception {
    ...
    if (null != page) {
      ...
      for (int i = 0; i < setors.length; i++)
        if (setors[i].getName().equals(setMethodName)) setMethod = setors[i];
  ...
  /**
     * 
     */
  public int updateTeachTaskByCriteria(String attr, Object value, TeachTask task, Long[] stdTypeIds,
      Long[] departIds) {
    getSessionFactory().evict(TeachTask.class);
    Map newParamsMap = new HashMap();
    String updateSql = getUpdateQueryString(attr, value, task, stdTypeIds, departIds, newParamsMap);
   ...
   * @return
   */
  private String getUpdateQueryString(String attr, Object value, TeachTask task, Long[] stdTypeIds,
      Long[] departIds, Map newParamsMap) {
    ...
    if (null != stdTypeIds && 0 != stdTypeIds.length) {
      entityQuery.add(new Condition("task.teachClass.stdType.id in (:stdTypeIds) ", stdTypeIds));
    }
    if (null != departIds && 0 != departIds.length) {
      entityQuery.add(new Condition("task.arrangeInfo.teachDepart.id in (:departIds) ", departIds));
    }
   ...
   * @return
   */
  public Criteria getExampleCriteria(Session session, TeachTask task, String[] additionExcludedProperties,
    ...
    if (null != task) {
      ...
      if (!task.getArrangeInfo().getTeachers().isEmpty()) {
        ...
        if (StringUtils.isNotEmpty(teacher.getName())) {
          teacherCriterions.add(Restrictions.like("teacher.name", teacher.getName(), MatchMode.ANYWHERE));
        }
        if (teacher.checkId()) {
          teacherCriterions.add(Restrictions.eq("teacher.id", teacher.getId()));
        }
        if (!teacherCriterions.isEmpty()) {
          Criteria teacherCriteria = criteria.createCriteria("arrangeInfo.teachers", "teacher");
      ...
      if (null != task.getRequirement().getRoomConfigType()
        ...
        criteria.createAlias("requirement.roomConfigType", "configType").add(
            Restrictions.like("configType.name", task.getRequirement().getRoomConfigType().getName().trim(),
                MatchMode.ANYWHERE));
      }
      /*---------添加对开课院系名称的模糊查询--------------*/
      if (null != task.getArrangeInfo().getTeachDepart()
        ...
        criteria.createAlias("arrangeInfo.teachDepart", "teachDepart").add(
            Restrictions.like("teachDepart.name", task.getArrangeInfo().getTeachDepart().getName().trim(),
                MatchMode.ANYWHERE));
      }
      /*---------添加对课程性质名称模糊查询--------------*/
      if (null != task.getCourseType() && StringUtils.isNotEmpty(task.getCourseType().getName())) {
        criteria.createAlias("courseType", "courseType").add(
            Restrictions.like("courseType.name", task.getCourseType().getName().trim(), MatchMode.ANYWHERE));
      }
      /*--------------发布课程组-----------*/
      if (null != task.getTaskGroup() && StringUtils.isNotEmpty(task.getTaskGroup().getName())) {
        criteria.createAlias("taskGroup", "taskGroup").add(
            Restrictions.like("taskGroup.name", task.getTaskGroup().getName(), MatchMode.ANYWHERE));
      }
  ...
  public Collection getTeachTasksWithMultiClass(TeachTask task, String stdTypeIds, String departIds) {
    String hql = "from TeachTask as task" + " where size(task.teachClass.adminClasses)>1"
        ...
        + " and task.arrangeInfo.teachDepart.id in (:teachDepartIds) " + " and task.calendar.id=:calendarId"
        + " and task.requirement.isGuaPai =false";
    Query query = getSession().createQuery(hql);
  ...
  public int statStdCount(TeachCalendar calendar) {
    ...
    try {
      ...
      try {
        ...
        info("execproduct is failed" + "in save_electresult" + ExceptionUtils.getStackTrace(e));
  ...
  public int statStdCount(String taskIdSeq) {
    ...
    try {
      String state = "update jxrw_t rw set rw.bjrs="
          + "(select count(*) from jxbxs_T jxb where jxb.jxrwid=rw.id) where instr('," + taskIdSeq
          + ",',','||rw.id||',')>0";
