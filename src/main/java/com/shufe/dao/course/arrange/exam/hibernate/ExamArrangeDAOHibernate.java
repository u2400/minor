...
public class ExamArrangeDAOHibernate extends BasicHibernateDAO implements ExamArrangeDAO {
  ...
  public void removeExamArranges(List tasks, ExamType examType) {
    ...
    if (examType.isPO()) query0.setParameter("examType", examType);
    ...
    if (ValidEntityPredicate.INSTANCE.evaluate(examType)) query0.setParameter("examType", examType);
    query0.setParameterList("tasks", tasks);
    query0.executeUpdate();

    // 删除监考信息
    if (ValidEntityPredicate.INSTANCE.evaluate(examType)) {
      removeQuery = "delete from ExamMonitor where id in(" + "select id from ExamMonitor as monitor  "
          + "where monitor.examActivity.task in (:tasks)" + "and monitor.examActivity.examType=:examType)";
    } else {
      removeQuery = "delete from ExamMonitor where id in(" + "select id from ExamMonitor as monitor  "
    ...
    if (ValidEntityPredicate.INSTANCE.evaluate(examType)) {
      removeQuery = "delete from ExamActivity where id in("
          + "select id from ExamActivity as ae where  ae.task in (:tasks)" + "and ae.examType=:examType)";
    } else {
    ...
    if (examType.isPO()) query1.setParameter("examType", examType);
