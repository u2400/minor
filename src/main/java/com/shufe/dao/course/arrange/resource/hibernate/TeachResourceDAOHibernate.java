...
public class TeachResourceDAOHibernate extends BasicHibernateDAO implements TeachResourceDAO {
  ...
  public boolean isAdminClassesOccupied(TimeUnit time, Collection adminClasses) {
    if (adminClasses.isEmpty()) return false;
  ...
  public boolean isTeacherCollision(Long examActivityId, Long teacherId) {
    if (examActivityId == null) { return true; }

    Query query = null;
    try {
      ExamActivity activity = (ExamActivity) utilDao.load(ExamActivity.class, examActivityId);
      if (activity == null || activity.getId() == null) { return true; }
      ...
      query.setParameter("V_TEACHER_ID", (teacherId == null ? activity.getTeacher().getId() : teacherId));
      ...
      if (logger.isDebugEnabled()) {
        ...
        logger.debug("V_TEACHER_ID:\t" + (teacherId == null ? activity.getTeacher().getId() : teacherId));
      }
    } catch (Exception e) {
      e.printStackTrace();
      return false;
    ...
    if (query.uniqueResult() == null || ((Number) query.uniqueResult()).longValue() == 0) { return false; }
    return true;
  }

  /*------------空闲教学资源查询--------------------*/
  public Classroom getFreeRoomIn(Collection roomIds, TimeUnit[] times, Classroom room, Class activityClass) {
  ...
  public Collection getFreeRoomsIn(Collection roomIds, TimeUnit[] times, Classroom room, Class activityClass) {
    return getFreeRoomsCriteria(roomIds, null, times, room, activityClass).list();
  }

  public Collection getFreeRoomsOf(Long[] departIds, TimeUnit[] times, Classroom room, Class activityClass) {
    return getFreeRoomsCriteria(null, departIds, times, room, activityClass).list();
  }

  public Pagination getFreeRoomsOf(Long[] departIds, TimeUnit[] times, Classroom room, Class activityClass,
      int pageNo, int pageSize) {
    return dynaSearch(getFreeRoomsCriteria(null, departIds, times, room, activityClass), pageNo, pageSize);
  ...
  public Collection getFreeTeacherIdsIn(Collection teacherIds, TimeUnit[] times, Teacher teacher,
    ...
    if (null != teacherIds && !teacherIds.isEmpty()) criteria.add(Restrictions.in("id", teacherIds));
    addTimesRestrictionsForTeacher(criteria, times, activityClass);
  ...
  public Collection getFreeClassIdsIn(Collection adminClassIds, TimeUnit[] times) {
    if (adminClassIds.isEmpty()) return Collections.EMPTY_LIST;
  ...
  /*------------获得教学资源的教学活动--------------------*/
  public List getAdminClassActivities(Serializable adminClassId, TimeUnit time, Class activityClass) {
    Criteria criteria = getSession().createCriteria(activityClass);
    criteria.createCriteria("task", "task").createCriteria("task.teachClass.adminClasses", "adminClass");
  ...
  public List getStdActivities(Long stdId, TimeUnit time, Class activityClass, TeachCalendar calendar) {
    Criteria criteria = getSession().createCriteria(activityClass);
    criteria.createCriteria("task", "task").createCriteria("task.teachClass.courseTakes", "courseTake");
  ...
  public List getResourceOccupyInfos(Serializable resourceId, Long validWeeksNum, Integer year, Query query) {
  ...
  public List getClassrooms(Long[] roomIds) {
    return getSession().getNamedQuery("getClassroomsByIdsSorted").setParameterList("roomIds", roomIds).list();
  }

  public List getTeachers(Collection teacherIds) {
    return getSession().getNamedQuery("getTeachersById").setParameterList("teacherIds", teacherIds).list();
  }

  public List getFreeRoomsIn(Collection roomIds, TimeUnit time) {
    Criteria criteria = getSession().createCriteria(Classroom.class);
    if (null != roomIds && !roomIds.isEmpty()) criteria.add(Restrictions.in("id", roomIds));

    addTimesRestrictionsForRoom(criteria, new TimeUnit[] { time }, CourseActivity.class);
    return criteria.list();
  ...
  private Criteria getFreeRoomsCriteria(Collection roomIds, Long[] departIds, TimeUnit[] times,
    ...
    if (null != departIds) {
      ...
      criteria.add(Restrictions.sqlRestriction(
          " {alias}.id in (select jsid from jcxx_bm_js_t where instr(?,','||bmid||',')>0)", departIdS,
          Hibernate.STRING));
    }
  ...
  private void addTimesRestrictionsFor(Criteria criteria, TimeUnit[] times, Class activityClass,
    ...
    if (null == times || times.length == 0) { return; }
    ...
    for (int i = 0; i < times.length; i++) {
      ...
      if (null == time.getEndTime()) {
        CourseUnit endCourseUnit = (CourseUnit) timeSettingService.getDefaultSetting().getCourseUnit(
            time.getEndUnit().intValue());
        values[i * 5 + 3] = endCourseUnit.getFinishTime();
      } else {
        values[i * 5 + 3] = time.getEndTime();
      }
      if (null == time.getStartTime()) {
        CourseUnit startCourseUnit = (CourseUnit) timeSettingService.getDefaultSetting().getCourseUnit(
    ...
    if (StringUtils.equals(property, "room")) {
      ...
      criteria.add(Restrictions.or(
          ...
          Restrictions.or(Restrictions.eq("isCheckActivity", null),
              Restrictions.eq("isCheckActivity", Boolean.FALSE))));
    } else {
      criteria.add(Subqueries.propertyNotIn("id", dc));
    }
  }

  private void addTimesRestrictionsForRoom(Criteria criteria, TimeUnit[] times, Class activityClass) {
    addTimesRestrictionsFor(criteria, times, activityClass, "room", "jsid");
  }

  private void addTimesRestrictionsForTeacher(Criteria criteria, TimeUnit[] times, Class activityClass) {
  ...
  public Collection getExamRoomUtilizations(TeachCalendar calendar, ExamType examType, DataRealmLimit limit,
      Float ratio) {
    ...
    params.put("stdTypeIds", SeqStringUtil.transformToLong(limit.getDataRealm().getStudentTypeIdSeq()));
    params.put("departIds", SeqStringUtil.transformToLong(limit.getDataRealm().getDepartmentIdSeq()));
