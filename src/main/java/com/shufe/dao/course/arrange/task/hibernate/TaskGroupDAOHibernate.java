 ...
 */
public class TaskGroupDAOHibernate extends BasicHibernateDAO implements TaskGroupDAO {
  ...
  public TaskGroup getTaskGroup(Serializable id) {
    TaskGroup group = (TaskGroup) getHibernateTemplate().get(TaskGroup.class, id);
    if (null == group) group = (TaskGroup) load(TaskGroup.class, id);
    return group;
  }
  ...
  public Integer getTaskCountOfGroup(Collection groups) {
    ...
    return new Integer(((Number) find("getDirectTaskCountOfGroups", params).iterator().next()).intValue());
  }

  /**
   * I want to user Criteria performance a dynamic query. but hibernate doen't support distint on
   * criteria Now!! so I assemble hql manually.
   */
  ...
  public Pagination getTaskGroups(TaskGroup group, Long[] stdTypeIds, Long[] teachDepartIds,
    ...
    String hql = "select distinct taskGroup from TaskGroup"
        + " as taskGroup join taskGroup.directTasks  as task " + " where task.calendar.id =:calendarId ";

    Map params = new HashMap();
    if (StringUtils.isNotBlank(courseName)) {
      hql += " and task.course.name like '%' || :courseName || '%'";
    ...
    if (null != group) {
      ...
      if (null != group.getPriority() && group.getPriority().intValue() != 0) {
