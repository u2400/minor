...
public class CourseTakeDAOHibernate extends BasicHibernateDAO implements CourseTakeDAO {
  ...
  public List getCourseTakeIdsNeedEvaluate(Long studentId, Collection teachCalendars) {
    if (teachCalendars == null || teachCalendars.isEmpty()) { return Collections.EMPTY_LIST; }
    ...
    query.add(new Condition("courseTake.isCourseEvaluated is null or courseTake.isCourseEvaluated = false"));
  ...
  public List getCourseTakes(Long stdId, TimeUnit timeUnit, TeachCalendar calendar) {
    ...
    if (null != timeUnit) {
      if (null != timeUnit.getWeekId() && timeUnit.getWeekId().intValue() != 0) conditions.add(Condition.eq(
          "activity.time.weekId", timeUnit.getWeekId()));
      if (null != timeUnit.getYear()) conditions.add(Condition.eq("activity.time.year", timeUnit.getYear()));
      if (null != timeUnit.getEndUnit() && timeUnit.getEndUnit().intValue() != 0) conditions.add(Condition
          .le("activity.time.startUnit", timeUnit.getEndUnit()));
      if (null != timeUnit.getStartUnit() && timeUnit.getStartUnit().intValue() != 0) conditions
          .add(Condition.ge("activity.time.endUnit", timeUnit.getStartUnit()));
      if (null != timeUnit.getValidWeeksNum() && timeUnit.getValidWeeksNum().intValue() != 0) conditions
          .add(new Condition("bitand(activity.time.validWeeksNum, :validWeeksNum)>0)", timeUnit
              .getValidWeeksNum()));
    }
    if (conditions.isEmpty()) entityQuery.add(new Condition("task.calendar=:calendar", calendar));
    else {
  ...
  public Map stdCourseValidator(String taskId, String stdId) {
    ...
    if (null == grades || grades.isEmpty()) {
      ...
      gradeTypeQuery.add(new Condition("gradeType.examType is not null and gradeType.state=true"));
      ...
      for (Iterator it = grade.getExamGrades().iterator(); it.hasNext();) {
        ...
        data.put("gradeType" + examGrade.getGradeType().getId() + "" + gradeMarkStyleId, score);
        data.put("examGradeId" + examGrade.getGradeType().getId() + "" + gradeMarkStyleId, examGrade.getId());
      }
      if (hasGradeCount == 0) {
