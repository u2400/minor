...
public class ExamTakeDAOHibernate extends BasicHibernateDAO implements ExamTakeDAO {
  ...
  public List statTakeCountWithTurn(TeachCalendar calendar, List examTypes) {
    ...
    if (null != examTypeId) {
      ...
      queryStr += " and instr('" + EntityUtils.extractIdSeq(examTypes) + "',','||kslxid||',')>0 ";
    }
  ...
  public List statTakeCountInCourse(TeachCalendar calendar, List examTypes) {
    ...
    if (null != examTypeId) {
      ...
      query.add(new Condition("instr(:examTypeIds,','||take.examType.id||',')>0", EntityUtils
          .extractIdSeq(examTypes)));
    }
    ...
    for (Iterator iter = dd.iterator(); iter.hasNext();) {
      ...
      takesOfCourse.add(new Object[] { get(Course.class, (Long) element[0]), element[1] });
    }
