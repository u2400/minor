...
public class BookRequirementDAOHibernate extends BasicHibernateDAO implements BookRequirementDAO {
  ...
  public Collection getBookRequirements(BookRequirement requirement, PageLimit limit) {
    ...
    if (requirement != null) {
      criteria.add(Restrictions.like("course.name", requirement.getTask().getCourse().getName(),
          MatchMode.ANYWHERE));
      criteria.add(Restrictions.like("task.teachClass.name", requirement.getTask().getTeachClass().getName(),
          MatchMode.ANYWHERE));
      criteria.add(Restrictions
          .like("textbook.name", requirement.getTextbook().getName(), MatchMode.ANYWHERE));
    }
    if (null != limit) return dynaSearch(criteria, limit);
