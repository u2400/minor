...
public class BaseInfoSearchHelper extends SearchHelper {
  ...
  public EntityQuery buildAdminClassQuery(HttpServletRequest request, Boolean isDataRealm) {
    ...
    QueryRequestSupport.populateConditions(request, query, "adminClass.stdType.id,calendar.studentType.id");
    ...
    if (Boolean.TRUE.equals(isDataRealm)) {
      ...
      if (null != resource && !resource.getPatterns().isEmpty()) {
        DataRealmUtils.addDataRealms(query, new String[] { "adminClass.stdType.id",
            "adminClass.department.id" }, dataRealmHelper.getDataRealmsWith(stdTypeId, request));
      } else {
        if (ValidEntityKeyPredicate.INSTANCE.evaluate(stdTypeId)) {
          dataRealmHelper.addStdTypeTreeDataRealm(query, stdTypeId, "adminClass.stdType.id", request);
        }
      }
    }

    query.join("left", "adminClass.speciality", "speciality").join("left", "adminClass.aspect", "aspect");
    // 查询一专业/二专业班级
    ...
    if (StringUtils.isNotEmpty(orderBy)) query.addOrder(OrderUtils.parser(orderBy));
    return query;
  }
  ...
  public EntityQuery buildTeacherQuery(HttpServletRequest request) {
    ...
    Boolean hasVideo = RequestUtils.getBoolean(request, "hasVideo");
    if (null != hasVideo) {
      if (hasVideo.booleanValue()) {
        entityQuery.add(new Condition("teacher.videoName is not null"));
      } else {
        entityQuery.add(new Condition("teacher.videoName is null"));
      }
    }
  ...
  public EntityQuery buildClassroomQuery(HttpServletRequest request) {
    ...
    if (StringUtils.isNotEmpty(departIds)) {
      entityQuery.add(new Condition("exists (from Department depart join depart.classrooms  room where"
          + " room.id=classroom.id and depart.id in(:departIds))", SeqStringUtil.transformToLong(departIds)));
    }
    // 考试人数
    Integer examCountFrom = RequestUtils.getInteger(request, "examCountFrom");
    if (null != examCountFrom) {
      entityQuery.add(new Condition("classroom.capacityOfExam >= (:examCountFrom)", examCountFrom));
    }
    Integer examCountTo = RequestUtils.getInteger(request, "examCountTo");
    if (null != examCountTo) {
      entityQuery.add(new Condition("classroom.capacityOfExam <= (:examCountTo)", examCountTo));
    }
    // 听课人数
    Integer courseCountFrom = RequestUtils.getInteger(request, "courseCountFrom");
    if (null != courseCountFrom) {
      entityQuery.add(new Condition("classroom.capacityOfCourse >= (:courseCountFrom)", courseCountFrom));
    }
    Integer courseCountTo = RequestUtils.getInteger(request, "courseCountTo");
    if (null != courseCountTo) {
      entityQuery.add(new Condition("classroom.capacityOfCourse <= (:courseCountTo)", courseCountTo));
    }
    // 正真容量
  ...
  public EntityQuery buildCourseQuery(HttpServletRequest request) {
    ...
    QueryRequestSupport.populateConditions(request, entityQuery);
    /* 时间模糊查询 */
    String courseCreateAt = request.getParameter("courseCreateAt");
    if (StringUtils.isNotEmpty(courseCreateAt)) {
      entityQuery.add(new Condition("to_char(course.createAt,'yyyy-MM-dd') like(:courseCreateAt)", "%"
          + courseCreateAt + "%"));
    }
    String courseModifyAt = request.getParameter("courseModifyAt");
    if (StringUtils.isNotEmpty(courseModifyAt)) {
      entityQuery.add(new Condition("to_char(course.modifyAt,'yyyy-MM-dd') like(:courseModifyAt)", "%"
          + courseModifyAt + "%"));
    }
    entityQuery.setLimit(getPageLimit(request));
    entityQuery.addOrder(OrderUtils.parser(request.getParameter("orderBy")));
