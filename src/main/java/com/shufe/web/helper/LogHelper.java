...
public class LogHelper {
  ...
  private void checkParams(HttpServletRequest request, String content) throws Exception {
    if (request == null || StringUtils.isEmpty(content)) { throw new Exception("==> the params of ["
        + this.getClass().getName() + ".info] of method is exception."); }
  }
   ...
   * @throws Exception
   */
  public SystemLog buildInfo(HttpServletRequest request, String content) throws Exception {
  ...
  public void info(HttpServletRequest request, String content) throws Exception {
    // 保存
    utilService.saveOrUpdate(buildInfo(request, content));
  }
