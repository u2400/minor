...
public class StdSearchHelper extends SearchHelper {
  ...
  public EntityQuery buildStdQuery(HttpServletRequest request, String extraStdTypeAttr) {
    ...
    QueryRequestSupport.populateConditions(request, query, "std.type.id");
    ...
    if (MajorType.FIRST.equals(majorTypeId)) {
      ...
      if (null != specialityId) {
        ...
        if (null != departId) query.add(new Condition("std.department.id=" + departId));
      }
      // 查询一专业就读的学生
      if (null != stdActive) {
      ...
      if (null != graduateAuditStatus) {
        query.add(new Condition("std.graduateAuditStatus=:graduateAuditStatus", graduateAuditStatus));
      }
    } else {
      query.add(new Condition("std.secondMajor.majorType.id = (:majorTypeId)", MajorType.SECOND));
      if (null != aspectId) {
        ...
        if (null != specialityId) {
          ...
          if (null != departId) query.add(new Condition("std.secondMajor.department.id=" + departId));
        }
      }
      // 查询双专业就读的学生
      if (Boolean.TRUE.equals(stdActive)) {
        ...
        query.add(new Condition("std.isSecondMajorStudy is null or std.isSecondMajorStudy = false"));
      }
      // 查询二专业毕业审核情况
      if (null != graduateAuditStatus) {
        query.add(new Condition("std.secondGraduateAuditStatus=:secondGraduateAuditStatus",
            graduateAuditStatus));
      }
    }
    ...
    if (orders.isEmpty()) {
      ...
      orders.add(new Order("std.enrollYear desc"));
    }
  ...
  public static void addSpecialityConditon(HttpServletRequest request, EntityQuery entityQuery, String stdAttr) {
    Long majorTypeId = RequestUtils.getLong(request, "majorTypeId");
    Long departId = RequestUtils.getLong(request, "department.id");
    Long specialityId = RequestUtils.getLong(request, "speciality.id");
    Long aspectId = RequestUtils.getLong(request, "specialityAspect.id");
    if (MajorType.FIRST.equals(majorTypeId)) {
      ...
      if (null != specialityId) {
        ...
        if (null != departId) entityQuery.add(new Condition(stdAttr + ".department.id=" + departId));
      }
    } else {
      ...
      if (null != aspectId) {
        ...
        if (null != specialityId) {
          ...
          if (null != departId) entityQuery.add(new Condition(stdAttr + ".secondMajor.department.id="
              + departId));
        }
      }
    }
