...
public class TeachCalendarHelper {
  ...
  public TeachCalendarHelper(TeachCalendarService teachCalendarService, UtilService utilService) {
  ...
  public void setCalendar(HttpServletRequest request, StudentType stdType) {
    ...
    TeachCalendar example = (TeachCalendar) RequestUtil.populate(request, TeachCalendar.class,
        Constants.CALENDAR);
    if (null != example.getStudentType() && StringUtils.isNotEmpty(example.getYear())
        && StringUtils.isNotEmpty(example.getTerm())) {
      calendar = teachCalendarService.getTeachCalendar(example.getStudentType(), example.getYear(),
          example.getTerm());
    }
    ...
    if (null == calendar) {
      stdTypeId = CookieUtils.getCookieValue(request, "calendar.studentType.id");
      try {
        if (null != stdTypeList) {
          ...
          if (inAuth) {
            ...
            if (StringUtils.isNotEmpty(stdTypeId) && StringUtils.isNotEmpty(year)
              ...
              calendar = teachCalendarService.getTeachCalendar(Long.valueOf(stdTypeId), year, term);
              // 重置学生类别
              stdType = (StudentType) utilService.get(StudentType.class, Long.valueOf(stdTypeId));
            }
          }
    ...
    if (null == calendar) { throw new CalendarNotExistException(stdType.getName() + "教学日历不存在！"); }
    request.setAttribute(Constants.CALENDAR, calendar);
  }
