...
public class RestrictionHelper {
  ...
  public List getDataRealmsWith(Long stdTypeId, HttpServletRequest request) {
    ...
    if (null == stdTypeId) {
      ...
      for (Iterator iterator = dataRealms.iterator(); iterator.hasNext();) {
        ...
        newRealm.setStudentTypeIdSeq(SeqStringUtil.intersectSeq(stdTypeIds, newRealm.getStudentTypeIdSeq()));
        if (StringUtils.isNotEmpty(newRealm.getStudentTypeIdSeq())) {
          newRealms.add(newRealm);
        }
      }
      if (newRealms.isEmpty()) { throw new StdTypeAuthInsufficientException(getUser(request.getSession()),
   ...
   * @param resourceName
   */
  public void addStdTypeTreeDataRealm(EntityQuery query, String stdTypeAttrPath, HttpServletRequest request,
      String resourceName) {
  ...
  public void addStdTypeTreeDataRealm(EntityQuery query, Long stdTypeId, String stdTypeAttrPath,
    ...
    if (null != stdTypeId) {
      ...
      if (StringUtils.isNotEmpty(stdTypeIds)) {
        DataRealmUtils.addDataRealm(query, new String[] { stdTypeAttrPath, null }, new DataRealm(stdTypeIds,
            null));
      }
    }
  }
  ...
  public List getDataRealms(HttpServletRequest request, AuthorityService authorityService) {
    ...
    if (null == realms || realms.isEmpty()) {
      ...
      if (resource.getPatterns().isEmpty()) {
        ...
        List restrictions = restrictionService.getRestrictions(getUser(request.getSession()), resource);
    ...
    // 没有权限就报错
    if (realms.isEmpty()) { throw new DepartAuthInsufficientException(getUser(request.getSession()),
        getResourceName(request)); }
    return realms;
  }
    ...
    if (null == request.getSession().getAttribute(Authentication.USERID)) {
      ...
      try {
        ...
        if (StringUtils.isNotEmpty(realm.getDepartmentIdSeq())) {
          ...
          throw new DepartAuthInsufficientException(getUser(request.getSession()), getResourceName(request));
        }
      } catch (Exception e) {
        e.printStackTrace();
        throw new DepartAuthInsufficientException(getUser(request.getSession()), getResourceName(request));
    ...
    if (null == request.getSession().getAttribute(Authentication.USERID)) {
      ...
      try {
        ...
        if (StringUtils.isNotEmpty(realm.getStudentTypeIdSeq())) {
          ...
          throw new StdTypeAuthInsufficientException(getUser(request.getSession()), getResourceName(request));
        }
      } catch (Exception e) {
        throw new StdTypeAuthInsufficientException(getUser(request.getSession()), getResourceName(request));
      }
