...
public class ExamArrangeAction extends ExamAction {
   ...
   * @throws Exception
   */
  public ActionForward arrangeList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
  ...
  public Map getExamTakeCountOfTasks(HttpServletRequest request, Collection teachTasks) {
    ...
    for (Iterator iterator = teachTasks.iterator(); iterator.hasNext();) {
      ...
      if (ExamType.FINAL.equals(examTypeId)) {
        ...
        query.add(new Condition("examTake.examType.id in (:examTypeId)", new Object[] { ExamType.DELAY_AGAIN,
            ExamType.AGAIN, ExamType.DELAY }));
      }
   ...
   * @throws Exception
   */
  public ActionForward arrangeSetting(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    for (Iterator iterator = rooms.iterator(); iterator.hasNext();) {
      ...
      if (null == room.getCapacityOfExam() || room.getCapacityOfExam().intValue() == 0) iterator.remove();
    }
   ...
   * @throws Exception
   */
  public ActionForward arrange(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ExamParams examParams = new ExamParams();
    RequestUtil.populate(request, examParams, "examParams");
    examParams.setFromCalendar(teachCalendarService.getTeachCalendar(getLong(request, "fromCalendar.id")));
    examParams.setToCalendar(teachCalendarService.getTeachCalendar(
    ...
    if (!StringUtils.isBlank(roomIds)) {
      examParams.setRooms(utilService.load(Classroom.class, "id", SeqStringUtil.transformToLong(roomIds)));
    }
    examParams.setLastWeek(examParams.getFromWeek() + getInteger(request, "weeks").intValue() - 1);
    ...
    for (int i = 0; i < weekAndTurns.length; i++) {
      ...
      examTimes.add(new ExamTime(null, examParams.getFromWeek() + NumberUtils.toInt(weekAndTurn[0]), WeekInfo
          .get(NumberUtils.toInt(weekAndTurn[1])), turn));
    }
   ...
   * @throws Exception
   */
  public ActionForward removeArrange(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String taskIds = request.getParameter("taskIds");

    Long examTypeId = getLong(request, "examType.id");
    if (StringUtils.isEmpty(taskIds) || null == examTypeId) return forwardError(mapping, request,
   ...
   * @throws Exception
   */
  public ActionForward examReport(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String taskIds = request.getParameter("taskIds");
    List tasks = null;
    ...
    if (request.getParameter("forRoom") != null) forRoom = true;
  ...
  public ActionForward allocateRoomSetting(ActionMapping mapping, ActionForm form,
    ...
    Pagination rooms = teachResourceService.getFreeRoomsOf(
        SeqStringUtil.transformToLong(getDepartmentIdSeq(request)), new TimeUnit[] { time }, room,
        ExamActivity.class, limit.getPageNo(), limit.getPageSize());
   ...
   * @throws Exception
   */
  public ActionForward examActivityReport(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    TeachCalendar calendar = teachCalendarService.getTeachCalendar(getLong(request,
        "examActivity.calendar.id"));
    if (null == calendar) { return forward(mapping, request, "error.parameters.needed", "error"); }
    ...
    for (Iterator it1 = results1.iterator(); it1.hasNext();) {
      ...
      timeMap.put(sdf.format(activity.getStartDay()) + "-" + sdf.format(activity.getEndDay()), activity);
    }
   ...
   * @throws Exception
   */
  public ActionForward allocateRoom(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    List classrooms = utilService.load(Classroom.class, "id", SeqStringUtil.transformToLong(classIds));
    ...
    boolean success = examArrangeService.allocateRooms(tasks, examType, classrooms, canCombineTask,
        sameDistrictWithCourse);
   ...
   * @throws Exception
   */
  public ActionForward seatReport(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String taskIds = request.getParameter("taskIds");
    Long examTypeId = getLong(request, "examType.id");
    ...
    if (StringUtils.isNotEmpty(taskIds)) {
      ...
      entityQuery.add(new Condition("activity.id in (:examActivityIds)", SeqStringUtil
  ...
  public ActionForward seatReportByClassroom(ActionMapping mapping, ActionForm form,
    ...
    if (StringUtils.isNotEmpty(taskIds)) {
      ...
      for (int i = 0; i < examActivityId.length; i++) {
        ...
        List examActivities = utilService
            .searchHQLQuery("from ExamActivity activity where activity.calendar.id=" + calendarId
                + " and activity.examType.id=" + examTypeId + " and activity.room.id=" + jsid
                + " and activity.time.year=" + year + " and activity.time.validWeeksNum=" + validWeeksNum
                + " and activity.time.weekId=" + weekId + " and activity.time.startUnit=" + startUnit
                + " and activity.time.endUnit=" + endUnit);
   ...
   * @throws Exception
   */
  public ActionForward mergeSetting(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String taskIds = request.getParameter("taskIds");
    List tasks = utilService.load(TeachTask.class, "id", SeqStringUtil.transformToLong(taskIds));
    addCollection(request, "taskList", tasks);

    ExamType examType = (ExamType) utilService.get(ExamType.class, getLong(request, "examType.id"));
    request.setAttribute("examType", examType);
    // 判断是否在统一时间考试
   ...
   * @throws Exception
   */
  public ActionForward mergeRoom(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    for (Iterator iterator = activities.iterator(); iterator.hasNext();) {
      ...
      if (null != activity.getRoom()) {
        int reminder = activity.getRoom().getCapacityOfExam().intValue() - activity.getExamTakes().size();
        if (reminder > 0) {
    ...
    if (Boolean.TRUE.equals(byClass)) {
      ...
      for (Iterator iter = tasks.iterator(); iter.hasNext();) {
        ...
        for (Iterator iterator = adminClassSet.iterator(); iterator.hasNext();) {
          ...
          if (null == courseAndClassMap.get(adminClass.getId() + "," + task.getCourse().getId())) {
            courseAndClassMap.put(adminClass.getId() + "," + task.getCourse().getId(), Boolean.TRUE);
  ...
  protected PropertyExtractor getPropertyExtractor(HttpServletRequest request) {
    Boolean byClass = getBoolean(request, "byClass");
    TeachTaskPropertyExtractor exporter = new TeachTaskPropertyExtractor(getLocale(request),
        getResources(request));
    if (Boolean.TRUE.equals(byClass)) {
      exporter = new TaskPerClassPropertyExtractor(getLocale(request), getResources(request));
    }
    Long examTypeId = getLong(request, "examType.id");
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    for (int i = 0; i < ((null == noTakedStdCount) ? 0 : noTakedStdCount.intValue()); i++) {
      ...
      ExamActivity examActivity = (ExamActivity) utilService.get(ExamActivity.class,
          getLong(request, "examActivityId" + i));

      if (null == examActivity) continue;
  ...
  public ActionForward freeRoomList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    ExamActivity examActivity = (ExamActivity) utilService.get(ExamActivity.class, examActivityId);
    Classroom room = (Classroom) populate(request, Classroom.class, "classroom");
    PageLimit limit = getPageLimit(request);

    Pagination rooms = teachResourceService.getFreeRoomsOf(
        SeqStringUtil.transformToLong(getDepartmentIdSeq(request)),
        new TimeUnit[] { examActivity.getTime() }, room, ExamActivity.class, limit.getPageNo(),
        limit.getPageSize());
    addOldPage(request, "rooms", rooms);
   ...
   * @throws Exception
   */
  public ActionForward addRoom(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long examActivityId = getLong(request, "examActivity.id");
    ExamActivity examActivity = (ExamActivity) utilService.get(ExamActivity.class, examActivityId);
  ...
  public ActionForward printPaperLabelForTask(ActionMapping mapping, ActionForm form,
    ...
    if (null == extraCount) extraCount = new Integer(0);
   ...
   * @throws Exception
   */
  public ActionForward roomUtilization(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long examTypeId = getLong(request, "examType.id");
    Long calendarId = getLong(request, "calendar.id");
    TeachCalendar calendar = teachCalendarService.getTeachCalendar(calendarId);
    ...
    addCollection(request, "utilizations",
        teachResourceService.getRoomUtilizationsOfExam(calendar, examType, limit, ratio));
    return forward(request);
  }
