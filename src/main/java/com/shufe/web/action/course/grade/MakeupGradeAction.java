...
import com.shufe.service.course.grade.CourseGradePropertyExtractor;
...
public class MakeupGradeAction extends CalendarRestrictionSupportAction {
  ...
  public ActionForward index(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    addSingleParameter(request, "END", GradeType.END);
   ...
   * @throws Exception
   */
  public ActionForward gradeAllInfo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    addCollection(request, "courseGrades", utilService.search(buildGradeAllInfoQuery(request)));
    addSingleParameter(
        request,
        "calendar",
        teachCalendarService.getTeachCalendar(getLong(request, "courseGrade.calendar.studentType.id"),
            get(request, "courseGrade.calendar.year"), get(request, "courseGrade.calendar.term")));
    addSingleParameter(request, "DELAY", baseCodeService.getCode(GradeType.class, GradeType.DELAY));
    addSingleParameter(request, "MAKEUP", baseCodeService.getCode(GradeType.class, GradeType.MAKEUP));
    addSingleParameter(request, "FINAL", baseCodeService.getCode(GradeType.class, GradeType.FINAL));
   ...
   * @return
   */
  protected EntityQuery buildGradeAllInfoQuery(HttpServletRequest request) {
    EntityQuery query = new EntityQuery(CourseGrade.class, "courseGrade");
    populateConditions(request, query);
    query.join("courseGrade.examGrades", "examGrade");
    query.add(new Condition("examGrade.gradeType.id in (:gradeTypeId)", new Long[] { GradeType.DELAY,
        GradeType.MAKEUP, GradeType.FINAL, GradeType.GA }));
  ...
  public ActionForward courseList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    query.add(new Condition(" examTake.calendar.id=:calendarId", getLong(request, "calendar.id")));
   ...
   * @throws Exception
   */
  public ActionForward getExamTakeState(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
   ...
   * @throws Exception
   */
  public ActionForward gradeTable(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
   ...
   * @throws Exception
   */
  public ActionForward batchAddGrade(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    getExamTakeAndMakeupExam(request);
    addSingleParameter(request, "MAKEUP", baseCodeService.getCode(ExamType.class, ExamType.AGAIN));
    return forward(request);
  }
  ...
  protected void getExamTakeAndMakeupExam(HttpServletRequest request) {
    ...
    addSingleParameter(request, "calendar", teachCalendarService.getTeachCalendar(calendarId));
    String params[] = StringUtils.split(courseIds, "@");
    addSingleParameter(request, "course",
        utilService.load(com.shufe.model.system.baseinfo.Course.class, Long.valueOf(params[0].toString())));
    addSingleParameter(request, "teachDepart",
        utilService.load(Department.class, Long.valueOf(params[1].toString())));
  }
   ...
   * @throws Exception
   */
  public ActionForward publishGradeBack(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String courseIds = get(request, "courseIds");
    String courseId[] = StringUtils.split(courseIds, ",");
    Long calendarId = getLong(request, "calendar.id");
    ...
    for (int i = 0; i < courseId.length; i++) {
      ...
      for (Iterator iter = examTakeList.iterator(); iter.hasNext();) {
        ...
        if (null == examGrade) { return forwardError(mapping, request, new String[] { "exam.apply.f", "" }); }
  ...
  public ActionForward batchSaveCourseGrade(ActionMapping mapping, ActionForm form,
    ...
    for (Iterator iter = activities.iterator(); iter.hasNext();) {
      ExamTake examTake = (ExamTake) iter.next();
      Float score = getFloat(request, "score_" + examTake.getId().toString());
      CourseGrade grade = getCourseGrade(examTake.getStd(), examTake.getTask().getCalendar(), examTake
          .getTask().getCourse());
      GradeType gradeType = null;
      if (examTake.getExamType().getId().equals(ExamType.DELAY)) {
        gradeType = new GradeType(GradeType.DELAY);
      ...
      if (null == score) {
        ...
        if (null != grade) {
          ...
          grade
              ...
              .calcScore()
              .updatePass()
              .calcGP(gradePointRuleService.getGradePointRule(grade.getStd().getType(), grade.getMarkStyle()));
        }
        grades.add(grade);
      }
    }
  ...
  private CourseGrade getCourseGrade(Student std, TeachCalendar calendar, Course course) {
    ...
    query.add(new Condition("cg.std =:std and cg.calendar=:calendar and cg.course=:course", std, calendar,
        course));
