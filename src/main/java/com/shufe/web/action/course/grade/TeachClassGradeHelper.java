...
public class TeachClassGradeHelper {
   ...
   * @throws Exception
   */
  public void statTask(String taskIdSeq, Long[] gradeTypeIds, HttpServletRequest request) throws Exception {
    ...
    request.setAttribute("courseStats",
        gradeService.statTask(taskIdSeq, segStat.getScoreSegments(), gradeTypes, null));
  }
   ...
   * @throws Exception
   */
  public void statCourse(List courses, TeachCalendar calendar, Long[] gradeTypeIds, HttpServletRequest request)
      throws Exception {
    List gradeTypes = new ArrayList();
    for (int i = 0; i < gradeTypeIds.length; i++) {
      gradeTypes.add(baseCodeService.getCode(GradeType.class, gradeTypeIds[i]));
    ...
    request.setAttribute("segStat", segStat);
    request.setAttribute("courseStats",
        gradeService.statCourse(courses, segStat.getScoreSegments(), gradeTypes, calendar));
  }
   ...
   * @throws Exception
   */
  public void report(List tasks, Long[] gradeTypeIds, HttpServletRequest request) throws Exception {
  ...
  public void info(TeachTask task, HttpServletRequest request) throws Exception {
    ...
    if (!orders.isEmpty()) {
      ...
      Collections.sort(grades, new CourseGradeComparator(order.getProperty(),
          order.getDirection() == Order.ASC, gradeTypes));
    }
