...
public class StdExamTableAction extends CalendarSupportAction {
   ...
   * @throws Exception
   */
  public ActionForward examTable(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long examTypeId = getLong(request, "examType.id");
    if (null == examTypeId) return forwardError(mapping, request, "error.parameters.needed");
    ...
    entityQuery.add(new Condition("take.student=:std", getStudentFromSession(request.getSession())));
  ...
  public ActionForward editApply(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isBlank(takeId)) { return forwardError(mapping, request, "error.parameter.needed"); }
    Long examTypeId = getLong(request, "examType.id");
    Long calendarId = getLong(request, "calendar.id");
    if (!ValidEntityKeyPredicate.INSTANCE.evaluate(examTypeId)
        || !ValidEntityKeyPredicate.INSTANCE.evaluate(calendarId)) { return forwardError(mapping, request,
   ...
   * @throws Exception
   */
  public ActionForward submitApply(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String takeId = request.getParameter("examTake.id");
    ...
    if (Boolean.TRUE.equals(print)) {
      return redirect(request, new Action(StdExamTableAction.class, "printDelayApply"), "info.set.success",
          "&calendar.id=" + request.getParameter("calendar.id") + "&examTakeIds=" + take.getId());
    } else {
      return redirect(request, new Action(StdExamTableAction.class, "examTable"), "info.set.success",
          new String[] { "calendar", "examType" });
    }
  }
   ...
   * @throws Exception
   */
  public ActionForward printDelayApply(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String takeIds = request.getParameter("examTakeIds");
    if (StringUtils.isBlank(takeIds)) { return forwardError(mapping, request, "error.parameters.needed"); }
    ...
    for (int i = 0; i < examTakeIds.length; i++) {
      ...
      if (take.getStd().equals(std) && take.getExamStatus().getId().equals(ExamStatus.APPLY_DELAY)) {
        examTakes.add(take);
      }
    }
    request.setAttribute("calendar", utilService.get(TeachCalendar.class, getLong(request, "calendar.id")));
    request.setAttribute("std", std);
  ...
  public ActionForward cancelApply(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String takeIds = request.getParameter("examTakeIds");
    if (StringUtils.isBlank(takeIds)) { return forwardError(mapping, request, "error.parameter.needed"); }
    Long[] examTakeIds = SeqStringUtil.transformToLong(takeIds);
    Student std = getStudentFromSession(request.getSession());
    for (int i = 0; i < examTakeIds.length; i++) {
      ExamTake take = (ExamTake) utilService.get(ExamTake.class, examTakeIds[i]);
      if (take.getStd().equals(std) && !take.getExamStatus().getId().equals(ExamStatus.NORMAL)) {
        take.setRemark("");
