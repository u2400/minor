...
public class SuggestAction extends DispatchBasicAction {
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (!suggest.getTime().isValid()) { throw new Exception("availTime.not.corrected"); }
    ...
    if (StringUtils.isNotEmpty(roomIds)) {
      suggest.getRooms().addAll(
          classroomService.getClassrooms(Arrays.asList(SeqStringUtil.transformToLong(roomIds))));
    }
    ...
    // 如果是不是弹出窗口,则返回前一个页面
    if (StringUtils.isNotEmpty(get(request, "inIframe"))) { return redirect(request, new Action(
        TeacherTimeAction.class, "taskList"), "info.save.success", new String[] { "task" }); }
    return null;
  }
