    ...
    for (Iterator iter = classroomList.iterator(); iter.hasNext();) {
      ...
      for (int i = 0; i < WeekInfo.MAX; i++) {
        for (int j = CourseUnit.MINUNITS - 1; j < TeachCalendar.MAXUNITS; j++) {
          if (CollectionUtils.isNotEmpty(unitMap.keySet()) && !unitMap.containsKey((i + 1) + "," + (j + 1))) {
            continue;
          }
          ...
          activityQuery.add(new Condition("BITAND(activity.time.validWeeksNum,:wnl)>0", wnl));
          activityQuery.add(new Condition("activity.time.weekId=:weekId", new Integer(i + 1)));
          activityQuery.add(new Condition("activity.time.startUnit<=:unit", new Integer(j + 1)));
          activityQuery.add(new Condition("activity.time.endUnit>=:unit", new Integer(j + 1)));
          ...
          if (it.hasNext()) {
            ...
            if (a instanceof TaskActivity) {
              ...
              String applyResult = (String) data[0] + " " + (String) data[1] + " " + (String) data[2];
