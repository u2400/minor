...
public class ElectScopeAction extends CalendarRestrictionSupportAction {
  ...
  public ActionForward batchUpdateEelectInfo(ActionMapping mapping, ActionForm form,
    ...
    ElectInitParams setting = (ElectInitParams) populate(request, ElectInitParams.class, "setting");
   ...
   * @throws Exception
   */
  public ActionForward setCancelable(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Boolean updateSelected = getBoolean(request, "updateSelected");
    List tasks = getTaskSearched(request, updateSelected);
    if (null == tasks) return forwardError(mapping, request, "error.model.id.needed");
    ...
    return redirect(request, "taskList", "info.set.success",
        "task.electInfo.isElectable=" + request.getParameter("task.electInfo.isElectable"));
  }
   ...
   * @throws Exception
   */
  public ActionForward electSetting(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
  ...
  public ActionForward edit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isEmpty(taskId)) return forward(mapping, request, "error.teachTask.id.needed", "failure");
    TeachTask task = null;
    if (taskId.indexOf(",") != 0) {
      ...
      for (int i = 0; i < taskIds.length; i++) {
        ...
        // 参选任务有一个默认的开课范围
        if (task.getElectInfo().getElectScopes().size() > 1) break;
      }
    }
    String stdTypeIdSeq = getStdTypeIdSeq(request);
    String departIdSeq = getDepartmentIdSeq(request);
    ...
    request.setAttribute(Constants.SPECIALITYASPECT_LIST, specialityAspectList);
    request.setAttribute(Constants.ADMINCLASS_LIST,
        classService.getAdminClasses(null, stdTypeIdSeq, departIdSeq));
    return forward(request);
  }
   ...
   * @throws Exception
   */
  public ActionForward maxStdCountCheck(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
  ...
  public ActionForward info(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    for (Iterator iter = task.getElectInfo().getElectScopes().iterator(); iter.hasNext();) {
      ...
      scopeMap.put("stdTypes",
          utilService.load(StudentType.class, "id", SeqStringUtil.transformToLong(scope.getStdTypeIds())));
      scopeMap.put("departs",
          utilService.load(Department.class, "id", SeqStringUtil.transformToLong(scope.getDepartIds())));
      scopeMap.put("specialities",
          utilService.load(Speciality.class, "id", SeqStringUtil.transformToLong(scope.getSpecialityIds())));
      scopeMap
          .put(
              "aspects",
              utilService.load(SpecialityAspect.class, "id",
                  SeqStringUtil.transformToLong(scope.getAspectIds())));
      scopeMap.put("adminClasses",
          utilService.load(AdminClass.class, "id", SeqStringUtil.transformToLong(scope.getAdminClassIds())));
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    String taskId = request.getParameter(Constants.TEACHTASK_KEY);
    // 先行保存样板任务的选课信息
    if (StringUtils.isEmpty(taskId) || newOrModifies == null) { return forwardError(mapping, request,
        new String[] { "entity.teachTask", "error.model.id.needed" }); }
    ...
    if (null == task) { return forwardError(mapping, request, new String[] { "entity.teachTask",
    ...
    for (int i = 1; i <= newOrModifies.intValue(); i++) {
      ...
      if (hasThis) {
        ElectStdScope scope = (ElectStdScope) populate(request, ElectStdScope.class, Constants.ELECTSTDSCOPE
            + i);
        scope.setTask(task);
    ...
    if (StringUtils.isNotEmpty(deleteScopeIdSeq)) {
      ...
      utilService.remove(ElectStdScope.class, "id", SeqStringUtil.transformToLong(deleteScopeIds));
    }
    // 更新选择的教学任务
    ...
    for (int i = 0; i < taskIds.length; i++) {
      if (!taskIds[i].equals(myId)) {
        ...
        if (null != other) {
          ...
          other.getElectInfo().getElectScopes().addAll(task.getElectInfo().getElectScopes());
