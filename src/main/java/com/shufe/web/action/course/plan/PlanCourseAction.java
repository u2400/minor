...
public class PlanCourseAction extends PlanUpdateAction {
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null == planCourse.getId()) {
      ...
      PropertyUtils.copyProperties(tPlanCourse,
          (PlanCourse) utilService.load(PlanCourse.class, planCourse.getId()));
    }
    ...
    if (planCourse.isPO()) {
      ...
      if (plan.hasCourse(planCourse.getCourse())) { return redirect(request, new Action(
          CourseGroupAction.class, "edit"), "teachPlan.duplicate.course", extra); }
    ...
    alteration.setIsModifyCourse(new Boolean(isNotEquals || c11.size() != c22.size() || hasC11 || hasC22));
    alteration.setAfterPlan(plan);
    utilService.saveOrUpdate(alteration);

    return redirect(request, new Action(CourseGroupAction.class, "edit"), "info.save.success", extra);
  }
