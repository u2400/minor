...
public class ExaminerAction extends ExamAction {
   ...
   * @throws Exception
   */
  public ActionForward examActivityList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    EntityQuery entityQuery = new EntityQuery(ExamActivity.class, "activity");
    populateConditions(request, entityQuery, "calendar.studentType.id,task.teachClass.stdType.id");
   ...
   * @throws Exception
   */
  public ActionForward batchSetExaminers(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String examActivityIds = request.getParameter("examActivityIds");
    if (StringUtils.isEmpty(examActivityIds)) return forward(mapping, request, "error.model.id.needed",
        "error");

    // 按照查询条件进行排序
    EntityQuery query = new EntityQuery(ExamActivity.class, "activity");
    query.add(new Condition("activity.id in (:ids)", SeqStringUtil.transformToLong(examActivityIds)));
    query.addOrder(OrderUtils.parser(request.getParameter("orderBy")));
  ...
  public ActionForward setExaminerByTeacher(ActionMapping mapping, ActionForm form,
    ...
    if (StringUtils.isEmpty(examActivityIds)) return forward(mapping, request, "error.model.id.needed",
        "error");
    List departmentList = getDeparts(request);
    List examActivities = utilService.load(ExamActivity.class, "id",
        SeqStringUtil.transformToLong(examActivityIds));
    for (Iterator iter = examActivities.iterator(); iter.hasNext();) {
      ExamActivity activity = (ExamActivity) iter.next();
      // 判断是否是主考院系
      if (departmentList.contains(activity.getTask().getArrangeInfo().getTeachDepart())) {
        if (!activity.getTask().getArrangeInfo().getTeachers().isEmpty()) {
          activity.getExamMonitor().setExaminer(
              (Teacher) activity.getTask().getArrangeInfo().getTeachers().iterator().next());
        }
      }
    }
   ...
   * @throws Exception
   */
  public ActionForward saveExaminers(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String[] activityIds = request.getParameterValues("examActivityId");
    Collection activities = utilService.load(ExamActivity.class, "id",
        SeqStringUtil.transformToLong(activityIds));
    ...
    for (Iterator iter = examActivities.iterator(); iter.hasNext();) {
      ...
      if (departmentList.contains(activity.getExamMonitor().getDepart())) {
        ...
        if (null != activity.getExamMonitor().getInvigilator()) {
          departMap.put(activity.getId().toString(), activity.getExamMonitor().getInvigilator()
              .getDepartment());
        } else if (null == activity.getExamMonitor().getDepart()) {
          departMap.put(activity.getId().toString(), activity.getTask().getTeachClass().getDepart());
        } else {
          departMap.put(activity.getId().toString(), activity.getExamMonitor().getDepart());
        }
      }
    }
   ...
   * @throws Exception
   */
  public ActionForward saveInvigilators(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String[] activityIds = request.getParameterValues("examActivityId");
    Collection activities = utilService.load(ExamActivity.class, "id",
        SeqStringUtil.transformToLong(activityIds));
   ...
   * @throws Exception
   */
  public ActionForward examinerReport(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    List examActivities = (List) getExportDatas(request);
    List notices = MonitorNotice.builidNotices(examActivities, new HashSet(getDeparts(request)));
   ...
   * @throws Exception
   */
  public ActionForward printPaperLabel(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    List examActivities = (List) getExportDatas(request);
    ...
    if (!orders.isEmpty()) {
      ...
      for (Iterator iterator = orders.iterator(); iterator.hasNext();) {
        ...
        comparator.addComparator(new PropertyComparator(StringUtils.replace(order.getProperty(), "activity.",
            ""), (order.getDirection() == Order.ASC)));
      }
   ...
   * @throws Exception
   */
  public ActionForward examinerStat(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long calendarId = getLong(request, "calendar.id");
    Long examTypeId = getLong(request, "examType.id");
    ...
    for (Iterator it1 = examActivityList.iterator(); it1.hasNext();) {
      ...
      if (StringUtils.equals("examMonitor", examTeacherType)) {
        if (null != examActivity.getExamMonitor() && null != examActivity.getExamMonitor().getInvigilator()
  ...
  protected PropertyExtractor getPropertyExtractor(HttpServletRequest request) {
    ExamActivityPropertyExtractor exporter = new ExamActivityPropertyExtractor(utilService, (Locale) request
        .getSession().getAttribute(Globals.LOCALE_KEY), getResources(request));
    return exporter;
  }
