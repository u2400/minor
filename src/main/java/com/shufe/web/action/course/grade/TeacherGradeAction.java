...
public class TeacherGradeAction extends CalendarSupportAction {
  ...
  public ActionForward index(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    addSingleParameter(request, "GAGrade", baseCodeService.getCode(GradeType.class, GradeType.GA));
    EntityQuery query = new EntityQuery(GradeInputSwitch.class, "switch");
    query.add(
        new Condition("switch.calendar=:calendar", (TeachCalendar) request.getAttribute(Constants.CALENDAR)));
    List rs = (List) utilService.search(query);
    if (!rs.isEmpty()) {
      addSingleParameter(request, "canInput", new Boolean(
          null == getCannotInputSwitch(request, (TeachCalendar) request.getAttribute(Constants.CALENDAR))));
    } else if (rs.isEmpty()) {
  ...
  protected GradeInputSwitch getCannotInputSwitch(HttpServletRequest request, TeachCalendar calendar) {
    ...
    if (!isTeacher) {
      ...
      if (!rs.isEmpty()) {
        ...
        if (!gradeInputSwitch.checkOpen(new Date())) { return gradeInputSwitch; }
      }
      return null;
    }
  }
  ...
  public ActionForward input(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null == task) { return forwardError(mapping, request, "error.parameters.illegal"); }
    ...
    if (state.isConfirmed(GAGradeType)) {
      if (GradeSetting.TEACHER_PRINTABLE) {
        return forward(request, new Action(TeachClassGradeReportAction.class, "report"));
      } else {
        return forward(request, new Action(TeacherGradeAction.class, "info"));
      }
    }
    ...
    addSingleParameter(request, "gradeConverterConfig",
        ConverterFactory.getConverter().getConfig(state.getMarkStyle()));
    // 开始录入
    return forward(request, (String) getSystemConfig().getConfigItemValue(GradeSetting.TEACHER_INPUT_MODE));
  }
   ...
   * @throws Exception
   */
  public ActionForward batchSaveGradeState(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String taskIds = get(request, "taskIds");
    ...
    for (Iterator iter = taskList.iterator(); iter.hasNext();) {
      TeachTask task = (TeachTask) iter.next();
      task.getGradeState().setMarkStyle((MarkStyle) baseCodeService.getCode(MarkStyle.class, markStyleId));
      tasks.add(task);
    }
   ...
   * @throws Exception
   */
  public ActionForward editGradeState(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    TeachTask task = checkTeachTask(request);
    if (null == task) { return forwardError(mapping, request, "error.parameters.illegal"); }
    GradeInputSwitch gradeInputSwitch = getCannotInputSwitch(request, task);
    ...
    // 总评或最终成绩发布的不能修改百分比
    GradeType GAGradeType = (GradeType) baseCodeService.getCode(GradeType.class, GradeType.GA);
    GradeType USUALGradeType = (GradeType) baseCodeService.getCode(GradeType.class, GradeType.USUAL);
    GradeType ENDGradeType = (GradeType) baseCodeService.getCode(GradeType.class, GradeType.END);
    if (EamsRole.TEACHER_USER.equals(getUserCategoryId(request))
        || null != state.getPercent(USUALGradeType) && null != state.getPercent(ENDGradeType)
            && 100F == state.getPercent(USUALGradeType).floatValue()
                + state.getPercent(ENDGradeType).floatValue()) { return forward(request,
                    new Action(this, "input")); }
    if (state.isPublished(GAGradeType) || state
        .isPublished(ENDGradeType)) { return forwardError(mapping, request, "error.grade.modifyPublished"); }
    ...
    addSingleParameter(request, "isAdmin",
        new Boolean(EamsRole.MANAGER_USER.equals(getUserCategoryId(request))));
   ...
   * @throws Exception
   */
  public ActionForward saveGradeState(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    TeachTask task = checkTeachTask(request);
    if (null == task) { return forwardError(mapping, request, "error.parameters.illegal"); }
   ...
   * @throws Exception
   */
  public ActionForward batchUpdateGrade(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String taskIds = get(request, "taskIds");
    List taskList = teachTaskService.getTeachTasksByIds(taskIds);
    for (Iterator iter = taskList.iterator(); iter.hasNext();) {
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    // 得到成绩的 教师录入模式
    String mode = (String) getSystemConfig().getConfigItemValue(GradeSetting.TEACHER_INPUT_MODE);
    // 找到任务,专业类型,总评成绩精度
    TeachTask task = checkTeachTask(request);
    GradeInputSwitch gradeInputSwitch = getCannotInputSwitch(request, task);
    if (null != gradeInputSwitch) {
    ...
    for (Iterator iter = task.getTeachClass().getCourseTakes().iterator(); iter.hasNext();) {
      ...
      String stuTypeName = null == take.getStudent().getStudentStatusInfo().getEnrollMode() ? null
          : take.getStudent().getStudentStatusInfo().getEnrollMode().getName();
      ...
      if (null == grade) {
        ...
        // 依据计划查找课程类别和学分
        CourseType planCourseType = teachPlanService.getCourseTypeOfStd(take.getStudent().getCode(),
            task.getCourse(), task.getCourseType(), grade.getMajorType());
      ...
      for (Iterator iterator = gradeTypes.iterator(); iterator.hasNext();) {
        ...
        // 1.查找考试情况
        ExamStatus examStatus = (ExamStatus) utilService.load(ExamStatus.class, ExamStatus.NORMAL);
        ...
        if (Boolean.FALSE.equals(examStatus.getIsAttend()) || (gradeType.getId().equals(GradeType.USUAL)
            && take.getCourseTakeType().getId().equals(CourseTakeType.REEXAM)
            && !(getSystemConfig().getBooleanParam("Grade.REEXAM.USUALISINPUT")))) {
        ...
        if (StringUtils.isNotEmpty(scoreStr) && ("-1".equals(scoreStr) || !NumberUtils.isNumber(scoreStr))) {
          incGradeCount(inputMap, gradeType);
          incGradeCount(confirmMap, gradeType);
        ...
        if (null != examGrade) {
          ...
          if (mode.equals(GradeSetting.TWICE_MODE)) {
            if (isTeacher) {
              if (Boolean.TRUE.equals(examGrade.getIsConfirmed())) incGradeCount(confirmMap, gradeType);
            } else {
              examGrade.setStatus(new Integer(Grade.CONFIRMED));
              incGradeCount(confirmMap, gradeType);
            }
          ...
          if (null != examScore || StringUtils.isNotEmpty(scoreStr)
            ...
            if (mode.equals(GradeSetting.TWICE_MODE)) {
              ...
              if (isTeacher) {
                examGrade.setStatus((minEnrol == 1) ? new Integer(Grade.NEW) : new Integer(Grade.CONFIRMED));
              } else {
                examGrade.setStatus(new Integer(Grade.CONFIRMED));
                incGradeCount(confirmMap, gradeType);
              }
      ...
      if (!StringUtils.equals("留学生", stuTypeName)) {
        grade.calcGA();
        grade.calcScore();
      } else {
        calcLXSGA(grade);
      ...
      grade.updatePass();
      // 计算平均绩点
      grade.calcGP(gradePointRuleService.getGradePointRule(grade.getStd().getType(), grade.getMarkStyle()));
      grades.add(grade);
  ...
  public void calcLXSFinal(CourseGrade grade) {
    GradeState state = (GradeState) grade.getGradeTask().getCourseGradeState();
    if (state == null) { return; }
    Float ga = grade.getGA();
    if (ga == null) grade.setScore(grade.getBestScoreCanConvertToFinal());
    else grade.setScore(grade.getGA());
  }

  public void calcLXSGA(CourseGrade grade) {
    if ((grade.getGradeTask() != null) && (grade.getCourseTakeType() != null)) {
      ...
      if (endScore == null) {
        ...
        if ((endGradePercent != null) && (endGradePercent.floatValue() != 0.0F)) {
          grade.setGA(null);
          return;
        }
      }
      ...
      Float endGradePercent = (Float) percentMap.get(new GradeType(GradeType.END));
      Float usualGradePercent = (Float) percentMap.get(new GradeType(GradeType.USUAL));
      float ga = 0.0F;
      if ((endGradePercent == null) && (usualGradePercent.floatValue() == 1.0D)) {
        grade.setGA(Float.valueOf(grade.getScore(new GradeType(GradeType.USUAL)).floatValue()));
      } else if ((endGradePercent.floatValue() == 1.0D) && (usualGradePercent == null)) {
        grade.setGA(endScore);
      } else {
        for (Iterator iter = percentMap.keySet().iterator(); iter.hasNext();) {
          GradeType gradeType = (GradeType) iter.next();
          if (gradeType.getId().equals(GradeType.END))
            ga = (float) (ga + 0.4D * endScore.floatValue());
          else {
            if (null != grade.getScore(gradeType)) {
              ga = (float) (ga + 0.6D * grade.getScore(gradeType).floatValue());
            }
          }
        }
  ...
  protected Float reservedPrecision(float ga, int precision) {
    int mutilply = (int) Math.pow(10.0D, precision);
    ga *= mutilply;
    if (ga % 1.0F >= 0.5D) ga += 1.0F;
    ga -= ga % 1.0F;
   ...
   * @throws Exception
   */
  public ActionForward reportForExam(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    new TeachClassGradeHelper(baseCodeService, gradeService).statTask(taskIdSeq, new Long[] { GradeType.END },
        request);
    return forward(request);
  }
   ...
   * @throws Exception
   */
  public ActionForward reportContent(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
