...
public class TeachTaskCheckAction extends TeachTaskCollegeAction {
  ...
  public ActionForward search(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    addCollection(request, "adminClasses",
        utilService.search(baseInfoSearchHelper.buildAdminClassQuery(request)));
    return forward(request);
  }
  ...
  public ActionForward info(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    TeachCalendar calendar = teachCalendarService.getTeachCalendar(getLong(request, "task.calendar.id"));
    ...
    for (Iterator it1 = adminClasses.iterator(); it1.hasNext();) {
      ...
      TeachPlan plan = null;
      try {
        int term = termCalc.getTerm(adminClass.getStdType(), adminClass.getEnrollYear(), Boolean.FALSE);
      ...
      query.add(new Condition("adminClass.id in (:adminClassIds)", new Long[] { adminClass.getId() }));
      ...
      adminClassMap.put(adminClass.getId().toString(), new Object[] { courses, taskCourses, all,
          onCampusTimeNotFound });
    }
  ...
  public ActionForward gen(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null == calendar || StringUtils.isEmpty(adminCourse)) { return forward(mapping, request,
        "error.parameters.illegal", "error"); }
    ...
    for (int i = 0; i < grouping.length; i++) {
      ...
      AdminClass adminClass = (AdminClass) utilService.load(AdminClass.class, new Long(element[1]));
      String key = adminClass.getEnrollYear() + "_" + adminClass.getStdType().getId() + "_"
          + adminClass.getDepartment().getId() + "_" + adminClass.getSpeciality().getId() + "_"
      ...
      if (StringUtils.equals(element[0], "P")) {
        ...
        TeachTask task = teachTaskService.genTeachTaskDetail(plan.getPlanCourse(course), calendar, plan,
            adminClass, params);
  ...
  protected List transferPlanToTasks(TeachPlan plan, AdminClass adminClass, TeachCalendar calendar,
      TaskGenParams params) {
    List tasks = new ArrayList();
    TermCalculator termCalc = new TermCalculator(teachCalendarService, calendar);
    int term = termCalc.getTerm(adminClass.getStdType(), adminClass.getEnrollYear(), Boolean.FALSE);
    for (Iterator iter = plan.getCourses(term + "").iterator(); iter.hasNext();) {
      tasks.add(teachTaskService.genTeachTaskDetail(plan.getPlanCourse((Course) iter.next()), calendar, plan,
          adminClass, params));
    }
    return tasks;
  }
