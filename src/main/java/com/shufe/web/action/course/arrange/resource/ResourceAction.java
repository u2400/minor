...
public abstract class ResourceAction extends CalendarRestrictionSupportAction {
   ...
   * @throws Exception
   */
  public ActionForward occupyTables(ActionMapping mapping, ActionForm form, HttpServletRequest request,
   ...
   * @throws Exception
   */
  public ActionForward digestTime(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    TeachCalendar calendar = (TeachCalendar) utilService.get(TeachCalendar.class,
        getLong(request, "calendar.id"));
    ...
    Integer startWeek = getInteger(request, "startWeek");
    Integer endWeek = getInteger(request, "endWeek");
    if (null == startWeek || startWeek.intValue() < 1) startWeek = new Integer(1);
    if (null == endWeek || endWeek.intValue() > calendar.getWeeks().intValue()) endWeek = calendar.getWeeks();
    List resources = getResourceList(request);
    ...
    for (Iterator iter = resources.iterator(); iter.hasNext();) {
      ...
      for (int i = 0; i < times.length; i++) {
        occupies.addAll(getOccupyInfos(entityId, times[i].getValidWeeksNum(), times[i].getYear()));
      }
