...
public class GradeLogAction extends CalendarRestrictionSupportAction {
  ...
  public ActionForward search(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isNotEmpty(stdCode)) {
      try {
        ...
        String logInfo = stdCode + " of Student CODE been removed or not exists when recording GradeLog!";
    ...
    if (null != calendar && StringUtils.isNotEmpty(taskSeqNo)) {
      try {
        ...
        String logInfo = taskSeqNo + " of Course CODE in [" + calendar.getStudentType().getName() + " "
      ...
      if (CollectionUtils.isEmpty(utilService.search(query))) {
        String logInfo = courseCode + " of Course CODE been removed or not exists when recording GradeLog!";
        logHelper.info(request, logInfo);
      }
    }
    // 处理记录时间条件
    ...
    if (StringUtils.isNotEmpty(userCode)) {
      try {
        ...
        String logInfo = userCode + " of User CODE been removed or not exists when recording GradeLog!";
        logHelper.info(request, logInfo);
      }
    }
    // /////////////// //
  ...
  public ActionForward info(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null == gradeCatalogId) { return forwardError(mapping, request, "error.model.id.needed"); }
  ...
  public ActionForward remove(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    try {
      ...
      if (null == gradeCatalogIds || gradeCatalogIds.length == 0) { throw new RuntimeException(); }
    } catch (Exception e) {
    ...
    for (Iterator it = catalogs.iterator(); it.hasNext();) {
      ...
      logHelper.info(request,
          "GradeCatalog with No." + StringUtils.repeat("0", 18 - String.valueOf(gradeLog.getId()).length())
