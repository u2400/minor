...
public class TeachTaskGenAction extends TeachTaskSearchAction {
   ...
   * @throws Exception
   */
  public ActionForward teachPlanList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse reponse) throws Exception {
    // get depart
    TeachPlan plan = (TeachPlan) populate(request, TeachPlan.class, Constants.TEACHPLAN);
    TeachCalendar calendar = (TeachCalendar) populate(request, TeachCalendar.class, Constants.CALENDAR);

    StudentType stdType = (StudentType) utilService
        .load(StudentType.class, calendar.getStudentType().getId());

    // get calendar from db
    if (StringUtils.isNotEmpty(calendar.getYear()) && StringUtils.isNotEmpty(calendar.getTerm())) {
      calendar = teachCalendarService.getTeachCalendar(stdType, calendar.getYear(), calendar.getTerm());
    } else {
    ...
    try {
      Collection teachPlanList = teachPlanService.getTeachPlansOfTaskGeneration(plan, calendar,
          getStdTypeIdSeq(request), getDepartmentIdSeq(request), active, omitSmallTerm.booleanValue());
      Results.addObject("teachPlanList", teachPlanList);
    } catch (OnCampusTimeNotFoundException e) {
      debug("cannot find onCompus time :", e);
      return forward(mapping, request, e.getMessage().substring(0, e.getMessage().indexOf(":")), "error");
    }
   ...
   * @throws Exception
   */
  public ActionForward listTask(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String planIdSeq = request.getParameter("fromPlan.ids");
    if (StringUtils.isEmpty(planIdSeq)) return forward(mapping, request, "error.teachPlan.id.needed", "error");
    ...
    query.add(new Condition("task.fromPlan.id in(:ids)", SeqStringUtil.transformToLong(planIdSeq)));
  ...
  public ActionForward gen(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    try {
      logHelper.info(request,
          "Generator TeachTask for cultivateScheme :" + ArrayUtils.toString(cultivateSchemeIds));
      response.setContentType("text/html; charset=utf-8");
      TaskGenObserver observer = (TaskGenObserver) getOutputProcessObserver(mapping, request, response,
          TaskGenObserver.class);
      ...
      logHelper.info(request,
          "Failure in generating TeachTask for cultivateScheme :" + ArrayUtils.toString(cultivateSchemeIds),
          e);
   ...
   * @throws Exception
   */
  public ActionForward genSetting(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    addSingleParameter(request, "hasPractice",
        new Boolean(CollectionUtils.isNotEmpty(utilService.search(query))));
    return forward(request);
  }
