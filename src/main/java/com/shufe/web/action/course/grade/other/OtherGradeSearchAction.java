...
public class OtherGradeSearchAction extends CalendarRestrictionSupportAction {
   ...
   * @return
   */
  public ActionForward searchOtherGrade(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    for (Iterator iter = ortherGradeDepartmentList.iterator(); iter.hasNext();) {
      ...
      List ortherGradeList = utilService
          .searchHQLQuery("select distinct(og.std.enrollYear) from OtherGrade og where og.category.code="
              + category + " and og.std.department.id =" + department.getId() + "order by og.std.enrollYear");
      for (Iterator iter_ortherGradeList = ortherGradeList.iterator(); iter_ortherGradeList.hasNext();) {
        ...
        gradeCount = (Long) utilService.searchHQLQuery(
            "select count(*) from OtherGrade og where og.category.code=" + category
                + " and og.std.department.id =" + department.getId() + "and og.std.enrollYear ='"
                + enrollYear + "' and og.score <> 0").get(0);
        gradeIspassCount = (Long) utilService.searchHQLQuery(
            "select count(*) from OtherGrade og where og.category.code=" + category
                + " and og.std.department.id =" + department.getId() + " and og.std.enrollYear ='"
    ...
    String otherExamCategoryName = (String) utilService.searchHQLQuery(
  ...
  public ActionForward searchOtherGradeByTerm(ActionMapping mapping, ActionForm form,
    ...
    for (Iterator iter = ortherGradeDepartmentList.iterator(); iter.hasNext();) {
      ...
      List ortherGradeList = utilService
          .searchHQLQuery("select distinct(og.std.enrollYear) from OtherGrade og where og.category.code="
              + category + " and og.std.department.id =" + department.getId() + " and og.calendar.year ='"
              + year + "' and og.calendar.term ='" + term + "' order by og.std.enrollYear");
      for (Iterator iter_ortherGradeList = ortherGradeList.iterator(); iter_ortherGradeList.hasNext();) {
        String enrollYear = (String) iter_ortherGradeList.next();
        Long gradeCount = null;
        Long gradeIspassCount = null;
        gradeCount = (Long) utilService.searchHQLQuery(
            "select count(*) from OtherGrade og where og.category.code=" + category
                + " and og.std.department.id =" + department.getId() + "and og.std.enrollYear ='"
                + enrollYear + "' and og.calendar.year ='" + year + "' and og.calendar.term='" + term
                + "' and og.score <> 0").get(0);
        gradeIspassCount = (Long) utilService.searchHQLQuery(
            "select count(*) from OtherGrade og where og.category.code=" + category
                + " and og.std.department.id =" + department.getId() + " and og.std.enrollYear ='"
                + enrollYear + "' and og.calendar.year ='" + year + "' and og.calendar.term='" + term
                + "' and og.score >=" + hgbz).get(0);
  ...
  protected EntityQuery buildQuery(HttpServletRequest request) {
    ...
    if (null != pass) {
      if (pass.equals(new Integer(-1))) {
        ...
        query.add(new Condition("otherGrade.isPass = (:isPass)", RequestUtils.getBoolean(request,
            "otherGrade.isPass")));
      }
    }
    DataRealmUtils.addDataRealms(query, new String[] { "otherGrade.std.type.id",
        "otherGrade.std.department.id" },
        getDataRealmsWith(getLong(request, "otherGrade.std.type.id"), request));
