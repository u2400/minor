...
public class TeachTaskCollegeAction extends TeachTaskMergeSplitAction {
  ...
  public ActionForward listSiva(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
   ...
   * @throws Exception
   */
  public ActionForward addMultiTask(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long taskId = getLong(request, "task.id");
    Long courseId = getLong(request, "task.course.id");
    if (null == courseId && null == taskId) { return selectCourse(mapping, form, request, response); }
    TeachTask task = null;
    ...
    task.getTeachClass().setStdType(studentTypeService.getStudentType(getLong(request, "stdType.id")));
  ...
  public ActionForward arrangeInfoSetting(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    if (StringUtils.isNotEmpty(taskIds)) {
      ...
      CollectionUtils.filter(tasks, new Predicate() {
        ...
        public boolean evaluate(Object arg0) {
          if (((TeachTask) arg0).getIsConfirm().equals(Boolean.FALSE)) return true;
          else return false;
        }
      });
    }
   ...
   * @throws Exception
   */
  public ActionForward batchEdit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String taskIds = get(request, "taskIds");
    if (StringUtils.isEmpty(taskIds)) { return forward(mapping, request, "error.teachTask.id.needed", "error"); }
    Collection taskList = taskListQuery(request, taskIds);

    taskList = DataAuthorityUtil.select(taskList, "TeachTaskForTeachDepart", getStdTypeIdSeq(request),
  ...
  public ActionForward batchEditSiva(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String taskIds = get(request, "taskIds");
    if (StringUtils.isEmpty(taskIds)) { return forward(mapping, request, "error.teachTask.id.needed", "error"); }
    Collection taskList = taskListQuery(request, taskIds);
   ...
   * @throws Exception
   */
  public ActionForward batchUpdate(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
  ...
  public ActionForward batchUpdateCourse(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null != taskIds) {
      ...
      for (Iterator iter = tasks.iterator(); iter.hasNext();) {
        ...
        if (null != courseType) {
          task.setCourseType((CourseType) baseCodeService.getCode(CourseType.class, courseType));
        }
        if (null != remark) {
          task.setRemark(remark);
        }
  ...
  public ActionForward batchUpdateElectInfo(ActionMapping mapping, ActionForm form,
    ...
    String taskRemark = get(request, "taskRemark");
    if (taskIds == null || hskId == null && StringUtils.isEmpty(code) && StringUtils.isBlank(taskRemark)) { return forward(
        request, new Action("", "search"), "info.save.failure"); }
    // check params
    ...
    for (int i = 0; i < taskIds.length; i++) {
      ...
      if (StringUtils.isNotBlank(taskRemark)) {
  ...
  public ActionForward batchUpdateRequirement(ActionMapping mapping, ActionForm form,
    ...
    TeachLangType type = (TeachLangType) baseCodeService.getCode(TeachLangType.class, teachLangTypeId);
    CourseCategory type1 = (CourseCategory) baseCodeService.getCode(CourseCategory.class, courseCategoryId);
    ExamMode examMode = (ExamMode) baseCodeService.getCode(ExamMode.class, examModeId);
  ...
  public ActionForward calcPlanStdCountByAdminClass(ActionMapping mapping, ActionForm form,
      HttpServletRequest request, HttpServletResponse response) throws Exception {
    String taskIds = get(request, "taskIds");
    if (StringUtils.isEmpty(taskIds)) { return forwardError(mapping, request, "error.model.id.needed"); }
   ...
   * @throws Exception
   */
  public ActionForward checkArrangeInfo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    entityQuery.add(new Condition(
        "task.teachClass.stdType in(:stdTypes) and task.arrangeInfo.teachDepart in(:departs)", getStdTypesOf(
            stdTypeId, request), getDeparts(request)));
    entityQuery.add(new Condition("task.calendar.id=" + get(request, "calendar.id")));
    entityQuery.add(new Condition(
        "task.arrangeInfo.weeks * task.arrangeInfo.weekUnits <> task.arrangeInfo.overallUnits"));
   ...
   * @throws Exception
   */
  public ActionForward checkInfo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long stdTypeId = getLong(request, "calendar.studentType.id");
    Long calendarId = getLong(request, "calendar.id");
    List teachTaskList = utilService.searchHQLQuery("from TeachTask tt where tt.calendar.id =" + calendarId
        + " and tt.teachClass.stdType.id =" + stdTypeId
  ...
  public ActionForward copy(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null != taskIds) {
      ...
      calendar = teachCalendarService.getTeachCalendar(calendar.getStudentType().getId(), calendar.getYear(),
          calendar.getTerm());
   ...
   * @throws Exception
   */
  public ActionForward copySetting(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
  ...
  public ActionForward declarationForm(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
   ...
   * @throws Exception
   */
  public ActionForward courseNotList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long[] taskIds = SeqStringUtil.transformToLong(get(request, "taskIds"));
    EntityQuery query1 = new EntityQuery(Student.class, "student");
    query1
        .add(new Condition(
            "exists (from "
                + TeachTask.class.getName()
                + " task where task.id in (:taskIds) and exists (from task.teachClass.adminClasses adminClass where exists (from adminClass.students student2 where student2 = student)) and not exists (from task.teachClass.courseTakes take where take.student = student))",
            taskIds));
    query1.addOrder(OrderUtils.parser("student.code"));
    addCollection(request, "students", utilService.search(query1));
    ...
    query2
        .add(new Condition(
            "exists (from "
                + TeachTask.class.getName()
                + " task2 where task.calendar= task2.calendar and task.course = task2.course and task2.id in (:taskIds))",
            taskIds));
    addCollection(request, "tasks", utilService.search(query2));
    return forward(request);
  }
   ...
   * @throws Exception
   */
  public ActionForward appointStd(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    for (Student student : students) {
      ...
      for (TeachTask task : tasks) {
        ...
        if (task.getTeachClass().getCourseTakes().size() >= (null == maxStdCount ? 0 : maxStdCount.intValue())
            || isConfictedActivityInStudent(student, task)) {
          continue;
        }
        take = new CourseTake(task, student, new CourseTakeType(CourseTakeType.COMPULSORY));
        break;
      }
      Map<String, Object> resultObj = new HashMap<String, Object>();
      resultObj.put("student", student);
      resultObj.put("isOk", null != take);
      if (null != take) {
        takes.add(take);
    ...
    if (CollectionUtils.isNotEmpty(takes)) {
      utilService.saveOrUpdate(takes);
    }
    addCollection(request, "results", results);
    addCollection(request, "tasks", tasks);
    return forward(request, "appointStdResult");
  }

  protected boolean isConfictedActivityInStudent(Student student, TeachTask task) {
    if (null == student || null == student.getId() || null == task || null == task.getId()
        || CollectionUtils.isEmpty(task.getArrangeInfo().getActivities())) { return true; }
    EntityQuery query = new EntityQuery(TaskActivity.class, "activity");
    query.add(new Condition("activity in (:activities)", task.getArrangeInfo().getActivities()));
    query
        .add(new Condition(
            "exists (from "
                + TaskActivity.class.getName()
                + " activity2 where exists (from "
                + CourseTake.class.getName()
                + " take where take.task = activity2.task and take.student = :student) and activity2.time.year = activity.time.year and bitand(activity2.time.validWeeksNum, activity.time.validWeeksNum) > 0 and activity2.time.weekId = activity.time.weekId and activity2.time.startTime <= activity.time.endTime and activity2.time.endTime >= activity2.time.startTime)",
            student));
   ...
   * @throws Exception
   */
  public ActionForward randomStd(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Collection<TeachTask> tasks = utilService.load(TeachTask.class,
        new String[] { "course.id", "calendar.id" },
        new Object[] { getLong(request, "courseId"), getLong(request, "calendarId") });
    Collection<Student> students = utilService.load(Student.class, "id",
        SeqStringUtil.transformToLong(get(request, "studentIds")));
  ...
  public ActionForward edit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null == taskId) {
      ...
      EntityUtils.merge(task, TeachTask.getDefault());
      task.setCourse(course);
      if (null != course.getExtInfo().getCourseType()) task
          .setCourseType(course.getExtInfo().getCourseType());
      task.setCalendar(calendar);
      task.getTeachClass().setName(course.getName());
      task.getTeachClass().setStdType(studentTypeService.getStudentType(getLong(request, "stdType.id")));
      task.getArrangeInfo().setWeeks(calendar.getWeeks());
      ...
      if (!new DataAuthorityPredicate(getStdTypeIdSeq(request), getDepartmentIdSeq(request),
          "teachClass.stdType", "arrangeInfo.teachDepart").evaluate(task)) return forward(mapping, request,
          "error.dataRealm.insufficient", "error");
    }
    if (task.getArrangeInfo().isSingleTeacher()) {
      Teacher teacher = (Teacher) task.getArrangeInfo().getTeachers().iterator().next();
    ...
    List stdTypes = studentTypeService.getStudentTypes(studentTypeService.getStdTypeIdSeqUnder(task
        .getCalendar().getStudentType().getId()));
    addSingleParameter(request, "stdTypeList", CollectionUtils.intersection(stdTypes, getStdTypes(request)));
  ...
  public ActionForward teachTaskCheck(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long taskId = getLong(request, "task.id");
    Long courseId = getLong(request, "task.course.id");
    if (null == courseId && null == taskId) { return selectCourse(mapping, form, request, response); }
    ...
    List list = (List) utilService.searchHQLQuery("from CourseTake ct where ct.task.id =" + task.getId());
    String stdString = "";
    ...
    query.add(new Condition("std.id not in (:stdIds)", SeqStringUtil.transformToLong(stdString)));
    query.join("tt.teachClass.adminClasses", "adminClass");
    StringBuilder hql = new StringBuilder();
    hql.append("exists (");
    hql.append("from AdminClass ac where ac.students.id=std.id");
    hql.append(" and exists (");
    hql.append("from TeachTask tt where adminClass.id=ac.id and tt.id = :taskId");
    hql.append("))");
    query.add(new Condition(hql.toString(), task.getId()));
    List stdList = (List) utilService.search(query);

    query = new EntityQuery(TeachTask.class, "tt");
    query.add(new Condition("tt.id = :taskId"));
    query.join("tt.teachClass.adminClasses", "adminClass");
    hql = new StringBuilder();
    hql.append("exists (");
    hql.append("from AdminClass ac where adminClass.id=ac.id");
    hql.append(")");
    query.add(new Condition(hql.toString()));
    List teachTaskForClassList = (List) utilService.search(query);
   ...
   * @throws Exception
   */
  public ActionForward exportSetting(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    String[] titles = StringUtils.split(message.getMessage(getLocale(request), "teachTask.showkeys"), ",");
  ...
  public ActionForward exportStdList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    EntityQuery entityQuery = new EntityQuery(TeachTask.class, "task");
    entityQuery.join("task.teachClass.courseTakes", "courseTake").join("courseTake.student", "std");
    entityQuery.join("left", "std.firstMajor", "firstMajor").join("left", "std.firstAspect", "firstAspect");
   ...
   * @throws Exception
   */
  public ActionForward listClassForTask(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    if (stdTypeId != null) {
      DataRealmUtils.addDataRealm(query, new String[] { "adminClass.stdType.id", null }, new DataRealm(
  ...
  public ActionForward printTaskForTeacher(ActionMapping mapping, ActionForm form,
    ...
    for (Iterator iter = tasks.iterator(); iter.hasNext();) {
      TeachTask task = (TeachTask) iter.next();
      arrangeInfos.put(task.getId().toString(), CourseActivityDigestor.digest(task.getCalendar(),
          task.getArrangeInfo(), getResources(request), getLocale(request), format));
      ArrayList myCourseTakes = new ArrayList();
      myCourseTakes.addAll(task.getTeachClass().getCourseTakes());
    }
    addCollection(request, "tasks", tasks);
   ...
   * @throws Exception
   */
  public ActionForward printTaskSummary(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    EntityQuery query = teachTaskSearchHelper.buildTaskQuery(request, Boolean.TRUE);
    query.setLimit(null);
  ...
  public ActionForward remove(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (dd.size() > 0) {
      calendar = ((TeachTask) dd.iterator().next()).getCalendar();
    }
    TermCalculator tc = new TermCalculator(teachCalendarService, calendar);
    List warnTasks = new ArrayList();
    for (Iterator iterator = dd.iterator(); null == getBoolean(request, "isRemove") && iterator.hasNext();) {
    ...
    if (CollectionUtils.isEmpty(tasks)) { return forward(mapping, request, "task.id.NotExists", "error"); }
    ...
    for (Iterator it = tasks.iterator(); it.hasNext();) {
      ...
      // 以开课院系做权限划分
      if (!SeqStringUtil.isIn(getDepartmentIdSeq(request), task.getArrangeInfo().getTeachDepart().getId()
          .toString())) { return forward(mapping, request, "error.depart.dataRealm.insufficient", "error"); }
      try {
        if (isToText) {
          ...
          msg.setBody("[" + task.getCalendar().getYear() + " " + task.getCalendar().getTerm() + "]学期，课程序号为 "
              + task.getSeqNo() + " ，课程代码为 " + task.getCourse().getCode() + " 的《"
          ...
          for (Iterator it2 = task.getTeachClass().getStds().iterator(); it2.hasNext();) {
            ...
            sysMsg.setRecipient((User) utilService.load(User.class, "name", student.getCode()).get(0));
        ...
        // utilService.remove(task);
        teachTaskService.removeTeachTask(task);
        // utilService.saveOrUpdate(gradeCatalogs);
        logHelper.info(request, "Remove teachTask with No.:" + task.getSeqNo() + " course No.:"
            + task.getCourse().getCode());
      } catch (Exception e) {
   ...
   * 删除或者保留
   */
  public ActionForward deleteOrSave(ActionMapping mapping, ActionForm form, HttpServletRequest request,
   ...
   * @throws Exception
   */
  public ActionForward saveTaskAdminClass(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    if (Boolean.TRUE.equals(getBoolean(request, "isToDo")) && StringUtils.isNotEmpty(operations)) {
      // 移除学生
      String removeAdminClassIds = get(request, "removeAdminClassIds");
      Set deleteObject = new HashSet();
      if (StringUtils.contains("," + operations + ",", ",1,") && StringUtils.isNotEmpty(removeAdminClassIds)) {
        for (Iterator it1 = adminClassService.getAdminClassesById(removeAdminClassIds).iterator(); it1
            .hasNext();) {
      ...
      if (StringUtils.contains("," + operations + ",", ",2,") && StringUtils.isNotEmpty(addAdminClassIds)) {
   ...
   * @throws Exception
   */
  public ActionForward saveMultiTask(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    // 准备任务
    ...
    for (int i = 0; i < teacherNum.intValue(); i++) {
      ...
      for (int j = 0, start = i; j < adminClassNum.intValue(); j++, start++) {
        ...
        // 获得教学班
        TeachClass teachClass = (TeachClass) populate(request, TeachClass.class, teachClassId);
   ...
   * @throws Exception
   */
  public ActionForward selectCourse(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    // 检查要添加的教学任务的学生类别是否在权限范围内
    TeachCalendar calendar = (TeachCalendar) populateEntity(request, TeachCalendar.class, Constants.CALENDAR);
    Long calendarId = getLong(request, "calendar.id");
    if (null == calendar.getStudentType().getId()) { return forward(mapping, request,
        "error.stdType.id.needed", "error"); }
    String stdTypeDataRealm = getStdTypeIdSeq(request);
    if (StringUtils.isEmpty(stdTypeDataRealm)) { return forward(mapping, request,
        "error.stdType.dataRealm.notExists", "error"); }
    if (!StringUtils.contains(stdTypeDataRealm, calendar.getStudentType().getId().toString())) { return forward(
        mapping, request, "error.stdType.dataRealm.insufficient", "error"); }
    addSingleParameter(request, "stdTypeList", baseInfoService.getBaseInfos(StudentType.class));
    addSingleParameter(request, Constants.STUDENTTYPE,
        utilService.load(StudentType.class, calendar.getStudentType().getId()));
  ...
  public ActionForward selfAdjustArrangeInfo(ActionMapping mapping, ActionForm form,
    ...
    saveErrors(request.getSession(), ForwardSupport.buildMessages(new String[] { "info.action.success" }));
    return mapping.findForward("actionResult");
  }
  ...
  protected void setDeparts(HttpServletRequest request, TeachTask task) {
    ...
    // 学生所在院系
    Collection departList = departmentService.getRelatedDeparts(getStdTypeIdSeqOf(task.getTeachClass()
        .getStdType().getId(), request));
  ...
  protected void setTeachersAndRooms(String teacherIdSeq, TeachTask task) {
    ...
    // 先清空教师
    task.getArrangeInfo().getTeachers().clear();
    if (null == staffIds || staffIds.length == 0) { return; }
    ...
    for (int i = 0; null != staffIds && i < staffIds.length; i++) {
      ...
      task.getArrangeInfo().getTeachers().addAll(utilService.load(Teacher.class, "id", staffIds[i]));
    }
    // 如果已经排课，处理要替换的教师
    ...
    if (oldTemps.size() < 1 || newTemps.size() < 1) { return; }
    for (Iterator it = task.getArrangeInfo().getActivities().iterator(); it.hasNext();) {
      ...
      if (null == activity.getTeacher()
          || ObjectUtils.equals(activity.getTeacher(), (Teacher) oldTemps.iterator().next())) {
  ...
  public ActionForward taskOfCourseTypeList(ActionMapping mapping, ActionForm form,
    ...
    List taskOfCourseTypes = teachTaskService.getTaskOfCourseTypes(calendar,
        getStdTypeIdSeqOf(stdTypeId, request), getDepartmentIdSeq(request), courseTypes);
