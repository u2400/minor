...
public class AutoArrangeAction extends CalendarRestrictionSupportAction {
  ...
  public ActionForward index(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    // 包含公有教室
    if (departDataRealm.indexOf(String.valueOf(Department.SCHOOLID)) == -1) departDataRealm += ","
        + Department.SCHOOLID;
  ...
  public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isEmpty(type)) return forward(mapping, request, "error.unarrangedType.needed", "error");
    ...
    if (type.equals("task")) {
      ...
      Pagination taskPage = teachTaskService.getTeachTasksOfLonely(task,
      ...
      if (isArrangeCompleted.equals(Boolean.FALSE)) prepareTimeInfo(request, new AvailableTime(
          AvailableTime.commonTeacherAvailTime));
      else {
      ...
      addOldPage(request, "groupList", groupService.getTaskGroups(group, stdTypeIdSeq, teachDepartIdSeq,
          calendar, getPageNo(request), getPageSize(request),
  ...
  public ActionForward arrange(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    /*--------------制定临时课程组---------------------------*/
    TaskGroup group = (TaskGroup) RequestUtil.populate(request, TaskGroup.class, Constants.TASKGROUP);
    ...
    if (type.equals("task")) {
      ...
      if (StringUtils.isEmpty(groupIds)) return forward(mapping, request, "error.taskGroup.id.needed",
          "error");
      List groupList = groupService.getTaskGroups(groupIds);
      group.getGroups().addAll(groupList);
      group.getSuggest().getTime().setAvailable(StringUtils.repeat("1", AvailableTime.MAX_LENGTH));
    }
    /*--------------检查教学任务是否满足自动排课条件----*/
    ...
    if (StringUtils.isNotEmpty(roomIdSeq)) group.getSuggest().getRooms()
        .addAll(roomService.getClassrooms(Arrays.asList(SeqStringUtil.transformToLong(roomIdSeq))));
   ...
   * @throws Exception
   */
  public ActionForward lastArrangeList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    // 查询重新加载的教学任务列表
   ...
   * @throws Exception
   */
  public ActionForward removeLastArrange(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String taskIds = request.getParameter(Constants.TEACHTASK_KEYSEQ);
    if (StringUtils.isEmpty(taskIds)) return forward(mapping, request, "error.teachTask.ids.needed");
    try {
