...
public class CourseGradeHelper {
  ...
  public void editGrade(HttpServletRequest request) {
    ...
    if (Boolean.FALSE.equals(courseGrade.getIsPass())) {
      ...
      if (!addedTypes.contains(new GradeType(GradeType.DELAY)) && !courseGrade.hasGrade(GradeType.DELAY)
          && !courseGrade.hasGrade(GradeType.MAKEUP)) {
        addedTypes.add((GradeType) baseCodeService.getCode(GradeType.class, GradeType.MAKEUP));
      }
    }
  ...
  public void saveGrade(HttpServletRequest request, User user) {
    ...
    courseGrade.setCourseType((CourseType) baseCodeService.getCode(CourseType.class, courseTypeId));
    Long courseTakeTypeId = RequestUtils.getLong(request, "courseGrade.courseTakeType.id");
    courseGrade.setCourseTakeType((CourseTakeType) baseCodeService.getCode(CourseTakeType.class,
        courseTakeTypeId));
    ...
    // 添加用户添加的其他考试成绩
    Long[] addGradeTypeIds = SeqStringUtil.transformToLong(request.getParameter("addGradeTypeIds"));
    if (null != addGradeTypeIds && addGradeTypeIds.length != 0) {
      for (int i = 0; i < addGradeTypeIds.length; i++) {
        ...
        if (null != score) {
          ...
          examGrade.setGradeType((GradeType) baseCodeService.getCode(GradeType.class, addGradeTypeIds[i]));
  ...
  public String removeTaskGrade(HttpServletRequest request, User user) throws Exception {
    ...
    GradeType FinalGradeType = (GradeType) baseCodeService.getCode(GradeType.class, GradeType.FINAL);
    // 发布的成绩不能修改或删除
    if (state.isPublished(GAGradeType) || state.isPublished(FinalGradeType)) { return "error.grade.modifyPublished"; }
