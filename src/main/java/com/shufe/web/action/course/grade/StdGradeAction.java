...
public class StdGradeAction extends StdGradeSearchAction {
   ...
   * @throws Exception
   */
  public ActionForward batchAdd(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    addCollection(request, "gradeTypes", utilService.search(queryGradeType(request)));
   ...
   * @return
   */
  protected EntityQuery queryGradeType(HttpServletRequest request) {
    ...
    // 屏蔽不需要的成绩类型
    String filterGradeTypeIdSeq = get(request, "filterGradeTypeIds");
    if (StringUtils.isNotBlank(filterGradeTypeIdSeq)) {
      query.add(new Condition("gradeType.id not in (:gradeTypeIds)", SeqStringUtil
          .transformToLong(filterGradeTypeIdSeq)));
    }
    return query;
  }
   ...
   * @return
   */
  private EntityQuery buildGradeTypeQuery(HttpServletRequest request) {
    ...
    EntityQuery query = queryGradeType(request);
  ...
  public ActionForward addGrade(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    addCollection(request, "stdTypeList", getStdTypes(request));
    EntityQuery query = buildGradeTypeQuery(request);
  ...
  public ActionForward batchEdit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isEmpty(courseGradeIdSeq)) { return forwardError(mapping, request,
        "error.parameters.needed"); }
    List grades = utilService.load(CourseGrade.class, "id", SeqStringUtil.transformToLong(courseGradeIdSeq));
   ...
   * @throws Exception
   */
  public ActionForward saveBatchEdit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String courseGradeIdSeq = request.getParameter("courseGradeIds");
    if (StringUtils.isEmpty(courseGradeIdSeq)) { return forwardError(mapping, request,
        "error.parameters.needed"); }
   ...
   * @throws Exception
   */
  public ActionForward saveAddGrade(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    List list = (List) utilService.search(buildGradeTypeQuery(request));
    for (Iterator it = list.iterator(); it.hasNext();) {
      GradeType gradeType = (GradeType) it.next();
      Long gradeTypeId = gradeType.getId();
      Float score = getFloat(request, "gradeType" + gradeTypeId + "" + grade.getMarkStyle().getId());
    ...
    grade.calcGA().calcScore().updatePass()
        .calcGP(gradePointRuleService.getGradePointRule(std.getType(), grade.getMarkStyle()));
  ...
  public ActionForward removeGrade(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    courseTakeQuery.add(new Condition("instr('," + courseGradeIdSeq + ",',','||take.courseGrade.id||',')>0"));
  ...
  public Object[] getCourseGradeInfo(String stdCode, Long stdTypeId, String calendarYear,
    ...
    TeachCalendar calendar = teachCalendarService.getTeachCalendar(stdTypeId, calendarYear, calendarTerm);
    if (null == calendar) return gradeInfo;
    ...
    if (null != grade.getStd()) {
      ...
      gradeInfo[6] = grade.getCourseType().getId();
      gradeInfo[7] = grade.getCourseTakeType().getName();
      if (null != grade.getCourseTake()) gradeInfo[8] = grade.getCourseTake().getId();
    }
    return gradeInfo;
  }
  ...
  public ActionForward batchSaveCourseGrade(ActionMapping mapping, ActionForm form,
    ...
    if (null == stdCount || stdCount.intValue() < 1) { return forwardError(mapping, request,
        "error.parameters.illegal"); }
    User who = getUser(request.getSession());
    TeachCalendar calendar = teachCalendarService.getTeachCalendar(
        getLong(request, "calendar.studentType.id"), request.getParameter("calendar.year"),
    ...
    for (int i = 0; i < stdCount.intValue(); i++) {
      ...
      CourseGrade grade = gradeService.getCourseGrade(stdCode, calendar, courseCode, gradeTypeId);
      Long courseTypeId = getLong(request, "courseType" + i);
      if (courseTypeId != null && courseTypeId != 0) {
        grade.setCourseType((CourseType) baseCodeService.getCode(CourseType.class, courseTypeId));
      }
      // 学生不存在
      if (null == grade.getStd()) {
        continue;
