...
public class ManualArrangeAction extends CalendarRestrictionSupportAction {
  ...
  public ActionForward index(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    TeachCalendar teachCalendar = getTeachCalendar(request);
  ...
  protected TeachCalendar getTeachCalendar(HttpServletRequest request) {
    ...
    if (null == calendar) {
      calendar = teachCalendarService.getTeachCalendar(getLong(request, "calendar.id"));
    }
    if (null == calendar) {
      calendar = teachCalendarService.getTeachCalendar(getLong(request, "task.calendar.id"));
    }
    return calendar;
  }
  ...
  public ActionForward willAndBeenArrange(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    // 院系排课需要此步
    if (isTimeOver(request, getTeachCalendar(request), null)) { return redirect(request, "search",
  ...
  public ActionForward updateTeachTask(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    // 院系排课需要此步
    Collection<TeachTask> tasks = utilService.load(TeachTask.class, "id", taskIds);
    if (isTimeOver(request, getTeachCalendar(request), tasks)) { return redirect(request, "search",
        "info.action.failure.timeover"); }
  ...
  public ActionForward saveActivities(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (count.intValue() == 0) {
      try {
        ...
        tasks.add(new TeachTask(taskId));
        if (isTimeOver(request, getTeachCalendar(request), tasks)) { return redirect(request, "search",
            "info.action.failure.timeover"); }
      ...
      Collection<TeachTask> tasks = new ArrayList<TeachTask>();
      tasks.add(task);
      if (isTimeOver(request, task.getCalendar(), tasks)) { return redirect(request, "search",
          "info.action.failure.timeover"); }
      /* 请暂不要删除 */
      ...
      for (Iterator iter = mergedActivityList.iterator(); iter.hasNext();) {
        ...
        if (null != one.getRoom() && Boolean.TRUE.equals(room.getIsCheckActivity())
            && teachResourceService.isRoomOccupied(one.getTime(), one.getRoom().getId(), task)) { return forward(
            request, "activityCollision"); }
      }
    ...
    // FIXME 2013-12-06 zhouqi 不明白为什么会有这一步？
    // if (null != task && Boolean.TRUE.equals(task.getElectInfo().getIsElectable())) {
    // return redirect(request, "editTask", "info.save.success", "&taskId=" + task.getId()
    // + "&params=" + get(request, "params") + "&forward=" + forward);
    // }

    return forwardInSave(mapping, request, forward);
  }
  ...
  protected ActionForward forwardInSave(ActionMapping mapping, HttpServletRequest request, String forward)
    ...
    if (StringUtils.isEmpty(forward)) {
      ...
      return redirect(request, isTaskList ? "taskList&order=Y" : "search&order=Y", "info.save.success");
    } else {
  ...
  public ActionForward editTask(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    request.setAttribute("task", utilService.load(TeachTask.class, taskId));
    return forward(request);
  }
  ...
  public ActionForward saveTask(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (CollectionUtils.isEmpty(task.getArrangeInfo().getActivities())) { return redirect(request,
        "taskList&order=Y", "info.save.success"); }
    ...
    String forward = get(request, "forward");
    if (StringUtils.isEmpty(forward)) {
      return redirect(request, "taskList&order=Y", "info.save.success");
    } else {
      saveErrors(request.getSession(), ForwardSupport.buildMessages(new String[] { "info.save.success" }));
      return mapping.findForward(forward);
    }
  ...
  public ActionForward shift(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null != offset) {
      List<TeachTask> tasks = teachTaskService.getTeachTasksByIds(taskIds);
  ...
  public ActionForward removeArrangeResult(ActionMapping mapping, ActionForm form,
    ...
    String taskIds = get(request, Constants.TEACHTASK_KEYSEQ);
    if (StringUtils.isEmpty(taskIds)) { return forward(request, new Action("", "taskList&order=Y"),
        "error.teachTask.id.needed"); }

    // 院系排课需要此步
    if (isTimeOver(request, getTeachCalendar(request),
        utilService.load(TeachTask.class, "id", SeqStringUtil.transformToLong(taskIds)))) { return redirect(
        request, "search", "info.action.failure.timeover"); }
    ...
    return redirect(request, (null == getBoolean(request, "task.arrangeInfo.isArrangeComplete")) ? "search"
        : "taskList&order=Y", "info.action.success");
  }
  ...
  public ActionForward freeRoomList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (Boolean.FALSE.equals(detectCollision)) {
      ...
      Results.addPagination("classroomList", teachResourceService.getFreeRoomsOf(
          ...
          (TimeUnit[]) timeList.toArray(new TimeUnit[timeList.size()]), room, Activity.class,
  ...
  public ActionForward shiftCalendar(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    // 院系排课需要此步
    if (isTimeOver(request, calendar, null)) { return redirect(request, "search",
        "info.action.failure.timeover"); }
  ...
  public ActionForward changeClassroom(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (isTimeOver(request, activity.getCalendar(), tasks)) { return redirect(request, "search",
        "info.action.failure.timeover"); }
    ...
    if (StringUtils.equals(requestURI, "courseTakeForTask")) { return redirect(request, new Action(
        "courseTakeForTask", "taskList&order=Y"), "info.save.success"); }
    return redirect(request, null == getBoolean(request, "task.arrangeInfo.isArrangeComplete") ? "search"
        : "taskList&order=Y", "info.save.success");
  }
