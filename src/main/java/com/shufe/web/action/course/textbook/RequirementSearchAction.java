...
public class RequirementSearchAction extends CalendarRestrictionSupportAction {
  ...
  public EntityQuery buildRequireQuery(HttpServletRequest request) {
    ...
    if (StringUtils.isNotEmpty(checkStatus)) {
      query.add(new Condition(
          "require.checked " + ("null".equals(checkStatus) ? "is null" : ("= " + checkStatus))));
    }
    ...
    Boolean matchWithCourse = getBoolean(request, "matchWithCourse");
    if (null != matchWithCourse) {
      if (matchWithCourse) {
        query.add(new Condition("require.textbook in elements(require.task.course.textbooks)"));
      } else {
        query.add(new Condition("require.textbook not in elements(require.task.course.textbooks)"));
      }
    }
  ...
  protected EntityQuery buildUnrequirementQuery(HttpServletRequest request) {
    ...
    query.add(new Condition("task.teachClass.stdType in (:stdTypes)", getStdTypesOf(stdTypeId, request)));
    DataRealmUtils.addDataRealms(query,
