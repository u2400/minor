...
public class CourseTableAction extends CalendarRestrictionSupportAction {
   ...
   * @throws Exception
   */
  public ActionForward stdHome(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    User user = getUser(request.getSession());
    Student std = studentService.getStudent(user.getName());
    if (null == std) { return forward(mapping, request, "error.std.stdNo.needed", "error"); }
    ...
    if (null == calendar) { return forward(mapping, request, "error.calendar.notMaded", "error"); }
    ...
    // 查询已有的课程
    List taskList = teachTaskService.getTeachTasksByCategory(std.getId(), TeachTaskFilterCategory.STD,
        teachCalendarService.getTeachCalendarsOfOverlapped(calendar));
    if (rs.isEmpty()) {
   ...
   * @throws Exception
   */
  public ActionForward teacherHome(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    request.setAttribute(Constants.CALENDAR,
        teachCalendarService.getTeachCalendar((StudentType) stdTypeList.iterator().next()));
   ...
   * @throws Exception
   */
  public ActionForward managerHome(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
  ...
  public ActionForward publicHome(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    addCollection(request, CalendarRestrictionSupportAction.CALENDAR_STDTYPES_KEY, calendarStdTypes);
   ...
   * @throws Exception
   */
  public ActionForward taskTable(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
  ...
  protected void putUserCategory(HttpServletRequest request) {
    ...
    Boolean isAdmin = Boolean.FALSE, isStudent = Boolean.FALSE;
    ...
    if (null != categories) {
      ...
      while (it.hasNext()) {
        ...
        if (temp.getId().equals(EamsRole.STD_USER)) {
   ...
   * @throws Exception
   */
  public ActionForward courseTableOfTask(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    Collections.sort(adminClasses, new MultiPropertyComparator("department.code,aspect.code,code"));
  ...
  public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isEmpty(kind)) return forward(mapping, request, "error.courseTable.unknown", "error");
    request.setAttribute(Constants.CALENDAR,
   ...
   * @throws Exception
   */
  public ActionForward courseTable(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    // 查找课表设置参数
    TeachCalendar calendar = getCalendar(request);
    if (calendar == null) { return forwardError(mapping, request, "error.calendar.id.notExists"); }
    ...
    if (StringUtils.isEmpty(setting.getKind())) return forward(mapping, request, "error.courseTable.unknown",
        "error");
    // 查找课表对象
    ...
    EntityQuery entityQuery = new EntityQuery(CourseTable.getResourceClass(setting.getKind()), "resource");
    ...
    Collections.sort(resources,
        new PropertyComparator(order.getProperty(), order.getDirection() == Order.ASC));
    ...
    List switches = utilService.load(CourseArrangeSwitch.class, "calendar.id", calendar.getId());
  ...
  public ActionForward classroomActivityStat(ActionMapping mapping, ActionForm form,
    ...
    for (Iterator it = activities.iterator(); it.hasNext();) {
      ...
      for (int i = activity.getTime().getStartUnit().intValue(); i <= activity.getTime().getEndUnit()
          .intValue(); i++) {
   ...
   * @throws Exception
   */
  public ActionForward roomActivityDetail(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
  ...
  protected void configExportContext(HttpServletRequest request, Context context) {
    ...
    context.put("calendar", teachCalendarService.getTeachCalendar(getLong(request, "calendar.id")));
    Map map = new HashMap();
    int i = 1;
    for (Iterator it = activities.iterator(); it.hasNext();) {
  ...
  private CourseTable buildCourseTable(CourseTableSetting setting, Entity resource) {
    ...
    if (CourseTable.CLASS.equals(setting.getKind())) {
      for (int j = 0; j < setting.getTimes().length; j++) {
        table.addActivities(teachResourceService.getAdminClassActivities(resource.getEntityId(),
      ...
      try {
        ...
        TermCalculator calc = new TermCalculator(teachCalendarService, setting.getCalendar());
        int term = calc.getTermBetween(time.getEnrollCalendar(), setting.getCalendar(), Boolean.TRUE);
        ...
        if (plan != null) {
          for (Iterator iter = plan.getCourseGroups().iterator(); iter.hasNext();) {
            ...
            for (Iterator it2 = group.getPlanCourses(String.valueOf(term)).iterator(); it2.hasNext();) {
      ...
      for (Iterator iter = taskList.iterator(); iter.hasNext();) {
        ...
        if (MapUtils.isNotEmpty(courseTypeMap) && null != courseTypeMap.get(task.getCourse())) {
      ...
      for (int j = 0; j < setting.getTimes().length; j++) {
        table.addActivities(teachResourceService.getTeacherActivities(resource.getEntityId(), setting
            .getTimes()[j], CourseActivity.class, setting.getForCalendar() ? setting.getCalendar() : null));
      }
      //
      if (setting.getIgnoreTask()) { return table; }
      if (setting.getForCalendar()) {
        ...
        taskList = teachTaskService.getTeachTasksByCategory(resource.getEntityId(),
            TeachTaskFilterCategory.TEACHER,
            teachCalendarService.getTeachCalendarsOfOverlapped(setting.getCalendar()));
      }
      Teacher teacher = (Teacher) utilService.load(Teacher.class, "id", resource.getEntityId()).get(0);
      ...
      for (int j = 0; j < setting.getTimes().length; j++) {
        table.addActivities(teachResourceService.getStdActivities((Long) resource.getEntityId(), setting
            .getTimes()[j], CourseActivity.class, setting.getForCalendar() ? setting.getCalendar() : null));
      }
      if (setting.getIgnoreTask()) return table;
      if (setting.getForCalendar()) {
      ...
      for (int j = 0; j < setting.getTimes().length; j++) {
        table.addActivities(teachResourceService.getRoomActivities(resource.getEntityId(),
            setting.getTimes()[j], CourseActivity.class, setting.getForCalendar() ? setting.getCalendar()
                : null));
      }
      if (setting.getIgnoreTask()) return table;
      // 教室的教学任务列表从教学活动中抽取
    ...
    if (null == table.getTasks()) table.setTasks(taskList);
