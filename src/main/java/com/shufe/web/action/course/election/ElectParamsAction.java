...
public class ElectParamsAction extends CalendarRestrictionSupportAction {
  ...
  public ActionForward edit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    Collection<Speciality> majors = specialityService.getSpecialities(new MajorType(MajorType.FIRST));
    addCollection(request, "majors", majors);
    addCollection(request, "majorFields", utilService.load(SpecialityAspect.class, "speciality", majors));
    initBaseCodes(request, "courseTypes", CourseType.class);
    initBaseCodes(request, "studentStates", StudentState.class);
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null == params.getCalendar()) {
      params.setCalendar(teachCalendarService.getTeachCalendar(getLong(request, "calendar.id")));
    }
    ...
    try {
      ...
      return forwardError(mapping, request, new String[] { "entity.electParams", "error.model.existed" });
    }
  ...
  private void setCollectionFields(HttpServletRequest request, ElectParams params) {
    ...
    params.getDeparts().addAll(
        utilService.load(Department.class, "id", SeqStringUtil.transformToLong(get(request, "departIds"))));
    ...
    params.getEnrollTurns().addAll(Arrays.asList(StringUtils.split(get(request, "enrollTurns"), ",")));
    params.getMajors().clear();
    params.getMajors().addAll(
        utilService.load(Speciality.class, "id", SeqStringUtil.transformToLong(get(request, "majorIds"))));
    params.getMajorFields().clear();
    params.getMajorFields().addAll(
        utilService.load(SpecialityAspect.class, "id",
            SeqStringUtil.transformToLong(get(request, "majorFieldIds"))));

    params.getStudentStates().clear();
    params.getStudentStates().addAll(utilService.load(StudentState.class, "id", SeqStringUtil.transformToLong(get(request, "studentStateIds"))));
    
    params.getStds().clear();
    String stdCodes = get(request, "stdCodes");
    if (StringUtils.isNotBlank(stdCodes)) {
      params.getStds().addAll(utilService.load(Student.class, "code", StringUtils.split(stdCodes, ",")));
    }
    params.getNotCurrentCourseTypes().clear();
    params.getNotCurrentCourseTypes()
        .addAll(
            utilService.load(CourseType.class, "id",
                SeqStringUtil.transformToLong(get(request, "courseTypeIds"))));
  ...
  public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    addCollection(request, Constants.ELECTPARAMS_LIST, paramsService.getElectParams(getStdTypeIdSeq(request),
        (TeachCalendar) teachCalendarService.getTeachCalendar(getLong(request, "electParams.calendar.id"))));
    return forward(request);
  }
  ...
  public ActionForward remove(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null == paramsIds || paramsIds.length == 0) { return forwardError(mapping, request,
        "error.electParams.id.needed"); }
    utilService.remove(utilService.load(ElectParams.class, "id", paramsIds));
    String calendarId = get(request, "electParams.calendar.id");
    return redirect(request, "list", "info.delete.success", "&electParams.calendar.id=" + calendarId);
  }
  ...
  public ActionForward info(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    request.setAttribute(Constants.ELECTPARAMS,
        utilService.load(ElectParams.class, getLong(request, "paramsId")));
    return forward(request);
  }
