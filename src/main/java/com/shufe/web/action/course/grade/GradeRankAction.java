...
public class GradeRankAction extends CalendarRestrictionSupportAction {
  ...
  public ActionForward list(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    // 查询统计
    list = gradeService.getGradeRank(courseSeq, courseName, yearfrom, termfrom, yearto, termto, searchType,
        departmentId);
    Map gradeForStudentCountMap = new MultiValueMap();
    if (null == departmentId) {
      gradeForStudentCountMap.put("全校", list);
    } else {
      department = (Department) utilService.searchHQLQuery("from Department dd where dd.id=" + departmentId)
          .get(0);
      gradeForStudentCountMap.put(department.getName(), list);
    }
    // 查询非正常考试学生
    Collection stdWithExamStatusList = gradeService.getWithExamStatus(courseSeq, courseName, yearfrom,
        termfrom, yearto, termto, searchType, departmentId, request);
  ...
  public ActionForward search(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    String searchType = get(request, "searchType");
    Long departmentId = getLong(request, "college.id");
    Collection stdWithExamIsNotPassList = gradeService.getWithExamIsNotPass(courseSeq, courseName, yearfrom,
        termfrom, yearto, termto, searchType, departmentId, request);
