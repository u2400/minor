...
public abstract class DispatchBasicAction extends DispatchActionSupport implements Constants {
  ...
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    try {
      ...
      if (null == request.getSession().getServletContext().getAttribute(SystemConfig.SYSTEM_CONFIG)) {
        request.getSession().getServletContext().setAttribute(SystemConfig.SYSTEM_CONFIG,
            SystemConfigLoader.getConfig());
      }
      request.getSession().setAttribute("nowDate", getNowDate());
      ...
      if (e3 instanceof AuthenticationException) { throw e3; }
      e3.printStackTrace();
  ...
  protected String getMethodName(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isEmpty(method)) { return Conventions.getProfile(this.getClass()).getDefaultMethod(); }
    return method;
  }
  ...
  protected ActionForward forward(ActionMapping mapping, HttpServletRequest request, String forwardTag,
   ...
   * @return
   */
  protected ActionForward forward(ActionMapping mapping, HttpServletRequest request, String errorKey,
      String forwardTag) {
    ActionMessages actionMessages = new ActionMessages();
    actionMessages.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage(errorKey));
    addErrors(request, actionMessages);
  ...
  protected ActionForward getRedirectForward(ActionMapping mapping, HttpServletRequest request,
    ...
    return getRedirectForwardWithParams(mapping, request, new String[] { messageKey }, null, forwardTag);
  }
   ...
   * @return
   */
  protected ActionForward getRedirectForwardWithParams(ActionMapping mapping, HttpServletRequest request,
      String messageKey, String params, String forwardTag) {
    return getRedirectForwardWithParams(mapping, request, new String[] { messageKey }, params, forwardTag);
  }
   ...
   */
  protected ActionForward getRedirectForwardWithParams(ActionMapping mapping, HttpServletRequest request,
      String[] messageKeys, String params, String forwardTag) {
   ...
   * @return
   */
  protected ActionForward forward(ActionMapping mapping, HttpServletRequest request, String[] errorKeys,
      String forwardTag) {
    ActionMessages actionMessages = new ActionMessages();
    for (int i = 0; i < errorKeys.length; i++) {
      actionMessages.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage(errorKeys[i]));
  ...
  protected boolean checkToken(javax.servlet.http.HttpServletRequest request, ActionMessages actionMessages) {
    if (isTokenValid(request)) {
      resetToken(request);
      return true;
    } else {
      actionMessages.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage("error.invaild.token"));
  ...
  protected User getUser(HttpSession session) {
    return (User) utilService.get(User.class, (Long) session.getAttribute(Authentication.USERID));
  }

  protected User getUser(HttpServletRequest request) {
    return (User) utilService.get(User.class,
        (Long) request.getSession().getAttribute(Authentication.USERID));
  }
  ...
  protected Student getStudentFromSession(HttpSession session) {
    List stds = utilService.load(Student.class, "code", getLoginName(session));
    if (stds.isEmpty()) {
      return null;
    } else {
   ...
   * @throws Exception
   */
  protected void outputExcelFile(HttpServletRequest request, HttpServletResponse response, List list,
   ...
   */
  protected void outputExcelFile(HttpServletRequest request, HttpServletResponse response, List list,
      String objectName, PropertyExtractor extracor) throws Exception {
  ...
  protected Collection getExportDatas(HttpServletRequest request) {
    ...
    if ("nomal".equals(kind)) {
      list = gradeService.getWithExamStatus(courseSeq, courseName, yearfrom, termfrom, yearto, termto,
          searchType, departmentId, null);
    } else if ("nopass".equals(kind)) {
      list = gradeService.getWithExamIsNotPass(courseSeq, courseName, yearfrom, termfrom, yearto, termto,
          searchType, departmentId, null);
  ...
  public static void populate(Map params, Entity entity) {
    if (null == entity) { throw new RuntimeException("Cannot populate to null."); }
