...
public class StudentAuditOperation extends StudentAuditSupportAction {
   ...
   * @throws Exception
   */
  public ActionForward batchAuditSecond(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long[] stdId = SeqStringUtil.transformToLong(request.getParameter("stdIds"));
    studentService.batchUpdateSecondGraduateAuditStatus(stdId,
  ...
  public ActionForward batchAutoSecondSpecialityAudit(ActionMapping mapping, ActionForm form,
    ...
    studentService.batchUpdateGraduateAuditStatus(unParseStudent, majorType, new Boolean(false));
    return redirect(request, new Action("secondSpecialityStudentAuditManager", "search"),
        "info.action.success");
  }
  ...
  public ActionForward detail(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (studentObject == null) { throw new RuntimeException("No such student!"); }
    ...
    try {
      getStudentTeachPlanAuditDetail(student, majorType, auditStandardId, request.getParameter("auditTerm"),
          null, Boolean.TRUE);
  ...
  public ActionForward changeCourseType(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
  ...
  public ActionForward saveCourseType(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    List grades = utilService.load(CourseGrade.class, new String[] { "std.id", "course.id" }, new Object[] {
        studentId, courseId });
   ...
   * @return
   */
  public ActionForward batchDetail(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
    ...
    if (CollectionUtils.isEmpty(stdList)) { throw new RuntimeException("No such student!"); }
    Long auditStandardId = null;
    if (NumberUtils.isNumber(request.getParameter("auditStandardId"))) {
      auditStandardId = Long.valueOf(request.getParameter("auditStandardId"));
    }
    ...
    for (Iterator iter = stdList.iterator(); iter.hasNext();) {
      ...
      try {
        resultList.addAll(graduateAuditService.getStudentTeachPlanAuditDetail(student, majorType,
            auditStandardId, graduateAuditService.getAuditTermList(request.getParameter("auditTerm")), null,
            Boolean.TRUE));
      } catch (PojoNotExistException e) {
        if (e.getMessage() != null && e.getMessage().equals(com.shufe.model.Constants.TEACHPLAN)) { return this
            .forwardError(mapping, request, "error.teachPlan.notExists"); }
   ...
   * @return
   */
  public ActionForward updateResultDetail(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
    // FIXME
    ...
    Results.addObject("auditResult", utilService.load(AuditResult.class, "stdNo", stdNo).get(0));
