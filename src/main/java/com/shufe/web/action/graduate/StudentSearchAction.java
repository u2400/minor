...
public class StudentSearchAction extends CalendarRestrictionSupportAction {
  ...
  protected EntityQuery buildQuery(HttpServletRequest request) {
    ...
    if (null != studenGraduateAuditStatus) {
      query.add(new Condition("std.graduateAuditStatus =:studenGraduateAuditStatus", Boolean
          .valueOf(studenGraduateAuditStatus)));
    }
   ...
   * @return
   */
  protected EntityQuery getStdQuery(HttpServletRequest request, String moduleName) {
    EntityQuery entityQuery = new EntityQuery(Student.class, "student");
    if (StringUtils.isNotEmpty(moduleName)) {
      entityQuery.add(new Condition("student.department in (:departDataRealm)", getDeparts(request)));
      entityQuery.add(new Condition("student.type in (:departDataRealm)", getStdTypes(request)));
    }
    ...
    if (adminClasssId1 != null && adminClasssId2 != null) {
      entityQuery.add(new Condition("exists (select class" + adminClasssId1 + " from AdminClass class"
          + adminClasssId1 + " join class" + adminClasssId1
          + ".students std where std.id=student.id and class" + adminClasssId1 + ".id = :classId"
          + adminClasssId1 + ")" + " and exists (select class" + adminClasssId2 + " from AdminClass class"
          + adminClasssId2 + " join class" + adminClasssId2
          + ".students std where std.id=student.id and class" + adminClasssId2 + ".id = :classId"
          + adminClasssId2 + ")", adminClasssId1, adminClasssId2));
  ...
  private void addAdminClassCondtion(EntityQuery entityQuery, Long adminClasssId) {
    entityQuery.add(new Condition("exists (select class" + adminClasssId + " from AdminClass class"
        + adminClasssId + " join class" + adminClasssId + ".students std where std.id=id and class"
        + adminClasssId + ".id = :classId" + adminClasssId + ")", adminClasssId));
  }
  ...
  protected void searchStudentWith2ndSpeciality(ActionForm form, HttpServletRequest request,
      String moduleName, Boolean is2ndMajor, String andOr) {
    String departmentIds = getDepartmentIdSeq(request);
    String studentTypeIds = getStdTypeIdSeq(request);
    initSearchStudentBar(request, departmentIds, studentTypeIds);
    ...
    if (StringUtils.isNotEmpty(searchFalg) && searchFalg.equals("search")) {
      ...
      if (Boolean.TRUE.equals(is2ndMajor)) {
        Collection stds = studentService.search2ndSpecialityStudent(std, new PageLimit(pageNo, pageSize),
        ...
        stds = studentService.searchStudent(std, pageNo, pageSize, studentTypeIds, departmentIds);
        Results.addObject("studentList", stds);
      } else {
        Collection stds = studentService.search2ndSpecialityStudent(std, new PageLimit(pageNo, pageSize),
            studentTypeIds, departmentIds, departmentIds, StringUtils.isBlank(andOr) ? "or" : andOr);
        addCollection(request, "studentList", stds);
      }
   ...
   */
  protected void initSearchStudentBar(HttpServletRequest request, String departmentIds, String studentTypeIds) {
   ...
   * @param moduleName
   */
  protected void searchAdminClass(ActionForm form, HttpServletRequest request, String moduleName) {
    String departmentIds = getDepartmentIdSeq(request);
    String studentTypeIds = getStdTypeIdSeq(request);
    initSearchStudentBar(request, departmentIds, studentTypeIds);
    String searchFalg = request.getParameter("searchFalg");
  ...
  protected void searchTeachTask(ActionForm form, HttpServletRequest request) throws Exception {
    ...
    if (obj != null) {
      Results.addList("courseTypeList",
          teachTaskService.getCourseTypesOfTask(studentTypeIds, departmentIds, (TeachCalendar) obj));
      ...
      Results.addList("teacherList",
          teachTaskService.getTeachersOfTask(studentTypeIds, departmentIds, (TeachCalendar) obj));

      String searchFalg = request.getParameter("searchFalg");
      if (StringUtils.isNotEmpty(searchFalg) && searchFalg.equals("search")) {
        int pageNo = QueryRequestSupport.getPageNo(request);
        int pageSize = QueryRequestSupport.getPageSize(request);
        TeachTask task = (TeachTask) RequestUtil.populate(request, TeachTask.class, "teachTask");
        Teacher teacher = (Teacher) RequestUtil.populate(request, Teacher.class, Constants.TEACHER);
        task.getArrangeInfo().getTeachers().add(teacher);
        /*-------------获得学年度学期信息--------------------*/
        Long stdTypeId = getLong(request, "calendar.studentType.id");
        task.getTeachClass().setStdType((StudentType) EntityUtils.getEntity(StudentType.class, stdTypeId));
        if (task.getCalendar().getId() == null) {
          ...
          if (StringUtils.isNotEmpty(year) && StringUtils.isNotEmpty(term)) {
            calendar = getTeachCalendarService().getTeachCalendar(new StudentType(stdTypeId), year, term);
        ...
        if (task.getArrangeInfo().getTeachDepart().isVO()) {
          ...
          tasks = teachTaskService.getTeachTasksOfTeachDepart(task, getStdTypeIdSeqOf(stdTypeId, request),
              departIds, pageNo, pageSize);
        } else tasks = teachTaskService.getTeachTasks(task, pageNo, pageSize);
  ...
  public Map getStdBeanMap(Map stdNoMap, HttpServletRequest request) {
    ...
    if (ArrayUtils.isEmpty(stdCodeArray)) { return Collections.EMPTY_MAP; }
    String departmentIds = getDepartmentIdSeq(request);
    String studentTypeIds = getStdTypeIdSeq(request);
    ...
    if (stdList.size() == 0) {
      ...
      for (Iterator iter = stdList.iterator(); iter.hasNext();) {
        ...
        if ((departmentIds.indexOf(String.valueOf(std.getDepartment().getId())) == -1)
            || (studentTypeIds.indexOf(String.valueOf(std.getType().getId())) == -1)) {
          continue;
        } else {
          ...
          stdMap.put("student.firstMajor.name", (null == std.getFirstMajor()) ? null : std.getFirstMajor()
              .getName());
          stdMap.put("student.firstMajor.engName", (null == std.getFirstMajor()) ? null : std.getFirstMajor()
              .getEngName());
          stdMap.put("student.firstAspect.name", (null == std.getFirstAspect()) ? null : std.getFirstAspect()
              .getName());
          stdMap.put("student.firstAspect.engName", (null == std.getFirstAspect()) ? null : std
              .getFirstAspect().getEngName());
          stdMap.put("student.basicInfo.homeAddress", (null == std.getBasicInfo().getHomeAddress()) ? null
              : std.getBasicInfo().getHomeAddress());
          stdMap.put("student.basicInfo.postCode", (null == std.getBasicInfo().getPostCode()) ? null : std
              .getBasicInfo().getPostCode());
          stdMap.put("student.basicInfo.phone", (null == std.getBasicInfo().getPhone()) ? null : std
