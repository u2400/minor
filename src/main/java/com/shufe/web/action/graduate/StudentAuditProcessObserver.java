...
public class StudentAuditProcessObserver extends OutputProcessObserver {
  ...
  private static class NullObserver extends StudentAuditProcessObserver {
     ...
     * （非 Javadoc）
     * @see
     * com.shufe.web.action.graduate.StudentAuditProcessObserver#addNoTeachPlanError(java.lang.String
     * ,
     ...
     * （非 Javadoc）
     * @see
     * com.shufe.web.action.graduate.StudentAuditProcessObserver#addStdOutputNotify(java.lang.String
     * ,
     * java.lang.String)
     */
  ...
  public void addNoTeachPlanError(String code, String remark, Long stdId) {
    try {
      writer.print("<script>addNoTeachPlanError('" + code + "','" + remark + "'," + stdId + ");</script>\n");
      writer.flush();
    } catch (Exception e) {
      // TODO: handle exception
    }
  ...
  public void addResult(String code, boolean flag, Long id, MajorType majorType, Long auditStandardId,
      String terms) {
    try {
      writer.print("<script>addResult('" + code + "'," + flag + "," + id + "," + majorType.getId() + ","
          + auditStandardId + ",'" + terms + "');</script>\n");
      writer.flush();
    } catch (Exception e) {
      // TODO: handle exception
    }
  ...
  public void notifyFinish(int count, long time) throws IOException {
    outputNotify(OutputWebObserver.good, new OutputMessage("", "本次毕业审核结束。"), false);
    outputNotify(OutputWebObserver.good, new OutputMessage("", "共审核" + count + "位学生,耗时" + formatTime(time)),
        false);
  }
  ...
  public void outputNotify(int term, TeachPlan plan, String messageKey) throws IOException {
    OutputMessage message = new OutputMessage("", StringUtils.repeat("&nbsp;", 9) + messageOf(messageKey)
        + StringUtils.repeat("&nbsp;", 8) + new TeachCommon(plan).toString());
    outputNotify(OutputWebObserver.good, message);
  }
  ...
  public void outputNotify(Map parameterMap) throws IOException {
    ...
    if (student.getFirstMajor() != null && student.getFirstMajor().isPO()) {
      buf.append(messageOf("entity.firstSpeciality") + "[" + student.getFirstMajor().getName() + "] "
    ...
    if (student.getSecondMajor() != null && student.getSecondMajor().isPO()) {
      buf.append(messageOf("entity.secondSpeciality") + "[" + student.getSecondMajor().getName() + "] "
  ...
  public String teachPlanCompletedStatus(Object teachPlanCompletedStatus) {
    if (teachPlanCompletedStatus == null) {
      ...
      } else if (!((Boolean) teachPlanCompletedStatus).booleanValue()) { return messageOf("attr.graduate.noPass"); }
    }
    return null;
  }
  ...
  public StringBuffer formatTime(long time) {
    ...
    hour = newTemp;
    StringBuffer buf = new StringBuffer();
    buf.append(messageOf("common.time.useTime") + ":" + hour + messageOf("common.time.hour") + minute
        + messageOf("common.time.minute") + second + messageOf("common.time.second") + millis
        + messageOf("common.time.millis"));
    return buf;
  }
}
