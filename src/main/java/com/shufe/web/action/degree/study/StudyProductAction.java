...
public class StudyProductAction extends StudyProductHelper {
  ...
  private EntityQuery buildStudyProductQuery(HttpServletRequest request, String productType) {
    EntityQuery query = new EntityQuery(StudyProduct.getStudyProductType(productType), "studyProduct");
    ...
    DataRealmUtils.addDataRealms(query, new String[] { "studyProduct.student.type.id",
        "studyProduct.student.department.id" },
        getDataRealmsWith(getLong(request, "studyProduct.student.type.id"), request));
    // FIXME 上面一句代替下面的一句
    // DataRealmUtils.addDataRealms(query, new String[] { "studyProduct.student.type.id",
  ...
  public ActionForward batchEdit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if ("studyThesis".equals(productType)) {
      request.setAttribute("studyProductTypes",
          utilService.load(PublicationLevel.class, "state", Boolean.TRUE));
    } else if ("literature".equals(productType)) {
      request
          .setAttribute("studyProductTypes", utilService.load(LiteratureType.class, "state", Boolean.TRUE));
    } else if ("project".equals(productType)) {
      request.setAttribute("studyProductTypes", utilService.load(ProjectType.class, "state", Boolean.TRUE));
    } else if ("studyMeeting".equals(productType)) {
      request.setAttribute("studyProductTypes", utilService.load(MeetingType.class, "state", Boolean.TRUE));
    }
    request.setAttribute("productType", productType);
   ...
   * @throws Exception
   */
  public ActionForward batchSave(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String productIds = request.getParameter("studyProductIds");
    String productType = request.getParameter("productType");
    List products = utilService.load(StudyProduct.getStudyProductType(productType), "id",
   ...
   * @throws Exception
   */
  public ActionForward statisticProducts(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Map valueMap = RequestUtils.getParams(request, "studyProduct");
    request.setAttribute("publicTypes", utilService.load(PublicationLevel.class, "state", Boolean.TRUE));
    request.setAttribute("projectTypes", utilService.load(ProjectType.class, "state", Boolean.TRUE));
    request.setAttribute("literatureTypes", utilService.load(LiteratureType.class, "state", Boolean.TRUE));
