...
public class TopicOpenSearchAction extends RestrictionSupportAction {
  ...
  protected EntityQuery buildQuery(HttpServletRequest request) {
    ...
    query.getConditions().addAll(
        QueryRequestSupport.extractConditions(request, TopicOpen.class, "topicOpen", ""));
    query.getConditions().addAll(
        QueryRequestSupport.extractConditions(request, Student.class, "student", "student.type.id"));
    // 权限
    Long stdTypeId = RequestUtils.getLong(request, "student.type.id");
    Resource resource = getResource(request);
    if (!resource.getPatterns().isEmpty()) {
      DataRealmUtils.addDataRealms(query, new String[] { "thesisManager.student.type.id",
          "thesisManager.student.department.id" }, getDataRealmsWith(stdTypeId, request));
    } else {
      if (null != stdTypeId) {
        DataRealmUtils.addDataRealm(query, new String[] { "thesisManager.student.type.id" }, new DataRealm(
            studentTypeService.getStdTypeIdSeqUnder(stdTypeId), null));
      }
    }
    QueryRequestSupport.addDateIntervalCondition(request, query, "topicOpen", "finishOn", "beginOn", "endOn");
