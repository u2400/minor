...
public class PreAnswerSearchAction extends RestrictionSupportAction {
  ...
  protected EntityQuery buildPreAnswerQuery(HttpServletRequest request) {
    ...
    if (null != resource && !resource.getPatterns().isEmpty()) {
      ...
      if (null != stdTypeId) {
        DataRealmUtils.addDataRealm(query, new String[] { "student.type.id" }, new DataRealm(
            studentTypeService.getStdTypeIdSeqUnder(stdTypeId), null));
      }
    }
    QueryRequestSupport.addDateIntervalCondition(request, query, "finishOn", "beginOn", "endOn");
