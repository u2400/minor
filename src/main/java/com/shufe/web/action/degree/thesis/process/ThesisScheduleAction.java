...
public class ThesisScheduleAction extends RestrictionSupportAction {
  ...
  public ActionForward remove(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    List schedules = utilService.load(Schedule.class, "id", SeqStringUtil.transformToLong(scheduleIds));
    try {
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    EntityQuery query = new EntityQuery(Schedule.class, "schedule");
    query.add(new Condition("schedule.studentType.id = :stId", schedule.getStudentType().getId()));
    ...
    if (list.size() > 0) {
      for (Iterator iterator = list.iterator(); iterator.hasNext();) {
        ...
        if (null == schedule.getId() || !exists.getId().equals(schedule.getId())) { return forward(request,
            new Action("", "edit"), "error.code.existed"); }
      }
    }
   ...
   * @throws Exception
   */
  public ActionForward loadCopyPage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    if ("single".equals(copyType)) {
      request.setAttribute("schedule", utilService.load(Schedule.class, Long.valueOf(scheduleIds)));
      setDataRealm(request, hasStdType);
    }
  ...
  public ActionForward copy(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (collect.size() > 0) {
      ...
      request.setAttribute("copyErrors", temp.getStudentType().getName() + "/" + temp.getEnrollYear() + "/"
          + temp.getStudyLength());
   ...
   * @throws Exception
   */
  public ActionForward batchCopy(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String scheduleIds = request.getParameter("scheduleIds");
    List schedules = utilService.load(Schedule.class, "id", SeqStringUtil.transformToLong(scheduleIds));
  ...
  public ActionForward editTache(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    addCollection(request, "supportDocuments",
        CollectionUtils.subtract(allDocuments, tacheSetting.getThesisDocuments()));
    addCollection(request, "supportModels",
        CollectionUtils.subtract(allDocuments, tacheSetting.getThesisModels()));
   ...
   * @throws Exception
   */
  public ActionForward batchEditTache(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String batch = request.getParameter("batch");
    String scheduleId = request.getParameter("scheduleId");
    String tacheSettingIds = request.getParameter("tacheSettingIds");
    List tacheSettings = utilService.load(TacheSetting.class, "id",
        SeqStringUtil.transformToLong(tacheSettingIds));
    if ("load".equals(batch)) {
      ...
      for (int i = 0; i < tacheSettingId.length; i++) {
        ...
        tacheSetting.setPlanedTimeOn(df.parse(request.getParameter("planTime" + tacheSettingId[i])));
  ...
  public ActionForward removeTache(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String tacheSettingIds = request.getParameter("tacheSettingIds");
    List tacheSettings = utilService.load(TacheSetting.class, "id",
        SeqStringUtil.transformToLong(tacheSettingIds));
    try {
      ...
      return redirect(request, "info", "info.delete.failure",
   ...
   * @throws Exception
   */
  public ActionForward saveTache(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    query.add(new Condition("tacheSetting.tache.id = :tacheId", tacheSetting.getTache().getId()));
    query.add(new Condition("tacheSetting.schedule.id = :scheduleId", tacheSetting.getSchedule().getId()));
    if (tacheSetting.getId() != null) {
      query.add(new Condition("tacheSetting.id <> :id", tacheSetting.getId()));
    }
    List list = (List) utilService.search(query);
    if (list.size() > 0) { return forward(request, new Action("", "editTache"),
    ...
    if (StringUtils.isNotBlank(thesisDocumentIdSeq)) {
      thesisDocuments.addAll(utilService.load(DegreeDocument.class, "id",
          SeqStringUtil.transformToLong(thesisDocumentIdSeq)));
    }
    ...
    if (StringUtils.isNotBlank(thesisModelIdSeq)) {
      thesisModels.addAll(utilService.load(DegreeDocument.class, "id",
          SeqStringUtil.transformToLong(thesisModelIdSeq)));
    }
    tacheSetting.setThesisModels(thesisModels);
    utilService.saveOrUpdate(tacheSetting);
    return redirect(request, "info", "info.save.success", "&scheduleId=" + request.getParameter("scheduleId"));
  }
  ...
  protected Collection getExportDatas(HttpServletRequest request) {
    ...
    if (StringUtils.isNotBlank(scheduleIds)) {
      ...
      for (Iterator iter = params.keySet().iterator(); iter.hasNext();) {
        ...
        conditions.add(new Condition("tacheSetting." + attr + " like :" + attr.replace('.', '_'), "%"
            + (String) strValue + "%"));
      }
  ...
  /*
   * (non-Javadoc)
   * @see com.shufe.web.action.common.DispatchBasicAction#getPropertyExporter(javax.servlet.http.
   * HttpServletRequest)
   */
   ...
   * @throws Exception
   */
  public ActionForward doDetailInfo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long scheduleId = getLong(request, "scheduleId");
