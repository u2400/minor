...
public class AnnotateAdminAction extends RestrictionSupportAction {
   ...
   * @throws Exception
   */
  public ActionForward doStdList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String hasWrite = get(request, "hasWrite");

    EntityQuery query = new EntityQuery(ThesisManage.class, "thesisManage");
    List stdConditions = QueryRequestSupport.extractConditions(request, Student.class, "student",
    ...
    for (Iterator it = stdConditions.iterator(); it.hasNext();) {
      ...
      condition.setContent(StringUtils.replace(fieldPath, "student.", "thesisManage.student."));
    }
    query.getConditions().addAll(stdConditions);
    DataRealmUtils.addDataRealms(query, new String[] { "thesisManage.student.type.id",
        "thesisManage.student.department.id" },
        restrictionHelper.getDataRealmsWith(getLong(request, "student.type.id"), request));
    query.add(new Condition("thesisManage.annotate is " + (StringUtils.equals(hasWrite, "true") ? "not" : "")
        + " null"));
    List bookConditions = QueryRequestSupport.extractConditions(request, ThesisAnnotateBook.class,
        "annotateBook", null);
    ...
    if (null != finishOnStart) {
      query.add(new Condition("thesisManage.annotate.finishOn >= (:finishOnStart)", finishOnStart));
    }
    Date finishOnEnd = RequestUtils.getDate(request, "finishOnEnd");
    if (null != finishOnEnd) {
      query.add(new Condition("thesisManage.annotate.finishOn <= (:finishOnEnd)", finishOnEnd));
    }
   ...
   * @throws Exception
   */
  public ActionForward doAnnotateBooks(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    for (Iterator it = stdConditions.iterator(); it.hasNext();) {
      Condition condition = (Condition) it.next();
      String fieldPath = condition.getContent();
      condition.setContent(StringUtils.replace(fieldPath, "student.", "annotateBook.thesisManage.student."));
    }
    query.getConditions().addAll(stdConditions);
    String isHasMark = get(request, "isHasMark");
    DataRealmUtils.addDataRealms(query, new String[] { "annotateBook.thesisManage.student.type.id",
        "annotateBook.thesisManage.student.department.id" },
        restrictionHelper.getDataRealmsWith(getLong(request, "student.type.id"), request));
    ...
    Date finishOnStart = RequestUtils.getDate(request, "finishOnStart");
    if (null != finishOnStart) {
      query.add(new Condition("annotateBook.thesisManage.annotate.finishOn >= (:finishOnStart)",
          finishOnStart));
    }
    Date finishOnEnd = RequestUtils.getDate(request, "finishOnEnd");
    if (null != finishOnEnd) {
      query.add(new Condition("annotateBook.thesisManage.annotate.finishOn <= (:finishOnEnd)", finishOnEnd));
    }
    query.setLimit(getPageLimit(request));
    query.addOrder(OrderUtils.parser(get(request, "orderBy")));
   ...
   * @throws Exception
   */
  public ActionForward doInsertIntoMark(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    if ("load".equals(isInsert)) {
      List annotateBooks = utilService.load(ThesisAnnotateBook.class, "id",
          SeqStringUtil.transformToLong(annotateBookIdSeq));
   ...
   * @throws Exception
   */
  public ActionForward doCalendarAvgMark(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String startNo = request.getParameter("startNo");
    String endNo = request.getParameter("endNo");
    annotateService.batchUpdateAvgMark(startNo, endNo, getDepartmentIdSeq(request), getStdTypeIdSeq(request));
   ...
   * @throws Exception
   */
  public ActionForward calcAvgMark(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String annotateBookIdSeq = request.getParameter("annotateBookIdSeq");
    List annotateBooks = utilService.load(ThesisAnnotateBook.class, "id",
        SeqStringUtil.transformToLong(annotateBookIdSeq));
   ...
   * @throws Exception
   */
  public ActionForward doubleChoose(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
   ...
   * @throws Exception
   */
  public ActionForward annotateInfo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    List thesisManagerList = utilService.load(ThesisManage.class, "student.id", student.getId());
   ...
   * @throws Exception
   */
  public ActionForward doInitaliBook(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    List thesisManages = utilService.load(ThesisManage.class, "id",
        SeqStringUtil.transformToLong(thesisManageIdSeq));
    ...
    for (Iterator iter = thesisManages.iterator(); iter.hasNext();) {
      ...
      for (int i = 0; i < teaerNumber; i++) {
        if (i < annotateBooksList.size()) {
          ThesisAnnotateBook thesisAnnotateBook = (ThesisAnnotateBook) annotateBooksList.get(i);
   ...
   * @throws Exception
   */
  public ActionForward doWriteBook(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String thesisId = request.getParameter("thesisId");
    ThesisManage thesisManage = (ThesisManage) utilService.load(ThesisManage.class, Long.valueOf(thesisId));
    ...
    for (int i = 1; i <= count; i++) {
      ...
      ThesisAnnotateBook thesisAnnotateBook = (ThesisAnnotateBook) RequestUtil.populate(request,
          ThesisAnnotateBook.class, "annotateBook" + i);
      if (thesisAnnotateBook.getAnswerTem().checkedId()) {
        AnswerTeam answerTeam = (AnswerTeam) utilService.load(AnswerTeam.class, thesisAnnotateBook
            .getAnswerTem().getId());
  ...
  protected Collection getExportDatas(HttpServletRequest request) {
    ...
    if (StringUtils.isNotBlank(flag) && "bookList".equals(flag)) {
      ...
      ThesisAnnotateBook thesisAnnotateBook = (ThesisAnnotateBook) RequestUtil.populate(request,
          ThesisAnnotateBook.class, "annotateBook");
      thesisAnnotateBook.setThesisManage(thesisManage);
      String isHasMark = request.getParameter("isHasMark");
      List tempList = annotateService.getAnnotateBooks(thesisAnnotateBook, departIdSeq, stdTypeIdSeq,
          isHasMark);
      ...
      stds = thesisManageService.getThesissNullNotNull(thesisManage, departIdSeq, stdTypeIdSeq, notNulls,
          nulls);
      return stds;
    }
  }
  ...
  public void transPortModels(List bookList, List returnList) {
    for (Iterator iter = bookList.iterator(); iter.hasNext();) {
      ...
      } else if (null != topicOpen.getThesisPlan() && StringUtils.isNotBlank(topicOpen.getThesisTopic())) {
   ...
   * @throws Exception
   */
  public ActionForward cancelAffirm(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String thesisManageIdSeq = request.getParameter("thesisManageIdSeq");
    List thesisManages = utilService.load(ThesisManage.class, "id",
        SeqStringUtil.transformToLong(thesisManageIdSeq));
   ...
   * @throws Exception
   */
  public ActionForward doDeleteNo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String thesisManageIdSeq = request.getParameter("thesisManageIdSeq");
    List thesisManages = utilService.load(ThesisManage.class, "id",
        SeqStringUtil.transformToLong(thesisManageIdSeq));
