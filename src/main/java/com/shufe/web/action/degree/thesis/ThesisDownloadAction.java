...
public class ThesisDownloadAction extends FileAction {
   ...
   * @throws Exception
   */
  public ActionForward doDownLoad(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long thesisManageId = getLong(request, "thesisManageId");
    ThesisManage thesisManage = (ThesisManage) utilService.load(ThesisManage.class, thesisManageId);
    ...
    if (StringUtils.isEmpty(thesisStore.getDownloadName())) {
      ...
      download(request, response, filePath + thesisStore.getDownloadName(), thesisStore.getDisplayName());
      return null;
    }
   ...
   * @throws Exception
   */
  public void doDownLoadSystemFile(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
  ...
  public ActionForward loadUploadPage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    request.setAttribute("thesisManageId", request.getParameter("thesisManageId"));
  ...
  public ActionForward doUpload(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (f != null) return f;
    return redirect(request, new Action(ThesisManageStdAction.class, "doLoadProcessPage"),
        "info.upload.success");
  }
  ...
  protected String getFileName(HttpServletRequest request, String uploadAbsolutePath) {
    uploadAbsolutePath = uploadAbsolutePath.substring(uploadAbsolutePath.lastIndexOf("."));
    String storeId = request.getParameter("storeId");
    Student student = getStudentFromSession(request.getSession());
    String fileName = storeId + "_" + student.getCode() + uploadAbsolutePath;
    return fileName;
  }
}
