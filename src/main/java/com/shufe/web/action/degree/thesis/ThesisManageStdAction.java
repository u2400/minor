...
public class ThesisManageStdAction extends FileAction {
   ...
   * @throws Exception
   */
  public ActionForward doLoadProcessPage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    while (null != stdType && null == schedule) {
      List schedules = utilService.load(
          Schedule.class,
          new String[] { "studyLength", "enrollYear", "studentType.id" },
          new Object[] { String.valueOf(student.getSchoolingLength()), student.getEnrollYear(),
              stdType.getId() });
  ...
  public void doDownLoad(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    DegreeDocument degreeDocument = (DegreeDocument) utilService.load(DegreeDocument.class, documentId);
  ...
  protected void afterUpload(HttpServletRequest request, File file, String updloadFilePath) {
    ...
    ThesisManage thesisManage = (ThesisManage) utilService.get(ThesisManage.class, thesisManageId);
    ThesisStore thesisStore = thesisManage.setProcessObject(manageType, file.getName(), updloadFilePath);
    utilService.saveOrUpdate(thesisStore);
  }

  /*
   * (non-Javadoc)
   * @see com.shufe.web.action.common.DispatchBasicAction#configExportContext(javax.servlet.http.
   * HttpServletRequest,
  ...
  protected void configExportContext(HttpServletRequest request, Context context) {
    ...
    if ("degreeInfo".equals(exportFlag)) {
      Map studyResults = studyProductService.getStudyProducts(student.getId(), Boolean.FALSE, null);
    ...
    if ("doctorDegreeCheck".equals(exportFlag)) {
      context.put(
          "studyThesiss",
          utilService.load(StudyThesis.class, new String[] { "student.id", "isPassCheck" }, new Object[] {
              student.getId(), Boolean.TRUE }));
    }
    EntityQuery query = new EntityQuery(Level2Subject.class, "level2Subject");
    query
        .add(new Condition("level2Subject.speciality.id = (:specialityId)", student.getFirstMajor().getId()));
