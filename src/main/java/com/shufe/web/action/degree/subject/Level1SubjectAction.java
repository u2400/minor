...
public class Level1SubjectAction extends DispatchBasicAction {
  ...
  public ActionForward index(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    addOldPage(request, "level1Subjects", subjectService.getLevel1Subject(
        (Level1Subject) populate(request, Level1Subject.class), getPageNo(request), getPageSize(request)));
  ...
  public ActionForward edit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isNotEmpty(level1SubjectId)) {
      level1Subject = (Level1Subject) utilService.get(Level1Subject.class, new Long(level1SubjectId));
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    try {
      ...
      if (null != level1Subject.getId()) {
        savedSubject = (Level1Subject) utilService.get(Level1Subject.class, level1Subject.getId());
      } else {
        savedSubject = new Level1Subject();
      }
      if (utilService.duplicate(Level1Subject.class, level1Subject.getId(), "code", level1Subject.getCode())) { return redirect(
          request, "index", "error.code.saveFailure"); }

      if (!level1Subject.isPO()) {
        logHelper.info(request, "Create a level1Subject with name:" + level1Subject.getName());
        ...
        logHelper.info(request, "Update a level1Subject with name:" + level1Subject.getName());
      }
    } catch (PojoExistException e) {
      logHelper.info(request, "Failure save or update a level1Subject with name:" + level1Subject.getName(),
          e);
      return forward(mapping, request, new String[] { "entity.level1Subject", "error.model.existed" },
          "error");
    }
    if (null != request.getParameter("addAnother")) {
      return redirect(request, "edit", "info.save.success");
    } else return redirect(request, "index", "info.save.success");
  }
  ...
  public ActionForward search(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    Level1Subject level1Subject = (Level1Subject) populate(request, Level1Subject.class);
    Pagination level1SubjectList = subjectService.getLevel1Subject(level1Subject, pageSize, pageNo);
  ...
  public ActionForward info(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String level1SubjectId = request.getParameter("level1Subject.id");
    if (StringUtils.isEmpty(level1SubjectId)) return forward(mapping, request, new String[] {
        "entity.level1Subject", "error.model.id.needed" }, "error");
    Level1Subject level1Subject = (Level1Subject) utilService.get(Level1Subject.class, new Long(
        level1SubjectId));
