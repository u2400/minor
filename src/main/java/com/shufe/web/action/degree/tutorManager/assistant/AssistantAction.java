...
public class AssistantAction extends RestrictionExampleTemplateAction {
  ...
  protected ActionForward saveAndForwad(HttpServletRequest request, Entity entity) throws Exception {
    ...
    if (teachers.isEmpty()) return forward(request, new Action(this, "edit"), "error.degree.noTutor");
  ...
  protected EntityQuery buildQuery(HttpServletRequest request) {
    ...
    DataRealmUtils.addDataRealms(query,
        new String[] { "assistant.std.type.id", "assistant.std.department.id" },
        getDataRealmsWith(getLong(request, "assistant.std.type.id"), request));
