...
public class FormalAnswerStdAction extends FileAction {
   ...
   */
  public ActionForward doApply(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isBlank(apply) || "load".equals(apply)) {
      if (null == thesisManage || !thesisManage.checkObjectId(thesisManage.getTopicOpen())) {
        request.setAttribute("reason", "field.formalAnswer.noOpenTopic");
  ...
  public ActionForward affirm(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    FormalAnswer formalAnswer = (FormalAnswer) utilService.load(FormalAnswer.class, formaAnswerId);
