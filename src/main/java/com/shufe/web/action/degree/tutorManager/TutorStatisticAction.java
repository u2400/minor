...
public class TutorStatisticAction extends RestrictionSupportAction {
  ...
  public ActionForward doStatisticTutorProperty(ActionMapping mapping, ActionForm form,
    ...
    if (StringUtils.isBlank(propertyName)) { return forward(request, "loadDefault"); }
    request.setAttribute("propertyName", propertyName);
    String departIdSeq = getDepartmentIdSeq(request);
    if ("gender".equals(propertyName)) {
      List genderList = tutorService.getPropertyOfTutor(new Tutor(), departIdSeq, "gender", Boolean.FALSE);
      List departmentList = tutorService.getPropertyOfTutor(new Tutor(), departIdSeq, "department",
          Boolean.FALSE);
      Map returnMap = tutorService.getStatisticMap(new Tutor(), departIdSeq, "department,gender");
  ...
  public ActionForward doStatisticSpecialityOfTutor(ActionMapping mapping, ActionForm form,
    ...
    List specialitys = level2SubjectService.getPropertyOfSubject(new Level2Subject(), "speciality",
        Boolean.FALSE);
    Collections.sort(specialitys, new Comparator() {
      ...
      public int compare(Object arg0, Object arg1) {
        ...
        return new CompareToBuilder()
            .append(specia0.getDepartment().getName(), specia1.getDepartment().getName())
            .append(specia0.getName(), specia1.getName()).toComparison();
      }
    });
    String departIdSeq = getDepartmentIdSeq(request);
   ...
   * @return
   */
  public ActionForward doStatisticByAge(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isBlank(doStatistic) && !"statistic".equals(doStatistic)) {
      ...
      Map tutorTypeGendarMap = tutorService.getTutorTypeMap(new Tutor(), departIdSeq, scoreList);
