...
public class ThesisAdminAction extends RestrictionSupportAction {
  ...
  private EntityQuery buildQuery(HttpServletRequest request) {
    ...
    from = "from Thesis  thesis join thesis.student std " + StringUtils.substringAfter(from, " std");
    query.setFrom(from);
    return query;
  }
  ...
  public ActionForward affirm(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    utilService.update(Thesis.class, "id", SeqStringUtil.transformToLong(thesisIdSeq), valueMap);
    return redirect(request, "search", "info.action.success");
  }

  /*
   * (non-Javadoc)
   * @see com.shufe.web.action.common.DispatchBasicAction#getExportDatas(javax.servlet.http.
   * HttpServletRequest)
   */
  ...
  public ActionForward importData(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    Transfer transfer = ImporterServletSupport.buildEntityImporter(request, ThesisManage.class, tr);
