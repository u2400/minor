...
public class ThesisStatisticAction extends RestrictionSupportAction {
   ...
   * @return
   */
  public ActionForward topicOpenStatistic(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if ("conditions".equals(flag)) {
      ...
      if (null != student.getDepartment().getId() && !new Long(0).equals(student.getDepartment().getId())) {
        departmentIdSeq = String.valueOf(student.getDepartment().getId());
      }
      List stdDeparts = thesisStatisticService.getstdGroupInfo(student, departmentIdSeq, stdTypeIdSeq);
      List thesisManages = thesisStatisticService.getTOpicOpenGroupInfo(thesisManage, departmentIdSeq,
          stdTypeIdSeq);
      ...
      List collegeAffirms = thesisStatisticService.getTOpicOpenGroupInfo(thesisManage, departmentIdSeq,
          stdTypeIdSeq);
      ...
      List tutorAffirms = thesisStatisticService.getTOpicOpenGroupInfo(thesisManage, departmentIdSeq,
          stdTypeIdSeq);
   ...
   * @return
   */
  public ActionForward annotateStatistic(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
    String departmentIdSeq = getDepartmentIdSeq(request);
    String stdTypeIdSeq = getStdTypeIdSeq(request);
    ...
    TeachCalendar teachcalendar = teachCalendarService.getTeachCalendar(stdType.getId(), year, term);
    ...
    // 等级
    Map levelMap = annotateService.getStdNumOfLevel(thesisAnnotateBook, departmentIdSeq, stdTypeIdSeq);
    request.setAttribute("levelMap", levelMap);

    // 平均分
    List avgMarkList = annotateService.getStdNumOfAvgMark(thesisAnnotateBook, departmentIdSeq, stdTypeIdSeq);
    ...
    List markList = annotateService.getStdNumOfMark(thesisAnnotateBook, departmentIdSeq, stdTypeIdSeq);
    Map markMap = getSegMentMap(markList, segMentMap);
    request.setAttribute("markMap", markMap);
    // 修改的书list
    thesisAnnotateBook.setIsReply("M");
    List modifyAnnotateBooks = annotateService.getAnnotateBooks(thesisAnnotateBook, departmentIdSeq,
        stdTypeIdSeq, "");
    request.setAttribute("modify", modifyAnnotateBooks);
    // 删除的书list
    thesisAnnotateBook.setIsReply("D");
    List noPassedBooks = annotateService.getAnnotateBooks(thesisAnnotateBook, departmentIdSeq, stdTypeIdSeq,
        "");
  ...
  public Map getSgeMentsFromForm(HttpServletRequest request) {
    ...
    returnMap.put("A", new FloatSegment(new Float(90).floatValue(), new Float(100).floatValue()));
    returnMap.put("B", new FloatSegment(new Float(75).floatValue(), new Float(89).floatValue()));
    returnMap.put("C", new FloatSegment(new Float(61).floatValue(), new Float(74).floatValue()));
    ...
    for (int i = 0; i < level.length; i++) {
      ...
      if (StringUtils.isNotBlank(minValue) || StringUtils.isNotBlank(maxValue)) {
        FloatSegment scoreSegment = new FloatSegment(Float.valueOf(minValue).floatValue(), Float.valueOf(
            maxValue).floatValue());
  ...
  public Map getSegMentMap(List markList, Map segMentMap) {
    ...
    for (Iterator iter = keySet.iterator(); iter.hasNext();) {
      String element = (String) iter.next();
      returnMap.put(element, new Integer(((FloatSegment) segMentMap.get(element)).getCount()));
    }
    return returnMap;
  }
   ...
   */
  public ActionForward scheduleStatistic(ActionMapping mapping, ActionForm form, HttpServletRequest request,
   ...
   * @return
   */
  protected EntityQuery buildQuery(HttpServletRequest request, String attr, String alias, String joinMode) {
    ...
    DataRealmUtils.addDataRealms(query, new String[] { "thesisManage.student.type.id",
        ...
        restrictionHelper.getDataRealmsWith(getLong(request, "thesisManage.student.type.id"), request));
    ...
    query.setSelect(groupby + ", sum(case when " + alias + ".finishOn is null then 0 else 1 end), count(*)");
   ...
   * @return
   */
  public ActionForward thesisAndTeachers(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
    Long stdTypeId = getLong(request, "stdTypeId");
