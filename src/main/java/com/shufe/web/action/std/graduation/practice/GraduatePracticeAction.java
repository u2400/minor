...
public class GraduatePracticeAction extends CalendarRestrictionSupportAction {
  ...
  protected EntityQuery buildQuery(HttpServletRequest request, PageLimit pageLimit) {
    ...
    DataRealmUtils.addDataRealms(entityQuery, new String[] { "graduatePractice.student.type.id",
  ...
  public ActionForward edit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null == practiceId || new Long(0).equals(practiceId)) {
      graduatePractice = (GraduatePractice) RequestUtil.populate(request, GraduatePractice.class,
          "graduatePractice");
      request.setAttribute("calendar", utilService.get(TeachCalendar.class, getLong(request, "calendarId")));
    } else {
      graduatePractice = (GraduatePractice) utilService.load(GraduatePractice.class, practiceId);
    }
    request.setAttribute("graduatePractice", graduatePractice);
    addCollection(request, "practiceSources", utilService.load(PracticeSource.class, "state", Boolean.TRUE));
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null != graduatePractice.getStudent()) {
      ...
      if (stds.size() != 1) {
        saveErrors(request.getSession(),
            ForwardSupport.buildMessages(new String[] { "error.model.notExist" }));
    ...
    if (StringUtils.isNotBlank(isNext)) { return redirect(request, "edit", "info.action.success"); }
  ...
  public ActionForward remove(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    List graduatePractices = utilService.load(GraduatePractice.class, "id",
        SeqStringUtil.transformToLong(graduatePracticeIds));
   ...
   * @throws Exception
   */
  public ActionForward practiceDepartStat(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    List departments = GraduatePracticeService.getPropertyOfPractices(graduatePractice,
        getDepartmentIdSeq(request), getStdTypeIdSeq(request), "student.department", Boolean.FALSE);
    List sources = GraduatePracticeService.getPropertyOfPractices(graduatePractice,
        getDepartmentIdSeq(request), getStdTypeIdSeq(request), "practiceSource", Boolean.FALSE);
    Map datasMap = GraduatePracticeService.getDatasOfDepartStat(getDataRealm(request), teachCalendar);
  ...
  public ActionForward importData(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    Transfer transfer = ImporterServletSupport.buildEntityImporter(request, GraduatePractice.class, tr);
  ...
  public ActionForward stdList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    query
        .add(new Condition(
            ...
            calendarId));
    query
        .add(new Condition("std.type in(:types)", getStdTypesOf(calendar.getStudentType().getId(), request)));
  ...
  public ActionForward batchAdd(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isEmpty(practiceCompany)) {
      ...
      List stds = utilService.load(Student.class, "id",
