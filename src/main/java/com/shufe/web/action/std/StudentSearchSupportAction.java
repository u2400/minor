...
public class StudentSearchSupportAction extends CalendarRestrictionSupportAction {
  ...
  protected EntityQuery buildQuery(HttpServletRequest request, String moduleName) {
    ...
    DataRealmUtils.addDataRealms(query, new String[] { "student.type.id", "student.department.id" },
    ...
    if (obj != null) {
      ...
      if (StringUtils.isNotEmpty(searchFalg) && searchFalg.equals("search")) {
        ...
        if (task.getArrangeInfo().getTeachDepart().isVO()) {
          ...
          tasks = getTaskService().getTeachTasksOfTeachDepart(task, getStdTypeIdSeqOf(stdTypeId, request),
              departIds, pageNo, pageSize);
        } else tasks = getTaskService().getTeachTasks(task, pageNo, pageSize);
  ...
  protected ActionForward forwardError(ActionMapping mapping, HttpServletRequest request, String errorKey,
      String[] errorValues) {
    addResults(request);
