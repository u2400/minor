...
public class StudentFileAction extends RestrictionSupportAction {
  ...
  public ActionForward importData(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null == importer) { return forward(request, "/pages/components/importData/error"); }
  ...
  public void exportStudentExcel(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
  ...
  public Collection getAdminClassStudentExcelData(HttpServletRequest request) {
    ...
    for (Iterator iter = adminClassList.iterator(); iter.hasNext();) {
      ...
      Object[] classStudentArray = new Object[adminClassKeyList.size() + studentKeyList.size()];
   ...
   * @return
   */
  private void addClassKeyArray(List adminClassKeyList, Object[] elementArray, AdminClass adminClass) {
    int classSize = 0;
  ...
  public Collection getSecondSpecialityStudentExcelData(HttpServletRequest request) {
    ...
    if (ArrayUtils.isEmpty(ids)) {
      ...
      entityQuery.add(new Condition("(student.firstMajor.department.id in (:firstMajor_department_id)) "
          + "or" + " (student.secondMajor.department.id in (:secondMajor_department_id))", SeqStringUtil
          .transformToLong(this.getDepartmentIdSeq(request)), SeqStringUtil.transformToLong(this
  ...
  public void exportDutyRecordExcel(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
  ...
  public Collection getDutyRecordExcelData(HttpServletRequest request) {
    ...
    return CollectionUtils.select(dutyRecordList, new Predicate() {
      ...
      public boolean evaluate(Object object) {
        /*
         * if(Boolean.TRUE.equals(isContainsNull)&&("null".equals(((DutyRecord)object).getCourseTakeType
         * ().getCode()))){
        ...
        return (Boolean.TRUE.equals(isContainsNull) && ("null".equals(((DutyRecord) object)
            ...
            || (ArrayUtils.contains(courseTakeTypeIdArray,
                ((DutyRecord) object).getCourseTakeType(Boolean.FALSE).getId()));
      }
    });
  }
   ...
   * @throws Exception
   */
  public void downloadTemplate(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    if (file_name == null) {
      response.getWriter().print("not file correspond to " + request.getParameter("fileName"));
      return;
    }
  ...
  protected String getExportKeys(HttpServletRequest request) {
    ...
    if (StringUtils.isBlank(keys)) { throw new RuntimeException("keys is empty!\n"); }
    return SeqStringUtil.keepUnique(keys);
  }

  protected String getExportTitles(HttpServletRequest request) {
    String titles = (String) request.getAttribute("titles");
    if (StringUtils.isBlank(titles)) { throw new RuntimeException("titles is empty!\n"); }
    return SeqStringUtil.keepUnique(titles);
  }

  protected PropertyExtractor getPropertyExtractor(HttpServletRequest request) {
    if ("getDutyRecordExcelData".equals(request.getAttribute("exportMethod"))) { return new DutyRecordPropertyExporter(); }
    return new DefaultPropertyExtractor();
  }

  protected Collection getExportDatas(HttpServletRequest request) {
    String exportMethod = (String) request.getAttribute("exportMethod");
    if (StringUtils.isBlank(exportMethod)) { throw new RuntimeException("EXPORT_METHOD is empty!\n"); }
    Method method = null;
    try {
