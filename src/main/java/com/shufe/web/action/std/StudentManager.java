...
public class StudentManager extends StudentSearchSupportAction {
  ...
  public ActionForward addSecondSpecialityStudent(ActionMapping mapping, ActionForm form,
    ...
    if (student == null) { return this.forwardError(mapping, request, "student.notExist.error"); }
    if (student.getSecondMajor() != null && student.getSecondMajor().isPO()) { return this.forwardError(
        mapping, request, "student.secondMajor.exists"); }
    ...
    studentService.updateStudentAdminClass(student.getId(),
        SeqStringUtil.transformToLong(request.getParameter("adminClassIds")), new Boolean(true));
  ...
  public ActionForward updateSecondSpecialityStudent(ActionMapping mapping, ActionForm form,
    ...
    for (Iterator iter = studentList.iterator(); iter.hasNext();) {
      ...
      studentService.updateStudentAdminClass(element.getId(),
          SeqStringUtil.transformToLong(request.getParameter("adminClassIds")), new Boolean(true));
    }
    Action action = new Action("studentManager", "managerSecondSpecialityStudentList");
    return this.redirect(request, action, null, "moduleName=SecondSpecialityStudentManage");
   ...
   * @throws Exception
   */
  public ActionForward printReview(ActionMapping mapping, ActionForm form, HttpServletRequest request,
   ...
   * (non-Javadoc)
   * @see
   * com.shufe.web.action.common.DispatchBasicAction#getExportKeys(javax.servlet.http.HttpServletRequest
   * )
   */
