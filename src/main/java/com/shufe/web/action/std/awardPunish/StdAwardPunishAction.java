...
public class StdAwardPunishAction extends DispatchBasicAction {
  ...
  public ActionForward awardList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    EntityQuery query = new EntityQuery(Award.class, "award");
    query.add(new Condition("award.std.code=:code", getUser(request.getSession()).getName()));
    addCollection(request, "awards", utilService.search(query));
    return forward(request);
  }

  public ActionForward punishList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    EntityQuery query = new EntityQuery(Punishment.class, "punish");
    query.add(new Condition("punish.std.code=:code", getUser(request.getSession()).getName()));
