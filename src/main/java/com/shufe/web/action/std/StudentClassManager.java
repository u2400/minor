...
public class StudentClassManager extends StudentSearchSupportAction {
   ...
   * @return
   */
  public ActionForward classManager(ActionMapping mapping, ActionForm form, HttpServletRequest request,
  ...
  public ActionForward batchPrintClass(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
    ...
    List adminClassList = studentService.findStudentClass(adminClass, studentTypeIds, departmentIds);
   ...
   * @return
   */
  public ActionForward printClass(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
  ...
  public ActionForward batchUpdateStdCount(ActionMapping mapping, ActionForm form,
    ...
    return redirect(request, new Action("adminClassManager", "classManager"), "info.update.success",
        new String[] { "pageNo" });
  }
  ...
  public ActionForward addClassStudentList(ActionMapping mapping, ActionForm form,
    ...
    Pagination page = studentService.searchStudent(student, getPageNo(request), getPageSize(request),
  ...
  protected void configExportContext(HttpServletRequest request, Context context) {
    ...
    if (null == stdPerClass) stdPerClass = new Integer(60);
    List adminClassList = (List) utilService.load(AdminClass.class, "id",
        SeqStringUtil.transformToLong(get(request, "ids")));
    List subClasses = new ArrayList();
    for (Iterator iter = adminClassList.iterator(); iter.hasNext();) {
      AdminClass adminClass = (AdminClass) iter.next();
      subClasses.addAll(SubClass.buildClasses(adminClass, stdPerClass.intValue(), isOnCompus));
    }
    // indexes perClasss
