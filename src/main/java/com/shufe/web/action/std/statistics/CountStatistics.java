...
public class CountStatistics extends GeneralStudentStatisticsAction {
   ...
   * @return
   */
  public ActionForward studentCountStat(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    DataRealmUtils.addDataRealms(entityQuery, new String[] { "student.type.id", "student.department.id" },
    ...
    if (StringUtils.isNotBlank(adminClasssId1String) || StringUtils.isNotBlank(adminClasssId2String)) {
      entityQuery.join("student.adminClasses", "adminClass");
      List adminClassIds = new ArrayList();
      if (StringUtils.isNotBlank(adminClasssId1String) && StringUtils.isNotBlank(adminClasssId2String)) {
        entityQuery.add(new Condition("exists (from AdminClass class" + adminClasssId1String + " join class"
            + adminClasssId1String + ".students std where std.id=student.id and class" + adminClasssId1String
            + ".id = :classId" + adminClasssId1String + ")" + " and exists (from AdminClass class"
            + adminClasssId2String + " join class" + adminClasssId2String
            + ".students std where std.id=student.id and class" + adminClasssId2String + ".id = :classId"
            + adminClasssId2String + ")", new Long(adminClasssId1String), new Long(adminClasssId2String)));
      }
    ...
    entityQuery.join("left", "student.firstMajor", "firstMajor").join("left", "student.firstAspect",
        "firstAspect");
    ...
    for (Iterator iter = result.iterator(); iter.hasNext();) {
      ...
      remPropertySet.add(element.get(((BaseValueBean) propertyList.get(propertyList.size() - 2))
          .getDataName()));
    }
  ...
  private void addIdCondition(HttpServletRequest request, EntityQuery entityQuery, String idName,
    ...
    if (StringUtils.isNotBlank(request.getParameter(idName))) {
      entityQuery.add(new Condition(toName + " = (:" + StringUtils.replace(idName, ".", "_") + ")", getLong(
          request, idName)));
    }
  }
   ...
   * @param name
   */
  private void addGraduateAuditStatus(HttpServletRequest request, EntityQuery entityQuery, String name) {
    String graduateAuditStatus = request.getParameter(name);
    if (NumberUtils.isNumber(graduateAuditStatus)) {
      entityQuery.add(new Condition("student." + name + " = (:" + name + ")", ConvertUtils.convert(
          graduateAuditStatus, Boolean.class)));
   ...
   * @return
   */
  protected Long[] getStdTypeIdArray(String stdTypeIds, HttpServletRequest request, String moduleName) {
    Set stdTypeIdSet = new HashSet();
    Long[] stdTypeIdArray = SeqStringUtil.transformToLong(stdTypeIds);
    if (ArrayUtils.isEmpty(stdTypeIdArray)) { return ArrayUtils.EMPTY_LONG_OBJECT_ARRAY; }
    for (int i = 0; i < stdTypeIdArray.length; i++) {
      stdTypeIdSet.addAll(EntityUtils.extractIds(getStdTypesOf(stdTypeIdArray[i], moduleName, request)));
    }
