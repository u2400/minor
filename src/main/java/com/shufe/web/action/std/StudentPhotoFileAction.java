...
public class StudentPhotoFileAction extends FileAction {
  ...
  protected String getFileName(HttpServletRequest request, String uploadAbsolutePath) {
    ...
    uploadAbsolutePath = getStdCode(request)
        + uploadAbsolutePath.substring(uploadAbsolutePath.lastIndexOf("."));
    try {
  ...
  protected String getStdCode(HttpServletRequest request) {
    ...
    if (!ValidEntityKeyPredicate.getInstance().evaluate(studentId)) { throw new RuntimeException(
        "without student with id:" + studentId); }
  ...
  protected void afterUpload(HttpServletRequest request, File file, String updloadFilePath) {
    Student student = (Student) utilService.load(Student.class, "code", getStdCode(request)).iterator()
        .next();
  ...
  public void download(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (!ValidEntityKeyPredicate.getInstance().evaluate(studentId)) {
      response.getWriter().write("without student with id:" + studentId);
      return;
    }
    Student student = (Student) utilService.get(Student.class, studentId);
    ...
    if (StringUtils.isEmpty(photoName)) {
       ...
       */
      download(request, response, request.getSession().getServletContext().getRealPath("") + DEFAULTPHOTO);
      return;
    }
    download(request, response, getFileRealPath("stdPhoto", request) + photoName);

  }

  public void downloadError(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    download(request, response, getFileRealPath("stdPhoto", request) + DEFAULTPHOTO);
