...
public class StudentOperation extends DispatchBasicAction {
   ...
   */
  public ActionForward loadAddForm(ActionMapping mapping, ActionForm form, HttpServletRequest request,
  ...
  public ActionForward add(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    StudentStatusInfo statusInfo = (StudentStatusInfo) RequestUtil.populate(request, StudentStatusInfo.class,
        "statusInfo");
    ...
    BasicInfo basicInfo = (BasicInfo) RequestUtil.populate(request, BasicInfo.class, "basicInfo");

    if (utilService.exist(Student.class, "code", student.getCode())) { return forward(mapping, request,
        "student.exist.error", FAILURE); }

    Student saved = studentService.saveStudent(student, statusInfo, basicInfo, abroadStudentInfo);
    studentService.updateStudentAdminClass(saved.getId(),
        SeqStringUtil.transformToLong(request.getParameter("adminClassIds")), new Boolean(false));
   ...
   */
  public ActionForward loadUpdateForm(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
    this.initForm();
  ...
  public ActionForward update(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    StudentStatusInfo statusInfo = (StudentStatusInfo) populateEntity(request, StudentStatusInfo.class,
        "statusInfo");
    ...
    studentService.updateStudentAdminClass(student.getId(),
        SeqStringUtil.transformToLong(get(request, "adminClassIds")), new Boolean(false));
  ...
  public ActionForward remove(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    try {
      ...
      return redirect(request, new Action("studentManager", "manager"), "error.remove.beenUsed");
    }
  }

  public ActionForward editCode(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String from = get(request, "from");
    String to = get(request, "to");
    if (StringUtils.isEmpty(from) || StringUtils.isEmpty(to) || ObjectUtils.equals(from, to)) { return forward(
        request, new Action("studentManager", "manager")); }
   ...
   * @return
   */
  public ActionForward studentDetail(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
  ...
  public ActionForward updateStudentSpeciality(ActionMapping mapping, ActionForm form,
    ...
    if (new NotZeroNumberPredicate().evaluate(adminClassId)) {
      for (Iterator iter = studentList.iterator(); iter.hasNext();) {
        ...
        for (Iterator adminClassSetIter = adminClassSet.iterator(); adminClassSetIter.hasNext();) {
          ...
          if (adminClassElement.getDepartment().equals(student.getDepartment())
            ...
            if (student.isInSchool()) {
              adminClassElement
                  .setActualStdCount(adminClassElement.getActualStdCount() == null ? new Integer(0)
                      : new Integer(adminClassElement.getActualStdCount().intValue() - 1));
            }
            if (student.isActive()) {
              adminClassElement.setStdCount(adminClassElement.getStdCount() == null ? new Integer(0)
                  : new Integer(adminClassElement.getStdCount().intValue() - 1));
            }
        ...
        } else if (new NotZeroNumberPredicate().evaluate(getLong(request, "specialityAspectId"))) {
          student.setFirstAspect(new SpecialityAspect(getLong(request, "specialityAspectId")));
        }
        AdminClass adminClass = (AdminClass) utilService.load(AdminClass.class, adminClassId);
        if (student.isInSchool()) {
          adminClass.setActualStdCount(adminClass.getActualStdCount() == null ? new Integer(0) : new Integer(
