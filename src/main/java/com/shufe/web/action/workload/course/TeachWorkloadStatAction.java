...
public class TeachWorkloadStatAction extends CalendarRestrictionSupportAction {
  ...
  public ActionForward search(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    List calendars = teachCalendarService
        .getTeachCalendars(stdTypeId, yearStart, termStart, yearEnd, termEnd);

    EntityQuery entityQuery = new EntityQuery(TeachWorkload.class, "teachWorkload");
    if (CollectionUtils.isEmpty(calendars)) {
      entityQuery.add(new Condition("teachWorkload.teachCalendar is null"));
    } else {
  ...
  public ActionForward doStatisticForTeacher(ActionMapping mapping, ActionForm form,
    ...
    TeachCalendar calendar = teachCalendarService.getTeachCalendar(Long.valueOf(stdTypeId), yearNo, termNo);
    if (null == calendar) {
    ...
    List propertylist = teachWorkloadStatService.getPropertyOfTeachWorkload(workloadString, calendar.getId());
    Map workloadMap = teachWorkloadStatService.getEduDegreeAndTitle(departmentIds, workloadString,
        calendar.getId(), age, studentTypeIds);
    Map teacherMap = teachWorkloadStatService
        .getEduDegreeAndTitleInTeacher(teacherString, departmentIds, age);
  ...
  public ActionForward doTeacherStdTypeDepartAndTitle(ActionMapping mapping, ActionForm form,
    ...
    TeachCalendar teachCalendar = teachCalendarService.getTeachCalendar(studentTypeId, year, term);
    String statisticContextId = request.getParameter("statisticContextId");
    List teacherTitleList = teachWorkloadStatService.getPropertyOfTeachWorkload("teacherInfo.teacherTitle",
        teachCalendar.getId());
    ...
    if ("1".equals(statisticContextId)) {
      dataMap = teachWorkloadStatService.getTeacherNumberByConditions(getDepartmentIdSeq(request),
          getStdTypeIdSeqOf(studentTypeId, request), teacherTypeId, teachCalendar);
      // 第二种,求教师所有的工作量
    } else {
      dataMap = teachWorkloadStatService.getTotalWorkloadByConditions(getDepartmentIdSeq(request),
          getStdTypeIdSeqOf(studentTypeId, request), teacherTypeId, teachCalendar);
    }
    Results.addList("teacherTitleList", teacherTitleList).addObject("statisticContextId", statisticContextId)
        .addList("collegeList", getColleges(request));
  ...
  public ActionForward doGetTeacherTitleOfWorkload(ActionMapping mapping, ActionForm form,
    ...
    TeachCalendar teachCalendar = teachCalendarService.getTeachCalendar(stdTypeId, yearNo, termNo);
    List teacherTitleAndWorkloadList = teachWorkloadStatService.getNumberAndWorkloadOfTitle(teachCalendar,
        studentTypeIdSeq);
    Results.addList("teacherTitleAndWorkloadList", teacherTitleAndWorkloadList).addObject("teachCalendar",
        teachCalendar);
   ...
   * @return
   */
  public ActionForward doRateOfDepartment(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    TeachCalendar teachCalendar = teachCalendarService.getTeachCalendar(studentTypeId, yearNo, termNo);
    ...
    for (Iterator iter = tempList.iterator(); iter.hasNext();) {
      ...
      if (null != teacherType && TeacherType.REGISTERTYPEID.equals(teacherType.getId())) {
        StatUtils.setValueToMap(departIdSeq, element[0], "integer", registerTeacherWorkloadMap);
      }
    }
  ...
  public ActionForward doGetCourseTypeNumber(ActionMapping mapping, ActionForm form,
    ...
    List courseTypeList = teachWorkloadStatService.getPropertyOfTeachWorkload("courseType",
        teachCalendar.getId());
    Map courseTypeCollegeMap = teachWorkloadStatService.getCourseTypesOfDepartment(teachCalendar,
  ...
  public ActionForward doGetRateOfTitleAndStdType(ActionMapping mapping, ActionForm form,
    ...
    List teacherTitleList = teachWorkloadStatService.getPropertyOfTeachWorkload("teacherInfo.teacherTitle",
        teachCalendar.getId());
    Map stdTypeAndTitleMap = teachWorkloadStatService.getTitleAndStdTypesNo(teachCalendar, departmentIds,
        studentTypeIds);
   ...
   * @return
   */
  public ActionForward doGetWorkloadByTeacherTypeAndDepartmentId(ActionMapping mapping, ActionForm form,
      HttpServletRequest request, HttpServletResponse response) {
    Long studentTypeId = getLong(request, "stdTypeId");
    if (null == studentTypeId) {
    ...
    Map teacherTypeAndDeparMap = teachWorkloadStatService.getWorkloadByDepartAndTeacherType(teachCalendar,
        getStdTypeIdSeqOf(studentTypeId, request), departmentIds, iscalc);
    request.setAttribute("departmentList", departmentService.getDepartments(departmentIds));
    request
        .setAttribute(
            "teacherTypeList",
            teachWorkloadStatService.getPropertyOfTeachWorkload("teacherInfo.teacherType",
                teachCalendar.getId()));
  ...
  public ActionForward doGetTeacherAcgWorkload(ActionMapping mapping, ActionForm form,
    ...
    List teacherAvgWorkloadList = teachWorkloadStatService.getAvgCourseNo(departIds, studentTypeIds,
        teachCalendar);
  ...
  public ActionForward doStatisticTeachAllDeparts(ActionMapping mapping, ActionForm form,
    ...
    request.setAttribute("stdTypeList",
        studentTypeService.getStudentTypes(getStdTypeIdSeqOf(stdTypeId, request)));
    Map deaprtAndStdTypeWorkloadMap = teachWorkloadStatService.getWorkloadByDataRealm(departIdSeq,
        stdTypeIdSeq, calendar);
   ...
   * @return
   */
  public Object getObjectFromList(List list, Long departmentId, int depatId, Long studentTypeId, int stdTypeId) {
    int count = -1;
    for (int i = 0; i < list.size(); i++) {
      ...
      if (depatId == 0 && stdTypeId > 0) {
        ...
        Department deprtment = (Department) objectArray[depatId];
        StudentType stdType = (StudentType) objectArray[stdTypeId];
        temp = departmentId.equals(deprtment.getId()) && studentTypeId.equals(stdType.getId());
      } else {
        break;
      }
      if (temp) {
  ...
  public ActionForward doTotalAndRegisterWorkload(ActionMapping mapping, ActionForm form,
    ...
    TeachCalendar calendar = teachCalendarService.getTeachCalendar(Long.valueOf(stdTypeId), year, term);
    List totleAndregisterList = teachWorkloadStatService.getTotalAndRegisterWorkload(departmentList,
        calendar.getId());
    ...
    for (int i = 0; i < departmentList.size(); i++) {
      ...
      Object[] objectTemp = (Object[]) getObjectFromList(totleAndregisterList, department.getId(), 4,
          new Long(0), 0);
      if (objectTemp != null) {
   ...
   * @throws Exception
   */
  public void doExpTeacherTitleOfWorkload(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long studentTypeId = getLong(request, "stdTypeId");
    String yearNo = request.getParameter("year");
    String termNo = request.getParameter("term");
  ...
  public ActionForward doGetDepartRegisterNumber(ActionMapping mapping, ActionForm form,
    ...
    TeachCalendar teachCalendar = teachCalendarService.getTeachCalendar(Long.valueOf(studentTypeId), yearNo,
        termNo);
    String departmentIds = getDepartmentIdSeq(request);
    List departmentList = departmentService.getDepartments(departmentIds);
    List list = new ArrayList();
    List tempList = teachWorkloadStatService.getPeopleOfDepartment(teachCalendar, studentTypeIds,
        departmentIds);
   ...
   * @return
   */
  public ActionForward doStatForDepart(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
   ...
   * @return
   */
  public ActionForward departList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
    EntityQuery query = new EntityQuery(Workload.class, "departWorkload");
    query.add(new Condition("departWorkload.teacherInfo.teachDepart.id=:id", Long.valueOf(request
        .getParameter("department"))));
    query.add(new Condition("departWorkload.teachCalendar.year=:year", request.getParameter("year")));
    query.add(new Condition("departWorkload.teachCalendar.term=:term", request.getParameter("term")));
    query.setSelect("departWorkload.calcWorkload,departWorkload.payReward,sum(departWorkload.totleWorkload)");
