...
public class TeachWorkloadMultiTermStatAction extends TeachWorkloadStatAction {
  ...
  public ActionForward index(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    request.setAttribute("departments", departmentService.getTeachDeparts(getDepartmentIdSeq(request)));
   ...
   * @return
   */
  public ActionForward mutiCalendarStat(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    if (StringUtils.isEmpty(yearStart) || StringUtils.isEmpty(termStart) || StringUtils.isEmpty(yearEnd)
        || StringUtils.isEmpty(termEnd)) {
      throw new CalendarNotExistException(
          ((StudentType) utilService.load(StudentType.class, stdTypeId)).getName() + "教学日历不存在！");
    } else {
      ...
      List condtionCalendars = teachCalendarService.getTeachCalendars(stdTypeId, yearStart, termStart,
          yearEnd, termEnd);
      ...
      Map departAndStdTypeMap = teachWorkloadStatService.getWorkloadByDataRealm(departmentId, stdTypeIds,
          condtionCalendars);
  ...
  public ActionForward info(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    List teachCalendars = utilService.load(TeachCalendar.class, "id",
        SeqStringUtil.transformToLong(allCalendarIdSeq));
    Map techerAndCalendarsMap = teachWorkloadStatService.getWorkloadByTeacherAndCalendar(selectDepartId,
        allStdTypeIdSeq, teachCalendars);
    Department depart = (Department) utilService.get(Department.class, Long.valueOf(selectDepartId));
