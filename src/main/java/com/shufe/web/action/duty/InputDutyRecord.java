...
public class InputDutyRecord extends DutyRecordSupportAction {
   ...
   * @throws Exception
   */
  public ActionForward searchTeachTask(ActionMapping mapping, ActionForm form, HttpServletRequest request,
  ...
  public ActionForward maintainRecordByTeachTask(ActionMapping mapping, ActionForm form,
    ...
    Map stdCourseUnitStatusMap = dutyService.getCourseUnitStatusMap(teachTaskId, beginDate, endDate);
    ...
    for (long i = 0; i < days; i++) {
      ...
      List courseUnitList = courseActivityService.getCourseUnits(teachTaskId, new java.sql.Date(calendar
          .getTime().getTime()));
  ...
  public ActionForward doMaintainRecordByTeachTask(ActionMapping mapping, ActionForm form,
    ...
    if (methodFlag) {
      String[] changeRecordString = StringUtils.split(request.getParameter("changeRecordString"), ";");
      for (int i = 0; i < changeRecordString.length; i++) {
        ...
        if (StringUtils.isEmpty(statusId)) {
          dutyService.deleteRecordDetail(Long.valueOf(stdRecordStatusArray[0]), teachTaskId, java.sql.Date
              .valueOf(stdRecordStatusArray[1]), new Long(beginUnitIdString), new Long(endUnitIdString));
        } else {
          dutyService.updateOrSaveRecordDetail(Long.valueOf(stdRecordStatusArray[0]), teachTaskId,
              java.sql.Date.valueOf(stdRecordStatusArray[1]), new Long(beginUnitIdString), new Long(
                  endUnitIdString), new Long(statusId));
        }
      }
    } else {
      ...
      for (int i = 0; i < dateAndCourseUnitStringArray.length; i++) {
        String[] dateAndCourseUnitArray = StringUtils.split(dateAndCourseUnitStringArray[i], ",");
        if (dateAndCourseUnitArray.length > 1) {
          ...
          for (int j = 1; j < dateAndCourseUnitArray.length; j++) {
            ...
            for (int k = 0; k < stdIdArray.length; k++) {
              ...
              String statusId = request
                  .getParameter("," + stdId + "," + day + "," + courseUnitIdString + ",");
              String[] unitIdArray = StringUtils.split(courseUnitIdString, "-");
              String beginUnitIdString = unitIdArray[0];
              String endUnitIdString = unitIdArray.length > 1 ? unitIdArray[1] : unitIdArray[0];
              if (StringUtils.isEmpty(statusId)) {
                dutyService.deleteRecordDetail(stdId, teachTaskId, java.sql.Date.valueOf(day), new Long(
                    beginUnitIdString), new Long(endUnitIdString));
              } else {
                dutyService.updateOrSaveRecordDetail(stdId, teachTaskId, java.sql.Date.valueOf(day),
                    new Long(beginUnitIdString), new Long(endUnitIdString), new Long(statusId));
              }
            }
          }
        }
  ...
  public ActionForward input(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    CourseUnit beginUnit = (CourseUnit) utilService.load(CourseUnit.class, new Long(beginUnitIdString));
    CourseUnit endUnit = (CourseUnit) utilService.load(CourseUnit.class, new Long(endUnitIdString));
    ...
    for (int i = 0; i < stdIdArray.length; i++) {
      ...
      if (!StringUtils.isEmpty(dutyStatusStdNo)) {
        for (Iterator iter = courseUnitList.iterator(); iter.hasNext();) {
          ...
          dutyService.updateOrSaveRecordDetail(stdIdArray[i], teachTaskId, java.sql.Date.valueOf(day),
              element.getId(), element.getId(), new Long(dutyStatusStdNo));
        }
      }
    }
