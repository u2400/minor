...
public class DutyRecordManager extends DutyRecordSupportAction {
  ...
  public ActionForward maintainRecordByTeachTask(ActionMapping mapping, ActionForm form,
    ...
    if (null == teachTaskId) {
      teachTaskId = (request.getAttribute("teachTaskId") == null) ? (new Long(0)) : ((Long) request
          .getAttribute("teachTaskId"));
    }
    TeachTask teachTask = (TeachTask) utilService.load(TeachTask.class, teachTaskId);
    Results.addObject("teachTask", teachTask);
    List recordList = utilService.load(DutyRecord.class, "teachTask.id", new Object[] { teachTaskId });
  ...
  public ActionForward modifyRecordByTeachTask(ActionMapping mapping, ActionForm form,
    ...
    Long teachTaskId = (request.getParameter("teachTaskId") == null) ? null : (Long.valueOf(request
    ...
    TeachTask teachTask = (TeachTask) utilService.load(TeachTask.class, teachTaskId);
    List dutyRecordDetailBatchList = dutyService.loadDutyRecordDetailsByTeachTaskId(teachTaskId);
   ...
   * @return
   */
  public ActionForward recordUpdateForm(ActionMapping mapping, ActionForm form, HttpServletRequest request,
   ...
   * @return
   */
  public ActionForward updateRecord(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
  ...
  public ActionForward updateDutyRecordForm(ActionMapping mapping, ActionForm form,
    ...
    List studentDutyRecordDetailsList = dutyService.loadStudentDutyRecordDetails(teachTaskId, dutyDate,
        beginUnitId, endUnitId);
    TeachTask teachTask = new TeachTask();
    if (studentDutyRecordDetailsList.size() >= 1) {
      teachTask = ((RecordDetail) studentDutyRecordDetailsList.get(0)).getDutyRecord().getTeachTask();
    }
    Results.addObject("teachTask", teachTask);
   ...
   * @return
   */
  public ActionForward deleteDutyRecord(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
    String dutyDate = request.getParameter("recordDetail.dutyDate");
    Long beginUnitId = getLong(request, "recordDetail.beginUnit.id");
    Long endUnitId = getLong(request, "recordDetail.endUnit.id");
  ...
  public ActionForward updateDutyRecordDetails(ActionMapping mapping, ActionForm form,
    ...
    if (oldDutyDate.equals(dutyDate) && oldBeginUnit.equals(beginUnitIdString)
      ...
      for (int i = 0; i < stdIdArray.length; i++) {
        dutyService.deleteRecordDetail(stdIdArray[i], teachTaskId, java.sql.Date.valueOf(oldDutyDate),
            new Long(oldBeginUnit), new Long(oldEndUnit));
      }
    }
    for (int i = 0; i < stdIdArray.length; i++) {
      String dutyStatusStdNo = request.getParameter("dutyStatus" + stdIdArray[i]);
      if (StringUtils.isNotBlank(dutyStatusStdNo)) {
        dutyService.updateOrSaveRecordDetail(stdIdArray[i], teachTaskId, java.sql.Date.valueOf(dutyDate),
            new Long(beginUnitIdString), new Long(endUnitIdString), new Long(dutyStatusStdNo));
      } else {
        dutyService.deleteRecordDetail(stdIdArray[i], teachTaskId, java.sql.Date.valueOf(dutyDate), new Long(
    ...
    if (((actionName == null) ? "" : actionName).equals("dutyRecordManagerWithTeacher.do")) { return this
        .redirect(request, "managerByTeacherForm", "flag=search"); }
    String from = request.getParameter("from");
    if (((from == null) ? "" : from).equals("inputDutyRecord")) { return this.redirect(request, new Action(
        "inputDutyRecord", "searchTeachTask"), "attr.updateSuccess", "flag=search"); }
    return this.forward(mapping, request, "recordUpdateSuccess");
  }
   ...
   * @throws Exception
   */
  public ActionForward deleteRecordDetail(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long recordDetailId = getLong(request, "recordDetail.id");
  ...
  public ActionForward studentStatisticsForm(ActionMapping mapping, ActionForm form,
    ...
    if (StringUtils.isEmpty(request.getParameter("flag"))
      ...
      if (StringUtils.isNotEmpty(departmentIds)) Results.addObject("departmentList",
          departmentService.getColleges(departmentIds));
      else Results.addObject("departmentList", Collections.EMPTY_SET);
  ...
  public ActionForward studentRecordStatistics(ActionMapping mapping, ActionForm form,
    ...
    TeachCalendar example = (TeachCalendar) RequestUtil.populate(request, TeachCalendar.class, "calendar");
    TeachCalendar calendar = teachCalendarService.getTeachCalendar(example.getStudentType(),
        example.getYear(), example.getTerm());
    if (null == calendar) { throw new RuntimeException("no calendar found!\n"); }
    ...
    int limit = 500;
    if (stdIds.length > limit) { return this.forwardError(mapping, request, "error.duty.studentDataOverflow",
        new String[] { String.valueOf(limit) }); }
    stdIds = (Long[]) studentIdList.toArray(stdIds);
    List calendarList = new ArrayList();
    calendarList.add(calendar);
    Predicate predicate = AndPredicate.getInstance(new BeanPredicateWrapper("state", new EqualPredicate(
        Boolean.TRUE)), new BeanPredicateWrapper("inSchool", new EqualPredicate(Boolean.TRUE)));

    String dateBegin = request.getParameter("dateBegin");
    String dateEnd = request.getParameter("dateEnd");
    Map partReslutMap = dutyService.dutyRecordStatistics(stdIds, calendarList, dateBegin, dateEnd, true,
        predicate);

    String totalDateBegin = request.getParameter("totalDateBegin");
    String totalDateEnd = request.getParameter("totalDateEnd");
    Map totalReslutMap = dutyService.dutyRecordStatistics(stdIds, calendarList, totalDateBegin, totalDateEnd,
        true, predicate);
  ...
  public ActionForward adminClassRecordStatistics(ActionMapping mapping, ActionForm form,
    ...
    AdminClass adminClass = (AdminClass) RequestUtil.populate(request, AdminClass.class, "adminClass");

    TeachCalendar example = (TeachCalendar) RequestUtil.populate(request, TeachCalendar.class, "calendar");
    TeachCalendar calendar = teachCalendarService.getTeachCalendar(example.getStudentType(),
        example.getYear(), example.getTerm());
    if (null == calendar) { throw new RuntimeException("calendar"); }
    List calendarList = new ArrayList();
    calendarList.add(calendar);
    adminClass.setStdType(example.getStudentType());
    String departmentIds = getDepartmentIdSeq(request);
    String studentTypeIds = getStdTypeIdSeq(request);
    List adminClassList = adminClassService.getAdminClasses(adminClass, studentTypeIds, departmentIds);
    int limit = 50;
    if (adminClassList.size() > limit) { return this.forwardError(mapping, request,
        "error.duty.adminClassDataOverflow", new String[] { String.valueOf(limit) }); }
    ...
    for (Iterator iter = adminClassList.iterator(); iter.hasNext();) {
      ...
      if (weeks == null) {
        ...
        for (int j = 0; j < weeks.intValue(); j++) {
          ...
          Map weekPartReslutTempMap = dutyService.dutyRecordStatistics(stdIdArray, calendarList, dateBegin,
              dateEnd, false, predicate);
          if (weekPartReslutTempMap.get("dutyRecordStatisticsMap") instanceof Map) {
            Object totalMapObject = ((Map) weekPartReslutTempMap.get("dutyRecordStatisticsMap"))
            ...
            if ((totalMapObject instanceof Map)
              ...
              DutyRecordStatisticsStatus dutyRecordStatisticsStatus = (totalClassReslutMap.get(String
                  .valueOf(new Integer(j + 1))) instanceof DutyRecordStatisticsStatus) ? (DutyRecordStatisticsStatus) totalClassReslutMap
                  .get(String.valueOf(new Integer(j + 1))) : new DutyRecordStatisticsStatus();
              dutyRecordStatisticsStatus.setAbsenteeismCount(dutyRecordStatisticsStatus
                  .getAbsenteeismCount(null) == null ? totlStatus.getAbsenteeismCount(null) : new Integer(
                  (dutyRecordStatisticsStatus.getAbsenteeismCount(null).intValue() + (totlStatus
                      .getAbsenteeismCount(null) == null ? 0 : totlStatus.getAbsenteeismCount(null)
                      .intValue()))));
              dutyRecordStatisticsStatus
                  .setAskedForLeaveCount(dutyRecordStatisticsStatus.getAskedForLeaveCount() == null ? totlStatus
                      .getAskedForLeaveCount() : new Integer((dutyRecordStatisticsStatus
                      .getAskedForLeaveCount().intValue() + (totlStatus.getAskedForLeaveCount() == null ? 0
                      : totlStatus.getAskedForLeaveCount().intValue()))));
              dutyRecordStatisticsStatus
                  .setLateCount(dutyRecordStatisticsStatus.getLateCount() == null ? totlStatus.getLateCount()
                      : new Integer((dutyRecordStatisticsStatus.getLateCount().intValue() + (totlStatus
                          .getLateCount() == null ? 0 : totlStatus.getLateCount().intValue()))));
              dutyRecordStatisticsStatus
                  .setLeaveEarlyCount(dutyRecordStatisticsStatus.getLeaveEarlyCount() == null ? totlStatus
                      .getLeaveEarlyCount() : new Integer((dutyRecordStatisticsStatus.getLeaveEarlyCount()
                      .intValue() + (totlStatus.getLeaveEarlyCount() == null ? 0 : totlStatus
                      .getLeaveEarlyCount().intValue()))));
              dutyRecordStatisticsStatus
                  .setPresenceCount(dutyRecordStatisticsStatus.getPresenceCount(null) == null ? totlStatus
                      .getPresenceCount(null) : new Integer((dutyRecordStatisticsStatus
                      .getPresenceCount(null).intValue() + (totlStatus.getPresenceCount(null) == null ? 0
                      : totlStatus.getPresenceCount(null).intValue()))));
              dutyRecordStatisticsStatus
                  .setTotalCount(dutyRecordStatisticsStatus.getTotalCount() == null ? totlStatus
                      .getTotalCount()
                      : new Integer((dutyRecordStatisticsStatus.getTotalCount().intValue() + (totlStatus
                          .getTotalCount() == null ? 0 : totlStatus.getTotalCount().intValue()))));
              totalClassReslutMap.put(String.valueOf(new Integer(j + 1)), dutyRecordStatisticsStatus);
            }
          }
        ...
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
        Map weekTotalReslutTempMap = dutyService.dutyRecordStatistics(stdIdArray, calendarList,
            simpleDateFormat.format(totalWeekBeginDates[0]), simpleDateFormat.format(totalWeekEndDates[1]),
            false, predicate);
        if (weekTotalReslutTempMap instanceof Map) {
          Object totalMapObject = ((Map) weekTotalReslutTempMap.get("dutyRecordStatisticsMap"))
              .get(DutyRecordStatisticsStd.totalNo);
          if ((totalMapObject instanceof Map)
              && (((Map) totalMapObject).get(DutyRecordStatisticsCourse.totalId) instanceof DutyRecordStatisticsStatus)) {
            Map totalMap = (Map) totalMapObject;
            ...
            DutyRecordStatisticsStatus dutyRecordStatisticsStatus = (totalClassReslutMap.get(String
                .valueOf(new Integer(0))) instanceof DutyRecordStatisticsStatus) ? (DutyRecordStatisticsStatus) totalClassReslutMap
                .get(String.valueOf(new Integer(0))) : new DutyRecordStatisticsStatus();
            dutyRecordStatisticsStatus.setAbsenteeismCount(dutyRecordStatisticsStatus
                .getAbsenteeismCount(null) == null ? totlStatus.getAbsenteeismCount(null)
                : new Integer(
                    (dutyRecordStatisticsStatus.getAbsenteeismCount(null).intValue() + (totlStatus
                        .getAbsenteeismCount(null) == null ? 0 : totlStatus.getAbsenteeismCount(null)
                        .intValue()))));
            dutyRecordStatisticsStatus.setAskedForLeaveCount(dutyRecordStatisticsStatus
                .getAskedForLeaveCount() == null ? totlStatus.getAskedForLeaveCount() : new Integer(
                (dutyRecordStatisticsStatus.getAskedForLeaveCount().intValue() + (totlStatus
                    .getAskedForLeaveCount() == null ? 0 : totlStatus.getAskedForLeaveCount().intValue()))));
            dutyRecordStatisticsStatus
                .setLateCount(dutyRecordStatisticsStatus.getLateCount() == null ? totlStatus.getLateCount()
                    : new Integer((dutyRecordStatisticsStatus.getLateCount().intValue() + (totlStatus
                        .getLateCount() == null ? 0 : totlStatus.getLateCount().intValue()))));
            ...
            dutyRecordStatisticsStatus
                .setPresenceCount(dutyRecordStatisticsStatus.getPresenceCount(null) == null ? totlStatus
                    .getPresenceCount(null) : new Integer((dutyRecordStatisticsStatus.getPresenceCount(null)
                    .intValue() + (totlStatus.getPresenceCount(null) == null ? 0 : totlStatus
            ...
            dutyRecordStatisticsStatus
                .setTotalCount(dutyRecordStatisticsStatus.getTotalCount() == null ? totlStatus
                    ...
                    : new Integer((dutyRecordStatisticsStatus.getTotalCount().intValue() + (totlStatus
                        .getTotalCount() == null ? 0 : totlStatus.getTotalCount().intValue()))));
            totalClassReslutMap.put(String.valueOf(new Integer(0)), dutyRecordStatisticsStatus);
          }
        }
    ...
    Results.addObject("weekBegin", weekBegin < totalWeekBegin ? new Integer(weekBegin) : new Integer(
        totalWeekBegin));
    Results.addObject("weekEnd", weekEnd > totalWeekEnd ? new Integer(weekEnd) : new Integer(totalWeekEnd));
  ...
  public Map searchTeachTaskForDuty(Long stdTypeId, HttpServletRequest request) {
    ...
    List teachDepartList = teachTaskService.getTeachDepartsOfTask(studentTypeIds, departmentIds,
        (TeachCalendar) request.getAttribute(Constants.CALENDAR));
