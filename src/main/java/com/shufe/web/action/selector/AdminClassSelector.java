...
public class AdminClassSelector extends RestrictionSupportAction {
   ...
   * @throws Exception
   */
  public ActionForward withSpeciality(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    Pagination adminClassList = adminClassService.getAdminClasses(adminClass, null, null, pageNo, pageSize);

    Results.addObject("adminClassList", adminClassList);
    Results.addObject("specialityId", adminClass.getSpeciality().isPO() ? adminClass.getSpeciality().getId()
        : null);
    Results.addObject("aspectId", adminClass.getAspect().isPO() ? adminClass.getAspect().getId() : null);
    Results.addObject("departmentId", adminClass.getDepartment().isPO() ? adminClass.getDepartment().getId()
        : null);
    Results.addObject("stdTypeId",
        adminClass.getStdType() != null && adminClass.getStdType().isPO() ? adminClass.getStdType().getId()
            : null);
    Results.addObject("enrollYearId",
        StringUtils.isEmpty(adminClass.getEnrollYear()) ? null : adminClass.getEnrollYear());
