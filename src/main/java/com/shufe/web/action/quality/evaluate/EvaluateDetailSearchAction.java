...
public class EvaluateDetailSearchAction extends CalendarRestrictionSupportAction {
  ...
  protected EvaluateStatService evaluateStatService;
  ...
  public ActionForward search(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null == calendarId && null == stdTypeId && StringUtils.isEmpty(year) && StringUtils.isEmpty(term)) { return forward(request); }
    ...
    EntityQuery query = buildQuery(request, calendar);
    addCollection(request, "evaluateTeachers", utilService.search(query));
    if (null == calendar) { return forward(request); }
    ...
    for (Iterator it = evaluateDs.iterator(); it.hasNext();) {
      ...
      evaluateDepartments.put(evaluateDepartment.getDepartment().getId().toString(), evaluateDepartment);
    }
    addSingleParameter(request, "evaluateDepartments", evaluateDepartments);
    addSingleParameter(request, "evaluateCollege", questionnairStatService.getEvaluateCollegeStat(calendar));

    addRankMapParams(request, query);
    return forward(request);
  }
   ...
   * @param query
   */
  protected Collection addRankMapParams(HttpServletRequest request, EntityQuery query) {
    // 准备按条件排名的数据
    query.setLimit(null);
    query.getOrders().clear();
    query.addOrder(OrderUtils.parser("evaluateTeacher.sumScore desc"));
    ...
    // conditionKey.append("_");
    // for (Iterator it = request.getParameterMap().keySet().iterator(); it.hasNext();) {
    // String key = (String) it.next();
    // if (StringUtils.lastIndexOf(key, "evaluateTeacher.") == 0
    // && StringUtils.lastIndexOf(key, "calendar") == -1) {
    // String value = get(request, key);
    // if (StringUtils.isNotBlank(value)) {
    // conditionKey.append(key + ":" + value + "_");
    // }
    // }
    // }
    // System.out.println(conditionKey.toString());
    // // 建立排名存储器
    // EvaluateRank rankCache = getEvaluateRank(request);
    // Map<String, Integer> rankMap = null;
    // if (null == rankCache.getRanksMap(calendar)) {
    // rankCache.addRank(calendar);
    // }
    // rankMap = rankCache.getRanksMap(calendar).get(conditionKey.toString());
    Map rankMap = new HashMap<String, Integer>();
    // 开始排名
    int previousRank = 0, conditionRank = 0;
    String previousSumScore = "";
    for (Iterator it = rs.iterator(); it.hasNext();) {
      EvaluateTeacherStat ets = (EvaluateTeacherStat) it.next();
      conditionRank++;
      if (!previousSumScore.equals(nf.format(ets.getSumScore()))) {
        previousRank = conditionRank;
      }
      rankMap.put(ets.getId().toString(), previousRank);
      previousSumScore = nf.format(ets.getSumScore());
    }
    addSingleParameter(request, "rankMap", rankMap);
    return rs;
  }
  ...
  protected EntityQuery buildQuery(HttpServletRequest request, TeachCalendar calendar) {
    EntityQuery query = new EntityQuery(EvaluateTeacherStat.class, "evaluateTeacher");
    query.add(new Condition("evaluateTeacher.teacher = (:teacher)", getTeacherFromSession(request
        .getSession())));
    ...
    if (null == calendar) {
      ...
      Collection switchs = utilService.load(EvaluateQuerySwitch.class, "teachCalendar.id", calendar.getId());
  ...
  public ActionForward info(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null == evaluateTeacherId) {
      ...
      QuestionnaireStat teacherStat = (QuestionnaireStat) utilService.load(QuestionnaireStat.class,
          teacherStatId);
      Teacher teacher = (Teacher) ((List) utilService.load(Teacher.class, "code",
          getUser(request.getSession()).getName())).get(0);
      evaluateTeacher = questionnairStatService.getEvaluateTeacherStat(teacherStat.getCalendar(),
          teacherStat.getCourse(), teacher);
    } else {
    ...
    addSingleParameter(request, "evaluateDepartment", questionnairStatService.getEvaluateDepartmentStat(
        evaluateTeacher.getCalendar(), (com.shufe.model.system.baseinfo.Department) evaluateTeacher
            .getTeacher().getDepartment()));
    addSingleParameter(request, "evaluateCollege",
        questionnairStatService.getEvaluateCollegeStat(evaluateTeacher.getCalendar()));

    EvaluateDepartmentStat evaluateDepartment = questionnairStatService.getEvaluateDepartmentStat(
        evaluateTeacher.getCalendar(), evaluateTeacher.getDepartment());
    ...
    for (Iterator it = evaluateDepartment.getQuestionsStat().iterator(); it.hasNext();) {
      ...
      departQuestionResults.put(evaluateTeacher.getDepartment().getId() + "_"
          + questionStat.getQuestion().getId(), questionStat);
    }
    addSingleParameter(request, "departQuestionResults", departQuestionResults);

    EvaluateCollegeStat evaluateCollege = questionnairStatService.getEvaluateCollegeStat(evaluateTeacher
        .getCalendar());
    ...
    for (Iterator it = evaluateCollege.getQuestionsStat().iterator(); it.hasNext();) {
      QuestionStat questionStat = (QuestionStat) it.next();
      collegeQuestionResults.put("college_" + questionStat.getQuestion().getId(), questionStat);
    }
  ...
  public void setEvaluateStatService(EvaluateStatService evaluateStatService) {
    this.evaluateStatService = evaluateStatService;
  }
