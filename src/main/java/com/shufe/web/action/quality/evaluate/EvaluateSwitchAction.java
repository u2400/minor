...
public class EvaluateSwitchAction extends CalendarRestrictionExampleTemplateAction {
  ...
  public ActionForward edit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
    EvaluateSwitch evaluateSwitch = (EvaluateSwitch) getEntity(request, EvaluateSwitch.class, "switch");
    ...
    addCollection(request, "stdTypes", CollectionUtils.subtract(stdTypes, evaluateSwitch.getStdTypes()));
  ...
  private EntityQuery builderEntityQuery(HttpServletRequest request) {
    ...
    DataRealmUtils.addDataRealms(query, new String[] { "stdType.id", "depart.id" }, getDataRealms(request));
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    EvaluateSwitch evaluateSwitch = (EvaluateSwitch) populateEntity(request, EvaluateSwitch.class, "switch");
    ...
    if (!ValidEntityPredicate.INSTANCE.evaluate(evaluateSwitch.getCalendar())) {
      evaluateSwitch.setCalendar(teachCalendarService.getTeachCalendar(
          getLong(request, "calendar.studentType.id"), get(request, "calendar.year"),
          get(request, "calendar.term")));
    }
    // 关联的学生类别和部门
    evaluateSwitch.setStdTypes(new HashSet(studentTypeService.getStudentTypes(get(request, "stdTypeIdSeq"))));
    evaluateSwitch.setDeparts(new HashSet(departmentService.getDepartments(get(request, "departmentIdSeq"))));
   ...
   * @throws Exception
   */
  public ActionForward clearData(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    evaluateResultService.batchDeleteEvaluateDatas(getDepartmentIdSeq(request), getStdTypeIdSeq(request),
        teachCalendarIdSeq);
    info(name + "删除了学生评教详细数据,教学日历id为(" + teachCalendarIdSeq + ")学生类别id为:(" + getStdTypeIdSeq(request)
        + ")部门id为:(" + getDepartmentIdSeq(request) + ")");
