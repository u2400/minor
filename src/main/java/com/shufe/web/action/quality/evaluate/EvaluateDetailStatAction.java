...
public class EvaluateDetailStatAction extends EvaluateDetailSearchAction {
  ...
  public ActionForward index(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    addCollection(request, "questionnaires", utilService.load(Questionnaire.class, "state", Boolean.TRUE));
    return forward(request);
  }

  public ActionForward isNotStdList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
  ...
  /**
   * 统计评教记录<br>
   * 包括： <li>初始化统计：是全部统计，即不传 id 的统计 <li>选择性统计：用户选择未统计的评教记录进行统计，即有 id 的统计
   * 
   * @param mapping
   * @param form
   * @param request
  ...
  public ActionForward stat(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    TeachCalendar calendar = teachCalendarService.getTeachCalendar(calendarId);
    // List results = questionnairStatService.buildStatResult(stdTypeIds, departmentIds,
    // calendar);
    if (evaluateStatService.evaluateStat(calendar, stdTypeIds, departmentIds)) {
  ...
  public ActionForward departmentChoiceConfig(ActionMapping mapping, ActionForm form,
    ...
    if (criterias.size() == 1) {
      ...
      return forward(request,
          new Action(this.getClass(), forwardPage, "&evaluationCriteriaId=" + criteria.getId()));
    } else {
   ...
   * @throws Exception
   */
  public ActionForward teacherHistory(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long evaluateTeacherId = getLong(request, "evaluateTeacherId");

    // 准备表头数据
    EvaluateTeacherStat evaluateTeacher = questionnairStatService.getEvaluateTeacherStat(evaluateTeacherId);
    ...
    queryTeacher.add(new Condition("evaluateTeacher.teacher = (:teacher)", evaluateTeacher.getTeacher()));
    queryTeacher.add(new Condition("evaluateTeacher.calendar.start <= (:calendarStart)", evaluateTeacher
        .getCalendar().getStart()));
    ...
    Map evaluateDepartMap = new HashMap();
    for (Iterator it3 = evaluateTeachers.iterator(); it3.hasNext();) {
      EvaluateTeacherStat stat = (EvaluateTeacherStat) it3.next();
      String key = stat.getCalendar().getId() + "";
      if (evaluateDepartMap.containsKey(key)) continue;
      EvaluateDepartmentStat evaluateDepartment = questionnairStatService.getEvaluateDepartmentStat(
          stat.getCalendar(), stat.getDepartment());
      if (null != evaluateDepartment) {
        evaluateDepartMap.put(key, evaluateDepartment);
      }
    }
   ...
   * @throws Exception
   */
  public ActionForward departmentHistory(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Department department = (Department) utilService.load(Department.class, getLong(request, "departmentId"));
    ...
    addCollection(request, "calendarDepartmentResults",
        questionnairStatService.getDepartResultList((List) criteriaItems, department));
    addCollection(request, "calendarCollegeResults",
        questionnairStatService.getCollegeResultList(criteriaItems));
    return forward(request);
  }
  ...
  public ActionForward departmentStatReport(ActionMapping mapping, ActionForm form,
    ...
    query.add(new Condition("questionTeacherStat.evaluateTeacherStat.calendar = :calendar", calendar));
    ...
    addCollection(request, "evaluateDepartments", questionnairStatService.getEvaluateDepartments(calendar));
    return forward(request);
  }
  ...
  public ActionForward departmentHistoryDetail(ActionMapping mapping, ActionForm form,
    ...
    Collection teacherInDepartmentResults = addRankMapParams(request, buildQuery(request, calendar));
    // EntityQuery query = ;
    // query.setLimit(null);
    // Collection teacherInDepartmentResults = utilService.search(query);
    // // System.out.println(teacherInDepartmentResults.size());
    addCollection(request, "teacherResults", teacherInDepartmentResults);

    addSingleParameter(request, "departmentResult",
        questionnairStatService.getDepartmentResults(teacherInDepartmentResults));
    // Collection teacherInCollegeResults =
    // questionnairStatService.getEvaluateTeachers(calendar);
   ...
   * @throws Exception
   */
  public ActionForward historyCollegeStat(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    EvaluationCriteria evaluationCriteria = evaluationCriteriaService.getEvaluationCriteria(getLong(request,
        "evaluationCriteriaId"));
    addSingleParameter(request, "evaluationCriteria", evaluationCriteria);
    addCollection(
        request,
        "evaluateCollegeCalendars",
        questionnairStatService.getCollegeResultList(
            teachCalendarService.getTeachCalendar(getLong(request, "calendar.id")),
            evaluationCriteriaService.getCriteriaItems(evaluationCriteria)));
    return forward(request);
  }
  ...
  public ActionForward departmentGroupItemInfo(ActionMapping mapping, ActionForm form,
    ...
    addSingleParameter(request, "groupItemResults",
        questionnairStatService.getDepartResultList(calendar, department));
    addSingleParameter(request, "collegeResults", questionnairStatService.getCollegeResultList(calendar));
  ...
  public ActionForward questionnaireCollege(ActionMapping mapping, ActionForm form,
    ...
    String existsStatement = "exists (from QuestionResult questionResult where questionResult.result.task.id "
        + "in (select task.id from TeachTask task where task.course.id = evaluateTeacher.course.id and "
        + "task.calendar.id = evaluateTeacher.calendar.id) and questionResult.result.questionnaire.id = :questionnaireId)";
    query.add(new Condition(existsStatement, questionnaire.getId()));
    addCollection(request, "teacherResults", utilService.search(query));

    addSingleParameter(request, "collegeEvaluate", questionnairStatService.getEvaluateCollegeStat(calendar));
    return forward(request);
  }
  ...
  public ActionForward depertmentCourseStat(ActionMapping mapping, ActionForm form,
    ...
    request.setAttribute("collegeResult", questionnairStatService.getCollegeResultList(departmentResults));
    return forward(request);
  }
  ...
  protected void removeBeenStat(HttpServletRequest request, boolean isAll) {
    ...
    Collection recordTs = utilService.load(EvaluateTeacherStat.class, "calendar.id", calendarId);
    Collection recordDs = utilService.load(EvaluateDepartmentStat.class, "calendar.id", calendarId);
    Collection recordCs = utilService.load(EvaluateCollegeStat.class, "calendar.id", calendarId);

    // 选择性删除
    if (!isAll) {
      Long[] evaluateTeacherIds = SeqStringUtil.transformToLong(get(request, "evaluateTeacherIds"));
      if (null != evaluateTeacherIds && evaluateTeacherIds.length > 0) {
        ...
        // 不清除使用中的开课院系
        EntityQuery queryD = new EntityQuery(EvaluateDepartmentStat.class, "evaluateDepartment");
        queryD.add(new Condition("evaluateDepartment.calendar.id = (:calendarId)", calendarId));
        queryD
            .add(new Condition(
        ...
        for (Iterator itD1 = recordDs.iterator(); itD1.hasNext();) {
          EvaluateDepartmentStat evaluateDepartment = (EvaluateDepartmentStat) itD1.next();
  ...
  protected EntityQuery buildQuery(HttpServletRequest request, TeachCalendar calendar) {
    ...
    if (get(request, "orderBy") != null) query.addOrder(OrderUtils.parser(get(request, "orderBy")));
    query.addOrder(OrderUtils.parser("evaluateTeacher.id"));
    return query;
  }
