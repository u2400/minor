...
public class QuestionnaireStatTeacherAction extends QuestionnaireStatSearchAction {
  ...
  public EntityQuery buildEntityQuery(HttpServletRequest request) {
    ...
    if (null != stdTypeId) {
      DataRealmUtils.addDataRealm(entityQuery, new String[] { "questionnaireStat.stdType.id" },
    ...
    if (teachCalendars.isEmpty()) {
      ...
      entityQuery.add(new Condition("questionnaireStat.calendar in (:calendars)", teachCalendars));
    }
    return entityQuery;
  }
  ...
  public ActionForward search(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    for (Iterator it = questionnaireStatTeachers.iterator(); it.hasNext();) {
      ...
      evaluateDetailMap.put(
          teacherStat.getId().toString(),
          questionnairStatService.isEmptyOfEvaluateTeacherStat(teacherStat.getCalendar(),
              teacherStat.getCourse(), teacherStat.getTeacher()));
    }
    ...
    request.setAttribute("criteria", utilService.get(EvaluationCriteria.class, EvaluationCriteria.DEFAULTID));
    return forward(request);
  }
   ...
   * @throws Exception
   */
  public ActionForward evaluatePersonInfo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
