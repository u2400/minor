...
public class TeachProductAction extends RestrictionExampleTemplateAction {
  ...
  private void prepare(HttpServletRequest request) {
    ...
    request.setAttribute("productionAwardTypes", baseCodeService.getCodes(ProductionAwardType.class));
    request.setAttribute("productionAwardLevels", baseCodeService.getCodes(ProductionAwardLevel.class));
    setDataRealm(request, hasStdTypeDepart);
  }
