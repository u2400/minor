  ...
  class CharacterExtractor extends DefaultPropertyExtractor {
    ...
    protected Object extract(Object target, String property) throws Exception {
      ...
      if ("isCourseEvaluate".equals(property)) {
        if (Boolean.TRUE.equals(value)) return getMessage(locale, "attr.evaluate.teacher");
        else return getMessage(locale, "attr.evaluate.course");
      }
      if ("affirm".equals(property)) {
        if (Boolean.TRUE.equals(value)) return getMessage(locale, "common.yes");
        else return getMessage(locale, "common.no");
      }
