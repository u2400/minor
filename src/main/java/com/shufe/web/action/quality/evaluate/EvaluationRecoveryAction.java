...
public class EvaluationRecoveryAction extends CalendarRestrictionSupportAction {
  ...
  public ActionForward index(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    request.setAttribute("stdTypes", getStdTypesOf(stdTypeId, "questionnaireRecycleRate", request));
  ...
  public ActionForward doStatisticInCourseTask(ActionMapping mapping, ActionForm form,
    ...
    Collection teachCalendars = utilService.load(TeachCalendar.class, "id",
        SeqStringUtil.transformToLong(teachCalendarIds));
    List statisticRecycleRateList = questionnaireRecycleRateService.getAllRateByCourseTask(departmentIds,
        studentTypeIds, teachCalendars);
    Map recycleMap = new HashMap();
    List recycleObjectList = questionnaireRecycleRateService.getRecycleRateObject(departmentIds,
        studentTypeIds, teachCalendars);
    ...
    for (Iterator iter = statisticRecycleRateList.iterator(); iter.hasNext();) {
      ...
      QuestionnaireRecycleRate recycleObject = (QuestionnaireRecycleRate) recycleMap.get(department.getId()
          + "-" + stdType.getId() + "-" + teachCalendar.getId());
    ...
    Collections.sort(tempList, new Comparator() {
      ...
      public int compare(Object arg0, Object arg1) {
        ...
        if (teachCalendar0.getStart().before(teachCalendar1.getStart())) { return 1; }
        return -1;
      }
    });
  ...
  public ActionForward search(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isNotBlank(query) && "query".equals(query)) {
      ...
      if (StringUtils.isNotBlank(teachCalendarId)) {
        teachCalendar = (TeachCalendar) utilService.load(TeachCalendar.class, Long.valueOf(teachCalendarId));
      } else {
        teachCalendar = teachCalendarService.getTeachCalendar(Long.valueOf(requireStdTypeIdSeq), year, term);
      ...
      List recycleRateList = questionnaireRecycleRateService.getRecycleRateObject(requireDeprtIdSeq,
          requireStdTypeIdSeq, teachCalendars);
      ...
      if (StringUtils.isNotBlank(teachCalendarId)) {
        calendar = (TeachCalendar) utilService.load(TeachCalendar.class, Long.valueOf(teachCalendarId));
      } else {
        List stdTypeList = studentTypeService.getStudentTypes(stdTypeIdSeq);
        calendar = teachCalendarService.getNearestCalendar((StudentType) stdTypeList.get(0));
      }
  ...
  public ActionForward doGetDetailInfoOfTask(ActionMapping mapping, ActionForm form,
    ...
    List detailTaskInfo = questionnaireRecycleRateService.getDetailInfoOfTask(
        departmentIds,
        departmentId,
        studentTypeIds,
        Collections.singleton(teachCalendarService.getTeachCalendar(getLong(request, "stdTypeId"),
            get(request, "year"), get(request, "term"))), studentOrTeacher);
