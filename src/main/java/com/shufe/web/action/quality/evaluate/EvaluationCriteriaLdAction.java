...
public class EvaluationCriteriaLdAction extends RestrictionExampleTemplateAction {
  ...
  public ActionForward remove(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    String evaluationCriteriaIds = request.getParameter("evaluationCriteriaIds");
    if (StringUtils.contains("," + evaluationCriteriaIds + ",", "," + EvaluationCriteria.DEFAULTID + ",")) {
  ...
  protected ActionForward saveAndForwad(HttpServletRequest request, Entity entity) throws Exception {
    ...
    if (null != criteriaCount) {
      for (int i = 0; i < criteriaCount.intValue(); i++) {
        String criteriaItemName = get(request, "criteriaItem" + i + ".name");
        if (StringUtils.isNotEmpty(criteriaItemName)) {
          evalutionCriteria.addItem((EvaluationCriteriaItem) populateEntity(request,
              EvaluationCriteriaItem.class, "criteriaItem" + i));
        }
      }
    }
