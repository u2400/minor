...
public class ClassroomAction extends ClassRoomSearchAction {
  ...
  public ActionForward edit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (ValidEntityPredicate.INSTANCE.evaluate(classroom)) {
      if (!DataAuthorityUtil.isInDataRealm("Classroom", classroom, null, getDepartmentIdSeq(request))) return forwardError(
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    // 检查是否重复
    if (utilService.duplicate(Classroom.class, roomId, "code", request.getParameter("classroom.code"))) { return forward(
    ...
    if (StringUtils.isEmpty(departIdSeq)) return forwardError(mapping, request, "useDepart.notExists");
    ...
    try {
      if (null == roomId) {
        ...
        populate(params, classroom);
        if (null == classroom.getDepartments()) classroom.setDepartments(userDeparts);
        else {
          Collection tobeDeleted = CollectionUtils.subtract(classroom.getDepartments(), userDeparts);
          Collection tobeAdded = CollectionUtils.subtract(userDeparts, classroom.getDepartments());
      ...
      logHelper.info(request, "Failure save or update a classroom with name:" + classroom.getName(), e);
      return forward(mapping, request, new String[] { "entity.classroom", "error.model.existed" }, "error");
    } catch (Exception e) {
      logHelper.info(request, "Failure save or update a classroom with name:" + classroom.getName(), e);
    ...
    } else return redirect(request, "search", "info.save.success");
  }
