...
public class TeachCalendarAction extends RestrictionSupportAction {
   ...
   * @throws Exception
   */
  public ActionForward schemeInfo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
   ...
   * @throws Exception
   */
  public ActionForward calendarList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long schemeId = getLong(request, "scheme.id");
    if (null == schemeId) return forward(mapping, request, "error.stdType.id.needed", "error");
   ...
   * @throws Exception
   */
  public ActionForward onCampusTimeList(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String stdTypeId = request.getParameter(Constants.STUDENTTYPE_KEY);
    if (StringUtils.isEmpty(stdTypeId)) return forward(mapping, request, "error.stdType.id.needed", "error");

    StudentType stdType = studentTypeService.getStudentType(Long.valueOf(stdTypeId));
    if (null == stdType) return forward(mapping, request, "error.stdType.notExists");

    Collection onCampusTimes = calendarService.getOnCampusTimesFor(stdType, getPageLimit(request));
   ...
   * @throws Exception
   */
  public ActionForward editCalendar(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long calendarId = getLong(request, Constants.CALENDAR_KEY);
    Long schemeId = getLong(request, "scheme.id");
    TeachCalendarScheme scheme = (TeachCalendarScheme) utilService.get(TeachCalendarScheme.class, schemeId);
    TeachCalendar calendar = null;
    if (null == calendarId) {
      calendar = (TeachCalendar) RequestUtil.populate(request, TeachCalendar.class, Constants.CALENDAR);
   ...
   * @throws Exception
   */
  public ActionForward editSchemeForm(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long schemeId = getLong(request, "scheme.id");
    Collection studentTypes = getStdTypes(request);
    if (null != schemeId) {
      // 得到该方案已经选学生类别
      TeachCalendarScheme scheme = (TeachCalendarScheme) utilService
          .load(TeachCalendarScheme.class, schemeId);
      request.setAttribute("scheme", scheme);
   ...
   * @throws Exception
   */
  public ActionForward editOnCampusTime(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    if (StringUtils.isEmpty(onCampusTimeId)) {
      time = (OnCampusTime) RequestUtil.populate(request, OnCampusTime.class, Constants.ONCAMPUSTIME);
      time.setStdType(studentTypeService.getStudentType(Long.valueOf(request
          .getParameter(Constants.STUDENTTYPE_KEY))));
    } else {
      time = (OnCampusTime) utilService.load(OnCampusTime.class, Long.valueOf(onCampusTimeId));
    }
   ...
   * @throws Exception
   */
  public ActionForward saveCalendar(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    try {
      if (!calendar.isPO()) {
        if (calendarService.checkDateCollision(calendar)) return forward(mapping, request,
            "error.calendar.dateCollision", "error");
        Long nextCalendarId = getLong(request, "nextCalendar.id");
        Long preCalendarId = getLong(request, "preCalendar.id");
        TeachCalendar preCalendar = null, nextCalendar = null;
        if (null != nextCalendarId) {
    ...
    return redirect(request, "calendarList", "info.save.success",
        "&scheme.id=" + request.getParameter("scheme.id"));
  }
   ...
   * @throws Exception
   */
  public ActionForward saveOnCampusTime(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    // 持久化，如果失败跳转到失败页面
    try {
      time.setEnrollCalendar(calendarService.getTeachCalendar(time.getEnrollCalendar().getStudentType(), time
          .getEnrollCalendar().getYear(), time.getEnrollCalendar().getTerm()));
      time.setGraduateCalendar(calendarService.getTeachCalendar(time.getGraduateCalendar().getStudentType(),
          time.getGraduateCalendar().getYear(), time.getGraduateCalendar().getTerm()));
      if (!time.isPO()) {
        if (calendarService.checkOnCampusTimeExist(time)) return forwardError(mapping, request, new String[] {
            "entity.onCampusTime", "error.model.existed" });
        utilService.saveOrUpdate(time);
      } else {
        OnCampusTime savedTime = (OnCampusTime) utilService.load(OnCampusTime.class, time.getId());
   ...
   * @throws Exception
   */
  public ActionForward saveScheme(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    TeachCalendarScheme scheme = (TeachCalendarScheme) populateEntity(request, TeachCalendarScheme.class,
        "scheme");
   ...
   * @throws Exception
   */
  public ActionForward removeCalendar(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    if (StringUtils.isNotEmpty(calendarId)) {
      ...
      try {
        ...
        return forward(request, new Action("", "calendarList"), "error.calendar.deletedFailure");
      }
   ...
   * @throws Exception
   */
  public ActionForward removeOnCampusTime(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String onCampusTimeId = request.getParameter(Constants.ONCAMPUSTIME_KEY);
    if (StringUtils.isNotEmpty(onCampusTimeId)) {
      OnCampusTime time = (OnCampusTime) utilService.load(OnCampusTime.class, Long.valueOf(onCampusTimeId));
      try {
   ...
   * @throws Exception
   */
  public ActionForward calendarInfo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String calendarId = request.getParameter(Constants.CALENDAR_KEY);
    if (StringUtils.isEmpty(calendarId)) return forwardError(mapping, request, new String[] {
        "entity.calendar", "error.model.id.needed" });
    TeachCalendar calendar = calendarService.getTeachCalendar(Long.valueOf(calendarId));

    if (null == calendar) return forwardError(mapping, request, new String[] { "entity.calendar",
