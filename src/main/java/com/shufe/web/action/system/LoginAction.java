...
public class LoginAction extends DispatchBasicAction {
  ...
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isNotEmpty(errorMsg)) {
      if(errorMsg.equals("security.error.userNotExist")){
        errorMsg="security.error.password";
      }
      request.getSession().removeAttribute(Authentication.USERID);
      if (StringUtils.equals("security.error.userUnactive", errorMsg)) {
        User user = (User) utilService.load(User.class, "name", loginForm.getName()).iterator().next();
  ...
  protected String login(HttpServletRequest request, LoginForm loginForm) {
    UserNamePasswordAuthentication auth = new UserNamePasswordAuthentication(loginForm.getName(),
