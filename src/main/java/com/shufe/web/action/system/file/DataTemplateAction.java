...
public class DataTemplateAction extends FileAction {
  ...
  public void download(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (null == path) {
      ...
      Document document = (Document) utilService.get(Document.class, documentId);
      if (null == document) {
        response.getWriter().write("without template with id:" + documentId);
        return;
      }
      download(request, response, getFileRealPath(FilePath.TEMPLATE_UPLOAD, request) + document.getPath());
    } else {
      download(request, response, getFileRealPath(FilePath.TEMPLATE_UPLOAD, request) + path);
    }
  }
}
