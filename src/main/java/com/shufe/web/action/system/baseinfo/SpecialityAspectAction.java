...
public class SpecialityAspectAction extends SpecialityAspectSearchAction {
  ...
  public ActionForward edit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    SpecialityAspect specialityAspect = (SpecialityAspect) getEntity(request, SpecialityAspect.class,
        "specialityAspect");
    if (ValidEntityPredicate.INSTANCE.evaluate(specialityAspect)) {
      if (!DataAuthorityUtil.isInDataRealm("Speciality", specialityAspect.getSpeciality(), stdTypeDataRealm,
          departDataRealm)) return forwardError(mapping, request, "error.depart.dataRealm.insufficient");
    }
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    // 检查是否重复
    if (utilService.duplicate(SpecialityAspect.class, specialityAspectId, "code",
        request.getParameter("specialityAspect.code"))) { return forward(request, new Action("", "edit"),
        "error.code.existed"); }
    ...
    try {
      if (null == specialityAspectId) {
        ...
        logHelper.info(request, "Create a specialityAspect with name:" + specialityAspect.getName());
      } else {
        specialityAspect = specialityAspectService.getSpecialityAspect(specialityAspectId);
        logHelper.info(request, "Update a specialityAspect with name:" + specialityAspect.getName());
        populate(params, specialityAspect);
      }
      ...
      logHelper.info(request,
          "Failure save or update a specialityAspect with name:" + specialityAspect.getName(), e);
      return forward(mapping, request, new String[] { "entity.specialityAspect", "error.model.existed" },
          "error");
    } catch (Exception e) {
      logHelper.info(request,
          "Failure save or update a specialityAspect with name:" + specialityAspect.getName(), e);
