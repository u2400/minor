...
public class StudentTypeAction extends StudentTypeSearchAction {
  ...
  public ActionForward edit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (ValidEntityPredicate.INSTANCE.evaluate(studentType)) {
      ...
      if (!StringUtils.contains(stdTypeIds, studentType.getId().toString())) { return forwardError(mapping,
          request, "error.stdType.dataRealm.insufficient"); }
    }
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    // 检查是否重复
    if (utilService.duplicate(StudentType.class, studentTypeId, "code",
        request.getParameter("studentType.code"))) { return forward(request, new Action("", "edit"),
        "error.code.existed"); }
    ...
    try {
      ...
      List departs = utilService.load(Department.class, "id", SeqStringUtil.transformToLong(departIdSeq));
      ...
      logHelper.info(request, "Failure save or update a studentType with name:" + studentType.getName(), e);
      return forward(mapping, request, new String[] { "entity.studentType", "error.model.existed" }, "error");
    } catch (Exception e) {
      logHelper.info(request, "Failure save or update a studentType with name:" + studentType.getName(), e);
