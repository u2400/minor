...
public class HomeAction extends DispatchBasicAction {
  ...
  public ActionForward index(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isEmpty(facade)) {
      facade = (String) getSystemConfig().getConfigItemValue(HomeAction.FACADE);
    }
    if (StringUtils.isEmpty(facade)) {
      facade = DEFAULT;
    }
    if (facade.equals(DEFAULT)) return forward(request, new Action(DefaultHomeAction.class, "index"));
