...
public class SpecialityStatAction extends RestrictionSupportAction {
   ...
   * @throws Exception
   */
  public ActionForward distribution(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    EntityQuery query = new EntityQuery(Speciality.class, "speciality");
    populateConditions(request, query);
   ...
   * @throws Exception
   */
  public ActionForward structure(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    EntityQuery query = new EntityQuery(Speciality.class, "speciality");
    query.setSelect("new  com.shufe.service.util.stat.StatItem(speciality.subjectCategory.id,count(*))");
    populateConditions(request, query);
    query.groupBy("speciality.subjectCategory.id");
    Collection stats = utilService.search(query);
    new StatHelper(utilService).setStatEntities(stats, SubjectCategory.class);
    addCollection(request, "stats", stats);
    return forward(request);
  }

  public void setSpecialityStatService(SpecialityStatService specialityStatService) {
