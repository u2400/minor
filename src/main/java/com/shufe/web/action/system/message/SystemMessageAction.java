...
public class SystemMessageAction extends RestrictionSupportAction {
  ...
  public ActionForward info(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    SystemMessage systemMessage = (SystemMessage) getEntity(request, SystemMessage.class, "systemMessage");
   ...
   * @throws Exception
   */
  public ActionForward sendbox(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    EntityQuery query = buildQuery(request);
  ...
  public ActionForward setStatus(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    if (StringUtils.isEmpty(messageIds)) {
      ...
      if (null == status) return forwardError(mapping, request, "error.parameters.needed");
      if (status.intValue() < SystemMessage.newly.intValue()
          || status.intValue() > SystemMessage.inTrash.intValue()) { return forwardError(mapping, request,
          "error.parameters.illegal"); }
      List messages = utilService.load(SystemMessage.class, "id", SeqStringUtil.transformToLong(messageIds));
  ...
  public ActionForward newMessage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ...
    if (StringUtils.isEmpty(who)) return forwardError(mapping, request, "error.parameters.needed");
    return forward(request);
  }
  ...
  public ActionForward reply(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    SystemMessage systemMessage = (SystemMessage) utilService.load(SystemMessage.class, Long.valueOf(id));
   ...
   * @throws Exception
   */
  public ActionForward quickSend(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    String receiptorIds = request.getParameter("receiptorIds");
    if (StringUtils.isEmpty(receiptorIds)) return forwardError(mapping, request, "error.parameters.needed");
    List recipients = utilService.load(User.class, "name", StringUtils.split(receiptorIds, ","));
    if (recipients.isEmpty()) return forwardError(mapping, request, "error.model.notExist");
