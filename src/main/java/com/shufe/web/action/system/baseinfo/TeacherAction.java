...
public class TeacherAction extends TeacherSearchAction {
  ...
  public ActionForward edit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (ValidEntityPredicate.INSTANCE.evaluate(teacher)) {
      if (!DataAuthorityUtil.isInDataRealm("Teacher", teacher, null, getDepartmentIdSeq(request))) return forwardError(
          mapping, request, "error.depart.dataRealm.insufficient");
    } else {
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    // 检查是否重复
    if (utilService.duplicate(Teacher.class, teacherId, "code", request.getParameter("teacher.code"))) { return forward(
        request, new Action("", "edit"), "error.code.existed"); }
    ...
    if (null == teacherId && StringUtils.isNotEmpty(code)) {
      ...
      if (null != existedTeacher) { return forward(mapping, request, new String[] { "entity.teacher",
    ...
    try {
      ...
      if (!teacher.isPO()) {
        ...
        depart = (Department) utilService.load(Department.class, teacher.getDepartment().getId());
      ...
      if (!codeGenerator.isValidCode(teacher.getCode())) {
        ...
        if (codeGenerator.isValidCode(teacherNO)) {
          ...
          messages.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage("error.codeGen.failure"));
          addErrors(request, messages);
          return forward(request, new Action(this.getClass(), "edit"));
        }
      }

      Boolean isRemoveInVideo = getBoolean(request, "isRemoveInVideo");

      File toBeRemoveFile = null;
      if (BooleanUtils.isTrue(isRemoveInVideo)) {
        ...
        toBeRemoveFile = new File(fileAbsolutePath);
        teacher.setVideoName(null);
      } else {
        String result = upload(request, getFileRealPath(FilePath.TEACHER_PHOTO, request), teacher);

        if (StringUtils.equals("____ERROR____", result)) {
          return forward(request, "/pages/system/systemConfig/filePathError");
        } else if (null != result) {
          teacher.setVideoName(result);
        } else if (null == result) {
          // 没有上传文件
        }
      }

      if (null != toBeRemoveFile) {
      ...
      userService.createTeacherUser(getUser(request.getSession()), teacher);
    } catch (PojoExistException e) {
      logHelper.info(request, "Failure save or upate a teacher with name:" + teacher.getName(), e);
      return forward(mapping, request, new String[] { "entity.teacher", "error.model.existed" }, "error");
    } catch (Exception e) {
      e.printStackTrace();
      logHelper.info(request, "Failure save or upate a teacher with name:" + teacher.getName(), e);
   ...
   * @return
   */
  protected String upload(HttpServletRequest request, String storeAbsolutePath, Teacher teacher)
      throws Exception {
    ...
    for (Iterator<?> iter = items.iterator(); iter.hasNext();) {
      ...
      if (!element.isFormField()) {
        ...
        if (StringUtils.isNotBlank(teacher.getVideoName())) {
          File oldFile = new File(storeAbsolutePath + EncryptUtil.encodePassword(teacher.getVideoName()));
          if (oldFile.exists()) {
            oldFile.delete();
          }
        }
        File sourceFile = new File(fileName);
        File targetFile = new File(storeAbsolutePath + EncryptUtil.encodePassword(sourceFile.getName()));
        FileUtils.touch(targetFile);
        itemtemp.write(targetFile);
