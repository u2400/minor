...
public class StdSubstituteCourseAction extends RestrictionExampleTemplateAction {
  ...
  protected ActionForward saveAndForwad(HttpServletRequest request, Entity entity) throws Exception {
    ...
    if (null != substitutionCourseId) {
      query.add(new Condition("substitutionCourse.id != (:substitutionCourseId)", substitutionCourseId));
    }
  ...
  public ActionForward search(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    MajorType majorType = new MajorType(getLong(request, "substitutionCourse.std.firstMajor.majorType.id"));
    ...
    DataRealmUtils.addDataRealms(query, new String[] { "substitutionCourse.std.type.id", departName },
        getDataRealmsWith(getLong(request, "substitutionCourse.std.type.id"), request));
  ...
  public ActionForward selector(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (Boolean.TRUE.equals(isWithPlan)) {
      ...
      if (list.size() != 0) {
        query.add(new Condition("plan.std.code = :stdCode", get(request, "substitutionCourse.std.code")));
      } else {
  ...
  public ActionForward info(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    addSingleParameter(request, "substitutionCourse",
        utilService.load(DefaultStdSubstituteCourse.class, getLong(request, "substitutionCourseId")));
    return forward(request);
  }
