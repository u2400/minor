...
public class DefaultCalculator implements GradeCalculator {
  ...
  public DefaultCalculator() {
    makeupIsFinal = SystemConfigLoader.getConfig().getBooleanParam("course.grade.makeupIsFinal");
  }
  ...
  private void calcLXSFinal(CourseGrade grade) {
  ...
  private void calcLXSGA(CourseGrade grade) {
    if ((grade.getGradeTask() != null) && (grade.getCourseTakeType() != null)) {
      ...
      if ((endGradePercent == null) && (usualGradePercent.floatValue() == 1.0D)) {
        ...
        for (Iterator iter = percentMap.keySet().iterator(); iter.hasNext();) {
          GradeType gradeType = (GradeType) iter.next();
          if (gradeType.getId().equals(GradeType.END)) ga = (float) (ga + 0.4D * endScore.floatValue());
          else {
