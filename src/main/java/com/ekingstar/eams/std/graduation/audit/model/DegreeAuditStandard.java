...
public class DegreeAuditStandard extends LongIdObject {
  ...
  public String audit(AuditResult result) {
    ...
    if (Boolean.TRUE.equals(getIsCompletePlan())) {
      if (!Boolean.TRUE.equals(result.getIsCompletePlan())) { return NOT_COMPLETEPLAN; }
    }
    // 审核GPA
    if (null != getGPA()) {
      if (null == result.getGPA()) return LOW_GPA;
      if (result.getGPA().floatValue() - getGPA().floatValue() < 0) return LOW_GPA;
    }
    // 审核处分
    if (null != getLowestPunishType() && null != result.getPunishmentType()) {
      if (result.getPunishmentType().equals(getLowestPunishType())
          || result.getPunishmentType().isSeriousThan(getLowestPunishType())) { return TOO_SERIOUSPUNISHMENT; }
    }
    // 审核外语水平
    if (!getLanguageExams().isEmpty()) {
      if (null == result.getLanguageGrades()) return NOTPASS_LANGUAGEEXAM;
      boolean pass = false;
      for (Iterator iter = getLanguageExams().iterator(); iter.hasNext();) {
        ...
        for (Iterator iterator = result.getLanguageGrades().iterator(); iterator.hasNext();) {
          ...
          if (examCategory.equals(grade.getCategory()) && grade.getIsPass().booleanValue()) {
            pass = true;
            break;
          }
        }
      ...
      if (!pass) return NOTPASS_LANGUAGEEXAM;
    }
    // 审核计算机水平
    if (!getComputerExams().isEmpty()) {
      if (null == result.getComputerGrades()) return NOTPASS_COMPUTEREXAM;
      boolean pass = false;
      ...
      if (!pass) return NOTPASS_COMPUTEREXAM;
    }
    // 审核论文答辩成绩
    if (null != getThesisScore()) {
      if (null == result.getThesisScore()) return LOW_THESISSCORE;
      if (result.getThesisScore().floatValue() - getThesisScore().floatValue() < 0) return LOW_THESISSCORE;
    }
    // 审核博士综合考试
    if (Boolean.TRUE.equals(getIsPassDoctorComprehensiveExam())) {
      if (!Boolean.TRUE.equals(result.getIsPassDoctorComprehensiveExam())) { return NOTPASS_DOCTORCOMPREHENSIVEEXAM; }
    }
    // 审核核心刊物发表数
    if (null != getThesisInCoreMagazine()) {
      if (null == result.getThesisInCoreMagazine()) return LOW_THESISINCOREMAGAZINE;
      if (result.getThesisInCoreMagazine().intValue() - getThesisInCoreMagazine().intValue() < 0) return LOW_THESISINCOREMAGAZINE;
    }
    return "";
  }
