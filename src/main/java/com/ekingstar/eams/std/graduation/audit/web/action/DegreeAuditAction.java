...
public class DegreeAuditAction extends AuditResultSearchAction {
  ...
  public ActionForward audit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (StringUtils.isNotEmpty(stdIds)) {
      ...
      List auditResults = utilService.load(AuditResult.class, "id",
          SeqStringUtil.transformToLong(auditResultIds));
  ...
  public ActionForward edit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long auditResultId = getLong(request, "auditResultId");
    AuditResult auditResult = null;
    if (null != auditResultId) {
      auditResult = (AuditResult) utilService.get(AuditResult.class, auditResultId);
    } else {
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Long auditResultId = getLong(request, "auditResult.id");
    AuditResult auditResult = null;
    if (null != auditResultId) {
      auditResult = (AuditResult) utilService.load(AuditResult.class, auditResultId);
    } else {
      auditResult = new AuditResult();
      Long stdCode = getLong(request, "auditResult.std.id");
      Student std = (Student) utilService.load(Student.class, stdCode);
      auditResult.setStd(std);
      Long yixi = getLong(request, "majorType");
      MajorType majorType = (MajorType) utilService.load(MajorType.class, yixi);
      auditResult.setMajorType(majorType);
    }

    Map params = getParams(request, "auditResult");
    populate(params, auditResult);
    String certificateNo = get(request, "diplomaNo");
    auditResult.setCertificateNo(certificateNo);
    String certificateNos = get(request, "score");
    auditResult.setDiplomaNo(certificateNos);

    utilService.saveOrUpdate(auditResult);
  ...
  public ActionForward importData(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    Transfer transfer = ImporterServletSupport.buildEntityImporter(request, AuditResult.class, tr);
