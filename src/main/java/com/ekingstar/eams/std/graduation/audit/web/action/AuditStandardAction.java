...
public class AuditStandardAction extends AuditStandardSearchAction {
   ...
   * @return
   */
  public ActionForward updateFrame(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
    ...
    Long auditStandardId = (auditStandardIdString == null || auditStandardIdString.equals("")) ? (request
        ...
        .getAttribute("auditStandardId")) : Long.valueOf(auditStandardIdString);
    Results.addObject("auditStandard", utilService.load(AuditStandard.class, auditStandardId));
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    String[] disauditCourseTypeIdArray = StringUtils.split(request.getParameter("disauditCourseTypeId"), ",");
    ...
    String[] convertableCourseTypeIdArray = StringUtils.split(
        request.getParameter("convertableCourseTypeId"), ",");
   ...
   * @return
   */
  public ActionForward detailById(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
    AuditStandard auditStandard = (AuditStandard) utilService.load(AuditStandard.class,
        Long.valueOf(request.getParameter("auditStandardId")));
    Results.addObject("auditStandard", auditStandard);
  ...
  public ActionForward detail(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) {
    Student student = (Student) utilService.load(Student.class, "code", getLoginName(request)).get(0);

    Map auditStandardMap = this.auditStandardService
        .searchStudentAuditStandard(student, AuditType.PLAN_AUDIT);
    if (!auditStandardMap.isEmpty()) Results
        .addObject("auditStandard", auditStandardMap.get(MajorType.FIRST));
