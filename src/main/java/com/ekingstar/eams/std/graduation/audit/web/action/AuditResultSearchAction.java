...
public class AuditResultSearchAction extends RestrictionSupportAction {
  ...
  public ActionForward index(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    addCollection(request, "standards", degreeAuditService.getAuditStandards(getStdTypes(request)));
  ...
  public ActionForward info(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    DegreeAuditStandard standard = (DegreeAuditStandard) utilService.get(DegreeAuditStandard.class,
        standardId);
    ...
    List degreePlanCourse = teachPlanService.getPlanCourseOfDegree(result.getStd(), result.getMajorType());
    for (Iterator iter = degreePlanCourse.iterator(); iter.hasNext();) {
      // 查询各学位课程对应成绩
      PlanCourse planCourse = (PlanCourse) iter.next();
      List gradeList = gradeService.getDegreeCourseGrade(result.getStd().getId(), planCourse.getCourse()
          .getId());
  ...
  protected EntityQuery buildQuery(HttpServletRequest request, Boolean audited) {
    ...
    Integer degreeState = getInteger(request, "degreeState");
    if (null != degreeState) {
      switch (degreeState.intValue()) {
      case 0: // ...
        // 以下四个情况的总和
        StringBuilder hql = new StringBuilder();
        hql.append("(");
        hql.append("std.graduateAuditStatus = true");
        hql.append(" and ");
        hql.append("std.degreeInfo.graduateOn is not null");
        hql.append(" and ");
        hql.append("std.degreeInfo.certificateNo is not null");
        hql.append(")");
        hql.append(" or ");
        hql.append("(");
        hql.append("std.graduateAuditStatus = true");
        hql.append(" and ");
        hql.append("std.degreeInfo.graduateOn is null");
        hql.append(" and ");
        hql.append("std.degreeInfo.certificateNo is null");
        hql.append(")");
        hql.append(" or ");
        hql.append("(");
        hql.append("std.graduateAuditStatus = false");
        hql.append(" and ");
        hql.append("std.degreeInfo.graduateOn is null");
        hql.append(" and ");
        hql.append("std.degreeInfo.certificateNo is null");
        ...
        hql.append(" or ");
        hql.append("(");
        hql.append("std.graduateAuditStatus is null");
        hql.append(" and ");
        hql.append("not exists (from AuditResult auditResult where auditResult.std = std and auditResult.majorType.id=:majorTypeId)");
        hql.append(")");
        query.add(new Condition(hql.toString(), majorTypeId));
        break;
      case 1: // 毕业+有学位
        query.add(new Condition("std.graduateAuditStatus = true"));
        query.add(new Condition("std.degreeInfo.graduateOn is not null"));
        query.add(new Condition("std.degreeInfo.certificateNo is not null"));
        break;
      case 2: // 毕业
        query.add(new Condition("std.graduateAuditStatus = true"));
        query.add(new Condition("std.degreeInfo.graduateOn is null"));
      ...
      case 4: // 未毕业
        ...
        query
            .add(new Condition(
                "not exists (from AuditResult auditResult where auditResult.std = std and auditResult.majorType.id=:majorTypeId)",
                majorTypeId));
        break;
      default:
        break;
    ...
    Boolean isActive = getBoolean(request, "isActive");
    if (null != isActive) {
      query.add(new Condition("std.active = :active", isActive));
    }
   ...
   * 按审核规则导出学生详细审核结果
   */
  public ActionForward exportDataWithInfo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    Boolean audited = getBoolean(request, "audited");
