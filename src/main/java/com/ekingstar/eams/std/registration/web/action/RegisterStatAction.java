...
public class RegisterStatAction extends RegisterSearchAction {
   ...
   * @throws Exception
   */
  public ActionForward statDepart(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    // 统计学籍
    EntityQuery stdQuery = new EntityQuery(Student.class, "std");
    List conditions = QueryRequestSupport.extractConditions(request, Student.class, "std", "std.type.id");
    ...
    regQuery.add(new Condition("register.calendar is null or register.calendar.id=(:calendarId)", getLong(
        request, "register.calendar.id")));
    ...
    DataRealmUtils.addDataRealms(regQuery, new String[] { "register.std.type.id",
        "register.std.department.id" }, getDataRealmsWith(getLong(request, "std.type.id"), request));
    ...
    request.setAttribute("departStat", new StatGroup(null, StatGroup.buildStatGroups((List) stdCounts, 2)));
