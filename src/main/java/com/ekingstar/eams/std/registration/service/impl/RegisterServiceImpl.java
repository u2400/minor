...
public class RegisterServiceImpl extends BasicService implements RegisterService {
  ...
  public void register(TeachCalendar calendar, Collection stds, String remark, RegisterState registerState) {
    ...
    for (Iterator iter = stds.iterator(); iter.hasNext();) {
      ...
      // 如果没有注册记录
      if (null == register) {
        registrations.add(new Register(calendar, std, registerAt, remark, registerState, isPassed,
            isTuitionFeeCompleted));
      } else {
        // 存在注册记录
  ...
  public Register getRegister(Student std, TeachCalendar calendar) {
    ...
    if (!collection.isEmpty()) { return (Register) (((List) collection).get(0)); }
    return null;
  }
  ...
  public Map getRegister(String stdCode, String calendarId) {
    ...
    if (null == std) { return null; }
    Boolean registerIsTuitionFeeCompleted = tuitionFeeService.isCompleted(std.getId(), new Long(calendarId));
  ...
  public int getRoleState(User user) {
    ...
    // 没有教务员权限
    if (null == userGroup) { return -1; }
    // 不在维护日期内
    Timestamp now = new Timestamp(new Date().getTime());
    if (now.before(userGroup.getBeginAt()) || now.after(userGroup.getEndAt())) { return 0; }
    // 在维护权限内
    return 1;
  }
