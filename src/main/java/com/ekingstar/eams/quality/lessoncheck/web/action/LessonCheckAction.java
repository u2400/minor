...
public class LessonCheckAction extends CalendarRestrictionExampleTemplateAction {
  ...
  protected ActionForward saveAndForwad(HttpServletRequest request, Entity entity) throws Exception {
    ...
    lessonCheck.setCalendar(teachCalendarService.getTeachCalendar(getLong(request, "studentType.id"), year,
        term));
    lessonCheck.setCheckRoom((Classroom) utilService.load(Classroom.class, getLong(request, "classRoomId")));
