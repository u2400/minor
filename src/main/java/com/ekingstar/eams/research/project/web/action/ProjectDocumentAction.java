...
public class ProjectDocumentAction extends RestrictionExampleTemplateAction {
  ...
  public ActionForward search(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if ((!"".equals(teachProjectIdSeq)) && (null != teachProjectIdSeq)) {
      TeachProject teachProject = (TeachProject) utilService.get(TeachProject.class,
          Long.valueOf(teachProjectIdSeq));
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    ProjectDocument projectDocument = (ProjectDocument) populateEntity(request, ProjectDocument.class,
        "projectDocument");
    ...
    if (null != projectDocument.getId()) {
      ProjectDocument document = (ProjectDocument) utilService.load(ProjectDocument.class,
    ...
    return redirect(request, new Action("", "search"), "info.save.success", new String[] { "teachProjectId" });
  }
  ...
  public ActionForward remove(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    for (int i = 0; i < projectDocumentIds.length; i++) {
      ...
      ProjectDocument projectDocument = (ProjectDocument) utilService.load(ProjectDocument.class,
          projectDocumentId);
      SystemConfig config = SystemConfigLoader.getConfig();
      String defaultPath = request.getSession().getServletContext().getRealPath(FilePath.fileDirectory);
      File file = new File(FilePath.getRealPath(config, FilePath.DOC, defaultPath) + "/"
          + projectDocument.getFilePath());
  ...
  public ActionForward download(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    ProjectDocument projectDocument = (ProjectDocument) utilService.load(ProjectDocument.class,
        Long.valueOf(projectDocumentId));
    ...
    if (!file.exists()) {
      response.getWriter().write(
          "without file path:[" + FilePath.getRealPath(config, FilePath.DOC, defaultPath) + "/"
              + new String(projectDocument.getFilePath().getBytes(), "ISO-8859-1") + "]");
      return null;
    }
