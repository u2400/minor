...
public class ExaminationPaperAction extends CalendarRestrictionSupportAction {
  ...
  public ActionForward search(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    query.add(new Condition(" examinationPaper.calendar.id=:calendarId", calendarId));
  ...
  public ActionForward edit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (!"".equals(examinationPaperId)) {
      ExaminationPaper examinationPaper = (ExaminationPaper) utilService.get(ExaminationPaper.class,
          Long.valueOf(examinationPaperId));
  ...
  public ActionForward save(ActionMapping mapping, ActionForm form, HttpServletRequest request,
      HttpServletResponse response) throws Exception {
    ExaminationPaper examinationPaper = (ExaminationPaper) populateEntity(request, ExaminationPaper.class,
        "examinationPaper");
    DynaActionForm dynaForm = (DynaActionForm) form;
    FormFile formFile = (FormFile) dynaForm.get("formFile");
    // 判断是否已有上传文档, 有则删除
    if (null != examinationPaper.getId()) {
      ExaminationPaper paper = (ExaminationPaper) utilService.load(ExaminationPaper.class,
          examinationPaper.getId());
      if (null != paper.getFilePath()) {
        ...
        File file = new File(FilePath.getRealPath(config, FilePath.DOC, defaultPath) + "/"
            + paper.getFilePath());
        // log.info("user [" + getUserFromSession(request.getSession()).getName() + "] delete file["
    ...
    return redirect(request, new Action("", "search"), "info.save.success", new String[] { "calendar.id" });
  }
  ...
  public ActionForward remove(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    for (int i = 0; i < examinationPaperIds.length; i++) {
      ...
      ExaminationPaper examinationPaper = (ExaminationPaper) utilService.load(ExaminationPaper.class,
          examinationPaperId);
      SystemConfig config = SystemConfigLoader.getConfig();
      String defaultPath = request.getSession().getServletContext().getRealPath(FilePath.fileDirectory);
      File file = new File(FilePath.getRealPath(config, FilePath.DOC, defaultPath) + "/"
          + examinationPaper.getFilePath());
      // log.info("user [" + getUserFromSession(request.getSession()).getName() + "] delete file[" +
      // file.getAbsolutePath()
      // + "]");
      file.delete();
    ...
    return redirect(request, new Action("", "search"), "info.delete.success", new String[] { "calendar.id" });
  }
  ...
  public ActionForward download(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    if (!file.exists()) {
      response.getWriter().write(
          "without file path:[" + FilePath.getRealPath(config, FilePath.DOC, defaultPath) + "/"
              + new String(examinationPaper.getFilePath().getBytes(), "ISO-8859-1") + "]");
      return null;
    }
