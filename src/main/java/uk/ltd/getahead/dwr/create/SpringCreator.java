 ...
 * <p>
 *  Patch: 注释掉无用的错误输出，导致控制台噪音太多，很难找到最初的错误
 * </p>
 */
public class SpringCreator extends AbstractCreator implements Creator {
  ...
  public Object getInstance() throws InstantiationException {
    try {
      ...
      if (factory == null) {
        ...
        log.info(
            "- Option 1. In dwr.xml, <create creator='spring' ...> add <param name='location1' value='beans.xml'/> for each spring config file."); //$NON-NLS-1$
  ...
  private BeanFactory getBeanFactory() {
    ...
    if (configLocation != null && configLocation.length > 0) {
      log.info("Spring BeanFactory via ClassPathXmlApplicationContext using " + configLocation.length //$NON-NLS-1$
          + "configLocations."); //$NON-NLS-1$
