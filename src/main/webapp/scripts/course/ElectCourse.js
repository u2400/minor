    ...
    errors.reStudySubstituteElected="重修替代课程";
    var reStudy="3";
    var elective="2";
    var reExam="4";
    ...
    displayColums[10]="electInfo.minStdCount";
    displayColums[11]="remark";
    ...
    function canElect(state,task){
        ...
        if(null!=state.hisCourses[task.course.id]){
           if(state.params.isRestudyAllowed){
              if(state.substituteIds[task.course.id]){
                 return errors.reStudySubstituteElected;
              }else{
...
//	    tr.appendChild(inputDiv);
    ...
    function cancelTask(stdCode){
        ...
        if(taskId!==""&& confirm("确定要退掉这门课吗？")){
           ...
           curTask.id=taskId;
           curTask.credit = new Number(curTask.credit);
           curTask.course.id=selectInput.nextSibling.value;
           
           if(curTask.electInfo.isCancelable=="false"){
              setMessage(errors.notCancelable);
              alert(errors.notCancelable);
              return;
           }
           /*if(!electState.params.isUnderMinAllowed){
           		alert(curTask.teachClass.stdCount);
              var minCount = new Number(curTask.electInfo.minStdCount);
              var stdCount = new Number(curTask.teachClass.stdCount);
              //alert(stdCount+"\n"+minCount)
