     ...
     function edit(){
        form.action="genderLimitGroup.do?method=edit&taskGroup.id="+taskGroupId;
        form.submit();
     }
     function deleteGroup(){
          var form = document.taskGroupForm;
          if(!confirm("确认删除?"))return;
          form.action="genderLimitGroup.do?method=deleteGroup&taskGroup.id="+taskGroupId;
          parent.setRefreshGroupListTime(500,true);
          form.submit();
     }
     function updatePlanStdCount(){
        var form = document.taskGroupForm;
         form.action="genderLimitGroup.do?method=divPlanCount&taskGroup.id="+taskGroupId;
         form.submit();
     }
     ...
     function deleteTask(){
         ...
         if(!confirm("确认删除?"))return;
         var form = document.taskGroupForm;
         form.action="genderLimitGroup.do?method=deleteTask&taskIds="+taskIds;
     ...
     function removeAdminClass(){
         ...
         var form = document.taskGroupForm;
         form.action="genderLimitGroup.do?method=removeAdminClass&adminClassIds="
                     +adminClassIds
                     +"&removeEmptyTask="
                     +(removeEmptyTask?"1":"0");
     ...
     function addTasks(){
          var form = document.taskGroupForm;
          form.action="genderLimitGroup.do?method=lonelyTaskList&calendar.studentType.id=";
