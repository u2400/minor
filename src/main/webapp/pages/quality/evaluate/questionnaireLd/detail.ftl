...
<script>
   var bar = new ToolBar('backBar','领导问卷查看页面',null,true,true);
   bar.setMessage('<@getMessage/>');
   bar.addBack('<@bean.message key="action.back"/>');
</script>
<#if questionnaireLd.title?exists>
<pre>
${questionnaireLd.title?if_exists}
</pre>
</#if>
<div align="center">问卷描述：${questionnaireLd.description?if_exists}&nbsp;&nbsp;&nbsp;问卷创建部门:${questionnaireLd.depart.name}&nbsp;&nbsp;&nbsp;</div>
		...
		<#list questionList?if_exists as question>
	   	...
	   	<tr class="${class}"  align="center">
			...
			<td width="300" align="center">
					<#if (questionnaireLd.optionGroup.options)?exists>
						<#list (questionnaireLd.optionGroup.options)?sort_by("proportion")?reverse as option>
