...
<script>
	var questionIds = ",";
	<#list questionLds?if_exists?reverse as question>
	...
	function addTr(questionTypeId){
		 var url="questionnaireLd.do?method=searchQuestion&questionTypeId="+questionTypeId+"&questionSeq="+questionIds.substring(1,questionIds.length);
...
function addContext(values){
	...
	function  addAllquestions(){
		var url="questionnaireLd.do?method=searchQuestion&questionSeq="+questionIds.substring(1,questionIds.length);
		window.open(url,'','scrollbars=yes,width=720,height=480,left=200,top=200,status=yes');
	}
	...
	function doValidate(){
		var errors ="";
		var flag="true";
		var simpleDescribe = document.questionnaireForm['questionnaireLd.description'].value;
		if(simpleDescribe==""||simpleDescribe.length<1){
			errors+="请填写问卷的描述\n";
		}
		var deparmentId = document.questionnaireForm['questionnaireLd.depart.id'].value;
		if(deparmentId==""||deparmentId.length<1){
			errors+="请选择一个问卷制作部门\n";
		}
		var stateValue = getCheckBoxValue(document.getElementsByName("questionnaireLd.state"));
		if(stateValue==""||stateValue.length<1){
			errors+="请选择一个该问卷的一个状态\n";
		}
		var remarkValue=document.questionnaireForm['questionnaireLd.remark'].value;
		if(remarkValue.length>500){
			errors+="请不要超过500个字符\n";
		}
		var title = document.questionnaireForm['questionnaireLd.title'].value;
		...
		alert(questionIds.substring(1,questionIds.length-1));
		document.questionnaireForm.action="questionnaireLd.do?method=save";
		addInput(document.questionnaireForm,"questionnaireLd.questionLdIds",questionIds.substring(1,questionIds.length-1))
...
<table id="tableName" align="center" class="formTable" width="100%">
	...
	<td><input type="text" name="questionnaireLd.description" value="${(questionnaireLd.description)?if_exists}" style="width:150px;"></td>
	<td class="title">问卷部门<font color="red">*</font>:</td>
	<td><@htm.i18nSelect datas=departments selected=(questionnaireLd.depart.id)?default(0)?string name="questionnaireLd.depart.id" style="width:150px;"/></td>
	</tr>
	<tr>
		<td class="title">问卷状态<font color="red">*</font>:</td>
		<td><@htm.radio2 name="questionnaireLd.state" value=questionnaireLd.state?default(false)/></td>
		
		<td valign="center" class="title">选项组<font color="red">*</font></td>
		<td><@htm.i18nSelect datas=optionGroups selected=(questionnaireLd.optionGroup.id)?default(0)?string name="questionnaireLd.optionGroup.id" style="width:300px;"/></td>
		</tr>
		<tr>
			<td valign="center" class="title">问卷表头</td>
			<td colspan="3"><textarea name="questionnaireLd.title" rows="3" cols="200" style="width:600px;">${(questionnaireLd.title)?if_exists}</textarea></td>
		</tr>
		<td class="title">备注:</td>
		<td colspan="3"><input type="text" name="questionnaireLd.remark" value="" style="width:100%"></td>
		<tr>
			<td colspan="4">
			...
			<tbody id="tbodyId">
				...
				<tr id="endTr" class="darkColumn" align="center">
					<td colspan="5" height="25px;">
							<input type="hidden" name="questionnaireLd.id" value="${(questionnaireLd.id)?if_exists}">
							<button name="button1" onclick="save()"><#if (questionnaireLd.id)?exists>更新<#else>添加</#if></button>
						</td>
				</tr>
			</tbody>
			</table>
