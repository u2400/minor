...
<body onload="parent.clearBarInfo()">
	...
	<table class="listTable" width="100%" style="text-align:justify;text-justify:inter-ideograph;line-height:5mm">
		<tr>
			<td width="9%" rowspan="2">评价时间</td>
			<td width="15%" rowspan="2">课程名称</td>
			<td rowspan="2" width="14%">院系</td>
			<td width="8%">课程评价</td>
			<td width="8%">学院课程</td>
			<td width="8%">学校课程</td>
		...
		<tr>
			...
			<td><@i18nName (evaluateTeacher.department)?if_exists/></td>
			<td>${evaluateTeacher.sumScore?string("0.00")}</td>
			<td>${(evaluateDepartMap[evaluateTeacher.calendar.id?string].sumScore?string("0.00"))?if_exists}</td>
			<td>${(evaluateCollegeMap[evaluateTeacher.calendar.id?string].sumScore?string("0.00"))?if_exists}</td>
			<td>${evaluateTeacher.departRank}/${(evaluateDepartMap[evaluateTeacher.calendar.id?string].count)?if_exists}</td>
			<td>${evaluateTeacher.rank}/${(evaluateCollegeMap[evaluateTeacher.calendar.id?string].count)?if_exists}</td>
		</tr>
		</#list>
	</table>
	<script>
