...
<body onload="SetPrintSettings()">
	...
	<table class="listTable" width="100%" style="text-align:justify;text-justify:inter-ideograph;line-height:5mm">
		...
		<tr>
			...
			<td>${questionStat.evgPoints?default(0)?string("0.00")}</td>
			<td>${(departQuestionResults[evaluateTeacher.department.id + "_" + questionStat.question.id].evgPoints)?default(0)?string("0.00")}</td>
			<td>${(collegeQuestionResults["college_" + questionStat.question.id].evgPoints)?default(0)?string("0.00")}</td>
		</tr>
		</#list>
	</table>
