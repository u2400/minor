...
<body onload="parent.clearBarInfo()">
	...
	<div style="width:85%">
		...
		<table class="listTable" width="100%" style="text-align:justify;text-justify:inter-ideograph;line-height:5mm">
			...
			<#assign teacherSum = 0/>
			<#assign allDeparts=[]/>
			<#assign allDepartResults={}/>
			
			<#list departmentResults?if_exists as departmentResult>
			<#assign allDeparts=allDeparts+[departmentResult[0].department]/>
			<#assign allDepartResults=allDepartResults+{departmentResult[0].department.code:departmentResult}/>
			</#list>
			<#list allDeparts?sort_by("code") as depart>
			<#assign departmentResult=allDepartResults[depart.code]>
			<tr>
