...
<body>
	...
	<@table.table id="result" width="100%" sortable="true">
		<@table.thead>
			...
			<@table.sortTd text="登录名" id="evaluateResult.student.code"/>
		...
		<@table.tbody datas=evaluateResults; result>
			...
			<td width="10%"><A href="#" onclick="info(${result.id})"><#if result.student??><@i18nName result.student/></#if><#if result.user??>${(result.user.userName)!}</#if></A></td>
	...
	<@htm.actionForm name="actionForm" action="evaluateResultLd.do" entity="evaluateResult">
        <input type="hidden" name="isEvaluate" valeu=""/>
	</@>
	<script>
		var bar = new ToolBar("bar", "领导评教结果明细列表", null, true, true);
