			...
			<td colspan="3">
				<#list tacheSetting.thesisModels?sort_by("id") as docuemnt>
   				...
   				<#if (thesisStore.id)?exists&&extra['personType']=="admin">
   				 <@msg.message key="thesis.std.moduleDoc"/>:<#if (thesisManage.thesis.displayName)?exists><a href="thesisDownload.do?method=doDownLoad&thesisManageId=${thesisManage.id}&storeId=${id}">${thesisManage.thesis.displayName}<img class="icon" src="images/action/paperclip.gif"></a></#if>
				</#if>
			</td>
		</tr>
	</#if>
