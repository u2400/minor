...
<#assign showGroupAll=(RequestParameters['showGroupAll']?default('false')=="true") />
       ...
       <td align="center" class="contentTableTitleTextStyle">
           <table align="center" cellpadding="0" cellspacing="0" border="1" width="95%" class="listTable">
               ...
               <tr>
                ...
                <#if offsetCredit?exists>
               <tr>
                <td align="center" class="grayStyle">可冲抵学分</td>
                <td class="brightStyle" colspan="3">${offsetCredit.offsetCredit}</td>
               </tr>
               </#if>
           </table>
       </td>
      ...
      <#if group.courseType.isCompulsory||showGroupAll>
        <#assign passCourseCount=group.passCourseCount/>
          ...
          <#if (show!="unpass")||!isAllCoursePass>
              <#if group.planCourseAuditResults?size!=0>
              <#assign typeRowspan=group.planCourseAuditResults?size+1><#else><#assign typeRowspan=2></#if>
                  ...
                  <#if (orderPlanCourseAuditResults?size>0) >   
                   ...
                   <#--开始绘制课程每学期上课情况-->
                           ...
                           <td align="center">--<#list planCourse.substitionScores as substitionScore><@i18nName substitionScore.course />(${substitionScore.course.credits},${substitionScore.course.code})<#if substitionScore_has_next>/</#if></#list></td>
                       <#--结束绘制课程每学期上课情况-->       
                      </tr>
                      </#if>
                      </#list>
                      <tr style="background-color:#BFFFBF">
                          <td align="center" colspan="3">小计</td>
                          <td align="center" colspan="2"><I><@msg.message key="grade.finalCredit"/>：</I>${group.creditAuditInfo.completed?if_exists}<#if group.creditAuditInfo.converted?exists&&group.creditAuditInfo.converted!=0>&nbsp;<I><@msg.message key="grade.conversionCredit"/>：</I>${group.creditAuditInfo.converted}</#if></td>
                          <td align="center"><#if !group.creditAuditInfo.isPass>未修学分:${group.creditAuditInfo.getCreditToComplete(false)}</#if></td>
                          <td align="center">${group.creditAuditInfo.required?if_exists}</td>
                          <td align="center">${(group.creditAuditInfo.isPass)?string("是","<font color=red>否</font>")}</td>
                          <#if termsShow=="true">
                           <#list 1..auditResult.termsCount?default(1) as i>
                           <td></td>
                           </#list>
                  ...
                  <tr>     
                    <td colspan="3" align="left">&nbsp;<@i18nName group.courseType/></td>
                    <td colspan="2" align="center" colspan="2"><I><@msg.message key="grade.finalCredit"/>：</I>${group.creditAuditInfo.completed?if_exists}<#if group.creditAuditInfo.converted?exists&&group.creditAuditInfo.converted!=0>&nbsp;<I><@msg.message key="grade.conversionCredit"/>：</I>${group.creditAuditInfo.converted}</#if></td>        
                    <td align="center"><#if !group.creditAuditInfo.isPass>未修学分:${group.creditAuditInfo.getCreditToComplete(false)}</#if></td>
                    <td align="center">${group.creditAuditInfo.required?if_exists}</td>
                       <td align="center"><#if (group.creditAuditInfo.isPass)?exists><#if group.creditAuditInfo.isPass><@msg.message key="yes"/><#else><font color='red'><@msg.message key="no"/></font></#if></#if></td>
                       <td align="center" <#if termsShow=="true">colspan="${auditResult.termsCount?default(1)}"</#if>></td>
            ...
            <#if typeRowspan=2>
                <td colspan="3" align="left" style="text-indent:10px"><a HREF="javascript:showGroup('${auditResult_index}_${group.courseType.id}','<@i18nName group.courseType/>');">${parentCourseType}<@i18nName group.courseType/></a></td>
                <td align="center" colspan="2"><I><@msg.message key="grade.finalCredit"/>：</I>${group.creditAuditInfo.completed?if_exists}<#if group.creditAuditInfo.converted?exists&&group.creditAuditInfo.converted!=0>&nbsp;<I><@msg.message key="grade.conversionCredit"/>：</I>${group.creditAuditInfo.converted}</#if></td>
                <td align="center"><#if !group.creditAuditInfo.isPass>未修学分:${group.creditAuditInfo.getCreditToComplete(false)}</#if></td>
                <td align="center">${group.creditAuditInfo.required?if_exists}</td>
                   <td align="center"><#if (group.creditAuditInfo.isPass)?exists><#if group.creditAuditInfo.isPass><@msg.message key="yes"/><#else><font color='red'><@msg.message key="no"/></font></#if></#if></td>
    ...
    <#if !group.courseType.isCompulsory>
      <div id="${auditResult_index}_${group.courseType.id}" style="visibility: hidden; display:none;">
        ...
        <tr>
            <td align="center">${planCourse.course.code}</td>
            <td align="left"><@i18nName planCourse.course/></td>
            <td align="left">${(planCourse.course.extInfo.courseType.name)?default("")}</td>
               <td align="center"><#list planCourse.getScoresOrderByScore(null) as score><#if (score.score)?exists>${score.scoreDisplay}<#else><@msg.message key="grade.noScore"/></#if><#if score_has_next>/</#if></#list>&nbsp;</td>
