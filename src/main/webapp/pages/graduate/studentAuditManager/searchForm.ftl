	...
	<table width="100%" class="searchTable">
		<form name="${formName}" method="post" action="${formAction}" target="${targetValue?default('_self')}" onsubmit="return false;">
			<tr class="infoTitle" valign="top" style="font-size:8pt">
       			<td colspan="2" style="text-align:left;text-valign:bottom;font-weight:bold"><img src="images/action/info.gif" align="top"/>&nbsp;学籍信息查询</td>
      		</tr>
      		<tr>
			...
			<tr>
         		...
         		<#assign adminClassId = "adminClasssId"/>
	   			...
	   			<td>
	   				<input type="text" name="adminClassName" value="${RequestParameters['adminClassName']?if_exists}" style="${formElementWidth}">
	     		</td>
         	</tr>
            <tr>
                <td>学制</td>
                <td>
                    <input type="text" name="std.schoolingLength" value="${RequestParameters['std.schoolingLength']?if_exists}" style="${formElementWidth}">
                </td>
            </tr>
			<tr>
			...
			<tr>
                <td>毕业状态</td>
                <td>
                    <select name="graduateStatus" style="${formElementWidth}">
						     		...
						     		<td>
							     		<select id="f_auditStandard" name="auditStandardId" style="width:90px;">
									     	<#list result.auditStandardList?if_exists as auditStandard >
									     		<option value="${auditStandard.id}"<#if RequestParameters['auditStandardId']?default("302")==auditStandard.id?string> selected</#if> /><@i18nName auditStandard /></option>
									     	</#list>
								        </select>
						     		</td>
						  		</tr>
