		...
		function editRemark(){
			var stdIds = getSelectIds("stdId");
			if(countId(stdIds)<=0){alert("请选择至少一条信息");return;}
			if(!confirm("确定修改这"+countId(stdIds)+"条备注信息?")) return;
			form.action="studentAuditManager.do?method=editRemark&stdIds="+stdIds;
			form.submit();
		}
