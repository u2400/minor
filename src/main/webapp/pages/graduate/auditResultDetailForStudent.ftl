...
<#assign secondMajorName><@i18nName auditResult.student.secondMajor?if_exists/></#assign>
   ...
   <tr>
    <td>可冲抵学分：${offsetCredit.offsetCredit}</td>
   </tr>
   </#if>
          ...
          <#if (show!="unpass")||!isAllCoursePass>
              ...
              <#assign typeRowspan=group.planCourseAuditResults?size+1><#else><#assign typeRowspan=2></#if>
                  ...
                  <#if (orderPlanCourseAuditResults?size>0) >   
                   ...
                   <#--开始绘制课程每学期上课情况-->
                      ...
                      <tr style="background-color:#BFFFBF">
                          ...
                          <td align="center" colspan="2"><I><@msg.message key="grade.finalCredit"/>：</I>${group.creditAuditInfo.completed?if_exists}</td>
                  ...
                  <tr>     
                    ...
                    <td align="center" colspan="2"><I><@msg.message key="grade.finalCredit"/>：</I>${group.creditAuditInfo.completed?if_exists}<#if group.creditAuditInfo.converted?exists&&group.creditAuditInfo.converted!=0>&nbsp;</#if></td
                    <td align="center"><#if !group.creditAuditInfo.isPass>未修学分:${group.creditAuditInfo.getCreditToComplete(false)}</#if></td>
                    <td align="center">${group.creditAuditInfo.required?if_exists}</td>
                  ...
                  <tr>     
                    <td colspan="3" align="left">&nbsp;<@i18nName group.courseType/></td>
                    <td colspan="2" align="center" colspan="2"><I><@msg.message key="grade.finalCredit"/>：</I>${group.creditAuditInfo.completed?if_exists}<#if group.creditAuditInfo.converted?exists&&group.creditAuditInfo.converted!=0>&nbsp;</#if></td>        
                    <td align="center"><#if !group.creditAuditInfo.isPass>未修学分:${group.creditAuditInfo.getCreditToComplete(false)}</#if></td>
                    <td align="center">${group.creditAuditInfo.required?if_exists}</td>
                       <td align="center"><#if (group.creditAuditInfo.isPass)?exists><#if group.creditAuditInfo.isPass><@msg.message key="yes"/><#else><font color='red'><@msg.message key="no"/></font></#if></#if></td>
                       <td align="center" <#if termsShow=="true">colspan="${auditResult.termsCount?default(1)}"</#if>></td>
