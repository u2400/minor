    ...
    <script>
        ...
        function save() {
            ...
            if (oValidator.exec()) {
                ...
                oForm.action = "teachingScheduleManagement.do?method=save&schedule.id=${(schedules.id)?if_exists}&schedule.task.id=${schedule.task.id}&schedulePath=" + oForm["schedule.path"].value + "&params=${(RequestParameters["params"]?replace("&", "%26"))?if_exists}";
                oForm.target = "_self"
                oForm.submit();
            }
        }
