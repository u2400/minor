<#include "/templates/head.ftl"/>
 ...
 <@table.thead>
      ...
      <@table.td width="10%" text="院系"/>
      <@table.td width="10%" text="性别"/>
      <@table.td width="10%" text="出生年月"/>
      <@table.td width="10%" text="教师职称"/>
  ...
  <@table.tbody datas=stats;statItem>
      ...
      <td>${(statItem.what.department.name)?if_exists}</td>
      <td>${(statItem.what.gender.name)?if_exists}</td>
      <td>${(statItem.what.birthday?string("yyyy-MM-dd"))?if_exists}</td>
      <td>${(statItem.what.title.name)?if_exists}</td>
