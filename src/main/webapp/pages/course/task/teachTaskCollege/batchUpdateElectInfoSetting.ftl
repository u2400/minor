...
<body>
	...
	<table class="formTable" style="width:95%;" align="center">
		...
		<tr>
            <td class="title"><input type="checkbox" name="taskRemarkCheck" value="taskRemark" onclick="toSelectItem(this.value)"/>备注：</td>
            <td colspan="3"><input type="text" name="taskRemark" cols="55" value="" style="width:350px" disabled/></td>
		</tr>
		<@searchParams/>
		</form>
		<tr>
	...
	<script>
		...
		function save() {
            ...
            if (form["taskRemarkCheck"].checked&& form['taskRemark'].value == "") {
                alert("备注不能为空！");
                return;
            }
            if(!form["HSKDegreeCheck"].checked&&!form["courseCodeCheck"].checked&&!form["taskRemarkCheck"].checked){
        ...
        function toSelectItem(xItemValue) {
            form[xItemValue].disabled = form[xItemValue + "Check"].checked ? "" : "disabled";
            if (!form[xItemValue + "Check"].checked) {
                form[xItemValue].value = "";
            } else {
                form[xItemValue].focus();
            }
        }
	</script>
</body>
