 ...
 <form name="taskListForm" method="post" action="" onsubmit="return false;">
    ...
    <#list taskList as task>
     ...
     <tr class="${class}" align="center" onmouseover="swapOverTR(this,this.className)" onmouseout="swapOutTR(this)" onclick="onRowChange(event)">
      ...
      <td>${task.seqNo}</td>
      <td>
      <A href="?method=info&task.id=${task.id}">
       <@i18nName task.course?if_exists/></a>
      </td>
      <td title="${task.teachClass.name?html?replace(",", ", ")}" nowrap><span style="display:block;width:150px;overflow:hidden;text-overflow:ellipsis;">${task.teachClass.name?html?replace(",", ", ")}</span></td>
      <#assign adminClassNames><#list task.teachClass.adminClasses as adminClass>${adminClass.name}<#if adminClass_has_next>, </#if></#list></#assign>
      <td title="${adminClassNames}" nowrap><span style="display:block;width:150px;overflow:hidden;text-overflow:ellipsis;">${adminClassNames?html}</span></td>
...
<script>
    function merge(){
       ...
       form.action="?method=merge&reservedId="+id+"&taskIds=" + taskIds+"&hold=true";
       form.submit();
    }
</script>
</body>
