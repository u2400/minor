...
<#include "/templates/back.ftl"/>
     ...
     <table width="90%" align="center" class="listTable">
       ...
       <tr>
         <td class="grayStyle" width="25%" id="f_count">
            <input type="radio" name="splitMode"  value="avgShare" /><@bean.message key="info.task.split.avgAll"/>：
         </td>
         <td class="brightStyle"  colspan="3">
       ...
       <tr>
         <td class="grayStyle" width="25%" id="f_count">
            <input type="radio" name="splitMode" checked value="keepShare"/><@bean.message key="info.task.split.keepShare"/>：
         </td> 
       ...
       <#if RequestParameters['splitNum']='2'>
       <tr>
           <td class="grayStyle" width="25%" id="f_count"><input type="radio" name="splitMode" value="adminClassSplit" id="mode_adminClassSplit"/>按班级拆分：</td>
           <td colspan="3"><#if task.teachClass.adminClasses?size == 0>当前没有班级可以拆分。<#else>
               <table style="text-align:center">
                   <tr style="text-align:left">
                       <td style="border-style:none;">第一个教学任务的班级：</td>
                       <td style="border-style:none;"></td>
                       <td style="border-style:none;">第二个教学任务的班级：</td>
                   </tr>
                   <tr valign="center">
                       <td rowspan="2" style="border-style:none;"><select name="adminClassOfFirstTask" MULTIPLE size="5" style="width:150px" onDblClick="selectRemoveAnyOne('adminClassOfFirstTask', 'adminClassOfSecondTask')" onfocus="setModeFocus('mode_adminClassSplit')"><#list task.teachClass.adminClasses as adminClass><option value="${adminClass.id}">${adminClass.name}</option></#list></select></td>
                       <td style="border-style:none;"><input OnClick="setModeFocus('mode_adminClassSplit');selectRemoveAnyOne('adminClassOfFirstTask', 'adminClassOfSecondTask')" type="button" value="&gt;"/></td>
                       <td rowspan="2" style="border-style:none;"><select name="adminClassOfSecondTask" MULTIPLE size="5" style="width:150px" onDblClick="JavaScript:moveSelectedOption(this.form['adminClassOfSecondTask'], this.form['adminClassOfFirstTask'])" onfocus="setModeFocus('mode_adminClassSplit')"></select></td>
                   </tr>
                   <tr>
                       <td style="border-style:none;"><input OnClick="setModeFocus('mode_adminClassSplit');selectRemoveAnyOne('adminClassOfSecondTask', 'adminClassOfFirstTask')" type="button" value="&lt;"/></td>
                   </tr>
               </table></#if></td>
       </tr>
       </#if>
  ...
  <script language="javascript">
      ...
      function save(){
         ...
         } else if (mode == "adminClassSplit") {
            if (form["adminClassOfFirstTask"].options.length == 0) {
                alert("第一个教学任务不能没有班级,故无法拆分班级。");
                return;
            }
            if (form["adminClassOfSecondTask"].options.length == 0) {
                alert("第二个教学任务不能没有班级，故无需拆分班级。");
                return;
            }
         }
         if (mode == "adminClassSplit") {
             var secondClassIds=getAllOptionValue(form['adminClassOfSecondTask']);
             addInput(form,"secondClassIds",secondClassIds);
             form.action="?method=split&splitNum=2&mode=" + mode + "&splitUnitNums=" + splitUnitNums;
         } else {
