     ...
     <form name="settingForm" action="electScope.do?method=batchUpdateEelectInfo" method="post" onsubmit="return false;">
       ...
       <tr>
         ...
         <td class="grayStyle">
          ...
          <input type="checkbox" name="initByTeachClass" value="64"/>性别
         </td>
       </tr>
       <tr>
       ...
       <tr>
         ...
         <td class="grayStyle">
           ...
           <tr class="infoTitle">
            ...
            <td>
                <input type="radio" name="setting.gender.id" value="" checked/> 不变
                <#list genders?sort_by("code") as gender>
                <input type="radio" name="setting.gender.id" value="${gender.id}"/> ${gender.name}
                </#list>
            </td>
           </tr>
           </table>
       ...
       <tr>
            ...
            <td class="grayStyle">
                <input type="checkbox" name="stdCountSwitch" value="1" onclick="stdCountSwitchChoice()"/>计划人数<br>
                <input type="checkbox" name="stdCountSwitch" value="0" onclick="stdCountSwitchChoice()"/>教室听课人数<br>
                <input type="checkbox" name="stdCountSwitch" value="2" onclick="stdCountSwitchChoice()"/>教室真正容量<br>
                <input type="hidden" name="setting.stdCountSwitchs" value=""/>
                <input type="hidden" name="setting.stdCountValue" value=""/>
                <input type="radio" name="stdCountType" value="0" onclick="stdCountTypeChoice()"/>取最小值&nbsp;<input type="radio" name="stdCountType" value="1" onclick="stdCountTypeChoice()"/>自定义人数：<input type="text" name="setting.stdCount" value="" disabled onblur="userDefinedChange()"/><br>
            </td>
       </tr>
       <tr>
           <td class="grayStyle">
...
<script>
    ...
    function save(){
       var stdCountSwitchs = form["setting.stdCountSwitchs"].value = getSelectIds("stdCountSwitch");
       ...
       var sct = form["setting.stdCountValue"].value = getSelectIds("stdCountType");
       if (sct != null && (sct == "0" && (null == stdCountSwitchs || "" == stdCountSwitchs) || sct == "1" && (null == form["setting.stdCount"].value || "" == form["setting.stdCount"].value || !/^\d+$/.test(form["setting.stdCount"].value)))) {
        alert("选择的人数上下限设置不正确。");
        return;
       }
    ...
    function changeInitByTeachClass(obj){
       var initByTeachClassInputs=document.getElementsByName('initByTeachClass');
       for(var i = 0;i < initByTeachClassInputs.length; i++) {
          input = initByTeachClassInputs[i];
    ...
    function stdCountTypeChoice() {
        switchValue = form["setting.stdCountSwitchs"].value = getSelectIds("stdCountSwitch");
        if (form["stdCountType"][0].checked == true) {
            form["setting.stdCount"].disabled = "disabled";
            form["setting.stdCount"].value = "";
            if (null == switchValue || "" == switchValue) {
                for (var i = 0; i < form["stdCountSwitch"].length; i++) {
                    form["stdCountSwitch"][i].checked = true;
                }
            }
        } else {
            for (var i = 0; i < form["stdCountSwitch"].length; i++) {
                form["stdCountSwitch"][i].checked = false;
            }
            form["setting.stdCount"].disabled = "";
            form["setting.stdCount"].value = 0;
        }
    }
    
    function stdCountSwitchChoice() {
        form["stdCountType"][0].checked = true;
        form["setting.stdCountSwitchs"].value = getSelectIds("setting.stdCountSwitchs");
        form["setting.stdCount"].disabled = "disabled";
        form["setting.stdCount"].value = "";
    }
    
    function userDefinedChange() {
        if (null == form["setting.stdCount"].value || "" == form["setting.stdCount"].value) {
            form["setting.stdCount"].value = 0;
        }
    }
</script>
