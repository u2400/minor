<script>   
	...
	<#list activityList?if_exists as activity>
	   ...
	   activity = new TaskActivity("${activity.teacher?if_exists.id?if_exists}","","${activity.task.course.id}(${activity.task.seqNo})","${activity.task.course.name}(${activity.task.seqNo})","${(activity.room.id)?if_exists}","<#if isAdmin?default(false) || (state.arrangeSwitch.isArrangeAddress)?default(false) && (state.arrangeSwitch.isPublished)?default(false)>${(activity.room.name)?if_exists}</#if>","${activity.time.validWeeks}");
