    ...
    <#list courseTable.tasksGroups?if_exists as group>
     ...
     <tr align="center" onmouseover="swapOverTR(this,this.className)" onmouseout="swapOutTR(this)" >
      ...
      <#if task.requirement.isGuaPai == true>
	      ...
	      <td colspan="3">
	       <a href="teachTaskSearch.do?method=courseInfo&courseId=${task.course.id}&type=Course" target="_blank" title="<@msg.message key="common.displayDetailCourse"/>"><@i18nName task.course/></a>
	      </td>
	      <td><@i18nName task.arrangeInfo.teachDepart/>
	      <td>${task.arrangeInfo.weekUnits}</td>
	      <td>${task.course.credits}</td>

	      <td>${task.firstCourseTime?if_exists}</td>
      ...
      <#else>
	      <td>${task.course.code}</td>
	      <td><A href="courseTableForStd.do?method=taskTable&task.id=${task.id}" target="_blank" title="<@bean.message key="info.courseTable.lookFormTaskTip"/>">${task.seqNo?if_exists}</a></td>
