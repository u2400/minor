...
<body >
  ...
  <@table.table  id="listTable" sortable="true" width="100%">
    ...
    function exportData(){
       ...
       addInput(form,"keys","std.code,std.name,std.adminClasses.name,task.course.code,task.seqNo,task.course.name,examType.name,task.arrangeInfo.teachDepart.name,activity.time.firstDay,activity.time.timeZone,activity.room.name,task.arrangeInfo.teacherCodes,task.arrangeInfo.teacherNames,task.teachClass.name");
       addInput(form,"titles","<@msg.message key="attr.stdNo"/>,<@msg.message key="attr.personName"/>,班级,<@msg.message key="attr.courseNo"/>,<@msg.message key="attr.taskNo"/>,<@msg.message key="attr.courseName"/>,考试类型,开课院系,考试日期,考试时间,考试地点,教师职工号,授课教师,教学班名称");
