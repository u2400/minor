...
<body >
 ...
 <@table.table width="100%" sortable="true" id="listTable">
  ...
  <script>
     ...
     var menu =bar.addMenu("批量设置主考老师","batchSet('batchSetExaminers')");
     menu.addItem("依据授课教师设置主考","setExaminerByTeacher()");
     bar.addItem("批量设置监考信息","batchSet('batchSetInvigilators')");
    ...
    function setExaminerByTeacher(){
        ...
        form.action="examiner.do?method=setExaminerByTeacher";
        form["examActivityIds"].value = examActivityIds;
        form.target="";
        form.submit();
    }
