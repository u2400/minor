...
<#if (stdTypes?first.id)?exists>
  ...
  defaultSelect.onclick();
    ...
    <#else>
   <center><h1><font color="red">请先配置“<a href="studentType.do" target="_blank">学生类别</a>”。</font></h1></center>
   <script>
    var bar=new ToolBar("calendarFrameBar","<@msg.message key="teachPlan.title"/>",null,true,true);
    bar.addBlankItem();
   </script>
   </#if>
  </body>
