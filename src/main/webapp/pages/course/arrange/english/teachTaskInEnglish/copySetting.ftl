...
<script>
  ...
  bar.addHelp("<@msg.message key="action.help"/>","teachTask/copySetting");
     ...
     <table width="100%" align="center" class="formTable">
        <form name="copySettingForm" action="teachTaskInEnglish.do?method=copy" method="post" onsubmit="return false;">
	    ...
	    <tr>
  	     <td class="darkColumn">
  	      3、<input type="checkbox" name="taskCopyParams.copyCourseTakes"/>
  	       复制学生名单 
  	     </td>
	    </tr>
	    <tr>
     ...
     <table class="listTable" width="100%">
	    ...
	    <#list taskList?sort_by("seqNo") as task>
	     <tr class="${(task_index % 2 == 0)?string("brightStyle", "grayStyle")}" align="center">
	      ...
	      <td><#list task.requirement.languageAbilities?sort_by("name") as languageAbility>${languageAbility.name}<#if languageAbility_has_next>, </#if></#list></td>
	    </tr>
		</#list>
     </table>
   </form>
