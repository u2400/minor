...
<body style="Z-index: 1">
    ...
    <table width="100%" align="center" class="formTable">
        ...
        <tr>
            ...
            <td class="title" id="f_adminClassName" rowspan="4">&nbsp;<@bean.message key="entity.adminClass"/>：</td>
            <td class="content" rowspan="4" valign="top">
                <table>
                    <tr>
                        <td style="border-style:none" rowspan="2">
                                <select name="adminClassSelect" id="adminClassSelect" multiple style="width:${formElementWidth};height:90px">
        ...
        <tr>
            <td class="title">语种能力：<input type="hidden" name="languageAbilityIds" value=""/></td>
            <td colspan="3" style="vertical-align:middle"><#list languageAbilities?sort_by("name") as languageAbility><input type="checkbox" id="languageAbility_${languageAbility.id}" name="languageAbilityId" value="${languageAbility.id}"<#if (task.requirement.languageAbilities)?exists && task.requirement.languageAbilities?seq_contains(languageAbility)> checked</#if>/><label for="languageAbility_${languageAbility.id}">${languageAbility.name}</label><#if languageAbility_has_next>&nbsp;</#if></#list></td>
        </tr>
        <tr class="darkColumn">
    ...
    <script language="javascript">
        var action = "teachTaskInEnglish.do";
        function save(form){
            ...
            if (v.exec()) {
                ...
                form["languageAbilityIds"].value = getSelectIds("languageAbilityId");
                form.submit();
            }
        }
