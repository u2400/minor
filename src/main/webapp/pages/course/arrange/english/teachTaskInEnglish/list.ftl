  ...
  <script>
   ...
   <form name="taskListForm" action="" method="post" onsubmit="return false;">
    ...
    <tr bgcolor="#ffffff" onKeyDown="javascript:enterQuery(event)">
      ...
      <td><input style="width:100%" type="text" name="languageAbility.name" maxlength="3" value="${RequestParameters['languageAbility.name']?if_exists}"/></td>
    ...
    <@table.tbody datas=tasks;task>
      ...
      <input type="hidden" name="${task.id}" id="${task.id}" value="<#if task.isConfirm == true>1<#else>0</#if>"/>
