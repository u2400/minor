...
<script>
	...
	bar.setMessage('切换学期如果不能操作,是为开放排课功能时只能查看默认学期,请联系管理员。');
	...
	function query(){
	    ...
	    form.action="ecuplManualArrange.do?method=taskList";
	    form.submit();
	}
   ...
   function editTeachTask(){
       ...
       window.open("teachTaskCollege.do?method=edit&forward=actionResult&task.id=" + id);
   }
  </script>
</body>
