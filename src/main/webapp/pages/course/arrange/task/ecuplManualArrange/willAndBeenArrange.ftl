...
<body>
    ...
    <@table.table id="willAndBeenArrange" width="100%" align="center">
        ...
        <@table.tbody datas=tasks;task>
            ...
            <td><A href="teachTaskCollege.do?method=info&task.id=${task.id}" title="<@bean.message key="info.task.info"/>">${task.course.name}</A></td>
    ...
    <script>
        ...
        function updateTeachTask() {
           ...
           if (confirm("要将排课结果同步于任务吗？")) {
               form.action = "ecuplManualArrange.do?method=updateTeachTask";
