...
<script>
	...
	<#if isArrangeSwitch>
		<#list activityList as activity>
	       activity = new TaskActivity("${(activity.teacher.id)?if_exists}","${(activity.teacher.name)?if_exists}","${activity.task.course.id}","","${(activity.room.id)?if_exists}","<#if isAdmin || (state.arrangeSwitch.isArrangeAddress)?default(false) && (state.arrangeSwitch.isPublished)?default(false) || (isArrangeAddress)?default(false) && (isArrangeSwitch)?default(false)>${(activity.room.name)?if_exists}</#if>","${activity.time.validWeeks}","${activity.task.id}");
