<@table.table id="electtable" width="100%" sortable="true" headIndex="1">
    <tr bgcolor="#ffffff" onkeypress="DWRUtil.onReturn(event, pageGoWithSize)">
      <td align="center" >
        <img src="images/action/search.gif"  align="top" onClick="pageGoWithSize()" alt="<@bean.message key="info.filterInResult" />"/>
      </td>
      ...
      <input type="hidden" name="requestURI" value="courseTakeForTask"/>
      <td>
    ...
    <@table.tbody datas=tasks;task>
      ...
      <td><A href="#" onclick="javascript:quickSearch(document.taskListForm, 'courseTakeForTask.do?method=taskList', 'task.course.code', '${(task.course.code)?default("")}', null, 'pageGoWithSize')">${task.course.code}</A></td>
      ...
      <td><#if (task.taskGroup.name)?exists><A href="#" onclick="javascript:quickSearch(document.taskListForm, 'courseTakeForTask.do?method=taskList', 'task.taskGroup.name', '${task.taskGroup.name}', null, 'pageGoWithSize')">${(task.taskGroup.name)?html}</A><br><span style="color:blue">（${(task.taskGroup.stdCountInClass)?default(0)}）</span></#if></td>
