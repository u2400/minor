...
<script>
   var bar = new ToolBar('backBar','修改组内每个教学任务计划人数,总人数${taskGroup.stdCountInClass}',null,true,true);
 ...
 <table width="100%" border="0" class="listTable">
    ...
    <#list taskGroup.taskList as task>
	 ...
	 <tr align="center" class="darkColumn">
	  <td colspan="9">
	     ...
	     <input type="radio" name="adjuestMode" id="avgAll" value="" onclick="if(this.checked)divPlanCount();"/>均分${taskGroup.stdCountInClass}名学生
	 </tr>	
	</form>
	</table>
   <script type="text/javascript">
     function divPlanCount(){
          ...
          if(taskCount>0){
              var allStdCount = ${taskGroup.stdCountInClass};
     ...
     function savePlanStdCount(){
        ...
        if(newStdCount!=${taskGroup.stdCountInClass}){
           if(!confirm("班级总人数为${taskGroup.stdCountInClass}\n调整后的人数总数为:"+newStdCount+"\n确定提交该设置?"))
           return;
        }
