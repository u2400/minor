...
<script>
    ...
    function getGroupInfo(groupId){
      self.parent.taskGroupContentFrame.window.location="genderLimitGroup.do?method=info&taskGroup.id=" +groupId;
    } 
    function loadFirstGroupInfo(){ 
      <#if groupPageList?size==0>
        self.parent.taskGroupContentFrame.window.location="genderLimitGroup.do?method=info&taskGroup.id=";
      <#else>
