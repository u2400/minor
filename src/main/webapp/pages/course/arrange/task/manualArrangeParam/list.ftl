...
<body LEFTMARGIN="0" TOPMARGIN="0">
    ...
    <@table.table width="100%" id="teachTaskParam" sortable="true">
        ...
        <@table.tbody datas=manualArrangeParamList;params>
          ...
          <td>${params.isOpenElection?if_exists?string('开放','关闭')}</td>
        </@>
    </@>
    <@htm.actionForm name="actionForm" action="manualArrangeParam.do" entity="params" onsubmit="return false;">
        <input type="hidden" name="calenarId" value="${RequestParameters["manualArrangeParam.calendar.id"]}"/>
        <#assign filterKeys = ["method"]/>
        <input type="hidden" name="params" value="<#list RequestParameters?keys as key><#if !filterKeys?seq_contains(key)>&${key}=${RequestParameters[key]?if_exists}</#if></#list>&calenarId=${RequestParameters["manualArrangeParam.calendar.id"]}"/>
    </@>
    <script>
	    var bar = new ToolBar('electParamsBar','院系排课开关',null,true,true);
