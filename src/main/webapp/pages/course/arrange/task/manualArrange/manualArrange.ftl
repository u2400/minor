...
<div id="courseInfo" style="display: block">
	<table class="infoTable" width="100%">
		...
		<tr>
			...
			<td class="content"><#if (task.taskGroup)?exists>${(task.taskGroup.name)?if_exists?replace("," , ", ")} / ${(task.taskGroup.stdCountInClass)?default(0)}</#if></td>
		</tr>
	</table>
</div>
	...
	<tr>
	    <td width="7%" class="darkColumn"><@i18nName week/></td>
  	    <#list 1..unitList?size as unit>
   		<td id="TD${week_index*unitList?size+unit_index}" onclick="selectUnits(this)" class="infoTitle"></td>
		</#list>
	</tr>
    </#list>
</table>
	...
	<tr class="infoTitle">
	 ...
	 <td align="right">
	   ...
	   <#if (selectUnits>4)><#assign selectUnits=4/><#elseif (selectUnits<1)><#assign selectUnits=1/></#if>
	     一次选择的节次：
	     <input type="radio" name="selectUnitNum" value='1' <#if selectUnits=1>checked</#if> onclick="settingUnitsValue(1)"/>单节
	     <input type="radio" name="selectUnitNum" value='2' <#if selectUnits=2>checked</#if> onclick="settingUnitsValue(2)"/>双节
	     <input type="radio" name="selectUnitNum" value='3' <#if selectUnits=3>checked</#if> onclick="settingUnitsValue(3)"/>三节
	     <input type="radio" name="selectUnitNum" value='4' <#if selectUnits=4>checked</#if> onclick="settingUnitsValue(4)"/>四节</td>
	 </td>
	</tr>
</table>
...
<script>
    ...
    var unitsValue = ${selectUnits};
    var perRowUnits = ${unitList?size};
    function selectUnits(tdItem) {
        var oneUnit = parseInt((tdItem.id + "").replace(new RegExp("TD", "gm"), ""));
        if (tdItem.style.backgroundColor == "yellow") {
            var count = 0;
            for (var i = oneUnit; i < ${unitList?size * weekList?size} && count < unitsValue && i % perRowUnits < perRowUnits && $("TD" + i).style.backgroundColor == "yellow"; i++) {
                count++;
                selectUnit("TD" + i);
            }
            for (var i = oneUnit - 1; i >= 0 && count < unitsValue && $("TD" + i).style.backgroundColor == "yellow"; i--) {
                count++;
                selectUnit("TD" + i);
            }
        } else {
            var xUnit = oneUnit % perRowUnits + unitsValue;
            if (xUnit > perRowUnits) {
                oneUnit = oneUnit - (xUnit - perRowUnits);
            }
            for (var i = oneUnit; i < oneUnit + unitsValue; i++) {
                if ($("TD" + i).style.backgroundColor == "yellow") {
                    if (i % perRowUnits < xUnit) {
                        continue;
                    }
                    break;
                }
    ...
    function selectUnit(tdIdValue){
        var td = $(tdIdValue);
        var index=tdIdValue.substring(2);
    ...
    function settingUnitsValue(value) {
        this.unitsValue = isEmpty(value) ? ${selectUnits} : value;
    }
</script>
