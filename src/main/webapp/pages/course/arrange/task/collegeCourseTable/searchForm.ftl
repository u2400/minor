	  ...
	  <table class="searchTable" onkeypress="DWRUtil.onReturn(event, searchTask)">
	    ...
	    <tr>
	     ...
	     <td><@htm.i18nSelect datas=courseTypes name="task.courseType.id" selected=RequestParameters["task.courseType.name"]?if_exists style="width:100px"><option value="">全部</option></@></td>
	    </tr>
	    <tr>
	    ...
	    <tr>
	       <td>教学班:</td>
	       <td><input name="task.teachClass.name" value="${RequestParameters["task.teachClass.name"]?if_exists}" style="width:100px" maxlength="50"/></td>
	    </tr>
	    <tr>
	     <td class="infoTitle">教师院系:</td>
	     <td>
	     <@htm.i18nSelect name="teacher.department.id" datas=departmentList selected=RequestParameters["teacher.department.id"]?default("") style="width:100px"><option value="">全部</option></@>
	     </td>
	    </tr>
	    <tr>
	    ...
	    <tr>
	     ...
	     <input type="hidden" name="isInterval" value="1"/>
	     <td>
		     <select name="courseActivity.time.startUnit" value="${RequestParameters["courseActivity.time.startUnit"]?if_exists}" style="width:45px">
		     	<option value=""><@bean.message key="common.all"/></option>
		     	<#list 1..14 as unit>
		     	<option value=${unit}>${unit}</option>
		     	</#list>
		     </select>－<select name="courseActivity.time.endUnit" value="${RequestParameters["courseActivity.time.endUnit"]?if_exists}" style="width:45px">
		     	<option value=""><@bean.message key="common.all"/></option>
		     	<#list 1..14 as unit>
		     	<option value=${unit}>${unit}</option>
		     	</#list>
