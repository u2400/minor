...
<script lang="text/javascript2">
    ...
    function initTable() {
        ...
        <#assign unitSize=unitList?size>
        <#assign beenBorrow><font color="HotPink">借用</font></#assign>
        <#list occupyTables?if_exists as occupyTable>
            <#list occupyTable.activities?if_exists as activity>
                document.getElementById("TD${activity.unit}_${occupyTable.classroom.id}").innerHTML="<#if RequestParameters["displayStyle"]?default("detail") == "detail">${activity.detail}<#elseif RequestParameters["displayStyle"]?default("detail") == "simple">${activity.isTask?string("占用", beenBorrow?js_string)}</#if>";
            </#list>
        </#list>
        <#list occupyTables?if_exists as occupyTable>
            for(var i=0;i<7;i++){
