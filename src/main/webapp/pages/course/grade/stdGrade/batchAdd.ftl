 ...
 <BODY LEFTMARGIN="0" TOPMARGIN="0" >
  ...
  <table align="center" width="90%" class="listTable"> 
   ...
   <tr>
     ...
     <td class="brightStyle">
        <#assign choiceGradeTypeId = RequestParameters["gradeTypeId"]?default("")/>
        <#if RequestParameters["entity"]?default("") == "makeupGrade">
        <input type="hidden" name="params" value="&entity=makeupGrade"/>
            <#assign choiceGradeTypeId = "4"/>
        </#if>
        <@htm.i18nSelect datas=gradeTypes name="gradeTypeId" selected=choiceGradeTypeId style="width:100px"/>
     </td>
     ...
     <td class="brightStyle">
        <@htm.i18nSelect datas=markStyles name="markStyleId" selected=RequestParameters["markStyleId"]?default("") style="width:100px" onChange='getMarkStyleSpan(this.value)'/>
     </td>
   </tr>
   <tr>
     ...
     <td>
        <#if RequestParameters["isOnlyPublished"]?default("") == "1">
        已发布<input type="hidden" name="courseGradeStatus" value="2"/>
        <#else>
  ...
  <table width="90%" align="center" class="listTable">
      <tbody id='addGradeTable'>
       ...
       <tr class="grayStyle" align="center">
            ...
            <td id="markStyleDiv${i}"><input name="score${i}" class="text" onchange="checkScore(event)" style="width:100px" maxlength="3"/></td>
            <td id="credit${i}"></td>
  ...
  <script>
     ...
     function getMarkStyleSpan(inputValue){
        if(inputValue=="1"){
          for(var i=0;i<${stdCount};i++){
            document.getElementById("markStyleDiv" + i).innerHTML = '<input name="score'+i+ '" class="text" onchange="checkScore(event)" style="width:100px" maxlength="3"/>';
          } 
        } 
        if(inputValue=="3"){
          for(var i=0;i<${stdCount};i++){
            document.getElementById("markStyleDiv" + i).innerHTML = '<select name="score' +i+ '"><option value=""></option><option value="95">优</option><option value="85">良</option><option value="75">中</option><option value="65">及格</option><option value="50">不及格</option></select>';
          }         
        }
        if(inputValue=="5"){
          for(var i=0;i<${stdCount};i++){
            document.getElementById("markStyleDiv" + i).innerHTML = '<select name="score' +i+ '"><option value=""></option><option value="65">合格</option><option value="49">不合格</option></select>';
          }
        }
     }
