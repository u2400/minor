     ...
     <table width="100%" align="center" class="listTable">
	   ...
	   <#list stdGPList as stdGP>
	    <tr align="center">
		   ...
		   <!--*${calendars?size}*-->
		   <#list calendars as calendar>
		   <#if stdGP.getGP(calendar)?exists><td><@reserve2 stdGP.getGP(calendar)+stdGp/><#assign count = count + 1/></td>
		   <#else><td><#assign count = count + 1/></td></#if>
		   </#list>
		   <#if (count < totalCount)>
		      <#list count..(totalCount - 1) as i>
		   <td></td>
		      </#list>
		   </#if>
		</tr>
