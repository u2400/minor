<#include "/templates/head.ftl"/>
 ...
 <script language="JavaScript" type="text/JavaScript" src="dwr/interface/gradeService.js"></script>
 ...
 <script>
   ...
   var stdScore = null;
   function notifyGA(){
      GAtd = document.getElementById("GA_"+this.stdId);
      if(null != GAtd){
   		stdScore = this.GA;
	      gradeService.getScoreDisplay(displayScore, this.GA, ${task.gradeState.markStyle.id});
      }
   }
   function displayScore(data) {
      if (null == data) {
       ...
       GAtd.innerHTML = "<font color='red'>"+data+"</font>";
      }else{
       GAtd.innerHTML = data;
      }
   }
 ...
 <#macro reExamTd(gradeType,courseTake)>
  <#if (gradeConverterConfig.converters?size==0)>
     <td><input type="text"  onfocus="this.style.backgroundColor='yellow'" onblur="this.style.backgroundColor='white'" onchange="checkScore(this)"
		        class="text" value="缺考" id="${gradeType.shortName}_${courseTake.student.id}"
