...
<script type='text/javascript' src='scripts/course/Textbook.js?ver=1'></script>
<BODY LEFTMARGIN="0" TOPMARGIN="0">
	<table id="bookRequirementBar"></table>
    <table class="listTable" width="100%" align="center">
       <tr><td colspan="7"><@msg.message key="attr.taskNo"/>:${teachTask.seqNo},<@msg.message key="attr.courseName"/> <@i18nName teachTask.course/>
    ...
    <table id="newRequireBar"></table>
	 <table class="formTable" width="100%" align="center">	
	   <form action="${currURL?default("bookRequirement.do")}?method=save" name="bookRequirementForm" method="post" onsubmit="return false;">	
	   ...
	   <input type="hidden" name="params" value="${RequestParameters['params']?if_exists}"/>
		<tr>  
			<td class="title">教材名称<font color="red">*</font></td>
		...
		<tr>
			<td class="title">出版时间</td>
			<td id="publishedOn"></td>
			<td class="title"></td>
			<td></td>
		</tr>
		<tr class="darkColumn" >
 ...
 function saveCount(requireId,obj){
   ...
   form["require.id"].value = requireId;
   ...
   form.action="${currURL?default("bookRequirement.do")}?method=updateCount";
   form.submit();
 }
 ...
 function remove(requireId){
    ...
    if(confirm("删除信息不可恢复,确认删除")){
	    form.action="${currURL?default("bookRequirement.do")}?method=remove";
	    form.submit();
    }
 }
