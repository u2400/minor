...
<#include "/templates/back.ftl"/>
    ...
    <form action="teacher.do?method=edit" name="teacherForm" method="post" onsubmit="return false;" enctype="multipart/form-data">
    <@searchParams/>
 ...
 <script language="javascript" > 
   ...
   function appendParams(form) {
    var queryStr = "";
    
    jQuery("input[type=hidden]", form).each(function(i, ele) {
        queryStr += "&" + ele.name + "=" + encodeURIComponent(jQuery(ele).attr("value"));
        jQuery(ele).attr("name", "f_" + ele.name);
    });
    
    jQuery(":text", form).each(function(i, ele) {
        queryStr += "&" + ele.name + "=" + encodeURIComponent(jQuery(ele).val());
        jQuery(ele).attr("name", "f_" + ele.name);
    });
    
    jQuery("select", form).each(function(i, ele) {
        queryStr += "&" + ele.name + "=" + encodeURIComponent(jQuery(ele).val());
        jQuery(ele).attr("name", "f_" + ele.name);
    });
    
    jQuery("textarea", form).each(function(i, ele) {
        queryStr += "&" + ele.name + "=" + encodeURIComponent(jQuery(ele).val());
        jQuery(ele).attr("name", "f_" + ele.name);
    });
    
    jQuery(":radio:checked", form).each(function(i, ele) {
        queryStr += "&" + ele.name + "=" + encodeURIComponent(jQuery(ele).val());
        jQuery(ele).attr("name", "f_" + ele.name);
    });
