...
<body>
	...
	<table class="infoTable" style="word-break:break-all">
		...
		<tr>
            ...
            <td class="content"><#if document.startDate?exists>${document.startDate?string("yyyy-MM-dd hh:mm")}<#else>无</#if></td>
            <td class="title">有效结束时间：</td>
            <td class="content"><#if document.finishDate?exists>${document.finishDate?string("yyyy-MM-dd hh:mm")}<#else>无</#if></td>
        </tr>
