...
<#include "/templates/back.ftl"/>
     <table class="infoTable">
	   <tr>
	     ...
	     <td width="30%">${award.code}</td>
	     <td class="title" style="width:20%">教材名称:</td>
	     <td>${award.name}</td>
	   </tr>
	   <tr>
	     <td class="title" style="width:20%"><@msg.message key="award.author"/>:</td>
	     <td>${award.auth}</td>
	     <td class="title" style="width:20%"><@msg.message key="entity.press"/>:</td>
         <td>${award.press.name}</td>
	   </tr>
	   <tr>
	     <td class="title" style="width:20%">定价:</td>
         <td>${award.price?string("##.##")}</td>
	     <td class="title">折扣:</td>
         <td>${(award.onSell)?if_exists}</td>
	   </tr>
	   <tr>
	     <td class="title" style="width:20%">出版时间:</td>
         <td>${award.publishedOn?string("yyyy-MM-dd")}</td>
