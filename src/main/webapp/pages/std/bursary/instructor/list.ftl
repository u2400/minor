...
<body >
 <table id="applyListBar"></table>
  ...
  <script>
    ...
    bar.addItem("审核", "approve()");
    bar.addItem("退回", "reject()");
    bar.addItem("导出列表", "exportList()");
    bar.addItem("导出申请表", "exportApply()");
    function exportList() {
    	var form = document.actionForm;
    	form.action = "bursaryInstructor.do?method=export<#if (RequestParameters['apply.award.id']!"")?length!=0>&apply.award.id=${RequestParameters['apply.award.id']}<#else>&template=bursaryApplyList.xls</#if>";
    ...
    function exportApply() {
    	var form = document.actionForm;
    	form.action = "bursaryInstructor.do?method=export&template=bursaryApply.xls";
    	var applyIds = getCheckBoxValue(document.getElementsByName("applyId"));
    	if (null != applyIds && '' != applyIds){
    		form.action = form.action + "&applyIds=" + applyIds;
    	}
	...
	function reject() {
    	var form = document.actionForm;
    	submitId(document.actionForm, "applyId", false, "bursaryInstructor.do?method=reject","确定退回?");
	}
	function approve() {
    	var form = document.actionForm;
    	submitId(document.actionForm, "applyId", false, "bursaryInstructor.do?method=approveForm");
	}
  </script>
</body> 
