...
<table id="myBar" width="100%"></table>
   ...
   <#if !(apply.collegeApproved??) && !(apply.approved??)>
   ...
   <input type="hidden" value="${apply.id}" name="apply.id"/>
     <table  width="90%" align="center" class="formTable">
       ...
       <tr>
          ...
          <td>
          <input type="radio" onclick="appendOpinon('初审通过',this.form,'apply.instructorOpinion')"  id="approved_yes" name="apply.instructorApproved" value="1" <#if apply.instructorApproved??><#if apply.instructorApproved>checked</#if></#if>> <label for="approved_yes">初审通过</label>
          <input type="radio" onclick="appendOpinon('初审未通过',this.form,'apply.instructorOpinion')"  id="approved_no" name="apply.instructorApproved" value="0" <#if apply.instructorApproved??><#if !apply.instructorApproved>checked</#if></#if>> <label for="approved_no">初审未通过</label>
          </td>
       </tr>
       <tr>
          ...
          <td>
          <textarea  rows="5" cols="80"  name="apply.instructorOpinion">${apply.instructorOpinion!}</textarea>
          </td>
       </tr>
   ...
   function save(form){
	    var a_fields = {
	         'apply.instructorApproved':{'l':'审核结果', 'r':true, 't':'f_approved'},
	         'apply.instructorOpinion':{'l':'审核意见', 'r':true, 't':'f_opinion',mx:500}
	     };
	     var v = new validator(form , a_fields, null);
	     if (v.exec()) {
	        form.action="bursaryInstructor.do?method=approve"
	        form.target = "_self";
	        form.submit();
	     }
     }
   ...
   <#if apply.collegeApproved??>
    <tr>
     ...
     院系审核：
      ...
      <td>
      ${apply.collegeApproved?string("同意","不同意")}
      <pre>${apply.collegeOpinion}
      </td>
    </tr>
    </#if>
    <#if apply.approved??>
