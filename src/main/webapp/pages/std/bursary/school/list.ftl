 ...
 <table id="applyListBar"></table>
  <@table.table  width="100%" id="listTable" sortable="true">
    <@table.thead>
        ...
        <@table.sortTd width="8%" text="姓名" id="apply.std.name"/>
        <@table.sortTd width="7%" text="年级" id="apply.std.grade"/>
        <@table.sortTd width="20%" text="院系" id="apply.department.name"/>
        <@table.sortTd width="35%" text="奖项" id="apply.award.name"/>
  ...
  <script>
    ...
    function exportList() {
        ...
        form.action = "bursarySchool.do?method=export<#if (RequestParameters['apply.award.id']!"")?length!=0>&apply.award.id=${RequestParameters['apply.award.id']}<#else>&template=bursaryApplyList.xls</#if>";
    ...
    function exportApply() {
        var form = document.actionForm;
        form.action = "bursarySchool.do?method=export&template=bursaryApply.xls";
        var applyIds = getCheckBoxValue(document.getElementsByName("applyId"));
        if (null != applyIds && '' != applyIds){
            form.action = form.action + "&applyIds=" + applyIds;
        }
    ...
    function batchApprove() {
        var form = document.actionForm;
        submitId(document.actionForm, "applyId", true, "bursarySchool.do?method=batchApproveForm");
    }
	function reject() {
    	var form = document.actionForm;
    	submitId(document.actionForm, "applyId", false, "bursarySchool.do?method=reject","确定退回?");
	}
    function approve() {
        var form = document.actionForm;
        submitId(document.actionForm, "applyId", false, "bursarySchool.do?method=approveForm");
    }
  </script>
</body> 
