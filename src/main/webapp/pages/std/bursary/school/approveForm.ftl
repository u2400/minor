...
<table id="myBar" width="100%"></table>
   ...
   <input type="hidden" value="${apply.id}" name="apply.id"/>
     <table  width="90%" align="center" class="formTable">
       ...
       <tr>
        ...
        <tr>
          <td class="title" >院系审核结果:</td>
          <td><#if apply.collegeApproved??><#if apply.collegeApproved>同意<#else>不同意</#if>
             <pre>
             ${apply.collegeOpinion}
             </pre>
             <#else>
             院系尚未审核
             </#if>
       ...
       <tr>
          ...
          <td>
          <input type="radio" onclick="appendOpinon('同意',this.form,'apply.schoolOpinion')" id="approved_yes" name="apply.approved" value="1" <#if apply.approved??><#if apply.approved>checked</#if></#if>> <label for="approved_yes">同意</label>
          <input type="radio" onclick="appendOpinon('未获奖',this.form,'apply.schoolOpinion')" id="approved_no" name="apply.approved" value="0" <#if apply.approved??><#if !apply.approved>checked</#if></#if>> <label for="approved_no">未获奖</label>
          </td>
       </tr>
       <tr>
          ...
          <td>
          <textarea  rows="5" cols="80"  name="apply.schoolOpinion">${apply.schoolOpinion!}</textarea>
          </td>
       </tr>
   ...
   function save(form){
	    var a_fields = {
	         'apply.approved':{'l':'审核结果', 'r':true, 't':'f_approved'},
	         'apply.schoolOpinion':{'l':'审核意见', 'r':true, 't':'f_opinion',mx:500}
	     };
	     var v = new validator(form , a_fields, null);
	     if (v.exec()) {
	        form.action="bursarySchool.do?method=approve"
	        form.target = "_self";
	        form.submit();
	     }
     }
