...
<#assign showInput>
    <tr>
        ...
        <td class="grayStyle">&nbsp;学制：</td>
        <td><input type="text" name="student.schoolingLength" value="${RequestParameters['std.schoolingLength']?if_exists}" maxlength="2" style="width:40px"></td>
    </tr>
</#assign>
    ...
    var totalSize = ${totalSize?default(-1)};
 	function exportData(){
 	    if (totalSize >= 0) {
	 	    ...
	 	    //var keys = "basicInfo.gender.name,basicInfo.birthday,basicInfo.idCard,schoolingLength,firstMajor.name,studentStatusInfo.enrollDate,graduateOn";
	 	    //var titles = "性别,出生日期,身份证,学制,专业,入学日期,毕业日期";
