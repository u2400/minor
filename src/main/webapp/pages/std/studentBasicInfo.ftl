	     ...
	     <script language="javascript">
	     	...
	     	function doRefresh(){
	     		...
	     		if(args[0]){
	     			...
	     			$('studentPhoto').src='<#if managerRank>studentPhoto.do?method=download&student.id=${(result.student.id)?default('')}<#elseif studentRank>photoByStudent.do?method=download</#if>';
	     		}	     		
	     	}
	     </script>
	     <td class="brightStyle">
	      <img id="studentPhoto" src="<#if managerRank>studentPhoto.do?method=download&student.id=${(result.student.id)?default('')}<#elseif studentRank>photoByStudent.do?method=download</#if>" width="100" height="100" style="vertical-align:top;margin-top:1px;border:0px" >
