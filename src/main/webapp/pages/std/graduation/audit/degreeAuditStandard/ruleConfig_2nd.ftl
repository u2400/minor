...
<#include "/templates/back.ftl"/>
 <table width="95%"  align="center" class="formTable">
  <form action="degreeAuditStandard.do?method=ruleConfig_3rd" name="standardForm" method="post" onsubmit="return false;">
   ...
   <#list standard.ruleConfigs?sort_by(["rule","name"]) as ruleConfig>
   <tr>
     <td><input type="checkbox" name="ruleConfigId" value="${ruleConfig.id?if_exists}" <#if ruleConfig.enabled==true>checked</#if>/>${ruleConfig.rule.name?default("")}</td>
  ...
  <script language="javascript" > 
   ...
   function save(form){
     ...
     var selectId = getSelectIds("ruleConfigId");
