...
<body>
				...
				<table width="80%" align="center" class="listTable">
					...
					<tr>
	   				...
	   				<tr>
	   					...
	   					<td><@htm.i18nSelect datas=courseTypes selected=(result.auditStandard.publicCourseType.id?string)?default("93") name="auditStandard.publicCourseType.id"/>　<input type="checkbox" id="isDualConvert" value="1"/>是否互抵（说明：选中互抵，否则多选抵单选）</td>
	   				</tr>
	   				<input type="hidden" name="auditStandard.isDualConvert" value="0"/>
                    ...
                    <tr>
                        <td class="grayStyle">是否(启用)同名课程：</td>
                        <td>
                            <select name="auditStandard.sameCourse" style="width:200px">
                                <option value="1"<#if (result.auditStandard.sameCourse)?default(false)> selected</#if>>是</option>
                                <option value="0"<#if !(result.auditStandard.sameCourse)?default(false)> selected</#if>>否</option>
                            </select>
                        </td>
                    </tr>
					<tr>
	...
	<script>
		...
		form["auditStandard.isDualConvert"].value = "${(result.auditStandard.isDualConvert?string('1', '0'))?default('0')}";
		document.getElementById("isDualConvert").checked = form["auditStandard.isDualConvert"].value == "1";
	    ...
	    function doAction(){
	     	...
	     	if (v.exec()) {
	     	    form["auditStandard.isDualConvert"].value = document.getElementById("isDualConvert").checked ? "1" : "0";
