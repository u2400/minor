...
<body>
	...
	<script>var oApplication = {};</script>
	...
	<script>oApplication["${application.id}"] = "";</script>
			</#if>
		</@>
	</@>
	...
	<script>
		...
		bar1.addItem("申请表", "report()", "print.gif");
		...
		function report() {
            ...
            var iApplicationId = getSelectId("applicationId");
            if (null == iApplicationId || undefined == iApplicationId || "" == iApplicationId || iApplicationId.split(",").length > 1) {
                alert("请选择一条要操作的记录。");
                return;
            }
            if (null == oApplication[iApplicationId] || undefined == oApplication[iApplicationId]) {
                alert("请选择一条“申请状态”为“新增”的记录。");
                return;
            }
            form.action = "specialityAlerationApplicaton.do?method=report";
            form["applicationId"].value = iApplicationId;
            form.target = "_blank";
			form.submit();
		}
	</script>
