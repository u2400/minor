...
<body>
	...
	<table class="infoTable" width="100%">
		...
		<tr>
			...
			<td><@getBeanListNames (alteration.std.adminClasses)?default("")/></td>
		</tr>
		<tr>
		...
		<tr>
			<td class="title" style="width:20%;vertical-align:top"><@msg.message key="attr.remark"/>：</td>
			<td colspan="3">${(alteration.remark)?default("<br><br>")}</td>
		</tr>
	</table>
	<table class="infoTable">
		...
		<tr>
			...
			<td><#if (alteration.afterStatus.isActive)?exists><#if alteration.afterStatus.isActive><@msg.message key="common.yes"/><#else><@msg.message key="common.no"/></#if><#else>${alteration.std.active?string("是", "否")}</#if></td>
			<td><#if (alteration.beforeStatus.isActive)?exists && (alteration.afterStatus.isActive)?exists><#if (alteration.beforeStatus.isActive!=alteration.afterStatus.isActive)>变更</#if></#if></td>
		</tr>
		<tr>
			...
			<td><#if (alteration.afterStatus.isInSchool)?exists><#if alteration.afterStatus.isInSchool><@msg.message key="common.yes"/><#else><@msg.message key="common.no"/></#if><#else>${alteration.std.inSchool?string("是", "否")}</#if></td>
			<td><#if (alteration.beforeStatus.isInSchool)?exists && (alteration.afterStatus.isInSchool)?exists><#if (alteration.beforeStatus.isInSchool!=alteration.afterStatus.isInSchool)>变更</#if></#if></td>
		</tr>
		<tr>
			...
			<td><#assign adminClassAfter = (alteration.afterStatus.adminClass)?if_exists/><#if adminClassAfter?exists && adminClassAfter != ""><@i18nName adminClassAfter/><#else><@getBeanListNames (alteration.std.adminClasses)?if_exists/></#if></td>
			<#if (adminClassBefore.id)?exists && (adminClassAfter.id)?exists>
