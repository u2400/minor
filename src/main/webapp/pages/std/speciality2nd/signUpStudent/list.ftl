...
<table id="myBar" width="100%"></table>
  ...
  <script>
     ...
     bar.addItem("重算绩点","document.actionForm.target = '_self';multiAction('reCalcGPA','是否重新计算选中学生的报名绩点?')");
     bar.addItem("<@msg.message key="action.delete"/>","document.actionForm.target = '_self';remove()","delete.gif","直接删除记录，不影响录取结果");
     bar.addItem("添加","addSignUp()");
     bar.addItem("本学期导出","exportData()");
     var menu1 = bar.addMenu("打印列表", "print()", "print.gif");
     menu1.addItem("学士位申请表", "reportInDegree()", "print.gif");
    ...
    function reportInDegree() {
        var signUpStdIds = getSelectIds("signUpStdId");
        if (isEmpty(signUpStdIds)) {
            alert("请选择要操作的记录。");
            return;
        }
        form.action = "speciality2ndSignUpStudent.do?method=reportInDegree";
        form.target = "_blank";
        addInput(form, "signUpStdIds", signUpStdIds, "hidden");
        form.submit();
    }
  </script>
</body>
